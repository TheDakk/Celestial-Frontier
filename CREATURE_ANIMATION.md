...
**Authored paint continuity — matches code 2026-09-25.** The offline observed-surface builder can weld explicit `paintBoundaryPairs` that each match one real source adjacency. This keeps an interior torso ownership patch continuous without welding independent near/far limbs. Unknown, duplicate, missing or conflicting declarations refuse; default and all-boundary behavior stay unchanged. The Cougar packet independently checks the declared seam and rejects the old unwelded hole. Existing accepted bindings are untouched.

**Planted faint excursion — matches code 2026-09-25.** A land/amphibious quadruped faint now compiles one constant torso gain from its observed four-leg geometry. The planner samples the complete canonical curve, retains a 10% angular authoring reserve inside the unchanged contact limits, and scales root translation plus root/pelvis/spine/chest rotations together. Timing, feet, head/neck/tail/limb tracks and secondary curves are unchanged. A full excursion that fits keeps its exact old timeline; absent bend direction keeps the old preview and still refuses in contact admission. This is a motion proposal, never a rig certificate: actual painted-support, skin, source-rest and native gates remain authoritative. Cougar passes 19 action rows plus 1,675 presentation samples after source-label ownership and the review clock correction; six S2 controls remain byte-identical. Evidence and all failures: `audits/ART_BATTLE_FOCUS_20260925/cougar-repair-03/`.
...
**Battle-stage roster, presentation and wet arena (Claude lane) — matches code as of September 24, 2026 (this block only; it does not cover motion, and every other block keeps its own date).** All 17 painted archetypes play on the battle stage as paint-skin parts rigs: Crab, Coconut Crab, Freshwater Crab, Mud Crab, Vent Crab, Civet, Salmon, Eagle, Beetle, Python, Tree Frog, Chimpanzee, Starfish, Tarantula, Octopus, Fruit Bat and Centipede. The list is `BATTLE2_PARTS_FITS` in `port/v2/apps/game/src/battle2-archetypes.ts`, generated by `port/v2/tools/morph/build-card-masters.mjs` from the same list as the Compendium card (`battle2-archetypes.test.ts` fails on drift). `battle2-wiring.ts` loads a parts rig for any record whose Earth name has a registered fit (`BATTLE2_ASSETS.partsFits`), when the asset loader supplies raw bytes; a failed load is reported and falls back to the fixture rig. `battle2/library-arena.test.ts` loads each archetype's fit directly and has it fight a portrait opponent as attacker, with its own compiled anatomy attack, and as target at 30 Hz with zero rig refusals.

Presentation is one rule, `combatantPresentation` in `port/v2/apps/game/src/battle2/stage.ts`, used both as the stage's own default and by the app wiring. It applies the mass rule (a guardian instead fills `GUARDIAN_FRAME_FILL` with its tallest pose), then caps width at `COMBATANT_WIDTH_FRACTION_MAX` (0.42 of the frame, `battle2/arena.ts`; guardians exempt), then, given its stand line and a rig that reports its extent, keeps the tallest pose (rest reach above the foot plus the measured rise) `COMBATANT_TOP_MARGIN` (0.02 of the frame) inside the top edge. The app wiring (and the native proof harness) centres each painted box on its stand with `standCentreShift` (stage.ts), by shifting the stands the stage and choreography read; the stage itself still places the foot at `stand.x`. With `fitToBand`, `selectHabitatArena` in `battle2/habitat-arena.ts` gives an air or water body taller than `BAND_FILL` (0.9) of its band a `fit` factor below 1 that shrinks it to that height; the wiring multiplies it into the presentation scale. The wet arena is the `BattleStage` option `water: { surfaceY, side? }`: it draws the `WATER_BANDS` depth bands behind the combatants; a full lake hides the dry near plate, while a swimmer facing a ground fighter gets only its own half and the near plate stays. `lakeArenaWorld` supplies the lake world. Stage file ownership: `port/v2/apps/game/src/battle2/README.md` (its table matches code as of 2026-09-19 and does not yet describe this presentation rule or the wet arena).

How each body plan moves is Codex's motion-anatomy program: `audits/MOTION_ANATOMY_20260923/CODEX_PROMPT.md`. Known issue, Codex's to fix in the anatomy chain: the Centipede's ARAP skin refuses at 0.85× its default presentation scale with "ARAP skin: unresolved folded triangles". The SCALE SWEEP in `battle2/library-arena.test.ts` runs every archetype as attacker through a four-turn bout at 0.85×, 1× and 1.15× and pins this as its only known refusal, so the pin goes red once the fold is fixed.
...
Observed/REST contact diagnostic (matches code2026-09-22): the0.25 gate uses native record pixels. On the unchanged Brown Bear fit, endpoint-only hind-far support correction approaches a two-cycle; a diagnostic32-pass clone is not runtime policy. REST stage admission does not establish painted-surface drift≤0.25px. Nick-declared original Mud/Vent Far chains are folded in presence.json and hash-bound record.anatomy; forward reach lifts, backward bounds do not. Runtime solver bytes unchanged. Full review: audits/BEAR_CONTACT_DIAGNOSTICS_20260922/README.md.
...
## Caller-owned atlas lifetime — 2026-09-22

`loadCreatureRigV1` accepts `{borrowedAtlas:true}` as its optional seventh argument after the existing custom decoder. That decoder returns the caller's cached Texture. Default/false retains owned-atlas disposal. Borrowed atlas and TextureSource survive rig disposal and decoded-dimension refusal; per-rig containers, geometries and Texture wrappers still release. Ownership is captured at load. The cache must retain its texture until all borrowing rigs are disposed, then destroy it itself. Hash admission, sampling guard, solver and source art remain unchanged. See audits/BORROWED_ATLAS_20260922/README.md for controls and the ordered S2 receipt.
...
### Selective fish seams and contact continuation — matches tooling as of 2026-09-26

Perch, Cod and Carp are review candidates only. Their selected weld bindings reproduce byte-exact; an audit-only extension measures the exact nonempty selected boundary inventory through every action and full presentation at the unchanged 0.0011959075927734375 px tolerance. All pass; original unwelded Perch fails with a 94.609 px gap despite its original static PASS. The existing runtime/static join contract still treats other excluded adjacencies as observations. `audits/C49_FISH_REVIEW_20260926` binds the historical native evidence, missing durable fit/film inputs, and proposed admission packet. No registry or pin rows were added. Trout/Herring remain visually refused; a physical collar-width/strain diagnostic is proposed before any weld-weight or reference change.

A bounded contact-endpoint ownership projection did not repair Grouse, Sparrow or Tapir; Mongoose already has correct endpoint ownership and was not rerun. Rejected candidates and original failures are retained in `audits/G1_FIT_CONTINUATION_C50_20260926`. Joint limits, compression, root pins and motion remain unchanged. The new G2 Robin passes static motion but remains semantically UNRESOLVED. Earlier Eagle/Sandpiper tail findings are unchanged.
