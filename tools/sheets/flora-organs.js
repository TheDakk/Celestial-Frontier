...
module.exports = {
  ...
  lift: ['mulberry32', 'hashInt', 'floraStat', 'STAT_KEYS',
  ...
  draw: `function(g){
    ...
    const NAMES=['Rafflesia','Banana','Coffee','Cacao','Durian','Mango','Papaya','Date Palm','Jackfruit','Avocado'];
    NAMES.forEach(function(nm,i){
      const x=8+(i%5)*310, y=20+((i/5)|0)*270;
      g.fillStyle='#8892b8'; g.fillText(nm, x+8, y-4);
      try{
        const gen={seed:30000+i*977, kingdom:'flora', color:i%17, accent:(i*3)%17, form:i%18, body:0, loco:0,
          trait:i%25, size:3, head:0, limbs:0, skin:1, tail:2, pattern:0, eyes:0, behavior:0, habitat:8, _earthName:nm, name:nm};
        const G=hdGenesFor(gen), cvb=hdFloraBare(G);
        const T=240, tc=document.createElement('canvas'); tc.width=tc.height=T;
        tc.getContext('2d').drawImage(cvb,0,0,T,T);
