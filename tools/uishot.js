...
const GV = (game.match(/const GAME_VERSION='([^']+)'/) || [,'0'])[1];   /* seeds track the build — a stale rn made every shot a bulletin screenshot */
...
const SEED = `<script>try{ localStorage.clear();
  localStorage.setItem('cfcc_save_v2', JSON.stringify({ me:'Proof Explorer', tut:1, rn:'${GV}' }));
}catch(e){}</script>`;
...
const SEED_FULL = `<script>try{ localStorage.clear();
  localStorage.setItem('cfcc_save_v2', JSON.stringify({ me:'Proof Explorer', tut:1, rn:'${GV}', essence:145,
