...
if(![3,4].includes(process.argv.length)||!process.argv[2].startsWith('--output=')||(process.argv.length===4&&!['--edge-runners','--weather-mat'].includes(process.argv[3])))throw Error('--output=NEW_DIRECTORY [--edge-runners|--weather-mat] required');
...
const settings={...prior.settings,finisherStrength:.35,...(process.argv[3]==='--weather-mat'?{compositionProfile:'weather-mat-v1'}:process.argv[3]==='--edge-runners'?{compositionProfile:'edge-runners-v1'}:{})};
