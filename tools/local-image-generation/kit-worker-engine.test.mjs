...
async function exercise(factory){
  ...
  const bytes=new Uint8Array(128*128*4).fill(255),sha=hash(bytes);
  ...
  globalThis.fetch=async url=>String(url).startsWith('/model/tokenizer/')?{ok:true,json:async()=>({})}:{ok:true,arrayBuffer:async()=>bytes.slice().buffer};
  ...
  const ort={env:{webgpu:{},wasm:{}},Tensor:class{constructor(type,data,dims){Object.assign(this,{type,data,dims});}dispose(){}},InferenceSession:{async create(graph){
    ...
    return {async run(feeds){calls.run[kind]++;let name,type,dims,data;
      ...
      else if(kind==='encode'){name='packed_latent';type='float32';dims=[1,128,feeds.sample.dims[2]/16,feeds.sample.dims[3]/16];data=new Float32Array(dims[1]*dims[2]*dims[3]);}
      ...
      else{name='sample';type='float32';dims=[1,3,feeds.packed_latent.dims[2]*16,feeds.packed_latent.dims[3]*16];const n=dims[2]*dims[3];data=new Float32Array(n*3).fill(1);data.fill(-1,n,n*2);for(let y=32;y<96;y++)for(let x=32;x<96;x++)for(let c=0;c<3;c++)data[c*n+y*dims[3]+x]=0;}
  ...
  const job={schema:'cf.kit-engine.v4',width:128,height:128,passSize:128,seed:133,steps:2,strength:.2,finisherStrength:.08,plate:ref,atlas:ref,triptych:ref,finisherPrompt:'f'.repeat(120),passes:names.map((name,i)=>({name,identityKey:'identity-'+i,prompt:'p'.repeat(120),reference:ref,placement:{x:.15+i*.13,groundY:.8,width:.1,flip:i%2===0}}))};
  let engine;
  try{
...
const reuseOracle=r=>{assert.deepEqual(r.calls.create,{text:1,encode:1,denoise:1,decode:1});assert.equal(r.calls.expansion,1);assert.equal(r.calls.draw,12);assert.equal(r.calls.run.decode,14);assert.equal(r.calls.run.denoise,26);assert.equal(r.second.boxes.length,6);assert.equal(r.second.qualityAccepted,false);};
test('actual engine composes six passes plus exactly one finisher; all four sessions stay warm for a second landing',async()=>{const r=await exercise(createKitWorkerEngine);reuseOracle(r);assert.equal(r.calls.release,4);assert.equal(r.calls.destroy,1);});
