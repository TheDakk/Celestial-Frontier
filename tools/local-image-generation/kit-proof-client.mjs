...
window.kitProof={
 ...
 report(){const {painting,finisherOriginal,composite,captures,maskCaptures,protectionMask,...details}=result??{};return {status,error,events,details,landings:landings.map(({painting,finisherOriginal,composite,captures,maskCaptures,protectionMask,...metrics})=>metrics),partial:partial.map(({name})=>name)};},
 async artifact(kind,index=0){const blob=kind==='landing'?landings[index].painting:kind==='finisher-original'?result.finisherOriginal:kind==='painting'?result.painting:kind==='composite'?result.composite:kind==='partial'?partial[index].blob:kind==='mask'?result.maskCaptures[index].blob:kind==='protection'?result.protectionMask:result.captures[index].blob;const bytes=new Uint8Array(await blob.arrayBuffer());let text='';for(let at=0;at<bytes.length;at+=32768)text+=String.fromCharCode(...bytes.subarray(at,at+32768));return btoa(text);},
