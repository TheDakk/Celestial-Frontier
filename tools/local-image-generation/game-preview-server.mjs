...
export async function createGamePreviewServer(options = {}) {
  const { port = 0, reference = path.join(ROOT, 'audits/AI_GAME_INTEGRATION_20260909/platypus-reference.png'),
    referenceIdentity, q8Block32 = 'auto', mode = 'development', fixture = null } = options;
  ...
  const createViteServer = fixture?.createViteServer
    ?? (await import(pathToFileURL(path.join(ROOT, 'port/v2/node_modules/vite/dist/node/index.js')).href)).createServer;
  let vite = null, server = null;
  const requests = [];
  const appendRequest = row => { requests.push(row); if (requests.length > 4096) requests.shift(); };
  try {
    vite = await createViteServer({ root: gameRoot, configFile: path.join(gameRoot, 'vite.config.ts'), mode,
      server: { middlewareMode: true, hmr: false, fs: { allow: [path.join(ROOT, 'port')] } }, appType: 'spa' });
