...
async function fixture(t) {
  ...
  const createViteServer = async options => {
    ...
    assert.equal(options.server.ws, false); assert.equal(options.server.forwardConsole, false);
    assert.equal(options.plugins.length, 1); assert.equal(options.plugins[0].name, 'cf-frozen-preview-client');
