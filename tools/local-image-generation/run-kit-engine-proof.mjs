...
try{
  ...
  if(receipt.status==='PASS'){receipt.artifacts.push(await capture('painting','painting.png'));receipt.artifacts.push(await capture('finisher-original','finisher-before-weather.png'));receipt.artifacts.push(await capture('composite','composite-before-finisher.png'));for(let i=0;i<6;i++){receipt.artifacts.push(await capture('cutout',`organism-${String(i+1).padStart(2,'0')}.png`,i));receipt.artifacts.push(await capture('mask',`organism-${String(i+1).padStart(2,'0')}-mask.png`,i));}receipt.artifacts.push(await capture('protection','protected-interiors.png'));}
