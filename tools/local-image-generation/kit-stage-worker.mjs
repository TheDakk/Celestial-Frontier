...
import {prepareCreatureFinishJob} from './creature-finish-math.mjs';
...
onmessage=async({data:job})=>{
 ...
 try{
  ...
  const recipe=creature?await prepareCreatureFinishJob(job.recipe):admitKitEngineJob(job.recipe);
  ...
  const result=await (creature?engine.finishCreature(recipe):engine.paint(recipe));postMessage({type:'complete',requestId:job.requestId,...result});
