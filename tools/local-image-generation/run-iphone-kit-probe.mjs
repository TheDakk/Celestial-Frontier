...
try{
 ...
 const prepared=path.join(root,embedded?'audits/ART_KIT_WEATHER_MAT_20260912/prepared':'audits/KIT_NORMAL_LAND_20260912/warm-prepared');
 ...
 if(embedded){
  ...
  if(report.initializerAudit.status!=='PASS'||report.initializerAudit.revision!==pin.revision||report.initializerAudit.parent.sha256!==pin.files.find(f=>f.path==='transformer_q8.onnx').sha256||report.initializerAudit.worker.largest.bytes>1073741824)throw Error('Initializer audit missing/mismatched');
 ...
 for(const row of pin.files)if(!embedded||!row.path.startsWith('text_encoder')&&!row.path.startsWith('tokenizer/'))routes.set('/model/'+row.path,path.join(cacheDir,row.path));
 ...
 if(!embedded)routes.set('/node_modules/@huggingface/tokenizers/dist/tokenizers.mjs',path.join(dir,'node_modules/@huggingface/tokenizers/dist/tokenizers.mjs'));
 ...
 if(!c.adapterAvailable||!c.shaderF16||(embedded&&c.maxBufferSize<report.initializerAudit.worker.largest.bytes)){report.status='DEVICE_UNSUPPORTED';report.modelLoad='Not attempted: required WebGPU/f16 unavailable';}
 else{
 ...
 const loaded=report.events.find(e=>e.phase==='loaded'&&e.stage==='denoise');
 report.transformerLoad={reached:report.events.some(e=>e.phase==='loading'&&e.stage==='denoise'),completed:!!loaded,memory:loaded?.memory??null,memoryScope:'Exposed worker JS heap only; native/GPU memory unavailable through these Safari APIs'};
 report.textEncoderRequests=report.requests.filter(r=>/text_encoder|tokenizer/.test(r.path));
 report.textEncoderEvents=report.events.filter(e=>e.stage==='text');
 if(embedded&&(report.textEncoderRequests.length||report.textEncoderEvents.length)){report.status='TEXT_ENCODER_BOUNDARY_FAILED';report.error='Phone accessed a forbidden text-encoder resource or session';}
