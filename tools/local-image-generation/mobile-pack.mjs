...
async function inventory(directory){const rows=[];let entries=0;
  async function walk(folder,prefix='',depth=0){need(depth<=12,'Package directory exceeds depth bound');
    for(const entry of (await fs.readdir(folder,{withFileTypes:true})).sort((a,b)=>a.name.localeCompare(b.name))){
      ...
      if(entry.isDirectory()){const count=rows.length;await walk(path.join(folder,entry.name),relative+'/',depth+1);need(rows.length>count,'Uninventoried empty package directory');}
...
export async function verifyMobilePack({directory,expectedManifestSha256}={}){
  ...
  need(manifest.sourceIdentity?.branch==='openai/mac'&&/^[a-f0-9]{40}$/.test(manifest.sourceIdentity.head)
    &&manifest.sourceIdentity.mode==='evidence'&&manifest.sourceIdentity.workingTree===true,'Invalid local diagnostic source identity');
  if(manifest.prior!==null)need(manifest.prior&&hex(manifest.prior.manifestSha256)&&hex(manifest.prior.buildId)
    &&Number.isSafeInteger(manifest.prior.totalBytes)&&manifest.prior.totalBytes>0,'Invalid retained prior package identity');
...
export async function createMobilePackServer({directory,expectedManifestSha256}={}){
  ...
  const requests=[];const server=http.createServer(async(request,response)=>{
    try{const url=new URL(request.url,'http://127.0.0.1'),name=url.pathname==='/'?'index.html':url.pathname.slice(1);
      const row=rows.get(name),receipt={method:request.method,path:url.pathname,status:null,closed:false};requests.push(receipt);
      response.once('finish',()=>{receipt.status=response.statusCode;});response.once('close',()=>{receipt.closed=true;receipt.status??=response.statusCode;});
      ...
      if(request.method==='HEAD')response.end();else{const stream=createReadStream(path.join(root,name));
        stream.once('error',()=>response.destroy());response.once('close',()=>stream.destroy());stream.pipe(response);}
