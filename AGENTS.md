...
8. **Versioning & release notes:** legacy v1 player-visible changes go into `RELEASES[0]` in `main.js`; v2 player-visible work goes into the unversioned `V2_DRAFT_RELEASE` in `port/v2/apps/game/src/release-content.ts`. `GAME_VERSION` and `V2_CURRENT_RELEASE_VERSION` bump **only when Dakk says so** — periodically suggest a bump when accumulated changes feel substantial. Use the existing categories (New Features & Systems / UI Enhancements / Gameplay / Balancing / Bug Fixes / Under the Hood). A v2 draft may appear in cumulative history but must never trigger the one-time update popup or mutate `rnSeen`; only a Dakk-authorized shipped version can do that.
