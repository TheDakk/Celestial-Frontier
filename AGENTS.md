...
**Phase 0 is COMPLETE on the automatable side; all 14 planned deterministic domain facades are present, and current development is a playable Phase-4 Pixi/browser slice in `port/v2/`.** That is the current implementation boundary, not a claim that Gates B–D are literally closed. The verbatim port rule, lifter (`port/v2/tools/lift.mjs`) and both fixture sources remain the conversion foundation. Gates from `port/v2/` include `npm test` + `npm run typecheck` (**root and app TypeScript configs**). Browser evidence is one immutable **Slice → Glass → recovery** chain on the same unchanged clean committed source: run and named-verify Slice, pass its exact run ID to full Glass and named-verify the exact Glass report with that Slice ID, then pass both exact predecessor IDs to recovery and named-verify the final three-report chain. Stop after any nonzero, red, or instrument result; do not run the next stage or automatically retry. `port/v2/README.md` owns the copy-ready commands and full current battery. A human-test package follows `port/DEVELOPMENT_PREVIEW.md` and must use a separate origin. Root-level fixture gates: `npm run preflight` / `goldenseeds` / `codefixtures` / `audioprofiles`.
