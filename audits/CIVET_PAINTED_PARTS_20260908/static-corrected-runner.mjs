...
const reportPath = path.join(audit, 'static-corrected-results.json');
const optionalMainJsPresent = fs.existsSync(path.join(root, 'main.js'));
...
const immutable = [
  ...
  ...(optionalMainJsPresent ? ['main.js'] : []), 'tools/validate.js', 'tools/build.js', 'tools/baseline.json',
  'tools/with-toolchain-lock.mjs', 'port/v2/tools/workspacelock.mjs',
  'audits/CIVET_PAINTED_PARTS_20260908/static-corrected-runner.mjs',
];
function requireUnchanged() {
  assert.equal(fs.existsSync(path.join(root, 'main.js')), optionalMainJsPresent, 'optional main.js presence changed');
...
function step(id, command, args) {
  requireUnchanged();
  const logPath = path.join(audit, `static-corrected-${id}.log`);
...
try {
  ...
  report.optionalMainJs = { present: optionalMainJsPresent, policy: 'Optional exactly as tools/validate.js; if absent, existing HTML remains the validation input. Presence must remain unchanged through the chain.' };
  report.priorInstrumentFailure = { runner: 'audits/CIVET_PAINTED_PARTS_20260908/static-runner.mjs', report: 'audits/CIVET_PAINTED_PARTS_20260908/static-results.json', diagnosis: 'Unconditional main.js source hashing threw ENOENT before any check ran; current validate.js deliberately treats this file as optional.' };
