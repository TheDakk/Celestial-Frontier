...
const receipt={schema:'cf.ambient-motion-preview.v1',status:'FAIL',startedAt:new Date().toISOString(),width:1024,height:576,fps:24,frames:288,durationSeconds:12,scope:'Ambient animation of one immutable local AI PNG; no creature articulation, inference or game integration',sources:[],browserEvents:[],frameHashes:[],cleanup:{}};
...
try{
release=acquireWorkspaceLock('local desert ambient motion capture');
const imagePath=path.join(root,'audits/LOCAL_AI_DESERT_TEST_20260909/raw-output.png'),htmlPath=path.join(out,'index.html');
for(const file of [fileURLToPath(import.meta.url),htmlPath,imagePath]){const b=await fs.readFile(file);receipt.sources.push({path:path.relative(root,file),bytes:b.length,sha256:sha(b)});}
...
cdp=await openChromiumCdp({label:'CF desert ambient motion',userDataPrefix:'cf-desert-motion-',commandTimeoutMs:30000,onEvent:e=>{if(['Runtime.exceptionThrown','Inspector.targetCrashed'].includes(e.method))receipt.browserEvents.push(e);}});receipt.browser=cdp.browser;
...
const first=await frame(0),quarter=await frame(3),end=await frame(12);assessLoop(sha(first),sha(quarter),sha(end));
...
await fs.writeFile(path.join(out,'frame-000.png'),first,{flag:'wx'});await fs.writeFile(path.join(out,'frame-072.png'),quarter,{flag:'wx'});
...
const args=['-hide_banner','-loglevel','warning','-f','image2pipe','-framerate','24','-vcodec','png','-i','pipe:0','-an','-c:v','libx264','-preset','medium','-crf','17','-pix_fmt','yuv420p','-movflags','+faststart',path.join(out,'desert-living-painting.mp4')];
...
const video=path.join(out,'desert-living-painting.mp4');
...
receipt.outputs=[];for(const name of ['desert-living-painting.mp4','preview.gif']){const b=await fs.readFile(path.join(out,name));receipt.outputs.push({path:name,bytes:b.length,sha256:sha(b)});}
