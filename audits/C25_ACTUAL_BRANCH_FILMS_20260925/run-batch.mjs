...
const dir=import.meta.dirname,plan=JSON.parse(fs.readFileSync(dir+'/cases.json')),producer=JSON.parse(fs.readFileSync(dir+'/producer.json'));const results=[];
for(const row of plan){
 ...
 const out=path.join(dir,row.name+'-actual-01');if(fs.existsSync(out))throw Error('no unchanged retry: '+out);
 ...
 const r=JSON.parse(fs.readFileSync(out+'/report.json'));results.push({name:row.name,code,status:r.status,error:r.error??null});fs.writeFileSync(dir+'/batch-results.json',JSON.stringify(results,null,2)+'\n');console.log('END '+row.name+' '+r.status+' '+JSON.stringify(r.capture?.refusalsAtEnd));
}
