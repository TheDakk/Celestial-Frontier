...
for subject in ['crab','coconut-crab','freshwater-crab']:
 ...
 with tempfile.TemporaryDirectory(prefix='cf-retained-frames-') as temp:
  ...
  for page in range((len(frames)+19)//20):
   subprocess.run(['/opt/homebrew/bin/magick','montage','-font','/System/Library/Fonts/Menlo.ttc',*[str(p) for p in frames[page*20:(page+1)*20]],'-tile','5x4','-geometry','+2+2','-background','#14242c',str(prefix)+f'-{page+1:02d}.png'],check=True)
