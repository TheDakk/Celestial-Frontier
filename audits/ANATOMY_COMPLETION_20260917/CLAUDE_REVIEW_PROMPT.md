...
Return one consolidated register: severity, exact source file/line or clip/action/frame, observed versus expected behavior, failing control, bounded shared repair and required acceptance proof. Separate code defects, missing anatomy/art/views, incomplete fits, visual failures, combat integration, audio, performance and physical-phone qualification. Inspect actual rendered evidence; a numerical PASS is not visual acceptance. Nick owns image/motion/listening acceptance.

Use this worktree as a read-only review input. Do not edit either lane, pull/merge/sync, copy implementation into your branch, change kits or signers, generate media/inference, rewrite history, run a broad certification chain or write to GitHub. PR42 remains parked. Report source/input drift if encountered; do not silently refresh evidence. No PR, deployment or promotion is requested.
