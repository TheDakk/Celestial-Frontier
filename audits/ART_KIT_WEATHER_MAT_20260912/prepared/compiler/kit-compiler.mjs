...
const EARTH_PAINTED_COMPOSITION_V1 = deepFreeze({
...
const EARTH_PAINTED_EDGE_RUNNERS_V1 = deepFreeze({
...
const EARTH_PAINTED_WEATHER_MAT_V1 = deepFreeze({
	...EARTH_PAINTED_COMPOSITION_V1,
...
function compileEarthArtKitV4(input, kit) {
	if (!kit.startsWith("# Celestial Frontier Art Kit\n") || !kit.includes("style_id: frontier   |   version 4,")) throw Error("Art Kit v4 required; retired kits are refused");
	...
	const system = systemFor(source.roster.starSeed);
	...
	const pigments = {
		...
		Frog: "green skin",
		Persimmon: "green leaves, orange fruit, brown wood",
		Cranberry: "green leaves and red berries",
		"Devil's Club": "green leaves and red berry cones"
	};
	const card = [
		...
		`  Light: ${scene.timeOfDay}; ${scene.weather}; ${system.binary || system.trinary ? JSON.stringify({
			binary: system.binary,
			trinary: system.trinary
		}) : "one primary, no companions"}; diffuse cloud-filtered light, soft neutral contact shadows; directional sun position absent from source, do not invent a visible second light.`,
	...
	const prompts = residents.map((row) => {
		...
		const accuracy = between(cls, "ACCURACY (paste and fill):\n", animal ? "\n\nLAYOUT, TOKEN POSE" : "\n\nLAYOUT (paste)").replace(/<[^>]+>/g, (token) => token.includes("count") || token.includes("stalks") ? counts : size);
		...
		return {
			key: row.name.toLowerCase().replace(/'/g, "").replace(/ /g, "-"),
			name: row.name,
			kind: "cut-out",
			width: 1024,
			height: 1024,
			identityKey: row.identityKey,
			sourceOwners: row.namedRule.sourceOwners,
			systemCard: card,
			prompt: [
				reference,
				style,
	...
	const plate = {
		key: "earth-temperate",
		name: "Earth temperate biome plate",
		kind: "scene",
		width: 2560,
		height: 1440,
		systemCard: card,
	...
	const exemplars = [
		...
		},
		{
			...
			name: "Pheasant",
			realm: "land",
			owner: "art/faunaoverrides.ts#birdB3Pheasant",
		...
		},
		{
			...
			name: "Trout",
			realm: "aquatic",
			owner: "art/faunaoverrides3.ts#FAUNA3_NAME.Trout",
		...
		},
		{
			...
			name: "Beetle",
			realm: "land",
			owner: "art/faunaoverrides.ts#FAUNA_NAME.Beetle/faunaBeetle",
		...
		},
		{
			...
			name: "Skink",
			realm: "land",
			owner: "art/faunaoverrides2.ts#faunaESquamata.Skink",
	...
	const familyReferences = exemplars.map((row) => {
		...
		const accuracy = between(familyClass, "ACCURACY (paste and fill):\n", "\n\nLAYOUT, TOKEN POSE").replace(/<[^>]+>/g, (token) => token.includes("count") ? row.counts : "a fraction of human height");
		return {
			key: "family-" + row.family.replace(/ /g, "-"),
			name: row.name,
			family: row.family,
			kind: "cut-out",
			width: 1024,
			height: 1024,
			realm: row.realm,
			sourceOwners: [row.owner],
			systemCard: familyCard,
			liveResident: false,
			prompt: [
				reference,
				style,
	...
	const frozenParagraph = "  Rich natural-history fantasy painting," + between(style, "  Rich natural-history fantasy painting,", "\n\nThe scene-contact");
	return freeze({
		schema: "cf.art.kit-earth-inputs.v4",
		...
		familyReferences,
		qualityAccepted: false,
		scope: "six-earth-cutouts-temperate-plate-five-named-family-exemplars"
	});
}
...
function compileEarthKitEngineV4(input, kit, assets, settings) {
	...
	const subject = model.recipe.residents.map((row) => `One ${row.name}: ${row.namedRule.diagnostics[0].required}.`).join(" ") + " Integrate feet and stems into wet ground with visible contact shadows under Civet and Platypus, shared light and natural ground reflections.";
	const finisherPrompt = [
		compiled.frozenParagraph,
		compiled.runtimeCard,
		subject,
		"A coherent riverbank scene with distinct readable subjects, the Civet as hero, a smaller Platypus, a clearly visible Frog, a taller Persimmon, and foreground grass overlapping the lowest animal feet."
	].join("\n\n");
	return freeze({
		schema: "cf.kit-engine.v4",
		experiment: settings.compositionProfile === "weather-mat-v1" ? "cf.kit-weather-mat.v1" : settings.compositionProfile === "edge-runners-v1" ? "cf.kit-edge-runners.v1" : "cf.kit-contact.v1",
		...settings,
		...
		qualityAccepted: false,
		mastersAccepted: true,
		textTokenCeiling: 512,
		finisherSteps: 1,
		skipOrganismPasses: true
	});
}
//#endregion
//#region port/v2/tools/landfall-snapshot/kit-entry.ts
function compileCanonicalEarthKit(kit) {
	return compileEarthArtKitV4(produceCanonicalEarthSnapshot().snapshot, kit);
}
function compileCanonicalEarthKitEngine(kit, assets, settings) {
	return compileEarthKitEngineV4(produceCanonicalEarthSnapshot().snapshot, kit, assets, settings);
}
//#endregion
export { compileCanonicalEarthKit, compileCanonicalEarthKitEngine };
