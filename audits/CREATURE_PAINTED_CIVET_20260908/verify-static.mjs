...
const reportPath = path.join(base, 'static-first.json');
if (fs.existsSync(reportPath)) throw Error('Immutable report exists; do not rerun unchanged input');
const sources = ['study-harness.ts', 'study.tsconfig.json', 'study-runner.mjs', 'serve-study.mjs'];
...
const stages = [
 ['study-typecheck', 'port/v2/node_modules/.bin/tsc', ['--noEmit','-p','audits/CREATURE_PAINTED_CIVET_20260908/study.tsconfig.json']],
 ['v2-three-typechecks', 'npm', ['--prefix','port/v2','run','typecheck']],
 ['root-validate', 'node', ['tools/validate.js']]
];
for (const [name, command, args] of stages) {
 const result=spawnSync(command,args,{encoding:'utf8',maxBuffer:32*1024*1024});
 fs.writeFileSync(path.join(base,name+'-first.log'),(result.stdout||'')+(result.stderr||''),{flag:'wx'});
 report.stages.push({name,command,args,exitCode:result.status,error:result.error?.message,log:name+'-first.log'});
