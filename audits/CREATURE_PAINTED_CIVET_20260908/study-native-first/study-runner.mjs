...
try {
  ...
  for (const mode of [{ name: 'desktop', width: 1440, height: 1200, dpr: 1, mobile: false }, { name: 'phone', width: 390, height: 844, dpr: 2, mobile: true }]) {
    ...
    const geometry = await evaluate(`(()=>{const v=document.getElementById('study-view'),c=v.querySelector('canvas');return{viewport:[innerWidth,innerHeight,devicePixelRatio],documentWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth,view:v.getBoundingClientRect().toJSON(),canvas:c.getBoundingClientRect().toJSON()}})()`);
    assert.equal(geometry.documentWidth, mode.width); assert.equal(geometry.scrollWidth, mode.width); assert(geometry.canvas.width > 0 && geometry.canvas.width <= geometry.view.width); result.geometry = geometry;
    ...
    assert(retired.actors.every(actor => actor.meshDestroyed && actor.geometryDestroyed)); assert(retired.resources.every(row => row.textureDestroyed && row.sourceDestroyed && row.bitmapWidth === 0 && row.bitmapHeight === 0)); assert.deepEqual(retired.failures, []); result.retired = retired;
