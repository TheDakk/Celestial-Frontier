...
const reportPath = path.join(base, 'static-final.json');
...
const stages = [
 ['rig-focused', 'npm', ['--prefix','port/v2','test','--','tests/painted-civet-rig.test.ts']],
...
for (const [name, command, args] of stages) {
 ...
 fs.writeFileSync(path.join(base,name+'-final.log'),(result.stdout||'')+(result.stderr||''),{flag:'wx'});
 report.stages.push({name,command,args,exitCode:result.status,error:result.error?.message,log:name+'-final.log'});
