...
INLINE void update(const double *p, const unsigned *d, const signed char *signs,
    ...
    int *location, double *priority, unsigned *state, int *unordered, unsigned t) {
  const unsigned k = t * 3, a = d[k], b = d[k + 1], c = d[k + 2];
  const double area = ((p[b] - p[a]) * (p[c + 1] - p[a + 1])
                    - (p[b + 1] - p[a + 1]) * (p[c] - p[a])) * signs[t];
  const double value = 1 - area / floors[t], previous = priority[t];
  priority[t] = value;
  if (value != value) *unordered = 1;
  if (value <= 1e-12 || movable[t] == 0) {
    remove_triangle(heap, location, priority, state, t); return;
  }
  int pos = location[t];
  /* All positions are final before incident updates. An already queued,
   * unchanged ordered priority leaves a valid heap exactly unchanged. Keep
   * ordinary repair after any NaN: its comparator is not a total order. */
  if (pos >= 0 && !*unordered && value == previous) return;
...
unsigned cf_orientation_active(double *p, const unsigned *d,
  ...
  for (unsigned t = 0; t < count; ++t)
    update(p, d, signs, floors, movable, heap, location, priority, state, &unordered, t);
