/** Packet adapter: same intake; imports/root routed to this lane. Prior manually authored landmarks retained explicitly; no fresh-observation claim. See lineage.json. */
/** Portable authored intake: run from the repository root with PACKET_DIR NEW_FIT_DIR.
 * Packet inputs: master.png, authoring.json, subject-source.json, presence.json.
 * Visible/hidden/absent/folded anatomy remains an explicit human-authored declaration.
 * New records normalize their source to this repository before recipe hashing.
 * Derived from audits/ARCHETYPE_SPRINT_20260922/intake.mjs; historical inputs unchanged.
 * Native review: port/v2/tools/battle2-proof/native-runner.mjs (this lane only).
 */
...
import {sealPortableFamilyRecord as sealFamilyRecord} from '/Users/nick/Projects/celestial-frontier-openai-mac/port/v2/tools/creature-animation/portable-record-writer.mjs';
...
try {
  ...
  const record=await sealFamilyRecord({kind:author.family, // Template id is not the optional taxonomy-family field.
    ...
    provenance:{authoringSha256:sha(bytes['authoring.json']),subjectSourceSha256:sha(bytes['subject-source.json']),presenceSha256:sha(bytes['presence.json']),sourceLabelsReused:false,sourceLandmarksReused:true,hiddenInference:false}
  });
  ...
  if(hasTransparent){
    ...
    write('label-authoring-receipt.json',{schema:'cf.sprint-authored-labels/v1',owner:'explicit new-master priority polygons; not source painter labels',counts,sourceLabelsReused:false,sourceLandmarksReused:true,rgbaChangedChannels:0,intake:checked.receipt,remainderPart:author.remainderPart});
