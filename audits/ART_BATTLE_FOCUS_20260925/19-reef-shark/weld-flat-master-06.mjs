...
const d=import.meta.dirname,root=path.resolve(d,'../../..'),old=path.join(d,'fit-05'),out=path.join(d,'fit-06'),req=createRequire(root+'/port/v2/package.json'),sharp=createRequire(req.resolve('free-tex-packer-core'))('sharp');
...
fs.writeFileSync(path.join(out,'flat-master-weld-receipt.json'),JSON.stringify({schema:'cf.flat-master-weld/v1',sourceFit:'fit-05',sourceRecordSha256:sha(old+'/record.json'),sourceBindingSha256:sha(old+'/pre-split-binding.json'),sourceAtlasSha256:sha(atlasFile),helperSha256:sha(import.meta.filename),options,receipt:split.receipt,bindingHash:split.binding.bindingHash,scope:'Only this unadmitted Reef Shark candidate. Existing opt-in preserves observed painted boundaries on a flat master. No solver/gate/limit change; no hidden pixels invented. Prior intake receipts describe inherited fit-05 inputs, this receipt owns the new split.'},null,2)+'\n');
console.log(JSON.stringify({fit:'fit-06',bindingHash:split.binding.bindingHash,...split.receipt}));
