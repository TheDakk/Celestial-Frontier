...
const d=import.meta.dirname,root=path.resolve(d,'../../..'),old=path.join(d,'fit-04'),out=path.join(d,'fit-05'),req=createRequire(root+'/port/v2/package.json'),sharp=createRequire(req.resolve('free-tex-packer-core'))('sharp');
if(fs.existsSync(out))throw Error('New output required');
const read=n=>JSON.parse(fs.readFileSync(path.join(old,n))),record=read('record.json'),binding=read('pre-split-binding.json'),atlasFile=path.join(old,'parts/atlas/bass.png'),atlas=await sharp(atlasFile).ensureAlpha().raw().toBuffer({resolveWithObject:true});
...
fs.writeFileSync(path.join(out,'flat-master-weld-receipt.json'),JSON.stringify({schema:'cf.flat-master-weld/v1',sourceFit:'fit-04',sourceRecordSha256:sha(old+'/record.json'),sourceBindingSha256:sha(old+'/pre-split-binding.json'),sourceAtlasSha256:sha(atlasFile),helperSha256:sha(import.meta.filename),options,receipt:split.receipt,bindingHash:split.binding.bindingHash,scope:'Only this unadmitted Bass candidate. Existing opt-in preserves observed painted boundaries on a flat master. No solver/gate/limit change; no hidden pixels invented. Prior intake receipts describe inherited fit-04 inputs, this receipt owns the new split.'},null,2)+'\n');
console.log(JSON.stringify({fit:'fit-05',bindingHash:split.binding.bindingHash,...split.receipt}));
