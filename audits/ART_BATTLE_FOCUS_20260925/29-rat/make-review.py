...
d.text((25,20),'RAT — item29 / Earth painting rank28 / plain master + six own masks',font=f,fill='white');d.text((25,65),'19/19 static + exact rest. Native0/0; p95 4.80 / max11.30ms at4x; zero over-budget frames.',font=sm,fill='#c5e3dc')
a=json.loads((p/'authoring.json').read_text());master=Image.open(p/'fit-03/parts/keyed.png').convert('RGBA');labels=Image.open(p/'fit-03/parts/ownership.png').convert('RGBA');fit=master.copy();fd=ImageDraw.Draw(fit);tiny=ImageFont.truetype('/System/Library/Fonts/Supplemental/Arial.ttf',25)
...
for i,(file,title) in enumerate([('turn1-hit-return-end.png','Return — opposing heads overlap'),('turn3-hit-idle-90.png','Final faint — connected, ground contact')]):
