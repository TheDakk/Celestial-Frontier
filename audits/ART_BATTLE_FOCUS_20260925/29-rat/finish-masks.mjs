...
const distributionFile=path.join(base,'distribution.json'),distribution=JSON.parse(read(distributionFile));
const partsBindingFile=path.join(fit,'parts/binding.json'),partsBinding=JSON.parse(read(partsBindingFile));
const partFiles=[],allowed=new Uint8Array(1254*1254);
assert(distribution.allowedPartIds.length===12);
for(const id of distribution.allowedPartIds){
 const part=partsBinding.parts.find(p=>p.id===id);assert(part,'Declared owner exists: '+id);
 const file=path.join(fit,'parts/parts',id+'.png');partFiles.push(file);const png=PNG.sync.read(read(file));
 for(let y=0;y<part.cutout.height;y++)for(let x=0;x<part.cutout.width;x++)if(png.data[(y*part.cutout.width+x)*4+3]>0)allowed[(y+part.cutout.y)*1254+x+part.cutout.x]=1;
}
const source=path.isAbsolute(record.source)?record.source:path.resolve(root,record.source);
const keyedFile=path.join(fit,'parts/keyed.png'),atlasFile=path.join(fit,'parts/atlas/rat.png');
...
const inputFiles=[distributionFile,partsBindingFile,...partFiles,source,recordFile,bindingFile,keyedFile,atlasFile,path.join(fit,'parts/manifest.json'),sourcePrompt];
...
function assess(mask){
  ...
  for(let k=0;k<count;k++){const i=k*4,a=mask.data[i+3],limit=allowed[k]?keyed.data[i+3]:0;
...
const report={schema:'cf.marking-conservation/v1',status:'RUNNING',createdAt:new Date().toISOString(),sourceHashes,helperSha256:sha(read(fileURLToPath(import.meta.url))),exclusion:{method:'Explicit authored torso and upper-leg fur owners only; no inferred absence',allowedParts:distribution.allowedPartIds,declaration:distribution},rows:[],controls:{},node:process.version};
...
try{
  ...
  for(const {pattern,rawFile,promptFile,rawBytes,prompt,raw} of prepared){
    ...
    for(let k=0;k<count;k++){const i=k*4,a=raw.data[i+3],limit=allowed[k]?keyed.data[i+3]:0,out=Math.round(a*limit/255);
    ...
    manifest.patterns[pattern]={file:rel(file),sha256:sha(read(file)),promptFile:rel(promptFile),promptSha256:sha(prompt),prompt:prompt.toString('utf8'),promptBytesBase64:prompt.toString('base64'),tool:'image_gen.imagegen',seed:null,rawFile:rel(rawFile),rawSha256:sha(rawBytes),transform:{resize:null,translationPx:[0,0],registration:null,rgb:[255,255,255],alpha:'generated alpha × keyed alpha / 255, rounded, then explicitly declared torso/upper-leg fur owners only; no automatic inference'}};
