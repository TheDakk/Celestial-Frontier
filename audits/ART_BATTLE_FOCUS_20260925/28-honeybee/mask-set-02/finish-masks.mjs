...
const fit=path.resolve(base,'../fit-02'),sourcePrompt=path.resolve(base,'../prompt.txt');
...
assert.deepEqual(distribution.allowedPartIds,['abdomen']);
const partsBindingFile=path.join(fit,'parts/binding.json'),abdomenFile=path.join(fit,'parts/parts/abdomen.png');
const abdomen=JSON.parse(read(partsBindingFile)).parts.find(p=>p.id==='abdomen'),abdomenPng=PNG.sync.read(read(abdomenFile));
const allowed=new Uint8Array(1254*1254);
for(let y=0;y<abdomen.cutout.height;y++)for(let x=0;x<abdomen.cutout.width;x++)allowed[(y+abdomen.cutout.y)*1254+x+abdomen.cutout.x]=abdomenPng.data[(y*abdomen.cutout.width+x)*4+3]>0?1:0;
const source=path.isAbsolute(record.source)?record.source:path.resolve(root,record.source);
const keyedFile=path.join(fit,'parts/keyed.png'),atlasFile=path.join(fit,'parts/atlas/honeybee.png');
...
const inputFiles=[distributionFile,partsBindingFile,abdomenFile,source,recordFile,bindingFile,keyedFile,atlasFile,path.join(fit,'parts/manifest.json'),sourcePrompt];
