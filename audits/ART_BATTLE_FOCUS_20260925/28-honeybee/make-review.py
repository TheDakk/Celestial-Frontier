...
d.text((25,20),'HONEYBEE — item28 / Earth painting rank27 / plain master + six own masks',font=f,fill='white');d.text((25,65),'13/13 static + exact rest. Native0/0; p95 5.80 / max37.10ms at4x; 25 over-budget frames.',font=sm,fill='#c5e3dc')
a=json.loads((p/'authoring.json').read_text());master=Image.open(p/'fit-02/parts/keyed.png').convert('RGBA');labels=Image.open(p/'fit-02/parts/ownership.png').convert('RGBA');fit=master.copy();fd=ImageDraw.Draw(fit);tiny=ImageFont.truetype('/System/Library/Fonts/Supplemental/Arial.ttf',25)
...
for i,(im,title) in enumerate([(master,'1254-square master, established keyer'),(labels,'21 authored paint owners'),(fit,'Manually observed joint placement')]):
 x=25+i*590;d.text((x,120),title,font=sm,fill='white');im.thumbnail((560,500));s.paste(im,(x,160),im)
for i,(file,title) in enumerate([('turn1-hit-return-end.png','Return — separated, connected paint'),('turn3-hit-idle-90.png','Final faint — intact limbs and wings')]):
