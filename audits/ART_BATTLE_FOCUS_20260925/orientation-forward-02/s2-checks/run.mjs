/** One changed-contact-source S2 execution; unchanged sentinel instrument, output routing only.
 * Invoke only after the parent authorizes final source. Fresh output is mandatory.
 * No browser, orientation wrapper, threshold change or automatic retry. */
...
const subjects=['crab','coconut-crab','freshwater-crab','mud-crab','vent-crab','civet'];
assert.deepEqual(baselineReport.subjects.map(x=>x.subject),subjects,'Exact six baseline controls');
remember(fileURLToPath(import.meta.url));remember(path.join(root,'audits/BORROWED_ATLAS_20260922/s2-controls.sources.json'));remember(path.join(baseline,'static.json'));
for(const subject of subjects)for(const suffix of ['-static.json','-support-samples.jsonl.gz'])remember(path.join(baseline,subject+suffix));
const retainedInputs=baselineReport.inputs.map(x=>{assert.equal(sha(read(x.path)),x.sha256,'Sentinel input changed: '+x.path);remember(x.path);return x;});
assert.equal(retainedInputs.length,12,'Exact twelve record/binding inputs');assert.equal(new Set(retainedInputs.map(x=>x.path)).size,12,'Unique sentinel inputs');
const execution={schema:'cf.s2-contact-regression-execution/v1',status:'RUNNING',command,cwd:root,startedAt:new Date().toISOString(),sourceHead:execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim(),sourceState:execFileSync('git',['status','--porcelain','--untracked-files=all'],{cwd:root,encoding:'utf8'}).trim().split('\n').filter(Boolean),node:process.version,owner,ownerSha256:sha(read(owner)),orchestratorSha256:sha(read(fileURLToPath(import.meta.url))),baseline,retainedInputs,noRetry:true,browser:false,workspaceLock:false,metadataNote:'The retained sentinel embeds historical signedProducer/sourceState prose. Actual current source hashes and dirty state are recorded separately here. Only the output path is transformed; no orientation oracle or altered sample/threshold logic. This contact regression makes no clean-source, signed-source, native-film or CPU certification claim.'};
const save=()=>fs.writeFileSync(executionFile,JSON.stringify(execution,null,2)+'\n');save();
const scratch=fs.mkdtempSync(path.join(os.tmpdir(),'cf-beetle-contact-s2-')),bundleFile=path.join(scratch,'s2.mjs');let transforms=0;
try{
  ...
  assert.deepEqual(identities.map(x=>x.subject),subjects,'Exact six current controls');assert.equal(identities.length,6);assert.equal(identities.reduce((n,x)=>n+x.sampleCount,0),13286,'Exact retained support sample count');
