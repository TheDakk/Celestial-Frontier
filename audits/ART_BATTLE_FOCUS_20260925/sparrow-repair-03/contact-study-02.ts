...
for(const [name,fit] of [['gull','audits/ART_BATTLE_FOCUS_20260925/12-gull/fit-03'],['goose','audits/ART_BATTLE_FOCUS_20260925/15-goose/fit-02'],['heron','audits/ART_BATTLE_FOCUS_20260925/16-heron/fit-01'],['sparrow','audits/ART_BATTLE_FOCUS_20260925/17-sparrow/fit-01'],['eagle','audits/ARCHETYPE_REPAIRS_20260922/03-biped-bird/fit-12']]){
 ...
 for(const id of ['melee:claw']){
  ...
  for(const [variant,tl]of [['original',base],['leg-scale-translation',buildActionTimeline(card,translated,card.identity.seed)],['level-strike',buildActionTimeline(card,{...action,poses:action.poses.map(p=>({...p,joints:{...p.joints,root:0},root:{...p.root,dy:0}}))},card.identity.seed)],...(id==='dodge'?[['grounded-duck',buildActionTimeline(card,duck,card.identity.seed)]]:[])])for(const mode of ['rest','observed']){
   ...
   for(let i=0;i<=120;i++){const ms=tl.durationMs*i/120,p=sampleTimeline(tl,ms),pose={...Object.fromEntries(Object.entries(p.joints).map(([j,rotation])=>[j,{rotation}])),root:{rotation:p.root.rotation,dx:p.root.dx,dy:p.root.dy}};maxHeadRotation=Math.max(maxHeadRotation,Math.abs(p.joints.head??0));try{solver.resolve(pose,{actionId:id,elapsedMs:ms,durationMs:tl.durationMs,weight:1,realm:card.realm,travel:'stage'});passes++;}catch(e){first??={i,ms,error:String(e)};}}
