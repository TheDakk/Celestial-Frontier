...
const d=import.meta.dirname,root=path.resolve(d,'../../..'),old=path.join(d,'fit-01'),out=path.join(d,'fit-03'),req=createRequire(root+'/port/v2/package.json'),sharp=createRequire(req.resolve('free-tex-packer-core'))('sharp');
...
const probe=createSourceJoinProbe({record,binding,atlas:{rgba:atlas.data,width:atlas.info.width,height:atlas.info.height}}),options={fixedJoints:['root'],shapeJoints:[...new Set(binding.parts.filter(p=>p.joint!=='root').map(p=>p.joint))],contactEndpoints:familyContactChains(familyContractForRecord(record)).map(c=>c.end),paintBoundaryPairs:[['root-patch','torso'],['root-patch','chest'],['torso','far-wing-shoulder'],['torso','near-wing-shoulder'],['torso','near-wing-flight']]};
