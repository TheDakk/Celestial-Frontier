...
it('preserves the shipped Eagle and already admitted new bird reactions exactly',()=>{
 for(const [fit,ids]of [['audits/ARCHETYPE_REPAIRS_20260922/03-biped-bird/fit-12',['dodge','hit','tame','faint']],[base+'12-gull/fit-03',['hit','tame']],[base+'16-heron/fit-01',['hit','tame']]] as const){const r=read(fit,'record.json'),card=compileBodyCard(r,r.genome);for(const id of ids)expect(buildTimeline(card,id,card.identity.seed)).toEqual(buildActionTimeline(card,actionsFor(card.template.id,card.anatomy)![id]!,card.identity.seed));}
});

it('holds the Goose faint inside both the solo and stage travel protocol without changing limits',()=>{
 const r=read(base+'15-goose/fit-02','record.json'),card=compileBodyCard(r,r.genome),action=actionsFor(card.template.id,card.anatomy)!.faint!,before=buildActionTimeline(card,action,card.identity.seed),after=buildTimeline(card,'faint',card.identity.seed),solver=createFamilyContactSolver(r);
 expect(after.notes.some(n=>n.startsWith('grounded-bird:planted-faint:'))).toBe(true);expect(after.durationMs).toBe(before.durationMs);expect(after.limitsRad).toEqual(before.limitsRad);expect(after.phases).toEqual(before.phases);let oldRefusals=0;
 for(const travel of ['solver','stage'] as const)for(let i=0;i<=120;i++)for(const [version,tl]of [['before',before],['after',after]] as const){const ms=tl.durationMs*i/120,p=sampleTimeline(tl,ms),pose={...Object.fromEntries(Object.entries(p.joints).map(([j,rotation])=>[j,{rotation}])),root:{rotation:p.root.rotation,dx:p.root.dx,dy:p.root.dy}},phase={actionId:'faint',elapsedMs:ms,durationMs:tl.durationMs,weight:1,realm:card.realm,travel};if(version==='after')expect(()=>solver.resolve(pose,phase)).not.toThrow();else try{solver.resolve(pose,phase);}catch{oldRefusals++;}}
 expect(oldRefusals).toBeGreaterThan(0);
});
