...
const originalDir = "/Users/nick/Projects/celestial-frontier-openai-mac/port/v2/tools/battle2-proof";
...
const repo = path.resolve(originalDir, '../../../..'), producer = path.resolve(repo, 'port/v2/apps/game/src'), arena = path.resolve(repo, 'audits/ARENA_EFFECTS_V42_PROOF_20260912');
...
try {
  ...
  const bundle = await rolldown({ input: path.join(originalDir, 'native-entry.mjs'), platform: 'browser', plugins: [{ name: 'read-only-producer', resolveId(id) { if (id.startsWith('cf-proof/')) return path.resolve(producer, id.slice(9)); }, transform(_, id) { if (path.isAbsolute(id) && fs.existsSync(id) && fs.statSync(id).isFile()) remember(id); if (id === "/Users/nick/Projects/celestial-frontier-openai-mac/port/v2/apps/game/src/battle2/choreography.ts") { const before=fs.readFileSync("/Users/nick/Projects/celestial-frontier-openai-mac/audits/ART_BATTLE_FOCUS_20260925/quadruped-repair-04/choreography-before.txt",'utf8'); if (_ !== before) throw Error('Faint proposal source mismatch'); const proposed=fs.readFileSync("/Users/nick/Projects/celestial-frontier-openai-mac/audits/ART_BATTLE_FOCUS_20260925/quadruped-repair-04/choreography-proposal-02.txt",'utf8'); report.faintProposal=JSON.parse(fs.readFileSync("/Users/nick/Projects/celestial-frontier-openai-mac/audits/ART_BATTLE_FOCUS_20260925/quadruped-repair-04/faint-transform-provenance-02.json",'utf8')); remember("/Users/nick/Projects/celestial-frontier-openai-mac/audits/ART_BATTLE_FOCUS_20260925/quadruped-repair-04/choreography-proposal-02.txt"); return {code:proposed,map:null}; } if (id === "/Users/nick/Projects/celestial-frontier-openai-mac/port/v2/apps/game/src/battle2/parts-rig.ts") { const before=fs.readFileSync("/Users/nick/Projects/celestial-frontier-openai-mac/audits/ART_BATTLE_FOCUS_20260925/approach-envelope-01/parts-rig-before.txt",'utf8'); if (_ !== before) throw Error('Proposal source mismatch'); const proposed=fs.readFileSync("/Users/nick/Projects/celestial-frontier-openai-mac/audits/ART_BATTLE_FOCUS_20260925/approach-envelope-01/parts-rig-proposal.txt",'utf8'); report.proposal=JSON.parse(fs.readFileSync("/Users/nick/Projects/celestial-frontier-openai-mac/audits/ART_BATTLE_FOCUS_20260925/approach-envelope-01/transform-provenance.json",'utf8')); remember("/Users/nick/Projects/celestial-frontier-openai-mac/audits/ART_BATTLE_FOCUS_20260925/approach-envelope-01/parts-rig-proposal.txt"); return {code:proposed,map:null}; } } }] });
  ...
  remember(path.join(originalDir,'capture-timeline.mjs'));
  remember(path.resolve(originalDir,'../animation-completion/review-schedule.mjs'));
  remember(path.resolve(originalDir,'../quadruped-proof/motion-proof-contract.mjs'));
  ...
  if (profiling) {
    ...
    remember(path.join(originalDir, 'cpu-profile.mjs'));
