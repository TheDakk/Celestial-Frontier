...
for(const [name,fit] of [['cattle','quadruped-repair-04/cattle-fit-02'],['lizard','quadruped-repair-04/lizard-fit-03']]){
 const out=path.join(p,'native-'+name+'-05'),log=out+'.log';if(fs.existsSync(out)||fs.existsSync(log))throw Error('New output required');
 const command=[path.join(p,'native-composed-proposal-02-runner.mjs'),path.join(base,fit),path.join(base,fit),out,path.join(p,name+'-script.json')],fd=fs.openSync(log,'wx');
