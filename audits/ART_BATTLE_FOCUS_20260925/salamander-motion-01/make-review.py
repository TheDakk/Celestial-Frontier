...
d.text((25,20),'SALAMANDER — item26 / Earth painting rank25 / plain master + six own masks',font=f,fill='white');d.text((25,65),'19/19 static +1557 presentation + exact rest. Native0/0; p95 4.10 / max11.80ms at4x.',font=sm,fill='#c5e3dc')
a=json.loads((p/'../26-salamander/authoring.json').read_text());master=Image.open(p/'../26-salamander/fit-05/parts/keyed.png').convert('RGBA');labels=Image.open(p/'../26-salamander/fit-05/parts/ownership.png').convert('RGBA');fit=master.copy();fd=ImageDraw.Draw(fit);tiny=ImageFont.truetype('/System/Library/Fonts/Supplemental/Arial.ttf',25)
...
for i,(im,title) in enumerate([(master,'1254-square master, established keyer'),(labels,'27 authored paint owners'),(fit,'Manually observed joint placement')]):
 x=25+i*590;d.text((x,120),title,font=sm,fill='white');im.thumbnail((560,500));s.paste(im,(x,160),im)
for i,(file,title) in enumerate([('turn1-hit-return-end.png','Return — opposing heads overlap'),('turn3-hit-idle-90.png','Final faint — continuous painted body')]):
