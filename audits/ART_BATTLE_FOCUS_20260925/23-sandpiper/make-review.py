...
d.text((25,20),'SANDPIPER — item23 / Earth painting rank18 / plain master + six own masks',font=f,fill='white');d.text((25,65),'14/14 static +1028 presentation + exact rest. Native0/0; p95 4.90 / max10.60ms at4x.',font=sm,fill='#c5e3dc')
a=json.loads((p/'authoring.json').read_text());master=Image.open(p/'master.png').convert('RGBA');labels=Image.open(p/'fit-02/labels.png').convert('RGBA');colors=[(0,0,0,0)]+[tuple(round(c*255) for c in colorsys.hsv_to_rgb(i*.618%1,.65,.95))+(255,) for i in range(18)];labels.putdata([colors[q[0]] for q in labels.getdata()]);fit=master.copy();fd=ImageDraw.Draw(fit);tiny=ImageFont.truetype('/System/Library/Fonts/Supplemental/Arial.ttf',25)
...
for i,(file,title) in enumerate([('turn1-hit-return-end.png','Return — positive painted head gap'),('turn3-hit-idle-90.png','Final faint — connected paint / no CPU spike')]):
