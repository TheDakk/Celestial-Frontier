...
const read=n=>JSON.parse(fs.readFileSync(path.join(old,n))),record=read('record.json'),binding=read('pre-split-binding.json'),atlasFile=path.join(old,'parts/atlas/eel.png'),atlas=await sharp(atlasFile).ensureAlpha().raw().toBuffer({resolveWithObject:true});
...
fs.writeFileSync(path.join(out,'flat-master-weld-receipt.json'),JSON.stringify({schema:'cf.flat-master-weld/v1',sourceFit:'fit-01',sourceRecordSha256:sha(old+'/record.json'),sourceBindingSha256:sha(old+'/pre-split-binding.json'),sourceAtlasSha256:sha(atlasFile),helperSha256:sha(import.meta.filename),options,receipt:split.receipt,bindingHash:split.binding.bindingHash,scope:'Only this new unadmitted Eel candidate: all observed boundaries belong to one continuous unoverlapped eel body. Retain head/jaw/neck and tiny root-patch paint continuity; no pixel, solver, contract or limits change. Original fit01 inputs retained; this receipt owns new split.'},null,2)+'\n');
