...
d.text((25,20),'EEL — item27 / Earth painting rank26 / plain master + six own masks',font=f,fill='white');d.text((25,65),'12/12 static +1015 presentation + exact rest. Native0/0; p95 2.30 / max7.00ms at4x.',font=sm,fill='#c5e3dc')
...
for i,(im,title) in enumerate([(master,'1254-square master, established keyer'),(labels,'13 authored paint owners'),(fit,'Manually observed joint placement')]):
 x=25+i*590;d.text((x,120),title,font=sm,fill='white');im.thumbnail((560,500));s.paste(im,(x,160),im)
for i,(file,title) in enumerate([('turn1-hit-return-end.png','Return — opposing heads overlap'),('turn3-hit-idle-90.png','Final faint — within water band')]):
