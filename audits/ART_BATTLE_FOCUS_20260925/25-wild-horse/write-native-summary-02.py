...
summary={'runId':run,'scope':r['scope'],'status':r['status'],
 ...
 'recipeHash':json.loads((p/'fit-05/record.json').read_text())['recipeHash'],
 'bindingHash':sha(p/'fit-05/binding.json'),'masterSha256':sha(p/'master.png')}
with (p/'native-summary-02.json').open('x') as f:f.write(json.dumps(summary,indent=2)+'\n')
