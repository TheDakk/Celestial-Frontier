...
const keyedFile=path.join(fit,'parts/keyed.png'),atlasFile=path.join(fit,'parts/atlas/brown-bear.png');
...
const prepared=patterns.map(pattern=>{
  ...
  assert(prompt.length>0 && prompt.toString('utf8').includes('1254 by 1254'), pattern+' exact standalone mask prompt retained; compiled master prompt is bound separately');
