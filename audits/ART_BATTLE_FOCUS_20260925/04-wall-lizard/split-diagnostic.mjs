...
import {smoothSkinWeights} from '/Users/nick/Projects/celestial-frontier-openai-mac/port/v2/tools/creature-animation/smooth-skin-weights.mjs';
import {selectPaintedContactVertex} from '/Users/nick/Projects/celestial-frontier-openai-mac/port/v2/tools/creature-animation/painted-contact-selector.mjs';
...
export async function splitObservedSurfaces(input,record,probe,{fixedJoints=['root'],shapeJoints=[],preservePaintBoundaries=false,contactEndpoints=[],preserveExistingWeights=false,releaseContactConflicts=false}={}){
 ...
 for(const {joint,supports}of contactPins)for(const i of supports){need(!locked.has(i)||locked.get(i)===joint,'contact conflicts with fixed owner '+JSON.stringify({joint,vertex:i,locked:locked.get(i),xy:[vertices[i].x,vertices[i].y]}));locked.set(i,joint);}
