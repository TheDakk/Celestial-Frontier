/** Explicit adhesive pad record and local source-owned near toe mesh refinement, final image unchanged. */
...
try {
  ...
  const record=await sealFamilyRecord({kind:author.family, // Template id is not the optional taxonomy-family field.
    ...
    geometry:{cutoutAssetHash:sha(bytes['master.png']),width:info.width,height:info.height,groundLineY:author.groundLineY,depthLayers:[{id:'far',order:0},{id:'near',order:1}],contactPads:{schema:author.contactPadsPx.schema,kind:author.contactPadsPx.kind,points:Object.fromEntries(Object.entries(author.contactPadsPx.points).map(([joint,p])=>[joint,normalized(p)]))}},
  ...
  stage='paint-skin';
  const compiled=await buildPaintSkin(path.join(out,'parts'),{seamBridges:{groups:[]}},record,{boundaryStep:40,interiorStep:80,refinements:provenance.options.refinements,includeTopology:true});
