...
const owner=path.join(root,'port/v2/tools/creature-animation/family-contracts.mjs'),relativeOwner=path.relative(root,owner),reportFile=path.join(packet,'contract-isolation-02.json');
...
try{
 ...
 fs.writeFileSync(path.join(packet,'family-contracts-before-02.mjs'),old,{flag:'wx'});
 ...
 fs.writeFileSync(path.join(packet,'resolved-before-02.json'),beforeBytes,{flag:'wx'});fs.writeFileSync(path.join(packet,'resolved-current-02.json'),afterBytes,{flag:'wx'});
