/** New opt-in terminal painted-triangle observer; retained prior static owner is unchanged. */
...
try {
  ...
  const supportModels=observedContactSupports(record,binding),contact=createFamilyContactSolver(record,supportModels);
  ...
  const marks=contact.chains.map(chain=>{
    if(record.geometry.contactPads){const surface=supportModels[chain.end]?.surface;assert(surface&&'triangle' in surface,'Declared painted triangle required');return {joint:chain.end,part:surface.partId,triangle:surface.triangle,barycentric:surface.barycentric,xy:supportModels[chain.end].rest};}
  ...
  const check=row=>{
    ...
    for(const c of lastContact?.contacts??[]){if(!c.stance)continue;const mark=marks.find(m=>m.joint===c.joint),p=positions[mark.part];
      let drift;
      if(mark.triangle){const actual=[0,0];for(let k=0;k<3;k++){actual[0]+=p[mark.triangle[k]*2]*mark.barycentric[k];actual[1]+=p[mark.triangle[k]*2+1]*mark.barycentric[k];}drift=Math.hypot((actual[0]-c.paintedTarget.x)*w,(actual[1]-c.paintedTarget.y)*h);assert(Number.isFinite(drift),'Finite actual painted pad');}
      else drift=contactPaintDriftPx([p[mark.index*2],p[mark.index*2+1]],mark.xy,[c.target.x,c.target.y],record.landmarks[c.joint],[w,h]);
