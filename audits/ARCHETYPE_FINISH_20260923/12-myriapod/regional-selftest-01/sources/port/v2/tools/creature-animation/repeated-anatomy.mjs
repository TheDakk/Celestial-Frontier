...
import {expandCompactMyriapod} from './myriapod-anatomy.mjs';
...
export function appendageCounts(id,anatomy){
 ...
 if(id==='myriapod'&&c&&typeof c==='object'&&!Array.isArray(c)){
  ...
  if(!Number.isInteger(c.walkingLegPairs)||c.walkingLegPairs<1||c.ultimateLegPairs!==1)fail('invalid myriapod appendage count');
  ...
  return {walkingLegPairs:c.walkingLegPairs,ultimateLegPairs:c.ultimateLegPairs};
 }
...
export function expandRepeatedAnatomy(template,anatomy){
 ...
 if(template.id==='myriapod')return freeze(expandCompactMyriapod(template,counts));
