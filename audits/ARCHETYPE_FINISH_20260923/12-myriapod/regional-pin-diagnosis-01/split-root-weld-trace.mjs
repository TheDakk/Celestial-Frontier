...
export async function splitObservedSurfaces(input,record,probe,{fixedJoints=['root'],shapeJoints=[],preservePaintBoundaries=false,contactEndpoints=[],preserveExistingWeights=false,releaseContactConflicts=false}={}){
 ...
 return [...groups.values()].map(members=>{const index=remap.get(members[0]);return{index,originalVertex:keys[members[0]].old,sourcePx:[vertices[index].x,vertices[index].y],parts:[...new Set(members.map(m=>keys[m].part))],lockedOwner:locked.get(index)??null};});
