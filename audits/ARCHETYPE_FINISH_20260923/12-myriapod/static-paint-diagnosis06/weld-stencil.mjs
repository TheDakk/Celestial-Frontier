...
const binding=json('fit-09/binding.json'),pre=json('fit-09/pre-split-binding.json'),probe=json('fit-09/source-join-probe.json'),author=json('candidate-11/authoring.json');
...
const master=PNG.sync.read(read(path.join(packet,'candidate-11/master.png'))),labels=PNG.sync.read(read(path.join(packet,'fit-09/labels.png')));
...
const rows=[1573,1576,1610].map(index=>{
...
const result={scope:'Retained fit09 two-level interpolation and exact split conservative support membership; no pose, contact, solve, intake, or acceptance rerun.',conventions:{fieldCoordinates:'source pixel coordinates',pixelDistance:'Euclidean distance to the center [pixelX+0.5,pixelY+0.5]',positiveCoefficient:'strict >0; no epsilon or alpha threshold used',zeroCoefficient:'strict ===0'},bodyLabel,joinName:join.name,maxReconstructionErrorPx:Math.max(...sourcePointErrors),controls:{zeroWeightConservativeMemberDetected:true,positiveWeightMutantDetected:true},rows,inputs:hashes.map(h=>({...h,unchanged:sha(fs.readFileSync(h.file))===h.sha256}))};
assert(result.inputs.every(h=>h.unchanged));
fs.writeFileSync(path.join(base,'weld-stencil.json'),JSON.stringify(result,null,2)+'\n',{flag:'wx'});
console.log(JSON.stringify({rows:rows.map(({samples,bodyPixels,...row})=>({...row,opaqueBrownWitnesses:bodyPixels.filter(p=>[[821,616],[822,617],[822,618],[823,619]].some(q=>q[0]===p.pixel[0]&&q[1]===p.pixel[1]))})),inputs:hashes.length,allUnchanged:true,maxReconstructionErrorPx:result.maxReconstructionErrorPx},null,2));
