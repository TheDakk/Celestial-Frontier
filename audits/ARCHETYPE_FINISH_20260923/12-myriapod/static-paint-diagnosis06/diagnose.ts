/** Two exact attempted canonical poses only; no row replay or acceptance. */
...
const base='/Users/nick/Projects/celestial-frontier-openai-mac/audits/ARCHETYPE_FINISH_20260923/12-myriapod',out=path.join(base,'static-paint-diagnosis06/report.json');
...
const record=read('fit-09/record.json'),binding=read('fit-09/binding.json'),original=read('static-06.json'),pre=read('fit-09/pre-regional-binding.json'),regions=read('fit-09/regional-authoring-receipt.json');
...
const findings=[];
for(const [action,expectedMs]of [['hit',15],['dodge',14]]){
