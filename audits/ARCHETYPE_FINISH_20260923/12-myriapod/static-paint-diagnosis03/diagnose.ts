...
const base='/Users/nick/Projects/celestial-frontier-openai-mac/audits/ARCHETYPE_FINISH_20260923/12-myriapod',out=path.join(base,'static-paint-diagnosis03/report.json');
...
const record=read('fit-06/record.json'),binding=read('fit-06/binding.json'),original=read('static-03.json'),pre=read('fit-06/pre-regional-binding.json'),regions=read('fit-06/regional-authoring-receipt.json');
...
const findings=[];
for(const [action,expectedMs]of [['hit',26.25],['dodge',25.666666666666668],['tame',144]]){
...
const targetScan={scope:'121 canonical target samples for each of hit/dodge/tame only; no ARAP solve, row acceptance or published geometry',samplesPerAction:121,rows:[],triangles:[]};
...
for(const action of ['hit','dodge','tame']){
...
console.log(JSON.stringify({targetScan:targetScan.rows.map(r=>({action:r.action,attempted:r.attemptedSamples,targets:r.targetSamples,invertedSamples:r.invertedSampleCount,errors:r.errors.length})),union:targetScan.triangles.map(t=>({triangle:t.triangle,parts:t.parts,vertices:t.vertices.map(v=>({index:v.index,restPx:v.restPx,weights:v.weights})),events:t.events.length}))},null,2));
