...
test('retained fit11 key includes its positive-alpha fringe without modifying source bytes',()=>{
 const file=path.join(import.meta.dirname,'fit-11/parts/keyed.png'),bytes=fs.readFileSync(file),im=PNG.sync.read(bytes),before=sha(im.data),box=positiveAlphaBox(im.data,im.width,im.height);
 assert.deepEqual([im.width,im.height],[1254,1254]);assert.deepEqual(box,{x:39,y:50,width:1189,height:1148});
 let positive=0,above8=0;const old=Uint8Array.from(im.data);for(let i=3;i<old.length;i+=4){if(old[i]>0)positive++;if(old[i]>8)above8++;else old[i]=0;}
