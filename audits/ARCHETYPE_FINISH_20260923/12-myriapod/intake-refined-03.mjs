/** Packet-local compact myriapod intake. Does not edit shared writers or source paint.
 * node audits/ARCHETYPE_FINISH_20260923/12-myriapod/intake-01.mjs PACKET_DIR NEW_FIT_DIR
 * Inputs and explicit source-coordinate semantics: regional-authoring-schema.md.
 * All results are authoring receipts, not static/native acceptance. No automatic retry.
 */
...
import {compileRegionalInfluences} from './regional-influences.mjs';
...
let stage='input-validation',failure=null,inputs=[];
const provenance={schema:'cf.sprint-authored-intake/v1',status:'RUNNING',sourceHead:execFileSync('git',['rev-parse','HEAD'],{cwd:ROOT,encoding:'utf8'}).trim(),helperSha256:sha(fs.readFileSync(fileURLToPath(import.meta.url))),regionalHelperSha256:sha(fs.readFileSync(new URL('./regional-influences.mjs',import.meta.url))),inputs,manualAuthoring:true,sourceLabelsReused:false,sourceLandmarksReused:false,hiddenInference:false,options:{boundaryStep:40,interiorStep:80,includeTopology:true,fixedJoints:['root'],shapeJoints:[],regionalDiffusionIterations:32},nativeAcceptance:false};
try {
...
inputs=inputFiles.map(name=>({path:path.join(packet,name),sha256:sha(bytes[name])}));
...
need(author.family==='myriapod','this packet requires explicit myriapod family');
...
need(Array.isArray(author.parts)&&author.parts.length===32,'32 explicit priority polygon texture parts required');
need(author.fixedAttachmentsPx&&typeof author.fixedAttachmentsPx==='object'&&!Array.isArray(author.fixedAttachmentsPx),'manual fixedAttachmentsPx object');
need(Array.isArray(author.regionalInfluences)&&author.regionalInfluences.length>0,'manual regionalInfluences required');
need(presence.appendages?.walkingLegPairs===14&&presence.appendages?.ultimateLegPairs===1,'explicit 14 walking pairs and one ultimate pair');
...
need(Array.isArray(author.meshRefinements)&&author.meshRefinements.length>0,'explicit local refinement rectangles required');
provenance.options.refinements=author.meshRefinements;
...
stage='family-record';
  ...
  const record=await sealFamilyRecord({kind:author.family, // Template id is not the optional taxonomy-family field.
    ...
    geometry:{cutoutAssetHash:sha(bytes['master.png']),width:info.width,height:info.height,groundLineY:author.groundLineY,depthLayers:[{id:'far',order:0},{id:'near',order:1}],fixedAttachments:Object.fromEntries(Object.entries(author.fixedAttachmentsPx).map(([joint,p])=>[joint,normalized(p)]))},
  ...
  const resolved=familyContractForRecord(record);
  need(resolved.joints.length===63,'compact myriapod requires exactly 63 declared joints');
  need(Object.keys(record.geometry.fixedAttachments).length===31,'31 authored fixed attachments required');
  ...
  stage='paint-skin';
  const compiled=await buildPaintSkin(path.join(out,'parts'),{seamBridges:{groups:[]}},record,{boundaryStep:40,interiorStep:80,refinements:author.meshRefinements,includeTopology:true});
  ...
  need(contactEndpoints.length===28&&new Set(contactEndpoints).size===28,'28 explicit walking contact endpoints required');
  need(contactEndpoints.every(j=>parts.filter(p=>p.joint===j).length===1),'each whole walking texture part must retain its Foot primary owner');
  const shapeJoints=[];
  const split=await splitObservedSurfaces(binding,record,probe,{fixedJoints:['root'],contactEndpoints,shapeJoints});
  write('pre-regional-binding.json',split.binding);
  write('observed-split-receipt.json',split.receipt);
  stage='regional-influences';
  const regional=await compileRegionalInfluences({binding:split.binding,record,atlas:{rgba:atlas.data,width:atlas.info.width,height:atlas.info.height},regions:author.regionalInfluences});
  stage='final-source-join-probe';
  const finalProbe=createSourceJoinProbe({record,binding:regional.binding,atlas:{rgba:atlas.data,width:atlas.info.width,height:atlas.info.height}});
  const {bindingHash:priorProbeBinding,...priorProbeBody}=probe;
  const {bindingHash:finalProbeBinding,...finalProbeBody}=finalProbe;
  need(await hashJSON(priorProbeBody)===await hashJSON(finalProbeBody),'source join inventory or interpolation changed');
  const {bindingHash:finalHash,...finalBody}=regional.binding;
  need(await hashJSON(finalBody)===finalHash,'final standard binding hash');
  write('regional-authoring-receipt.json',regional.receipt);
  write('source-join-probe.json',finalProbe);
  write('binding.json',regional.binding);
  write('receipt.json',{schema:'cf.sprint-fit/v1',intake,paintSkin:compiled.receipt,surfaces:split.receipt,regional:regional.receipt,sourceJoinProbe:{sourceBindingHash:priorProbeBinding,bindingHash:finalProbeBinding,geometryHash:await hashJSON(finalProbeBody),joins:finalProbe.joins.length,sourceSamples:finalProbe.joins.reduce((n,j)=>n+j.samples.length,0),unchangedSourceInventory:true},recordRecipeHash:record.recipeHash,bindingHash:regional.binding.bindingHash,masterSha256:record.geometry.cutoutAssetHash,manualAuthoring:true,nativeAcceptance:false});
  provenance.status='FIT_COMPILED';
  provenance.recordRecipeHash=record.recipeHash;
  provenance.bindingHash=regional.binding.bindingHash;
  provenance.contactEndpoints=contactEndpoints;
  provenance.shapeJoints=shapeJoints;
  console.log(JSON.stringify({status:provenance.status,out,parts:intake.parts,recordRecipeHash:record.recipeHash,bindingHash:regional.binding.bindingHash}));
} catch(error) {
  ...
  provenance.unchangedInputs=inputs.map(input=>({...input,unchanged:fs.existsSync(input.path)&&sha(fs.readFileSync(input.path))===input.sha256}));
