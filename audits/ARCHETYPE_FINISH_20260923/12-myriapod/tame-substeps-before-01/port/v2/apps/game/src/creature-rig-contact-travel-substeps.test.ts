...
it('retains exact root arithmetic at source waypoints, substep midpoints and endpoints in both reactions',()=>{
 const r=fixture(),solver=createFamilyContactSolver(r),old=withoutCadence(r);
 for(const id of['hit','dodge']){const{card,timeline}=at(r,id,0),travel=createContactTravel(timeline.root.dx,card.bodyLength),times=new Set([0,timeline.durationMs,timeline.durationMs+100]);for(let i=1;i<timeline.root.dx.length;i++){const a=timeline.root.dx[i-1]!.ms,b=timeline.root.dx[i]!.ms;for(const t of[.25,.5,.75,1])times.add(a+(b-a)*t);}
...
it('other actions, stationary holds, after-end poses and stage-owned travel keep old complete results',()=>{
 const r=fixture(),solver=createFamilyContactSolver(r),old=withoutCadence(r);expect(contracts.familyContractForRecord(r).contactStance!.travelSubsteps).toEqual({hit:2,dodge:2});
 for(const [id,ms]of[['tame',100],['tame',400],['hit',450],['dodge',280]]as const){const s=at(r,id,ms);expect(JSON.stringify(solver.resolve(s.pose,s.phase))).toBe(JSON.stringify(old.resolve(s.pose,s.phase)));}
 for(const id of['hit','dodge']){const s=at(r,id,50),phase={...s.phase,travel:'stage' as const};expect(JSON.stringify(solver.resolve(s.pose,phase))).toBe(JSON.stringify(old.resolve(s.pose,phase)));}
});
it.each([null,{},[],{hit:1},{hit:3},{hit:2.5},{hit:'2'},{tame:2},{other:2},Object.create({hit:2}),{[Symbol('hit')]:2}])('rejects malformed or out-of-scope subdivision declarations (%j)',travelSubsteps=>{
