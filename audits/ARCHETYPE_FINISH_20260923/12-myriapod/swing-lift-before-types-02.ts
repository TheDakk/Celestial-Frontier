...
it('foreign families/models and unbound fixed sockets cannot opt in',()=>{
 ...
 for(const changed of[{...t,id:'insect'},{...t,anatomyModel:undefined},{...t,fixedPivots:undefined}]){const spy=vi.spyOn(contracts,'familyContractForRecord').mockReturnValue(changed);try{expect(()=>createFamilyContactSolver(r)).toThrow(/Fixed attachments:|invalid swing lift declaration/);}finally{spy.mockRestore();}}
});
