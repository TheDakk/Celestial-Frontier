...
const relativeOwner='port/v2/tools/creature-animation/family-contracts.mjs',owner=path.join(root,relativeOwner),reportFile=path.join(packet,'contact-isolation-01.json');
...
try{
 ...
 report.priorSourceChecks=prior.sources.map(source=>{
  ...
  if(source.path===owner)assert.equal(source.sha256,sha(before),'Signed family bytes must equal the previously measured family bytes');
  else assert.equal(hash,source.sha256,'Previously measured S2 source changed '+source.path);
  remember(source.path);return {path:source.path,priorSha256:source.sha256,currentSha256:hash,expectedPrimateOnlyChange:source.path===owner};
 });
