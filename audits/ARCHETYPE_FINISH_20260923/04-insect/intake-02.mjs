/** Retains all23 painted regions while merging same-joint thorax/elytra labels into one source owner. No shared writer/gate change. */
/** New Beetle authored mesh uses existing writer options40/80 as in accepted Eagle/Octopus. No writer or numerical gate changes. */
...
try {
  ...
  if(hasTransparent){
    ...
    // Explicit manual paint regions sharing a joint become one source owner.
    // Elytra and thoracic shield cannot independently articulate in insect-v1.
    const jointOrder=[...new Set(parts.map(p=>p.joint))];
    const ownerParts=jointOrder.map(joint=>{const regions=parts.filter(p=>p.joint===joint);const chosen=regions.find(p=>p.id==='thorax')??regions[0];return {id:chosen.id,joint,layer:regions.some(p=>p.layer==='near')?'near':'far'};});
    const ownerIndex=parts.map(p=>jointOrder.indexOf(p.joint));
    const fallback=parts.findIndex(part=>part.id===author.remainderPart);
    const labels=Buffer.alloc(info.width*info.height*4),counts=ownerParts.map(()=>0);
    for(let y=0;y<info.height;y++)for(let x=0;x<info.width;x++){
      const i=y*info.width+x;let label=0;
      if(data[i*4+3]){let owner=parts.findIndex(part=>inside((x+.5)/info.width,(y+.5)/info.height,part.polygon));if(owner<0)owner=fallback;label=ownerIndex[owner]+1;counts[ownerIndex[owner]]++;}
    ...
    declarationBody={schema:'cf.painter-part-intake/v1',recordRecipeHash:record.recipeHash,cutoutSha256:record.geometry.cutoutAssetHash,labelsFile:'labels.png',labelsSha256:sha(labelsPng),parts:ownerParts};
    write('label-authoring-receipt.json',{regionOwnerMap:parts.map((p,i)=>({region:p.id,joint:p.joint,owner:ownerParts[ownerIndex[i]].id})),schema:'cf.sprint-authored-labels/v1',owner:'explicit new-master priority polygons; not source painter labels',counts,sourceLabelsReused:false,sourceLandmarksReused:false,rgbaChangedChannels:0,intake:checked.receipt,remainderPart:author.remainderPart});
