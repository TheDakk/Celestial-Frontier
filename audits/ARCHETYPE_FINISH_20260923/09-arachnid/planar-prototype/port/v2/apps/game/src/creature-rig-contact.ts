...
import {planarContactCandidates} from './creature-planar-contact.js';
...
export function createFamilyContactSolver(record:CreatureRigRecordV1,paintedSupports:Readonly<Record<string,ContactSupport>>={}){
 ...
 const rootModel=template.contactStance?.rootAccommodation,hasRootModel=!!template.contactStance&&Object.hasOwn(template.contactStance,'rootAccommodation');
 if(hasRootModel&&(template.id!=='arachnid'||rootModel!=='planar'))throw Error('Contact: unsupported root accommodation model');
 ...
 return {chains,scaleLength,stride,resolve(input:CreaturePoseV1,phase:ContactPhase){
  ...
  const planarBaseline=rootModel==='planar'&&rigidEligible?Object.fromEntries(Object.entries(pose).map(([j,k])=>[j,{...k}])):null;
  const planarEndpoints=planarBaseline?contacts.map(c=>({...c.endpointTarget})):null;
  let rootAccommodation:{model:'planar';requestedTranslation:{x:number;y:number};translation:{x:number;y:number};norm:number;attemptedCandidates:number}|undefined;
  ...
  const attemptPlanar=()=>{
   if(!planarBaseline||!planarEndpoints)throw Error('Contact: planar root accommodation unavailable');
   const restore=()=>{for(const key of Object.keys(pose))delete pose[key];for(const [j,k]of Object.entries(planarBaseline))pose[j]={...k};for(let i=0;i<contacts.length;i++)contacts[i]!.endpointTarget={...planarEndpoints[i]!};};
   const rigid=activeChains.map(c=>{let chain=rigidSupportChains.get(c.id);if(!chain){const cross=(c.support.x-c.root.x)*(c.joint.y-c.root.y)-(c.support.y-c.root.y)*(c.joint.x-c.root.x);chain=createTwoBoneChain({root:c.root,joint:c.joint,end:c.support,bend:cross<0?-1:1});rigidSupportChains.set(c.id,chain);}return chain;});
   const baselineMatrices=program.evaluate(planarBaseline),baselineRoot=transformPoint(baselineMatrices.root!,point(record.landmarks.root!)),bound=scaleLength*.08;
   const reach=activeChains.map((c,i)=>{const h=transformPoint(baselineMatrices[c.hip]!,c.root),lengths=rigid[i]!.lengths,target=contacts[i]!.paintedTarget;return {center:{x:target.x-h.x,y:target.y-h.y},min:Math.abs(lengths.upper-lengths.lower),max:lengths.upper+lengths.lower};});
   const candidates=planarContactCandidates(reach,bound),attempts:Array<{translation:{x:number;y:number};reason:string}>=[];
   for(const translation of candidates){restore();try{
    const norm=Math.hypot(translation.x,translation.y);if(translation.y<0||norm>bound)throw Error('Contact: planar root exceeds accommodation bound');
    pose.root={rotation:0,...pose.root,dx:(pose.root?.dx??0)+translation.x/program.bodyLength,dy:(pose.root?.dy??0)+translation.y/program.bodyLength};
    const matrices=program.evaluate(pose),actualRoot=transformPoint(matrices.root!,point(record.landmarks.root!)),actual={x:actualRoot.x-baselineRoot.x,y:actualRoot.y-baselineRoot.y},actualNorm=Math.hypot(actual.x,actual.y);
    if(!Number.isFinite(actualNorm)||actual.y<0||actualNorm>bound)throw Error('Contact: transformed planar root exceeds accommodation bound');
    for(let i=0;i<activeChains.length;i++){const c=activeChains[i]!,contact=contacts[i]!,parent=matrices[c.hip]!,root=transformPoint(parent,c.root),solved=rigid[i]!.solve(root,contact.paintedTarget);
     const upper=wrapped(angle(solved.root,solved.joint)-angle(c.root,c.joint)),lower=wrapped(angle(solved.joint,solved.end)-angle(c.joint,c.support));
     pose[c.knee]={rotation:wrapped(upper-Math.atan2(parent[1],parent[0]))};pose[c.end]={rotation:wrapped(lower-upper)};if(c.terminal)pose[c.terminal]={rotation:wrapped(-lower)};
    ...
    final=program.evaluate(pose);const measured=measure();if(measured.maxPaintTargetErrorPx>.25)throw Error('Contact: painted support iteration residual '+measured.maxPaintTargetErrorPx);
    compression=actual.y;rootAccommodation={model:'planar',requestedTranslation:{...translation},translation:actual,norm:actualNorm,attemptedCandidates:attempts.length+1};return measured;
   }catch(error){attempts.push({translation:{...translation},reason:String(error)});}}
   restore();throw Object.assign(Error('Contact: no sampled admissible planar root candidate'),{attempts});
  };
  ...
  if((iterativeFailure!==undefined||measured.maxPaintTargetErrorPx>.25)&&rigidEligible){try{
   ...
   if(planarBaseline&&measured.maxPaintTargetErrorPx>.25)throw Error('Contact: painted support iteration residual '+measured.maxPaintTargetErrorPx);
  }catch(error){
   if(planarBaseline){try{measured=attemptPlanar();}catch(planarFailure){if(error instanceof Error)error.cause=planarFailure;if(iterativeFailure!==undefined){if(iterativeFailure instanceof Error)iterativeFailure.cause=error;throw iterativeFailure;}throw error;}}
   else{if(iterativeFailure!==undefined){if(iterativeFailure instanceof Error)iterativeFailure.cause=error;throw iterativeFailure;}throw error;}
  }}
  ...
  return {pose,contacts,maxError:measured.maxError,compression,maxPaintTargetErrorPx:measured.maxPaintTargetErrorPx,...rootAccommodation?{rootAccommodation}:{}};
 }};
}
