...
try {
 ...
 for(const size of [132,300,440]){
  ...
  await wait(`[...document.querySelectorAll('.p-native-pair img')].length===2 && [...document.querySelectorAll('.p-native-pair img')].every(i=>i.dataset.pilotSpecimen==='wolf'&&i.dataset.pilotSize==='${size}'&&i.complete&&i.naturalWidth===${size})`);
  const state=await evaluate(`(async()=>{const i=document.querySelector('.p-native-pair img'),c=document.createElement('canvas');c.width=i.naturalWidth;c.height=i.naturalHeight;const ctx=c.getContext('2d');ctx.drawImage(i,0,0);const bytes=ctx.getImageData(0,0,c.width,c.height).data;const hash=await crypto.subtle.digest('SHA-256',bytes);return {visualKey:i.dataset.visualKey,width:c.width,height:c.height,pixelsSha256:[...new Uint8Array(hash)].map(b=>b.toString(16).padStart(2,'0')).join(''),png:c.toDataURL('image/png').split(',')[1]};})()`);
