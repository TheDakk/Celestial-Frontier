...
export function createArapScratch(vertices,triangles,width,height,options={}){
 ...
 const neighbours=new Uint32Array(starts[n]),deltas=new Float64Array(starts[n]*2);
 for(let i=0;i<n;i++){let k=starts[i];for(const j of [...adj[i]].sort((a,b)=>a-b)){neighbours[k]=j;deltas[k*2]=rest[i*2]-rest[j*2];deltas[k*2+1]=rest[i*2+1]-rest[j*2+1];k++;}}
 ...
 return {n,width,height,rest,starts,neighbours,deltas,pins,iterations,globalIterations,targetWeight,axis,triangles:Uint32Array.from(triangles),areas,orientationIterations,minimumAreaRatio,
...
export function solveArapSkin(s,targets,output){
 ...
 const {n,width,height,rest,position:p,target:t,starts,neighbours,deltas,rotation:r,rhs,pins}=s;
 ...
 if(rigid)for(let k=0;k<s.triangles.length;k+=3){const a=s.triangles[k]*2,b=s.triangles[k+1]*2,c=s.triangles[k+2]*2,ratio=((t[b]-t[a])*(t[c+1]-t[a+1])-(t[b+1]-t[a+1])*(t[c]-t[a]))/s.areas[k/3];if(!Number.isFinite(ratio)||ratio<=0){rigid=false;break;}}
 ...
 for(let pass=0;pass<s.iterations;pass++){
  for(let i=0;i<n;i++){let dot=0,cross=0;const ix=p[i*2],iy=p[i*2+1];
   for(let k=starts[i];k<starts[i+1];k++){const j=neighbours[k]*2,px=ix-p[j],py=iy-p[j+1],x=deltas[k*2],y=deltas[k*2+1];dot+=x*px+y*py;cross+=x*py-y*px;}
   const length=Math.hypot(dot,cross);r[i*2]=length>1e-12?dot/length:1;r[i*2+1]=length>1e-12?cross/length:0;
  }
  for(let i=0;i<n;i++){const lambda=(starts[i+1]-starts[i])*s.targetWeight;let x=lambda*t[i*2],y=lambda*t[i*2+1];
   for(let k=starts[i];k<starts[i+1];k++){const j=neighbours[k]*2,c=(r[i*2]+r[j])*.5,q=(r[i*2+1]+r[j+1])*.5,dx=deltas[k*2],dy=deltas[k*2+1];x+=c*dx-q*dy;y+=q*dx+c*dy;}
   rhs[i*2]=x;rhs[i*2+1]=y;
  }
  ...
  for(let sweep=0;sweep<s.globalIterations;sweep++)for(let direction=0;direction<2;direction++)for(let step=0;step<n;step++){
   const i=direction?n-1-step:step,ii=i*2;
   if(pins[i]){p[ii]=t[ii];p[ii+1]=t[ii+1];continue;}
   let x=rhs[ii],y=rhs[ii+1];for(let k=starts[i];k<starts[i+1];k++){const j=neighbours[k]*2;x+=p[j];y+=p[j+1];}
   const divisor=(starts[i+1]-starts[i])*(1+s.targetWeight);p[ii]=x/divisor;p[ii+1]=y/divisor;
  }
 }
 ...
 for(let pass=0;pass<s.orientationIterations;pass++){
  let changed=0;for(let k=0;k<s.triangles.length;k+=3){
   const ai=s.triangles[k],bi=s.triangles[k+1],ci=s.triangles[k+2],a=ai*2,b=bi*2,c=ci*2,original=s.areas[k/3],sign=Math.sign(original);
   const area=(p[b]-p[a])*(p[c+1]-p[a+1])-(p[b+1]-p[a+1])*(p[c]-p[a]),constraint=area*sign-Math.abs(original)*s.minimumAreaRatio;
   if(constraint>=0)continue;changed++;
   const ax=p[b+1]-p[c+1],ay=p[c]-p[b],bx=p[c+1]-p[a+1],by=p[a]-p[c],cx=p[a+1]-p[b+1],cy=p[b]-p[a];
   const norm=(pins[ai]?0:ax*ax+ay*ay)+(pins[bi]?0:bx*bx+by*by)+(pins[ci]?0:cx*cx+cy*cy);if(norm<1e-20)continue;
   const scale=-constraint*sign/norm;if(!pins[ai]){p[a]+=scale*ax;p[a+1]+=scale*ay;}if(!pins[bi]){p[b]+=scale*bx;p[b+1]+=scale*by;}if(!pins[ci]){p[c]+=scale*cx;p[c+1]+=scale*cy;}
  }
  orientationPasses=pass+1;if(!changed)break;
 }
 let flipped=0,minRatio=Infinity;for(let k=0;k<s.triangles.length;k+=3){const a=s.triangles[k]*2,b=s.triangles[k+1]*2,c=s.triangles[k+2]*2,ratio=((p[b]-p[a])*(p[c+1]-p[a+1])-(p[b+1]-p[a+1])*(p[c]-p[a]))/s.areas[k/3];if(!Number.isFinite(ratio)||ratio<=0)flipped++;minRatio=Math.min(minRatio,ratio);}
