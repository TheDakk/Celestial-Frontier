...
import{createWasmArapSweep}from'./wasm-arap-sweep.mjs';
...
export function createArapScratch(vertices,triangles,width,height,options={}){
 ...
 for(let row=0;row<freeDofs.length;row++){const ii=freeDofs[row],i=ii/2,start=starts[i],degree=starts[i+1]-start,base=row*11;solveRows[base]=ii;solveRows[base+1]=degree;solveRows[base+2]=start;solveDivisors[row]=divisor[i];solveReciprocals[row]=1/divisor[i];for(let k=0;k<Math.min(8,degree);k++)solveRows[base+3+k]=neighbourDofs[start+k];}
 ...
 const sweepKernel=createWasmArapSweep(n*2,solveRows,neighbourDofs,solveReciprocals);
 return {sweepBackend:sweepKernel?'wasm':'js',sweepKernel,n,width,height,rest,starts,neighbours,neighbourDofs,deltas,pins,freeDofs,lambda,divisor,solveRows,solveDivisors,solveReciprocals,triangleDofs,triangleSigns,triangleFloors,triangleMovable,iterations,globalIterations,targetWeight,axis,triangles:Uint32Array.from(triangles),areas,orientationIterations,minimumAreaRatio,
  position:sweepKernel?.position??new Float64Array(n*2),target:new Float64Array(n*2),rotation:new Float64Array(n*2),rhs:sweepKernel?.rhs??new Float64Array(n*2),
...
export function solveArapSkin(s,targets,output){
 ...
 for(let pass=0;pass<s.iterations;pass++){
  for(let i=0;i<n;i++){let dot=0,cross=0;const ix=p[i*2],iy=p[i*2+1];
   ...
   // The ordinary finite range needs no scaling. Extremes and underflow retain
   // the robust norm; this changes rounding, never iteration or motion budgets.
   const squared=dot*dot+cross*cross,length=squared>=1e-200&&squared<=1e200?Math.sqrt(squared):Math.hypot(dot,cross);r[i*2]=length>1e-12?dot/length:1;r[i*2+1]=length>1e-12?cross/length:0;
  }
  ...
  if(s.sweepKernel)s.sweepKernel.run(s.globalIterations);else for(let sweep=0;sweep<s.globalIterations;sweep++){
   for(let step=0;step<freeDofs.length;step++){const base=step*11,ii=solveRows[base],degree=solveRows[base+1];let x=rhs[ii],y=rhs[ii+1],k=0;
    ...
    const start=solveRows[base+2];for(;k<degree;k++){const j=neighbourDofs[start+k];x+=p[j];y+=p[j+1];}p[ii]=x*solveReciprocals[step];p[ii+1]=y*solveReciprocals[step];
   }
