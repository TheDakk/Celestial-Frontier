...
export function createArapScratch(vertices,triangles,width,height,options={}){
 ...
 return {n,width,height,rest,starts,neighbours,neighbourDofs,deltas,pins,freeDofs,lambda,divisor,triangleDofs,triangleSigns,triangleFloors,triangleMovable,iterations,globalIterations,targetWeight,axis,triangles:Uint32Array.from(triangles),areas,orientationIterations,minimumAreaRatio,
...
export function solveArapSkin(s,targets,output){
 ...
 for(let pass=0;pass<s.iterations;pass++){
  ...
  for(let sweep=0;sweep<s.globalIterations;sweep++){
   for(let step=0;step<freeDofs.length;step++){const ii=freeDofs[step],i=ii/2;let x=rhs[ii],y=rhs[ii+1];
    const end=starts[i+1];let k=starts[i];
    for(;k+3<end;k+=4){const a=neighbourDofs[k],b=neighbourDofs[k+1],c=neighbourDofs[k+2],d=neighbourDofs[k+3];x+=p[a];y+=p[a+1];x+=p[b];y+=p[b+1];x+=p[c];y+=p[c+1];x+=p[d];y+=p[d+1];}
    for(;k<end;k++){const j=neighbourDofs[k];x+=p[j];y+=p[j+1];}p[ii]=x/divisor[i];p[ii+1]=y/divisor[i];
   }
   for(let step=freeDofs.length-1;step>=0;step--){const ii=freeDofs[step],i=ii/2;let x=rhs[ii],y=rhs[ii+1];
    const end=starts[i+1];let k=starts[i];
    for(;k+3<end;k+=4){const a=neighbourDofs[k],b=neighbourDofs[k+1],c=neighbourDofs[k+2],d=neighbourDofs[k+3];x+=p[a];y+=p[a+1];x+=p[b];y+=p[b+1];x+=p[c];y+=p[c+1];x+=p[d];y+=p[d+1];}
    for(;k<end;k++){const j=neighbourDofs[k];x+=p[j];y+=p[j+1];}p[ii]=x/divisor[i];p[ii+1]=y/divisor[i];
   }
