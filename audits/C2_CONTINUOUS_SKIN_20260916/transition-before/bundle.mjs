...
function createProbe(record, skin, plans) {
	...
	return {
		sample(ms) {
			const rev = ms >= 5e3, t = rev ? ms - 5e3 : ms, plan = plans[rev ? 1 : 0], planted = rev || t < plan.beats.commandEnd || t >= plan.beats.returnEnd;
			const raw = structuredClone(sampler.sample(plan, t, rev)), resolved = solver.resolve(raw, planted);
			return {
				ms,
				planted,
				raw,
