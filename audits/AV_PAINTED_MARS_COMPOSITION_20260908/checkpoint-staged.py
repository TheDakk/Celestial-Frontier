...
for file in manifest['files']:
    ...
    verified+=1
subprocess.run(['git','diff','--cached','--check','--','.',':(exclude)audits/**'],cwd=r,check=True)
...
prior=[]
for name in ['painted-mars','earth-material','earth-turn','painted-direction','charm-study']:
...
p=r/'port/v2/apps/game/smoke/painted-mars-composition-staged-20260908.patch.gz';jpath=p.with_name('painted-mars-composition-staged-20260908.json')
...
'paintedMarsCompositionManifestSha256':sha((a/'manifest.json').read_bytes()),'manifestCarriersVerified':verified,
'codeWhitespace':'PASS excluding immutable audit logs','unstagedTracked':unstaged,'untracked':untracked,'priorRecoveriesVerified':prior,
...
'next':'Continue approved painted direction with a complete canonical creature or inhabited scene; preserve Earth identity and current UI layout. Codex owns openai/mac. Claude need not open or sync now. No GitHub step/PR or release.'}
