...
// node audits/AV_PAINTED_MARS_COMPOSITION_20260908/native-composition-waveoff-runner.mjs DIST FRESH_OUTPUT phone|desktop|blocked|default
...
const report = { schema: 'cf-native-painted-mars-composition-waveoff/v1', certification: false, status: 'RUNNING',
  ...
  limitations: [
    ...
    'At most one further native Land is permitted after a newly observed exact durable Mars wave-off and a 100% learned-approach CTA. This does not reconstruct the earlier failed run witness.',
...
const sourceNames = [
  ...
  'audits/AV_PAINTED_MARS_COMPOSITION_20260908/native-composition-waveoff-runner.mjs',
...
try {
  ...
  const landReceipt = (before,after) => {
    ...
    assert.equal(facts.worldKey,address);assert.equal(facts.planetSeed,134);assert.equal(facts.planetOrdinal,3);
    assert.equal(facts.receiptOrdinal,br.ordinal);assert.equal(facts.training,false);
    assert(['landed','wave-off'].includes(d.kind),'Refused or unknown landing result');
    assert.equal(d.policy.key,address);assert.equal(d.policy.planetType,'desert');assert.equal(d.policy.biomeKey,'dunesea');
    assert.equal(d.policy.successPercent,before.cta.success);assert.equal(facts.descentWeather,null);
    assert.equal(d.drawsConsumed,2);assert.equal(ar.ordinal,br.ordinal+2);
    const draws={...br.draws};for(const domain of ['descent.success','descent.damage'])draws[domain]=(draws[domain]??0)+1;
    assert.deepEqual(ar.draws,draws,'Land consumed other or repeated random draws');
  ...
  const landMars = async () => {
    const before=await landCheckpoint('Mars Land predecessor');
    let result=await nativeLandAttempt(before,'Mars first native Land');
    if(result.facts.descent.kind==='wave-off'){
      const after=result.after,d=result.facts.descent;
      assert.equal(d.navigation,'orbit');assert.equal(d.persistenceOutcome,'failure');
      assert.equal(d.waveOffCountBefore,0);assert.equal(d.waveOffCountAfter,1);
      assert.equal(d.rawDamage,2);assert.equal(d.gearAdjustedDamage,2);assert.equal(d.damage,Math.min(2,d.hpBefore-1));
      assert.equal(result.facts.permanentLanding,false);assert.equal(result.facts.identityLandedAfter,false);
      assert.deepEqual(after.state.save,before.state.save,'Wave-off granted rewards or changed the saved route');
      assert.equal(after.state.mode,'system');assert.equal(after.state.navWorldKey,null);
      assert.equal(after.state.planet,null);assert.equal(after.state.planetOrdinal,null);
      assert.deepEqual([after.state.gal,after.state.galX,after.state.galY,after.state.star,after.state.starX,after.state.starY],[999,90,-60,424242,560,170]);
      assert.deepEqual(after.state.renderedScene,before.state.renderedScene);
      assert.equal(after.state.cardOpen,true);
      const priorCarrier=before.raw.catalogRow.extensions['descent.wave-offs'];
      const prior=priorCarrier?JSON.parse(priorCarrier.json):{schema:'cf-v2-descent-wave-offs/v1',version:1,records:[],unresolved:[...before.raw.legacy.wvo].sort(([a],[b])=>a-b)};
      assert(!prior.records.some(([key])=>key===address));assert(!prior.unresolved.some(([seed])=>seed===134));
      const carrier=after.raw.catalogRow.extensions['descent.wave-offs'];assert.equal(carrier.version,1);
      assert.deepEqual(JSON.parse(carrier.json),{...prior,records:[...prior.records,[address,1]].sort(([a],[b])=>a<b?-1:a>b?1:0)});
      assert.deepEqual(after.raw.legacy.wvo,[...new Map([...before.raw.legacy.wvo,[134,1]])].sort(([a],[b])=>a-b));
      assert.deepEqual(after.raw.catalogRow.data.wvo,after.raw.legacy.wvo);
      assert.equal(after.cta?.world,address);assert.equal(after.cta.success,100);
      assert.equal(Math.min(100,d.policy.successPercent+20),100);
      after.learnedApproachAdmitted=true;persist();
      result=await nativeLandAttempt(after,'Mars one learned native Land');
      assert.equal(result.facts.descent.policy.successPercent,100);
      assert.equal(result.facts.descent.waveOffCountBefore,1);
    }
