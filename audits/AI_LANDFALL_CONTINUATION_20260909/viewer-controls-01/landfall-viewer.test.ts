...
import { createLandfallViewerV1 } from '../apps/game/src/landfall-viewer.js';
...
describe('retained painting viewer owner; explicit DOM/decoder doubles, no native-layout claim', () => {
  ...
  it('owns late/mutated background roots for the entire lifetime and restores exact prior attributes', async () => {
    ...
    const prior = [main.outerHTML, aside.outerHTML]; await h.viewer.open(original());
    ...
    expect([main.outerHTML, aside.outerHTML]).toEqual(prior); expect(late.inert).toBe(false); expect(late.getAttribute('aria-hidden')).toBe('false');
