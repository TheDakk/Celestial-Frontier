...
try {
  ...
  if(hasTransparent){
    ...
    for(const c of corrections){
      ...
      const region=c.selectionRegionPxInclusive??c.boundsPxInclusive,selected=queue.filter(i=>{const x=i%info.width,y=Math.floor(i/info.width);return x>=region[0]&&x<=region[2]&&y>=region[1]&&y<=region[3];});
