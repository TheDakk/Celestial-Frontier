...
const base=path.join(root,'audits/ARCHETYPE_REPAIRS_20260922/03-biped-bird');
const output=path.join(base,'fold-diagnosis-09.json');
...
const record=read('fit-09/record.json'),binding=read('fit-09/binding.json'),report=read('static-10.json');
if(report.recordRecipeHash!==record.recipeHash||report.bindingHash!==binding.bindingHash)throw Error('Recorded failure authority does not match fit-09');
...
const rows=report.rows.filter(row=>row?.id==='melee:kick'&&row.firstRefusal);
if(rows.length!==1)throw Error('Exactly one stored failed kick pose required');
if(rows[0].firstRefusal.ms!==302.75555555555553||rows[0].firstRefusal.arapStats.flippedTriangles!==13)throw Error('Unexpected recorded failed-pose identity');
const findings=[];
for(const row of rows){
 ...
 const foldedPoints=folded.flatMap(f=>f.vertices.map(v=>v.restPx));
 const sourceBounds=foldedPoints.length?[Math.min(...foldedPoints.map(p=>p[0])),Math.min(...foldedPoints.map(p=>p[1])),Math.max(...foldedPoints.map(p=>p[0])),Math.max(...foldedPoints.map(p=>p[1]))]:null;
 const reproduced=error===refusal.error.split('\n')[0]&&scratch.stats.flippedTriangles===refusal.arapStats.flippedTriangles&&folded.length===13&&published.every(v=>v===123);
 findings.push({row:row.id,recordedMs:refusal.ms,recordedError:refusal.error.split('\n')[0],replayedError:error,reproduced,stats:{...scratch.stats},outputUnpublished:published.every(v=>v===123),sourceBounds,partCounts,folded});
}
