/** Candidate10 retains fine24/56 mesh around the observed tail; global40/80 elsewhere. Rectangle explicitly authored from tail cutout[29,744,599,205] plus collar margin after retained13-tail-fold diagnosis. */
/** Candidate09 changes only authored mesh sampling to40/80. All source pixels, masks, declaration, landmark and fixed/contact/shape policies retained. Shared writers, family contracts and numerical gates unchanged. */
...
try {
  ...
  if(hasTransparent){
    ...
    // Explicit manually declared ownership repairs, never presence inference or source-pixel edits.
    const corrections=(author.labelOwnershipCorrections??[]).map(c=>{
      const from=parts.findIndex(p=>p.id===c.fromPart),to=parts.findIndex(p=>p.id===c.toPart),box=c.boundsPxInclusive;
      need(from>=0&&to>=0&&from!==to&&Array.isArray(box)&&box.length===4&&box.every(Number.isInteger)&&box[0]>=0&&box[1]>=0&&box[2]<info.width&&box[3]<info.height&&box[0]<=box[2]&&box[1]<=box[3]&&Number.isInteger(c.expectedPixels)&&c.expectedPixels>0&&typeof c.basis==='string','explicit ownership correction');
      need([4,8].includes(c.connectivity)&&Array.isArray(c.seedPx)&&c.seedPx.length===2&&c.seedPx.every(Number.isInteger),'explicit connected seed');
      if(c.selectionRegionPxInclusive)need(Array.isArray(c.selectionRegionPxInclusive)&&c.selectionRegionPxInclusive.length===4&&c.selectionRegionPxInclusive.every(Number.isInteger)&&c.selectionRegionPxInclusive[0]>=box[0]&&c.selectionRegionPxInclusive[1]>=box[1]&&c.selectionRegionPxInclusive[2]<=box[2]&&c.selectionRegionPxInclusive[3]<=box[3]&&c.selectionRegionPxInclusive[0]<=c.selectionRegionPxInclusive[2]&&c.selectionRegionPxInclusive[1]<=c.selectionRegionPxInclusive[3],'explicit selection region');
      if(c.selectionPolygonPx)need(Array.isArray(c.selectionPolygonPx)&&c.selectionPolygonPx.length>=3&&c.selectionPolygonPx.every(p=>Array.isArray(p)&&p.length===2&&p.every(Number.isFinite)),'explicit manual selection polygon');
      return {...c,from,to,observedPixels:0};
    });
    ...
    const sourceLabels=Buffer.from(labels),claimed=new Set();
    for(const c of corrections){
      const [x0,y0,x1,y1]=c.boundsPxInclusive,[sx,sy]=c.seedPx;
      need(sx>=x0&&sx<=x1&&sy>=y0&&sy<=y1&&sourceLabels[(sy*info.width+sx)*4]===c.from+1,'seed must belong to declared source part');
      const queue=[sy*info.width+sx],seen=new Set(queue);
      for(let head=0;head<queue.length;head++){const i=queue[head],x=i%info.width,y=Math.floor(i/info.width);for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){const xx=x+dx,yy=y+dy,n=yy*info.width+xx;if((dx||dy)&&(c.connectivity===8||!dx||!dy)&&xx>=x0&&xx<=x1&&yy>=y0&&yy<=y1&&!seen.has(n)&&sourceLabels[n*4]===c.from+1){seen.add(n);queue.push(n);}}}
      const region=c.selectionRegionPxInclusive??c.boundsPxInclusive,selected=queue.filter(i=>{const x=i%info.width,y=Math.floor(i/info.width);return x>=region[0]&&x<=region[2]&&y>=region[1]&&y<=region[3]&&(!c.selectionPolygonPx||inside(x+.5,y+.5,c.selectionPolygonPx));});
      c.observedPixels=selected.length;need(c.observedPixels===c.expectedPixels,'ownership correction count for '+c.toPart+': '+c.observedPixels);
      for(const i of selected){need(!claimed.has(i),'overlapping explicit ownership corrections');claimed.add(i);labels.set([c.to+1,c.to+1,c.to+1,255],i*4);counts[c.from]--;counts[c.to]++;}
    }
    provenance.explicitOwnershipCorrections=corrections;
  ...
  const shapeJoints=[...new Set(parts.filter(p=>p.joint!=='root').map(p=>p.joint))];
