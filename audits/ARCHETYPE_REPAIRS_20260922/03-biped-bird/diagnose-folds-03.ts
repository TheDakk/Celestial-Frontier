...
const output=path.join(base,'fold-diagnosis-03.json');
...
const record=read('fit-03/record.json'),binding=read('fit-03/binding.json'),report=read('static-03.json');
...
const rows=[report.presentation].filter(row=>row?.id==='presentation'&&row.firstRefusal);
if(rows.length!==1)throw Error('Exactly one failed presentation pose required');
const findings=[];
