...
// node audits/STATIC_LANDING_PORTRAIT_20260908/native-landing-still-v2-chain.mjs DIST FRESH_OUTPUT_ROOT
...
const observer=path.join(repo,'audits/STATIC_LANDING_PORTRAIT_20260908/native-landing-still-v2-runner.mjs');
...
const report={schema:'cf-static-earth-landing-native-chain/v2',certification:false,status:'RUNNING',
...
try{
  ...
  for(const mode of ['desktop','phone','blocked','default']){
    ...
    assert.equal(review.schema,'cf-native-earth-landing-still/v2');assert.equal(review.mode,mode);assert.equal(review.status,'PASS');
