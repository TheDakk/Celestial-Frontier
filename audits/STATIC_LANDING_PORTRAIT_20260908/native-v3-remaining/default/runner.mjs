...
// node audits/STATIC_LANDING_PORTRAIT_20260908/native-landing-still-v3-runner.mjs SAME_FROZEN_DIST FRESH_OUTPUT blocked|default
const repo = fs.realpathSync(fileURLToPath(new URL('../..', import.meta.url)));
assert.equal(process.argv.length, 5, 'Expected unchanged evidence-dist, fresh-output and blocked|default only');
const mode = process.argv[4]; assert(['blocked', 'default'].includes(mode), 'Passed desktop/phone modes must not rerun');
...
const report = { schema: 'cf-native-earth-landing-still/v3', certification: false, status: 'RUNNING',
  ...
  predecessor: 'audits/STATIC_LANDING_PORTRAIT_20260908/native-landing-still-v2-runner.mjs',
  serverAssetRequests: [], servedMeaning: 'Content digest inventory of every HTTP server response, across request owners; not a selected-page request trace',
  limitations: [
...
const sourceNames = [
  ...
  'audits/STATIC_LANDING_PORTRAIT_20260908/native-landing-still-v3-runner.mjs',
  'audits/STATIC_LANDING_PORTRAIT_20260908/native-landing-still-v3-remaining-chain.mjs',
  'audits/STATIC_LANDING_PORTRAIT_20260908/native-v2/chain-report.json',
  'audits/STATIC_LANDING_PORTRAIT_20260908/native-v2/desktop/review.json',
  'audits/STATIC_LANDING_PORTRAIT_20260908/native-v2/phone/review.json',
  'audits/STATIC_LANDING_PORTRAIT_20260908/native-v2/blocked/review.json',
...
const server = http.createServer((req, res) => {
  try {
    ...
    if(key===report.asset?.path){
      report.serverAssetRequests.push({method:req.method,path:key,bytes:bytes.length,sha256:digest,
        fetchDestination:req.headers['sec-fetch-dest']??null,fetchMode:req.headers['sec-fetch-mode']??null,
        referer:typeof req.headers.referer==='string'?req.headers.referer.slice(0,256):null,
        role:'unattributed server request; no page-session identity on this channel'});
    }
...
// The page's CDP session and request ID own delivery evidence. The HTTP digest
// inventory can also contain background install/cache requests and cannot prove
// that an intercepted page request escaped to the server.
const acceptPageAssetWitness = (w, expected) => {
  assert.equal(w.artVariant,'canonical-v1');assert.equal(w.mounted,true);
  assert.equal(w.painting,false);assert.equal(w.ownedPaintedCanvases,0);
  assert.equal(w.cacheEntries,1);assert.equal(w.workerStarts,1);
  if(expected.mode==='default'){
    assert.deepEqual(w.network,[]);assert.deepEqual(w.intercepted,[]);
    assert.equal(w.loader,null);assert.equal(w.faults,0);return;
  }
  assert.equal(expected.mode,'blocked');assert.equal(w.faults,1);
  assert.equal(w.loader?.status,'failed');assert.equal(w.loader.error,'painted vista SHA-256 mismatch');
  assert.equal(w.loader.fetchStarts,1);assert.equal(w.loader.canvasPixels,0);
  assert.equal(w.loader.decodePending,false);assert.equal(w.loader.aborted,true);
  const requests=w.network.filter(x=>x.method==='Network.requestWillBeSent');
  const responses=w.network.filter(x=>x.method==='Network.responseReceived');
  const finished=w.network.filter(x=>x.method==='Network.loadingFinished');
  assert.equal(w.network.length,3);assert.equal(requests.length,1);assert.equal(responses.length,1);assert.equal(finished.length,1);
  const id=requests[0].requestId;assert(typeof id==='string'&&id.length>0);
  assert.equal(requests[0].url,expected.url);
  for(const row of w.network){assert.equal(row.sessionId,expected.sessionId);assert.equal(row.requestId,id);}
  assert.equal(responses[0].status,200);
  assert.equal(w.intercepted.length,1);const intercepted=w.intercepted[0];
  assert.equal(intercepted.sessionId,expected.sessionId);assert.equal(intercepted.networkId,id);
  assert.equal(intercepted.url,expected.url);assert.equal(intercepted.status,200);
  assert.equal(intercepted.bytes,expected.bytes);assert.equal(intercepted.sha256,expected.wrongSha256);
  assert.notEqual(intercepted.sha256,expected.correctSha256);
  assert.equal(intercepted.fulfillmentAcknowledged,true);
};
...
try {
  ...
  browser = await openChromiumCdp({ label: 'native static Earth landing ' + mode,
    userDataPrefix: 'cf-earth-still-v3-' + mode + '-20260908', commandTimeoutMs: 15000,
    onEvent: event => {
      ...
      if (event.method === 'Network.requestWillBeSent' && p.request.url === assetUrl) {
        assetRequests.add(p.requestId); report.network.push({ method: event.method, sessionId:event.sessionId, requestId: p.requestId, url: p.request.url, initiator: p.initiator?.type });
      }
      if (['Network.responseReceived', 'Network.loadingFailed', 'Network.loadingFinished'].includes(event.method) && assetRequests.has(p.requestId)) {
        report.network.push({ method: event.method, sessionId:event.sessionId, requestId: p.requestId, status: p.response?.status,
      ...
      if (event.method === 'Fetch.requestPaused') {
        const job = (async () => {
          ...
          const interception={ requestId:p.requestId,sessionId:event.sessionId,networkId:p.networkId,url:p.request.url,
            status:200,bytes:wrongAssetBytes.length,sha256:report.wrongAsset.sha256,fulfillmentAcknowledged:false };
          report.intercepted.push(interception);
          ...
          interception.fulfillmentAcknowledged=true;
  ...
  await Promise.all(interceptionJobs);
  const observedResources=await evaluate(resources);
  const assetWitness={network:structuredClone(report.network),intercepted:structuredClone(report.intercepted),
    artVariant:observedResources.surfaceVistaArtVariant,mounted:observedResources.surfaceVistaMounted,
    painting:observedResources.surfacePaintedComposition,ownedPaintedCanvases:observedResources.surfaceEarthLayeredCanvasCount,
    cacheEntries:observedResources.surfaceVistaCacheEntries,workerStarts:observedResources.surfaceVistaWorkerStarts,
    faults:observedResources.surfaceVistaFaults,loader:observedResources.surfacePaintedVista};
  const expectedTransport=Object.freeze({mode,url:assetUrl,sessionId:pageSession,bytes:expectedAssetBytes,
    wrongSha256:hash(wrongAssetBytes),correctSha256:report.asset.sha256});
  report.pageAssetWitness={expected:expectedTransport,observed:assetWitness,
    method:'Actual page-session Network IDs correlate one acknowledged Fetch fulfillment with exact payload bytes/hash and the independent loader/fallback outcome. Server-wide digest entries are separate inventory.',
    serverInventoryEntry:report.served[report.asset.path]??null,serverRequests:structuredClone(report.serverAssetRequests)};
  persist();acceptPageAssetWitness(assetWitness,expectedTransport);
  const mutants=blocked?[
    ['missing interception',w=>{w.intercepted=[]}],
    ['wrong page session',w=>{w.network[0].sessionId='wrong-page-session'}],
    ['unrelated network ID',w=>{w.intercepted[0].networkId='unrelated-network-id'}],
    ['correct asset instead of wrong payload',w=>{w.intercepted[0].sha256=report.asset.sha256}],
    ['wrong delivered byte count',w=>{w.intercepted[0].bytes++}],
    ['unacknowledged fulfillment',w=>{w.intercepted[0].fulfillmentAcknowledged=false}],
    ['missing completed response',w=>{w.network=w.network.filter(x=>x.method!=='Network.loadingFinished')}],
    ['loader accepted payload',w=>{w.loader.status='ready';w.loader.error=null}],
  ]:[
    ['default made a page asset request',w=>{w.network=[{method:'Network.requestWillBeSent',requestId:'unexpected'}]}],
    ['default started a painted loader',w=>{w.loader={status:'pending',fetchStarts:1}}],
  ];
  report.pageAssetMutationControls=[];
  for(const [name,mutate]of mutants){
    const mutant=structuredClone(assetWitness);mutate(mutant);let rejected=false,diagnosis=null;
    try{acceptPageAssetWitness(mutant,expectedTransport)}catch(error){rejected=true;diagnosis=String(error.message)}
    report.pageAssetMutationControls.push({name,rejected,diagnosis,kind:'same-acceptor mutation of retained observed record; no additional native run'});persist();
    assert(rejected,'Page-asset acceptor missed mutant: '+name);acceptPageAssetWitness(assetWitness,expectedTransport);
  }
