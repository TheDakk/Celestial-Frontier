...
const resultFile=path.join(audit,'integration-static-results.json');
assert.equal(process.argv.length,2); assert(!fs.existsSync(resultFile),'Fresh result required');
const tests=[
  'tests/painted-earth-landing-binding.test.ts', 'tests/painted-earth-mount-ownership.test.ts',
  'apps/game/src/painted-vista-load.test.ts', 'apps/game/src/earth-layered-load.test.ts',
  'apps/game/src/earth-layered-resources.test.ts', 'tests/earth-layered-recipe.test.ts',
  'apps/game/src/earth-layered-layout.test.ts', 'tests/earth-layered-shake-settlement.test.ts',
  'tests/biome-vista-surface.test.ts', 'tests/biome-vista-cache.test.ts', 'tests/scene-texture-owner.test.ts',
];
...
const names=[...new Set([
  ...
  'audits/STATIC_LANDING_PORTRAIT_20260908/integration-static-runner.mjs',
])];
...
function step(id,cmd,args,cwd=root){
  unchanged(); const log=path.join(audit,'integration-static-'+id+'.log'),fd=fs.openSync(log,'wx');
