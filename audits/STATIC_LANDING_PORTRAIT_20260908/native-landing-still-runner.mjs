...
// node audits/STATIC_LANDING_PORTRAIT_20260908/native-landing-still-runner.mjs DIST FRESH_OUTPUT phone|desktop|blocked|default
...
const report = { schema: 'cf-native-earth-landing-still/v1', certification: false, status: 'RUNNING',
  ...
  limitations: [
    ...
    'Blocked control returns the prior valid Earth WebP at only the exact new asset URL; its wrong SHA must reject before canonical fallback.',
...
const wrongAssetBytes = fs.readFileSync(path.join(repo, 'port/v2/apps/game/src/assets/painted/earth-riverbank-v1.webp'));
report.wrongAsset = { path: 'port/v2/apps/game/src/assets/painted/earth-riverbank-v1.webp', bytes: wrongAssetBytes.length, sha256: hash(wrongAssetBytes), purpose: 'Valid prior Earth image, wrong bytes for new still identity' };
...
try {
  ...
  browser = await openChromiumCdp({ label: 'native static Earth landing ' + mode,
    userDataPrefix: 'cf-earth-still-' + mode + '-20260908', commandTimeoutMs: 15000,
    onEvent: event => {
  ...
  const shot = async name => {
    ...
    report.screenshots.push({path:name+'.png',bytes:bytes.length,sha256:hash(bytes)}); persist();
  };
  ...
  if (blocked) {
    ...
    assert(report.network.some(event => event.method === 'Network.responseReceived' && event.status === 200), 'Wrong valid-asset response absent');
