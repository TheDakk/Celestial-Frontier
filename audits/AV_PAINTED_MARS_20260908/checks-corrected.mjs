...
const steps=[
 ['producer-final','node',[path.join(audit,'producer-corrected.mjs')],root],
 ['producer-refresh-final','node',[path.join(audit,'refresh-producer-corrected.mjs')],root],
 ['final-focused','npm',['test','--','apps/game/src/painted-vista-load.test.ts','tests/painted-mars-binding.test.ts','tests/biome-vista-surface.test.ts','tests/biome-vista-cache.test.ts','tests/biome-vista-protocol.test.ts','tests/biome-vista-worker-error.test.ts','tests/pwa-offline.test.ts','tests/current-producer-authorities.test.ts','tests/compendium-budget.test.ts','tests/guide-release.test.ts','tests/slicesmoke-guide-release-scroll-contract.test.ts','tests/slicesmoke-dtrain-release-convergence.test.ts'],v2],
...
for(const [name,cmd,args,cwd]of steps){
 const log=path.join(audit,name+'-corrected.log'),fd=fs.openSync(log,'wx'),start=Date.now();
...
fs.writeFileSync(path.join(audit,'corrected-browser-free.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});
