...
const report=JSON.parse(fs.readFileSync('audits/AV_PAINTED_MARS_20260908/producer.json','utf8'));
...
assert.equal(previous.sha256,'cf8ad1a39c98d1464b34b6ccd69b92bbeb2ea2bf87b43a9be443428ab9a3fd9c');
...
budget.calibration.selectionRule+=' Opt-in exact Mars painted vista (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after adding one exact-request-bound Mars dune-sea asset with bounded verified image loading, native cache/texture ownership and canonical worker fallback. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('audits/AV_PAINTED_MARS_20260908/producer-refresh.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
