...
const report=JSON.parse(fs.readFileSync('audits/AV_PAINTED_MARS_20260908/producer-corrected.json','utf8'));
...
assert.equal(previous.sha256,'12515e894c04c51fa46bf3abcdeccd03861b9230741a1870d34966890b14f605');
...
budget.calibration.selectionRule+=' Opt-in exact Mars painted vista (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after consolidating painted/native request binding into one common environment owner, preserving the existing missing-binding negative control. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('audits/AV_PAINTED_MARS_20260908/producer-refresh-corrected.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
