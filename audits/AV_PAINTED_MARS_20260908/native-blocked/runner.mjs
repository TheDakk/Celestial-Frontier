...
// node audits/AV_PAINTED_MARS_20260908/native-painted-mars-runner.mjs DIST FRESH_OUTPUT phone|desktop|blocked
const repo = fs.realpathSync(fileURLToPath(new URL('../..', import.meta.url)));
assert.equal(process.argv.length, 5, 'Expected evidence-dist, fresh-output and phone|desktop|blocked');
const mode = process.argv[4]; assert(['phone', 'desktop', 'blocked'].includes(mode));
...
const report = { schema: 'cf-native-painted-mars/v1', certification: false, status: 'RUNNING',
...
try {
  ...
  browser = await openChromiumCdp({ label: 'native painted Mars ' + mode,
    userDataPrefix: 'cf-painted-mars-' + mode + '-20260908', commandTimeoutMs: 15000,
    onEvent: event => {
  ...
  const checkVista = async (name, initial = false) => {
    ...
    assert.equal(r.surfaceVistaArtVariant, blocked ? 'canonical-v1' : variant);
    ...
    assert.equal(r.surfaceVistaWorkerStarts, blocked ? 1 : 0); assert.equal(r.surfaceVistaFaults, blocked ? 1 : 0);
  ...
  const capturePaint = async name => {
    ...
    try {
      const control = await evaluate(`(()=>{
        const s=${S},r=s.app.renderer,m=${actualVista},canvas=m.texture.source.resource;
        const q={sprite:m,texture:m.texture,source:m.texture.source,canvas,parent:m.parent,index:m.parent.getChildIndex(m),
        ...
        q.restore=()=>{const errors=[],attempt=f=>{try{f()}catch(e){errors.push(String(e))}};
          attempt(()=>{m.visible=q.visible});attempt(()=>{r.render(s.app.stage)});
          attempt(()=>{if(q.started)s.app.ticker.start();else s.app.ticker.stop()});
          return{errors,visible:m.visible,tickerStarted:s.app.ticker.started,originalTicker:q.started}};
        ...
        return{method:'One actual vista visible/hidden/restored; app ticker paused; explicit unchanged full-stage frame; warm owned renderer pools',
          ...
          identities:q.identities,pools:pools(),eventMode:m.eventMode,tickerStopped:!s.app.ticker.started};
      })()`);
      ...
      try {
        ...
        if ('visible' in restored) assert(restored.visible && restored.tickerStarted === restored.originalTicker);
  ...
  const exitMars = async () => {
    ...
    assert.equal(r.surfaceVistaFaults, blocked ? 1 : 0); assert.equal(r.surfaceVistaWorkerStarts, blocked ? 1 : 0);
    checkRegistry(r.registry); await evaluate('delete window.__cfPaintedMarsRetired;true');
  };
  await viewport(...dimensions);
  await send('Page.navigate', { url: report.origin + '/?paintedvista=1' });
  ...
  await wait(blocked ? 'canonical fallback after controlled asset failure' : 'painted Mars ready', `${S}.api.state().mode==='surface'&&${resources}.surfaceVistaMounted&&${resources}.surfaceVistaArtVariant===${JSON.stringify(blocked ? 'canonical-v1' : variant)}&&${resources}.surfacePaintedVista?.status===${JSON.stringify(blocked ? 'failed' : 'ready')}&&${ready}`);
  ...
  assert.equal(assetRequests.size, 1, 'Expected one observed page request for Mars asset');
  if (blocked) {
  ...
  await exitMars(); guard(); assert.equal(assetRequests.size, 1, 'Asset fetch repeated'); report.status = 'PASS';
} catch (error) {
