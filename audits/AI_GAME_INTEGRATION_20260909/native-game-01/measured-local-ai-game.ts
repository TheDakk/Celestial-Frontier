...
const button = (action: string, label: string, jobId = ''): string => `<button type="button" data-ai-act="${action}" data-ai-job="${esc(jobId)}" style="min-height:44px;padding:8px 12px">${label}</button>`;
...
export async function createLocalAiGameV1(options: LocalAiGameOptionsV1): Promise<LocalAiGameV1> {
  ...
  const rowHtml = (job: AiLandfallJobV1): string => {
    ...
    return `<div data-ai-landfall-job="${job.jobId}" style="flex-basis:100%;min-width:0;padding:8px 0"><strong>${active ? 'Landing' : job.status === 'ready' ? 'Landfall ready' : 'Landfall ' + esc(job.status)}</strong>`
      + (active ? `<div>${esc(job.progress.phase)} · ${eta}</div><progress aria-label="Landing work completed" max="100" value="${job.progress.completed}" style="width:100%"></progress>${job.status !== 'canceling' ? button('cancel', 'Cancel painting', job.jobId) : ''}`
  ...
  const api: LocalAiGameV1 = {
    ...
    async action(action, jobId) {
      ...
      } else if (action === 'retry' && job && ['failed', 'canceled'].includes(job.status)) api.enqueue(job.input);
