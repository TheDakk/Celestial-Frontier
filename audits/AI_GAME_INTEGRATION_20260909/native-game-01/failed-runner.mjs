...
try{
  ...
  await until('small viewport settled','innerWidth===320&&innerHeight===568&&window.__CF_SLICE__.api.state().viewportWidth===320');
  receipt.smallViewport=await evaluate(`(()=>{const s=window.__CF_SLICE__.api.state();return {width:innerWidth,height:innerHeight,
    scrollWidth:document.documentElement.scrollWidth,scrollHeight:document.documentElement.scrollHeight,
    mounted:s.localAi.mounted,mode:s.mode,sceneResources:s.sceneResources};})()`);
  need(receipt.smallViewport.mounted&&receipt.smallViewport.mode==='surface','Small viewport lost retained painting');
