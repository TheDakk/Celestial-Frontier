...
const report={schema:'cf-u3-color-icon-study-browser-export/v1',status:'RUNNING',scope:'Three colorful original SVG concepts beside current emoji at22/32px; standalone review only; product emoji unchanged',htmlSha256:digest(html),startedAt:new Date().toISOString(),errors:[],authority:'Nick explicitly said Please proceed with U2-U4 and requested colorful function-specific icon concepts; one bounded study only.',inkscape:'NOT_USED_STOPPED_FOR_SESSION'};
const output=path.join(here,'color-study.png'),receipt=path.join(here,'browser-export-receipt.json');
...
try{
  ...
  browser=await openChromiumCdp({label:'U3 three-color-icon study export',userDataPrefix:'cf-u3-color-icon-study',onEvent:e=>{if(e.method==='Runtime.exceptionThrown')report.errors.push(e.params.exceptionDetails.exception?.description??e.params.exceptionDetails.text);}});
  ...
  try{
    ...
    while(!await evaluate("document.readyState==='complete'&&document.querySelectorAll('.card').length===3")){assert(Date.now()<until,'standalone document readiness expired');await new Promise(r=>setTimeout(r,25));}
    ...
    report.observation=await evaluate("(()=>({title:document.title,cards:document.querySelectorAll('.card').length,current:[...document.querySelectorAll('.emoji')].map(n=>n.textContent),svg:document.querySelectorAll('svg').length,horizontalOverflow:document.documentElement.scrollWidth>innerWidth,sizes:[...document.querySelectorAll('.samples svg')].map(n=>({width:n.getBoundingClientRect().width,height:n.getBoundingClientRect().height})),height:document.documentElement.scrollHeight}))()");
    assert.equal(report.observation.cards,3);assert.equal(report.observation.svg,6);assert(!report.observation.horizontalOverflow);assert.deepEqual(report.observation.sizes,[22,32,22,32,22,32].map(n=>({width:n,height:n})));
    assert.deepEqual(report.observation.current,['🔭','🔭','🛠','🛠','🌍','🌍']);
    const image=Buffer.from((await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:true,clip:{x:0,y:0,width:1120,height:report.observation.height,scale:1}})).data,'base64');
