...
const masterPaths=new Set([...fs.readFileSync(new URL('../../port/v2/apps/game/src/battle2-master-pins.generated.ts',import.meta.url),'utf8').matchAll(/masterPath: '([^']+)'/g)].map(x=>'/battle2/'+x[1]));
...
const root = path.resolve(pkgArg), out = path.resolve(outArg); if(fs.existsSync(out))throw Error('fresh output required');fs.mkdirSync(out, { recursive: true });
...
const server = http.createServer((req, res) => { let rel = decodeURIComponent(new URL(req.url, 'http://x').pathname); if (rel.endsWith('/')) rel += 'index.html'; const file = path.join(root, rel);
  if(masterPaths.has(rel))masterRequests.push(rel);
...
try {
  ...
  await send('Page.addScriptToEvaluateOnNewDocument',{source:`{const paths=new Set(${JSON.stringify([...masterPaths])});const fetch0=window.fetch;window.fetch=function(input,...args){const u=new URL(typeof input==='string'?input:input instanceof URL?input.href:input.url,location.href);if(paths.has(u.pathname)){const old=JSON.parse(sessionStorage.getItem('cf-c23-master-requests')||'[]');old.push(u.pathname);sessionStorage.setItem('cf-c23-master-requests',JSON.stringify(old));}return fetch0.call(this,input,...args);};}`});
  // every page error (a throw inside a ticker callback stops Pixi's shared ticker, so it must be seen) — reported in the report
  await send('Page.addScriptToEvaluateOnNewDocument', { source: "window.__cfErrors=JSON.parse(sessionStorage.getItem('cf-c23-errors')||'[]');const push0=window.__cfErrors.push;window.__cfErrors.push=function(...args){const n=push0.apply(this,args);sessionStorage.setItem('cf-c23-errors',JSON.stringify(this));return n;};addEventListener('error',e=>window.__cfErrors.push(String(e.error&&e.error.stack||e.message).slice(0,600)));addEventListener('unhandledrejection',e=>window.__cfErrors.push('rejection: '+String(e.reason&&e.reason.stack||e.reason).slice(0,600)));" }); await send('Emulation.setDeviceMetricsOverride', PHONE ? { width: 390, height: 844, deviceScaleFactor: 3, mobile: true } : { width: 1280, height: 800, deviceScaleFactor: 1, mobile: false }); if (PHONE) await send('Emulation.setTouchEmulationEnabled', { enabled: true, maxTouchPoints: 5 });
  ...
  report.runs[1].rigs = await paintedSides();
  if (SW_CONTROL) {
  ...
  report.masterRequests={server:masterRequests,page:await evaluate("JSON.parse(sessionStorage.getItem('cf-c23-master-requests')||'[]')")};
  ...
  report.masterRequests={server:masterRequests,page:await evaluate("JSON.parse(sessionStorage.getItem('cf-c23-master-requests')||'[]')")};
  if(report.masterRequests.server.length||report.masterRequests.page.length)throw Error('C23: master fetch attempted');
  if(report.runs.some(r=>r.rigs?.painted!==2))throw Error('C23: every run must stage two painted rigs');
  report.status = 'PASS';
