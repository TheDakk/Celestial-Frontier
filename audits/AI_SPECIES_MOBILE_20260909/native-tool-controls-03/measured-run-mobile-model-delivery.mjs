...
export async function runMobileModelDelivery({pack,sha256,output}){
  ...
  try{
    ...
    const shaImports=[...deliveryModule.matchAll(/(['"])\.\/local-model-sha256\.js\1/g)];
    need(shaImports.length===1,'Read-only module import rewrite is not exact');
    const shaImport=shaImports[0][0];
    ...
    while(performance.now()<workerDeadline){
      ...
      for(const [id,row] of workers)if(!beforeWorkerTargets.has(id)&&row.info.type==='worker'&&row.sessionId&&row.ready){
