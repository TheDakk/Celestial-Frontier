...
test('read-only diagnostic transformation imports the exact production owner and preserves its parameter-property error code',async()=>{
  const source=await fs.readFile(new URL('../../port/v2/apps/game/src/local-model-delivery.ts',import.meta.url),'utf8');
  const dependency=await fs.readFile(new URL('../../port/v2/apps/game/src/local-model-sha256.ts',import.meta.url),'utf8');
  const result=stripTypeScriptTypes(source,{mode:'transform'}),shaSource=stripTypeScriptTypes(dependency,{mode:'transform'});
  const matches=[...result.matchAll(/(['"])\.\/local-model-sha256\.js\1/g)];assert.equal(matches.length,1);
  const shaUrl='data:text/javascript;base64,'+Buffer.from(shaSource).toString('base64');
  const ownerUrl='data:text/javascript;base64,'+Buffer.from(result.replace(matches[0][0],JSON.stringify(shaUrl))).toString('base64');
