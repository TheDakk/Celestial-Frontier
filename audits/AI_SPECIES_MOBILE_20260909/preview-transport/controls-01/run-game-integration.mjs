...
let output=null,speciesReferences=false;
for(const argument of process.argv.slice(2)){
  if(argument.startsWith('--output=')&&output===null)output=path.resolve(argument.slice(9));
  else if(argument==='--species-references'&&!speciesReferences)speciesReferences=true;
  else throw Error('Usage: run-game-integration.mjs --output=NEW_AUDITS_DIRECTORY [--species-references]');
}
...
try{
  ...
  const species=speciesReferences?await loadSpeciesReferenceSet():null;
  if(species){await collectSource('tools/local-image-generation/species-references.mjs');await collectSource('port/v2/packages/art/src/speciesidentity.ts');
    await collectSource(SPECIES_REFERENCE_SET);
    for(const row of species.files)if(row.source!==binding.image.path)await collectSource(row.source);
    receipt.speciesReferenceSet={manifestSha256:species.manifestSha256,references:species.references};}
  ...
  preview=await createGamePreviewServer({mode:'evidence',reference,
    referenceIdentity:binding.binding.subjectIdentityKey,q8Block32:'auto',speciesReferences});
  ...
  need(recipe.qualityAccepted===false&&recipe.q8Block32===true&&(species
    ? recipe.schema==='cf.ai-landfall-render.v2'&&JSON.stringify(recipe.references)===JSON.stringify(species.references.map(({url,...row})=>row))
    : recipe.reference.sha256===binding.image.sha256&&recipe.reference.speciesVisualKey===binding.binding.subjectIdentityKey),
  ...
  need(workerRequests().length===(species?9:4)&&workerRequests().every(row=>row.status===200),'Expected exactly the declared text/reference/denoise/decode worker requests');
