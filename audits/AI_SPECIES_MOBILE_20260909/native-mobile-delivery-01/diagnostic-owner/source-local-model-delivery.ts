...
export interface LocalModelFileV1 { readonly path: string; readonly bytes: number; readonly sha256: string }
export interface LocalModelManifestV1 {
  readonly schema: 'cf.local-model-delivery-manifest.v1'; readonly modelId: string; readonly revision: string;
  readonly sourceManifestSha256: string; readonly totalBytes: number; readonly files: readonly LocalModelFileV1[];
}
export interface LocalModelWriterV1 {
  write(data: Uint8Array<ArrayBuffer> | string): Promise<void>; close(): Promise<void>; abort(): Promise<void>;
}
export interface LocalModelFileHandleV1 { getFile(): Promise<Blob>; createWritable(): Promise<LocalModelWriterV1> }
export interface LocalModelDirectoryV1 {
  getDirectoryHandle(name: string, options?: { create?: boolean }): Promise<LocalModelDirectoryV1>;
  getFileHandle(name: string, options?: { create?: boolean }): Promise<LocalModelFileHandleV1>;
}
export interface LocalModelStorageV1 {
  getDirectory(): Promise<LocalModelDirectoryV1>;
  estimate(): Promise<{ usage?: number; quota?: number }>;
}
export interface LocalModelLockV1 {
  run<T>(name: string, signal: AbortSignal, action: () => Promise<T>): Promise<T>;
}
export type LocalModelDeliveryPhaseV1 = 'unknown' | 'missing' | 'partial' | 'verifying' | 'downloading'
  | 'ready' | 'canceled' | 'paused' | 'invalid' | 'failed';
export interface LocalModelDeliveryStatusV1 {
  readonly phase: LocalModelDeliveryPhaseV1; readonly ready: boolean; readonly manifestSha256: string;
  readonly totalBytes: number; readonly storedBytes: number; readonly verifiedBytes: number;
  readonly downloadedBytes: number; readonly verifiedFiles: number; readonly totalFiles: number;
  readonly file: string | null; readonly error: string | null; readonly attemptId: string | null;
  readonly qualityAccepted: false; readonly deviceQualified: false;
}
export interface LocalModelDeliveryOptionsV1 {
  readonly manifest: LocalModelManifestV1; readonly baseUrl?: string; readonly storage?: LocalModelStorageV1;
  readonly locks?: LocalModelLockV1; readonly fetch?: typeof fetch; readonly headroomBytes?: number;
  readonly fileTimeoutMs?: number; readonly createAttemptId?: () => string;
  readonly onStatus?: (status: LocalModelDeliveryStatusV1) => void;
}
export interface LocalModelDeliveryV1 {
  readonly manifest: LocalModelManifestV1; readonly manifestSha256: string; readonly baseUrl: string;
  status(): LocalModelDeliveryStatusV1;
  install(options?: { signal?: AbortSignal; restart?: boolean }): Promise<LocalModelDeliveryStatusV1>;
  verify(options?: { signal?: AbortSignal }): Promise<LocalModelDeliveryStatusV1>;
  /** Only after this instance verified readiness. Composes immutable OPFS Files;
   * delivery never materializes a whole multi-GB file as a JS ArrayBuffer. */
  openFile(path: string): Promise<Blob>;
}
class DeliveryError extends Error {
  constructor(readonly code: string, message = code) { super(message); }
}
const requireValue = (condition: unknown, code: string): void => { if (!condition) throw new DeliveryError(code); };
const count = (value: number): boolean => Number.isSafeInteger(value) && value >= 0;
const digest = (value: string): string => new LocalModelSha256V1().update(new TextEncoder().encode(value)).digestHex();
const notFound = (error: unknown): boolean => error instanceof Error && error.name === 'NotFoundError';
const canceled = (signal: AbortSignal): void => { if (signal.aborted) throw new DeliveryError('canceled'); };
function normalizeManifest(source: LocalModelManifestV1): LocalModelManifestV1 {
  ...
  const files = source.files.map(file => {
    ...
    paths.add(file.path); return Object.freeze({ path: file.path, bytes: file.bytes, sha256: file.sha256 });
  });
...
async function maybeFile(directory: LocalModelDirectoryV1, name: string): Promise<Blob | null> {
  try { return await (await directory.getFileHandle(name)).getFile(); } catch (error) { if (notFound(error)) return null; throw error; }
}
async function readMarker(directory: LocalModelDirectoryV1, name: string): Promise<unknown> {
  const file = await maybeFile(directory, name); if (!file) return null;
  requireValue(file.size > 0 && file.size <= 65_536, 'invalid-marker');
  try { return JSON.parse(await file.text()) as unknown; } catch { throw new DeliveryError('invalid-marker'); }
}
async function atomicWrite(directory: LocalModelDirectoryV1, name: string, data: Uint8Array<ArrayBuffer> | string,
  signal: AbortSignal): Promise<void> {
  canceled(signal);
  const writer = await (await directory.getFileHandle(name, { create: true })).createWritable();
  let closed = false;
  try { await writer.write(data); canceled(signal); await writer.close(); closed = true; }
  finally { if (!closed) await writer.abort(); }
}
interface AttemptMarker { readonly schema: 'cf.local-model-attempt.v1'; readonly manifestSha256: string; readonly attemptId: string }
function markerAttempt(value: unknown, manifestSha256: string): AttemptMarker {
  requireValue(value !== null && typeof value === 'object', 'invalid-marker');
  const row = value as AttemptMarker;
...
export function createLocalModelDeliveryV1(options: LocalModelDeliveryOptionsV1): LocalModelDeliveryV1 {
  ...
  let readyAttempt: AttemptMarker | null = null, busy = false;
  let current: LocalModelDeliveryStatusV1 = Object.freeze({ phase: 'unknown', ready: false, manifestSha256,
  ...
  const publish = (patch: Partial<LocalModelDeliveryStatusV1>): LocalModelDeliveryStatusV1 => {
    current = Object.freeze({ ...current, ...patch }); options.onStatus?.(current); return current;
  };
  const chunkName = (fileIndex: number, chunkIndex: number): string => `f${fileIndex}-c${chunkIndex}`;
  const activeName = `active-${manifestSha256}.json`, readyName = `ready-${manifestSha256}.json`;
  const getNamespace = async (storage: LocalModelStorageV1): Promise<LocalModelDirectoryV1> =>
    (await storage.getDirectory()).getDirectoryHandle(NAMESPACE, { create: true });
  const getAttempt = (directory: LocalModelDirectoryV1, marker: AttemptMarker, create = false): Promise<LocalModelDirectoryV1> =>
    directory.getDirectoryHandle(marker.attemptId, { create });
  async function quota(storage: LocalModelStorageV1, additional: number): Promise<void> {
  ...
  async function scan(directory: LocalModelDirectoryV1, signal: AbortSignal, full: boolean): Promise<number> {
    let stored = 0, verified = 0, verifiedFiles = 0;
    for (let index = 0; index < manifest.files.length; index++) {
      const file = manifest.files[index]!, hash = new LocalModelSha256V1(); let bytes = 0;
      for (let part = 0; bytes < file.bytes; part++) {
        canceled(signal); const blob = await maybeFile(directory, chunkName(index, part));
        ...
        hash.update(new Uint8Array(await blob.arrayBuffer())); bytes += expected; stored += expected;
      }
      if (bytes === file.bytes) {
        requireValue(hash.digestHex() === file.sha256, 'hash-mismatch'); verified += bytes; verifiedFiles++;
  ...
  async function verifyReady(directory: LocalModelDirectoryV1, value: unknown, signal: AbortSignal): Promise<boolean> {
    ...
    publish({ phase: 'verifying', attemptId: marker.attemptId }); await scan(attempt, signal, true); canceled(signal);
    readyAttempt = marker; publish({ phase: 'ready', ready: true, file: null, error: null }); return true;
  }
  async function downloadFile(attempt: LocalModelDirectoryV1, index: number, storage: LocalModelStorageV1,
    signal: AbortSignal): Promise<void> {
    const file = manifest.files[index]!, hash = new LocalModelSha256V1(); let offset = 0, part = 0;
    // Rehash every persisted prefix, never trust a journal's byte count or hash.
    while (offset < file.bytes) {
      canceled(signal); const blob = await maybeFile(attempt, chunkName(index, part));
      if (!blob || blob.size === 0) break;
      requireValue(blob.size === Math.min(LOCAL_MODEL_CHUNK_BYTES_V1, file.bytes - offset), 'invalid-chunk-length');
      hash.update(new Uint8Array(await blob.arrayBuffer())); offset += blob.size; part++;
    }
    if (offset < file.bytes) {
      ...
      const timer = setTimeout(abort, timeout); let reader: ReadableStreamDefaultReader<Uint8Array<ArrayBuffer>> | null = null;
      try {
        ...
        const response = await fetcher(new URL(file.path.split('/').map(encodeURIComponent).join('/'), address),
          { method: 'GET', headers, signal: controller.signal, credentials: 'omit', cache: 'no-store', referrerPolicy: 'no-referrer' });
        canceled(signal);
        if (offset) requireValue(response.status === 206
        ...
        requireValue(response.body !== null, 'missing-response-body'); reader = response.body!.getReader();
        ...
        while (true) {
          canceled(signal); const next = await reader.read(); canceled(signal);
          if (next.done) break;
          ...
          for (let at = 0; at < value.byteLength;) {
            ...
            chunk.set(value.subarray(at, at + take), used); used += take; at += take;
            if (used === chunk.length) {
              ...
              offset += chunk.length; part++; used = 0;
    ...
    canceled(signal); requireValue(hash.digestHex() === file.sha256, 'hash-mismatch');
    publish({ verifiedBytes: current.verifiedBytes + file.bytes, verifiedFiles: current.verifiedFiles + 1, file: file.path });
  }
  async function run(mode: 'verify' | 'install', signal: AbortSignal, restart: boolean): Promise<LocalModelDeliveryStatusV1> {
    if (busy) throw new DeliveryError('delivery-busy');
    busy = true;
    readyAttempt = null; publish({ phase: 'verifying', ready: false, storedBytes: 0, verifiedBytes: 0,
    ...
    try {
      ...
      return await locks.run(NAMESPACE, signal, async () => {
        canceled(signal); const directory = await getNamespace(storage);
        ...
        if (mode === 'verify') {
          if (!attempt) return publish({ phase: 'missing', file: null });
          publish({ attemptId: marker!.attemptId });
          if (failed !== null) return publish({ phase: 'failed', error: 'failed-attempt', file: null });
          await scan(attempt, signal, false); return publish({ phase: 'partial', file: null });
        }
        if (restart || !attempt || failed !== null) {
          ...
          try { await directory.getDirectoryHandle(attemptId); throw new DeliveryError('attempt-id-collision'); }
        ...
        try {
          ...
          canceled(signal); await quota(storage, MARKER_MARGIN);
          ...
          readyAttempt = marker; return publish({ phase: 'ready', ready: true, error: null, file: null });
        } catch (error) {
      ...
      const phase: LocalModelDeliveryPhaseV1 = code === 'canceled' ? 'canceled'
      ...
      readyAttempt = null; return publish({ phase, ready: false, error: code });
    } finally { busy = false; }
  }
  return Object.freeze({ manifest, manifestSha256, baseUrl: address.href, status: () => current,
    install: (args: { signal?: AbortSignal; restart?: boolean } = {}) => run('install', args.signal ?? new AbortController().signal, args.restart === true),
    verify: (args: { signal?: AbortSignal } = {}) => run('verify', args.signal ?? new AbortController().signal, false),
    async openFile(filePath: string): Promise<Blob> {
      requireValue(readyAttempt !== null && current.ready, 'model-not-verified-ready');
      const index = manifest.files.findIndex(file => file.path === filePath); requireValue(index >= 0, 'unknown-model-file');
      const file = manifest.files[index]!, directory = await getNamespace(options.storage ?? nativeStorage());
      const attempt = await getAttempt(directory, readyAttempt!); const parts: Blob[] = [];
...
export interface LocalModelCapabilityV1 {
  readonly secureContext: boolean; readonly opfs: boolean; readonly webLocks: boolean;
  readonly webgpu: boolean; readonly shaderF16: boolean; readonly fallbackAdapter: boolean | null;
  readonly limits: Readonly<{ maxBufferSize: number; maxStorageBufferBindingSize: number; maxStorageBuffersPerShaderStage: number }> | null;
  readonly storage: Readonly<{ usageBytes: number | null; quotaBytes: number | null; persisted: boolean | null }>;
  readonly supported: boolean; readonly deviceQualified: false; readonly errors: readonly string[];
}
/** Read capabilities/estimates only. No model bytes, inference, benchmark or UA/RAM guesses. */
export async function probeLocalModelCapabilitiesV1(host: Pick<Navigator, 'storage' | 'locks' | 'gpu'> = navigator,
  secureContext = globalThis.isSecureContext === true): Promise<LocalModelCapabilityV1> {
  const errors: string[] = []; let usageBytes: number | null = null, quotaBytes: number | null = null, persisted: boolean | null = null;
  try { const estimate = await host.storage?.estimate(); usageBytes = estimate?.usage !== undefined && count(estimate.usage) ? estimate.usage : null;
  ...
  let webgpu = false, shaderF16 = false, fallbackAdapter: boolean | null = null, limits: LocalModelCapabilityV1['limits'] = null;
  try {
    const adapter = await host.gpu?.requestAdapter({ powerPreference: 'high-performance' });
    if (adapter) { webgpu = true; shaderF16 = adapter.features.has('shader-f16'); fallbackAdapter = adapter.info.isFallbackAdapter;
