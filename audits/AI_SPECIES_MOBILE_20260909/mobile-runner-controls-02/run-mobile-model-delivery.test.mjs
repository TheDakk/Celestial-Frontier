...
test('read-only diagnostic transpilation retains exactly one production SHA import and no side-effect invocation',async()=>{const source=await fs.readFile(new URL('../../port/v2/apps/game/src/local-model-delivery.ts',import.meta.url),'utf8');const result=stripTypeScriptTypes(source,{mode:'strip'});assert.equal(result.split("'./local-model-sha256.js'").length,2);assert.match(result,/export function createLocalModelDeliveryV1\(options\s+\)/);assert.match(result,/async openFile\(filePath\s+\)/);});
