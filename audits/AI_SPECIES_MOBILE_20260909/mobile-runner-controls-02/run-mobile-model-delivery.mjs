 ...
 * read-only diagnostics measure OPFS/CacheStorage. No inference, actual remote
 * model download, personal profile, shipping/phone qualification or hosted work.
...
export async function runMobileModelDelivery({pack,sha256,output}){
  ...
  const receipt={schema:'cf.native-optional-mobile-model-delivery.v1',status:'FAIL',startedAt:new Date().toISOString(),
    sourcePack:{directory:pack,manifestSha256:sha256},sources:[],observations:[],clicks:[],screenshots:[],browserEvents:[],network:[],interceptions:[],cleanup:{},
    actualRemoteDownload:false,modelExecuted:false,physicalPhoneQualified:false,qualityAccepted:false,
    scope:'Desktop native browser: actual optional static package, explicit native install/pause/resume/verify; exact pinned model bytes from disclosed loopback transport; full OPFS verification and offline app/runtime.'};
  ...
  try{
    ...
    async function intercept(event){
      ...
      if(!file||request.method!=='GET'||offline){
        receipt.interceptions.push({url:url.href,method:request.method,action:'blocked'});
      ...
      receipt.interceptions.push({url:url.href,rewritten,path:file.path,method:request.method,range,origin:requestOrigin,action:'loopback-byte-mirror'});
    ...
    async function attachWorker(info){
      ...
      workers.set(info.targetId,{info,sessionId:null});
    ...
    cdp=await openChromiumCdp({label:'CF optional model delivery',userDataPrefix:'cf-mobile-model-delivery-',commandTimeoutMs:60000,
      onEvent:event=>{
        ...
        if(event.method==='Target.targetCreated'&&['service_worker','worker'].includes(event.params?.targetInfo?.type))track(attachWorker(event.params.targetInfo));
    ...
    await until('installed active service worker','!!navigator.serviceWorker.controller',120000);
    ...
    const transpile=async file=>stripTypeScriptTypes(await fs.readFile(path.join(root,file),'utf8'),
      {mode:'strip',sourceUrl:file});
    const shaModule=await transpile('port/v2/apps/game/src/local-model-sha256.ts');
    const deliveryModule=await transpile('port/v2/apps/game/src/local-model-delivery.ts');
    need(deliveryModule.split("'./local-model-sha256.js'").length===2,'Read-only module import rewrite is not exact');
    ...
    await evaluate(`(()=>{window.__cfModelReadback={phase:'verifying'};void(async()=>{let shaUrl,ownerUrl;try{
      ...
      ownerUrl=URL.createObjectURL(new Blob([${JSON.stringify(deliveryModule)}.replace("'./local-model-sha256.js'",JSON.stringify(shaUrl))],{type:'text/javascript'}));
    ...
    // Load the actual stage-worker's complete ESM closure while offline. No
    // message is posted, so no graph, model, inference or tensor is initialized.
    const beforeWorkerTargets=new Set(workers.keys());
    await evaluate(`(()=>{window.__cfModuleOnlyWorkerError=null;window.__cfModuleOnlyWorker=new Worker('/__local_ai/stage-worker.mjs',{type:'module',name:'cf-module-only-offline'});window.__cfModuleOnlyWorker.onerror=e=>{window.__cfModuleOnlyWorkerError=e.message;};return true;})()`);
    const workerDeadline=performance.now()+60000;let loadedWorker=null;
    while(performance.now()<workerDeadline){
      if(fatal)throw fatal;const error=await evaluate('window.__cfModuleOnlyWorkerError');need(!error,'Offline AI worker module failed: '+error);
      for(const [id,row] of workers)if(!beforeWorkerTargets.has(id)&&row.info.type==='worker'&&row.sessionId){
        const result=await cdp.send('Runtime.evaluate',{expression:'typeof self.onmessage === "function"',returnByValue:true},row.sessionId);
        if(result.result?.value===true){loadedWorker={targetId:id,url:row.info.url,sessionId:row.sessionId};break;}
      }
      if(loadedWorker)break;await pause(100);
    }
    need(loadedWorker,'Actual offline worker import never completed');receipt.moduleOnlyWorker=loadedWorker;
    await evaluate('window.__cfModuleOnlyWorker.terminate();delete window.__cfModuleOnlyWorker;true');
...
async function main(){const options={};for(const argument of process.argv.slice(2)){const match=/^--(pack|sha256|output)=(.+)$/.exec(argument);need(match&&!Object.hasOwn(options,match[1]),'Usage: run-mobile-model-delivery.mjs --pack=EXACT_STATIC_PACK --sha256=EXTERNAL_MANIFEST_SHA --output=NEW_AUDIT_DIRECTORY');options[match[1]]=match[2];}need(options.pack&&options.sha256&&options.output,'All three arguments required');await runMobileModelDelivery(options);}
