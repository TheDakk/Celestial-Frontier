...
export async function runOfflineRuntime({pack,sha256,output}){
  ...
  const result={schema:'cf.offline-runtime-module-diagnostic.v1',status:'FAIL',startedAt:new Date().toISOString(),
    ...
    sources:[],clicks:[],network:[],browserEvents:[],serviceWorkerFetches:[],cleanup:{},modelExecuted:false,modelDownloaded:false,physicalPhoneQualified:false};
  await write('start.json',result);
  let release,server,cdp,targetId,sessionId,swSessionId,overflow=false;
  try{
    ...
    server=await createMobilePackServer({directory:pack,expectedManifestSha256:sha256});result.pack=server.verified;result.url=server.url;
    ...
    cdp=await openChromiumCdp({label:'CF isolated offline runtime',userDataPrefix:'cf-offline-runtime-',commandTimeoutMs:30000,onEvent:event=>{
      ...
      if(event.method==='Runtime.bindingCalled'&&event.params.name==='__cfObserveOfflineFetch'){if(result.serviceWorkerFetches.length<300)result.serviceWorkerFetches.push(JSON.parse(event.params.payload));else overflow=true;}
    ...
    const targets=await cdp.send('Target.getTargets');const sw=targets.targetInfos.filter(t=>t.type==='service_worker'&&t.url===origin+'/service-worker.js');need(sw.length===1,'No exact service-worker observer target');
    ({sessionId:swSessionId}=await cdp.send('Target.attachToTarget',{targetId:sw[0].targetId,flatten:true}));
    await cdp.send('Runtime.enable',{},swSessionId);await cdp.send('Runtime.addBinding',{name:'__cfObserveOfflineFetch'},swSessionId);
    const observer="self.addEventListener('fetch',e=>self.__cfObserveOfflineFetch(JSON.stringify({url:e.request.url,mode:e.request.mode,destination:e.request.destination,clientId:e.clientId,resultingClientId:e.resultingClientId}))); true";
    await cdp.send('Runtime.evaluate',{expression:observer,returnByValue:true},swSessionId);result.serviceWorkerObserver={source:observer,purpose:'Read-only fetch metadata; no response or policy mutation',targetId:sw[0].targetId};
    ...
    result.workerReply=await evaluate('window.__cfOfflineRuntime');assessOfflineWorkerReply(result.workerReply.messages);
    ...
    if(swSessionId)try{const d=await cdp.send('Runtime.evaluate',{expression:"(async()=>({clients:(await self.clients.matchAll({type:'all',includeUncontrolled:true})).map(c=>({id:c.id,type:c.type,url:c.url})),pins:await Promise.all((await (await caches.open('cf-v2-pwa-control-v1')).keys()).map(async r=>({url:r.url,value:await (await (await caches.open('cf-v2-pwa-control-v1')).match(r)).json()}))}))()",awaitPromise:true,returnByValue:true},swSessionId);result.failedWorkerOwnership=d.result?.value??d.exceptionDetails;}catch(e){result.ownershipCaptureError=String(e);}
  }
  finally{
