...
export async function createLocalAiGameV1(options: LocalAiGameOptionsV1): Promise<LocalAiGameV1> {
  ...
  /** Exact historical V1 compatibility, not a general cross-recipe lookup. Both
   * inputs are rebuilt from the same admitted snapshot, model and derivative;
   * existing stored input/content verification remains mandatory. */
  const historicalInput = (input: AiLandfallInputV1): AiLandfallInputV1 | null => {
    if (!runtime.references) return null;
    try {
      const candidate = JSON.parse(input.recipeJson) as { schema: string; q8Block32: boolean; conditioning: { sourceSnapshot: unknown } };
      if (candidate.schema !== 'cf.ai-landfall-render.v2' || typeof candidate.q8Block32 !== 'boolean') return null;
      const current = buildLandfallConditioningV2(candidate.conditioning.sourceSnapshot);
      if (!current.ok || JSON.stringify(renderInput(current, candidate.q8Block32)) !== JSON.stringify(input)) return null;
      const previous = buildLandfallConditioningV1(candidate.conditioning.sourceSnapshot);
      if (!previous.ok || previous.recipe.referenceRequirements[0]?.subjectIdentityKey !== runtime.reference.speciesVisualKey) return null;
      return renderInput(previous, candidate.q8Block32);
    } catch { return null; }
  };
  const readOriginal = async (input: AiLandfallInputV1, originalId: string): Promise<AiLandfallOriginalV1 | null> => {
    const current = await store.read(input, originalId);
    if (current) return current;
    const previous = historicalInput(input);
    return previous ? store.read(previous, originalId) : null;
  };
  const api: LocalAiGameV1 = {
    prepare(request, roster) {
      ...
      return renderInput(compiled, useInstalled ? false : runtime.q8Block32);
    },
    enqueue: input => {
      ...
      const current = await store.find(input);
      if (current) return current;
      const previous = historicalInput(input);
      return previous ? store.find(previous) : null;
