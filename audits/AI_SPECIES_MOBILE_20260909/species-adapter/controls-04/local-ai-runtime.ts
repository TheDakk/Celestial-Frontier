...
export async function generateLocalLandfallV1(
  ...
  try {
    ...
    if ('imageIndex' in reference) { ctx.imageSmoothingEnabled = true; ctx.imageSmoothingQuality = 'high'; }
  ...
  // Fetch/decode/encode one reference at a time; completed source bytes leave
  // scope before the next fetch (GC timing is not guaranteed). Only six
  // 307,200-byte latent arrays are retained until transfer to denoising.
