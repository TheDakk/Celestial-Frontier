...
import { buildLandfallConditioningV2 } from '../apps/game/src/landfall-conditioning.js';
import { createHash, webcrypto } from 'node:crypto';
import { setTimeout as delay } from 'node:timers/promises';
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from 'vitest';
import { generateLocalLandfallV1, localAiReferenceBindingV2, type LocalAiRuntimeConfigV1, type LocalAiReferenceV2 } from '../apps/game/src/local-ai-runtime.js';
...
function fixture(options: {
  ...
  const referenceBytes = Array.from({ length: 6 }, (_, index) => new Uint8Array([...BYTES, index]));
  if (options.multi) {
    ...
    const compiled = buildLandfallConditioningV2(retained.conditioning.sourceSnapshot);
    if (!compiled.ok) throw Error(compiled.reason);
    const references: LocalAiReferenceV2[] = compiled.recipe.referenceRequirements.map((row, index) => ({
      imageIndex: row.imageIndex, url: `/__local_ai/reference-${index + 1}.png`,
      sha256: createHash('sha256').update(referenceBytes[index]!).digest('hex'),
      width: 480, height: 320, sourceWidth: 1536, sourceHeight: 1024, speciesVisualKey: row.subjectIdentityKey,
    }));
    Object.assign(config, { references });
    Object.assign(recipe, { schema: 'cf.ai-landfall-render.v2', conditioning: compiled.recipe,
      references: JSON.parse(JSON.stringify(references.map(localAiReferenceBindingV2))) });
    delete recipe.reference;
  }
  const controller = new AbortController(); controllers.push(controller);
  const bitmap = { width: 1536, height: 1024, close: vi.fn(() => { if (options.closeThrows) throw Error('Bitmap close failed'); }) };
  const decodeBitmap = vi.fn(() => options.bitmap ?? Promise.resolve(bitmap));
  const fetcher = vi.fn((url: string) => options.response?.() ?? Promise.resolve(new Response(
    options.multi ? referenceBytes[Number(url.match(/reference-(\d+)/)?.[1]) - 1] : BYTES)));
  const canvases: FakeCanvas[] = [];
  class FakeCanvas {
    initialWidth: number; initialHeight: number; width: number; height: number;
    puts: Uint8ClampedArray[] = []; fills = 0; draws = 0; encodes = 0;
    smoothing: { imageSmoothingEnabled?: boolean; imageSmoothingQuality?: string } | null = null;
    ...
    getContext() {
      ...
      const context = { fillStyle: '', imageSmoothingEnabled: true, imageSmoothingQuality: 'low',
        fillRect: () => { this.fills++; },
        drawImage: () => { this.smoothing = { imageSmoothingEnabled: context.imageSmoothingEnabled, imageSmoothingQuality: context.imageSmoothingQuality }; this.draws++; if (options.canvasFailure === 'draw') throw Error('Draw failed'); },
        getImageData: () => {
...
describe('local AI stage orchestrator — synthetic workers, no inference claim', () => {
  it('runs four serial workers, transfers correct tensors and returns a real Blob only after RGBA conversion', async () => {
    ...
    expect(f.canvases[0]!.smoothing).toEqual({ imageSmoothingEnabled: true, imageSmoothingQuality: 'low' });
...
describe('six canonical reference orchestration with synthetic image and worker fixtures', () => {
  it('hashes and encodes six references serially, transfers ordered latent slots and bounds retained work', async () => {
    let encodedIndex = 0;
    const f = fixture({ multi: true, result: (stage, value) => {
      if (stage === 'encode') value.fill(++encodedIndex); return value;
    } }); await f.run();
    expect(f.jobs.map(row => row.job.stage)).toEqual(['text', 'encode', 'encode', 'encode', 'encode', 'encode', 'encode', 'denoise', 'decode']);
    expect(f.peak()).toBe(1);
    expect(f.fetcher.mock.calls.map(row => row[0])).toEqual(f.config.references!.map(row => row.url));
    expect(f.decodeBitmap).toHaveBeenCalledTimes(6); expect(f.bitmap.close).toHaveBeenCalledTimes(6);
    const denoise = f.jobs[7]!;
    expect(denoise.transfers).toBe(7); expect(denoise.detached).toBe(true);
    const slots = denoise.job.references as Array<{ data: Float32Array; width: number; height: number }>;
    expect(slots.map(row => [row.data[0], row.data.byteLength, row.width, row.height])).toEqual(
      [1, 2, 3, 4, 5, 6].map(index => [index, 307200, 480, 320]));
    expect(f.progress.at(-1)).toEqual({ phase: 'Finishing the painting', completed: 12, total: 12, etaMs: null });
    expect(f.canvases).toHaveLength(7);
    expect(f.canvases.slice(0, 6).every(canvas => canvas.smoothing?.imageSmoothingEnabled === true && canvas.smoothing?.imageSmoothingQuality === 'high')).toBe(true);
    f.retired();
  });

  it('refuses missing/excess/reordered/duplicate references, unbound geometry, identities and digests before allocation', async () => {
    for (const mutate of [
      (f: ReturnType<typeof fixture>) => { Object.assign(f.config, { references: f.config.references!.slice(0, 5) }); },
      (f: ReturnType<typeof fixture>) => { Object.assign(f.config, { references: [...f.config.references!, f.config.references![0]] }); },
      (f: ReturnType<typeof fixture>) => { Object.assign(f.config, { references: [...f.config.references!].reverse() }); },
      (f: ReturnType<typeof fixture>) => { Object.assign(f.config.references![1]!, { sha256: f.config.references![0]!.sha256 }); },
      (f: ReturnType<typeof fixture>) => { Object.assign(f.config.references![1]!, { speciesVisualKey: f.config.references![0]!.speciesVisualKey }); },
      (f: ReturnType<typeof fixture>) => { Object.assign(f.config.references![1]!, { sourceWidth: 1535 }); },
      (f: ReturnType<typeof fixture>) => { Object.assign(f.config.references![1]!, { width: 320 }); },
      (f: ReturnType<typeof fixture>) => { (f.recipe.references as Array<Record<string, unknown>>)[0]!.sha256 = 'a'.repeat(64); },
      (f: ReturnType<typeof fixture>) => { (f.recipe.references as Array<Record<string, unknown>>)[0]!.sourceHeight = 320; },
      (f: ReturnType<typeof fixture>) => { const value = JSON.parse(JSON.stringify(f.recipe.conditioning)); value.referenceRequirements[0].fullGenome.seed++; f.recipe.conditioning = value; },
      (f: ReturnType<typeof fixture>) => { const value = JSON.parse(JSON.stringify(f.recipe.conditioning)); value.prompt += ' Added unreviewed directive'; f.recipe.conditioning = value; },
      (f: ReturnType<typeof fixture>) => { f.recipe.schema = 'cf.ai-landfall-render.v3'; },
    ]) {
      const f = fixture({ multi: true }); mutate(f);
      await expect(f.run()).rejects.toThrow(); expect(f.workers).toHaveLength(0);
      expect(f.fetcher).not.toHaveBeenCalled(); f.retired();
    }
  });

  it('rejects a later mismatched PNG before its decode and never requests subsequent references', async () => {
    const f = fixture({ multi: true });
    f.fetcher.mockImplementationOnce(async () => new Response(new Uint8Array([...BYTES, 0])))
      .mockImplementationOnce(async () => new Response(new Uint8Array([1, 2, 3])));
    await expect(f.run()).rejects.toThrow(/reference changed/);
    expect(f.fetcher).toHaveBeenCalledTimes(2); expect(f.decodeBitmap).toHaveBeenCalledTimes(1);
    expect(f.jobs.map(row => row.job.stage)).toEqual(['text', 'encode']); f.retired();
  });

  it('rejects actual decoded source geometry before canvas allocation or further references', async () => {
    const f = fixture({ multi: true }); f.bitmap.width = 768;
    await expect(f.run()).rejects.toThrow(/source geometry changed/);
    expect(f.bitmap.close).toHaveBeenCalledOnce(); expect(f.canvases).toHaveLength(0);
    expect(f.fetcher).toHaveBeenCalledOnce(); f.retired();
  });

  it('cancels between reference encoders without fetching or encoding the remaining images', async () => {
    const f = fixture({ multi: true, onProgress: progress => {
      if (progress.completed === 2) f.controller.abort();
    } });
    await expect(f.run()).rejects.toMatchObject({ name: 'AbortError' });
    expect(f.fetcher).toHaveBeenCalledOnce(); expect(f.jobs.map(row => row.job.stage)).toEqual(['text', 'encode']); f.retired();
  });
});
