...
const root=process.cwd(),fit=path.join(root,'audits/PYTHON_OPEN_POSE_20260923/fit-01');
...
const out=path.join(import.meta.dirname,'join-diagnosis.json');fs.writeFileSync(out,JSON.stringify({scope:'Three synthetic pose diagnostics of unchanged source-owned mesh, not a native pose reproduction, threshold or new admission test. Root rigid motion is the negative control for differential separation.',recordRecipeHash:record.recipeHash,bindingHash:binding.bindingHash,inputs,results},null,2)+'\n',{flag:'wx'});
