...
const root=process.cwd(),fit=path.join(root,'audits/PYTHON_OPEN_POSE_20260923/candidate-02/fit-01');
...
const record=JSON.parse(read(path.join(fit,'record.json'))),binding=JSON.parse(read(path.join(fit,'binding.json'))),atlas=PNG.sync.read(read(path.join(fit,'parts/atlas/python-open.png')));
...
const out=path.join(import.meta.dirname,'join-diagnosis-02.json');fs.writeFileSync(out,JSON.stringify({scope:'Three synthetic pose diagnostics of unchanged source-owned mesh, not a native pose reproduction, threshold or new admission test. Root rigid motion is the negative control for differential separation.',recordRecipeHash:record.recipeHash,bindingHash:binding.bindingHash,inputs,results},null,2)+'\n',{flag:'wx'});
