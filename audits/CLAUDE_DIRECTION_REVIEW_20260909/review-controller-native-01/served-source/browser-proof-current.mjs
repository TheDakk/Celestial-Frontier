...
let preparationView=null,completedLanding=null;
const showPanel=panel=>{byId('planet-panel').hidden=panel!=='planet';byId('journal-panel').hidden=panel!=='journal';};
// Display binding only. The server still owns canonical admission; this proof
// can return only to its existing Earth panel, never to an invented world route.
function completionDestination(config){
  const request=config?.request,roster=config?.roster,snapshot=config?.appearanceSnapshot;
  const worldKey=request?.worldKey,environmentFingerprint=request?.environmentFingerprint;
  if(typeof worldKey!=='string'||worldKey!==byId('planet').getAttribute('data-world-key')
    ||typeof environmentFingerprint!=='string'||!environmentFingerprint.length
    ||roster?.worldKey!==worldKey||roster?.environmentFingerprint!==environmentFingerprint
    ||snapshot?.displayPlan?.worldKey!==worldKey||snapshot?.displayPlan?.environmentFingerprint!==environmentFingerprint
    ||!Number.isSafeInteger(roster?.ecologyEpoch)||roster.ecologyEpoch<0
    ||typeof snapshot?.sha256!=='string'||!/^[a-f0-9]{64}$/.test(snapshot.sha256))
    throw Error('Landing destination does not match the Earth preview');
  return Object.freeze({worldKey,environmentFingerprint,ecologyEpoch:roster.ecologyEpoch,snapshotSha256:snapshot.sha256});
}
function sameCompletionDestination(left,right){
  return left.worldKey===right.worldKey&&left.environmentFingerprint===right.environmentFingerprint
    &&left.ecologyEpoch===right.ecologyEpoch&&left.snapshotSha256===right.snapshotSha256;
}
...
async function generate({referenceEnabled=true,profile=false}={}) {
  ...
  generationController=new AbortController();completedLanding=null;delete window.cfImageProof.completionDestination;
  delete window.cfImageProof.png;window.cfImageProof.profiles=[];
  byId('landing-error').hidden=true;byId('landing-error').textContent='';
  ...
  try {
    ...
    const destination=completionDestination(config),jobKey=JSON.stringify({config,referenceEnabled,profile});
    landing.begin({key:jobKey,steps:config.steps,referenceCount:referenceEnabled?config.references.length:0,nowMs:started});preparationView=null;renderLanding();
    ...
    // An input event cannot interrupt the synchronous conversion/publication
    // below. Honor cancellation already requested before this await resumes.
    if(generationController.signal.aborted)throw Error('Canceled before publication');
    if(JSON.stringify({config:window.cfImageProof.recipe,referenceEnabled,profile})!==jobKey
      ||!sameCompletionDestination(destination,completionDestination(window.cfImageProof.recipe)))
      throw Error('Landing destination changed before publication');
    ...
    completedLanding=Object.freeze({destination,jobKey,referenceEnabled,profile,png:window.cfImageProof.png});
    window.cfImageProof.completionDestination=destination;
    byId('notice-open').setAttribute('data-world-key',destination.worldKey);
    ...
    const canceled=generationController?.signal.aborted===true;
    completedLanding=null;delete window.cfImageProof.completionDestination;delete window.cfImageProof.png;
    byId('notice').hidden=true;byId('painting').hidden=true;
    window.cfImageProof.state='failed';window.cfImageProof.error=String(error.stack??error);record({phase:'failed',canceled,message:window.cfImageProof.error});
    byId('status').textContent=canceled?'Landing canceled':'Landing unavailable';
    byId('landing-error').textContent=canceled?'Landing canceled. Select Earth to start again.':'Landing unavailable. Select Earth to try again.';
    byId('landing-error').hidden=false;
  }
...
byId('notice-open').onclick=()=>{
  if(!completedLanding||window.cfImageProof.state!=='complete'||window.cfImageProof.landing?.state!=='complete'
    ||window.cfImageProof.png!==completedLanding.png||landing.snapshot().key!==completedLanding.jobKey
    ||window.cfImageProof.landing.key!==completedLanding.jobKey
    ||byId('notice-open').getAttribute('data-world-key')!==completedLanding.destination.worldKey)return;
  try{
    const {referenceEnabled,profile,jobKey,destination}=completedLanding;
    if(JSON.stringify({config:window.cfImageProof.recipe,referenceEnabled,profile})!==jobKey
      ||!sameCompletionDestination(destination,completionDestination(window.cfImageProof.recipe)))return;
  }catch{return;}
  showPanel('planet');byId('notice').hidden=true;
};
