...
try{
  receipt.priorInstrumentFailure={file:'../recipe-preparation-native-01/run.mjs',sha256:'00d017a1af48be89689ac77045edae3359727d7f57525263abf2a0da9b1badf2',
    preserved:true,repair:'Recipe evidence contains data only; cleanup refuses late HTTP before closing the target and retiring held responses'};
  ...
  async function runCase(id,variant,kind,negative=false){
    ...
    try{
      ...
      // Refuse late/retried requests before target teardown. Only the held Map
      // owns response objects; evidence rows stay serializable throughout awaits.
      item.closing=true;
      for(const entry of item.recipeRequests)entry.cleanupStarted=true;
      ...
      // Closing the page comes first, so destroying an unfinished response cannot
      // make a still-open page retry after the evidence was serialized.
      for(const [response,entry] of held)if(entry.caseId===id){entry.cleanupStarted=true;response.destroy();held.delete(response);}
      item.cleanup.heldResponsesRetired=true;
