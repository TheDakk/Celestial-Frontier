...
const report=JSON.parse(fs.readFileSync('audits/AV_CHARTER_HEADER_PREVIEW_20260908/producer.json','utf8'));
...
assert.equal(previous.sha256,'2152279dbe394a2f6ed0cfe15157b75dd907ee80dc810f5a72ee6662fe75bace');
...
budget.calibration.selectionRule+=' Shared sticky panel title paint (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after opaque shared panel title paint covers existing padding and the Close gutter without changing geometry; the existing UI draft bullet records that behavior. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('audits/AV_CHARTER_HEADER_PREVIEW_20260908/producer-refresh.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
