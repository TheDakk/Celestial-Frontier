...
const steps=[
 ['refresh-producer','node',[path.join(audit,'refresh-producer.mjs')],root],
 ['focused-binding-repair','npm',['test','--','tests/current-producer-authorities.test.ts','tests/compendium-budget.test.ts','tests/exceptional-crafting-evidence-contract.test.ts','tests/guide-release.test.ts','tests/slicesmoke-sixth-red-contract.test.ts'],v2],
 ['typescript','npm',['run','typecheck'],v2],
...
const report={status:'RUNNING',continuationOf:'develop-profile.json',steps:[]};
...
fs.writeFileSync(path.join(audit,'continued-verification.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});
