...
const report = { schema: 'cf-native-earth-surface-turn/v1', certification: false, status: 'RUNNING',
  ...
  limitations: [
    ...
    'Two emulated Chromium viewports, not physical iPhone/Safari/PWA or human art acceptance.',
    ...
    'Preference controls are exercised on the phone viewport. Hidden-document/context-loss and malformed-worker cases are outside this native run.',
  ] };
for (const name of [
  ...
  'audits/AV_EARTH_SURFACE_TURN_20260908/native-runner.mjs',
...
try {
  ...
  const captureCycle = async name => {
    ...
    assert.equal(cache.turnUniformGroupCount,initial.resources.surfacePlanetTurnProgramCache.turnUniformGroupCount);
  ...
  const phoneEntry=await enterEarth('phone');assert.deepEqual(phoneEntry.viewport,[390,844,2]);
  const phone=await captureCycle('earth-phone');
  ...
  await escape();await exitEarth('phone');
  await viewport(1440,1000,1,false);await frames();
  const desktopEntry=await enterEarth('desktop');assert.deepEqual(desktopEntry.viewport,[1440,1000,1]);
  assert.equal(desktopEntry.documentToken,phoneEntry.documentToken,'Re-entry replaced the application');
  assert.equal(desktopEntry.resources.surfacePlanetTurn.appProgramKey,phone.resources.surfacePlanetTurn.appProgramKey);
  assert.equal(desktopEntry.resources.surfacePlanetTurn.appUniformUid,phone.resources.surfacePlanetTurn.appUniformUid);
  const desktop=await captureCycle('earth-desktop');
  assert.equal(desktop.resources.surfacePlanetTurnProgramCache.programCount,phone.resources.surfacePlanetTurnProgramCache.programCount,'Renderer program cache grew on re-entry');
  assert.equal(desktop.resources.surfacePlanetTurnProgramCache.turnUniformGroupCount,phone.resources.surfacePlanetTurnProgramCache.turnUniformGroupCount,'Turn uniform cache grew on re-entry');
  await exitEarth('desktop');
