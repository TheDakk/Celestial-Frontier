...
const report=JSON.parse(fs.readFileSync('audits/AV_EARTH_SURFACE_TURN_20260908/producer.json','utf8'));
...
assert.equal(previous.sha256,'16c4a7b9f07089dae7e8dc9aae1b6b6fb5378891e54a2ba29aadf15ea4746f60');
...
budget.calibration.selectionRule+=' Opt-in finite canonical Earth surface turn (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after separating an unlit Earth atlas, spherical finite yaw and fixed light with shared application program ownership and matching draft copy. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('audits/AV_EARTH_SURFACE_TURN_20260908/producer-refresh.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
