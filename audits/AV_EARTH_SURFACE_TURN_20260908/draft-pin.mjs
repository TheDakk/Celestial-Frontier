...
if(bullets.length!==84)throw Error('draft count changed');
...
const pin=old.match(/const GUIDE_DRAFT_BULLET_AUTHORITY = Object\.freeze\(\{\s*count: 83,\s*sha256: '([a-f0-9]{64})'/g);
...
fs.writeFileSync(p+'.avtmp',old.replace(prev,hash).replace('count: 83,', 'count: 84,'),{flag:'wx'});fs.renameSync(p+'.avtmp',p);
fs.writeFileSync(process.argv[2],JSON.stringify({previous:prev,count:84,sha256:hash},null,2)+'\n',{flag:'wx'});console.log(hash);
