...
function sameFilters(a: readonly Filter[] | null | undefined, b: readonly Filter[] | null | undefined): boolean {
  // Pixi changes an initial undefined chain to null when removing its first
  // filter. All empty representations mean no filters; nonempty chains still
  // require exact ordered filter identities, so foreign state is never erased.
