...
for(const phase of ['launch','travel','impact']){
 ...
 const registeredRaw=await sharp(registered).ensureAlpha().raw().toBuffer();
 let minX=size,minY=size,maxX=-1,maxY=-1;
 for(let y=0;y<size;y++)for(let x=0;x<size;x++)if(registeredRaw[(y*size+x)*4+3]>0){minX=Math.min(minX,x);minY=Math.min(minY,y);maxX=Math.max(maxX,x);maxY=Math.max(maxY,y);}
 assert.ok(maxX>=minX&&maxY>=minY);
 const registeredBounds={x:minX,y:minY,width:maxX-minX+1,height:maxY-minY+1};
 const mapped=f.source.map((n,i)=>(n*info.width*ratio+[left,top][i])/size);
 assert.ok(mapped.every((n,i)=>Math.abs(n-f.target[i])*size<=.51));
 rows.push({phase,image:name+'.png',imageSha256:sha(bytes),canvasSize:{width:info.width,height:info.height},originAnchor:f.origin,contactAnchor:f.contact,keyedImage:'keyed/'+name+'.png',keyedSha256:sha(png),alphaBoundsPixels:keyed.bounds,keyer:keyed.receipt,registeredImage:'registered/'+name+'.png',registeredSha256:sha(registered),registeredBounds,registration:{uniformScale:ratio,left,top,sourceAnchorNormalized:f.source,targetAnchorNormalized:f.target,mappedAnchorNormalized:mapped,anchorResidualPixels:mapped.map((n,i)=>(n-f.target[i])*size),method:'visual anchor measurement; uniform downscale and translation of keyed copy; no paint changes, rotation, warping or cropping'},qualityAccepted:false});
 assert.equal(sha(fs.readFileSync(path.join(out,name+'.png'))),sha(bytes));
}
const anchors={schema:'cf.effect-sequence-anchors/v1',sequenceId:'wild-maw-proof-v43',theme:'wild',abilityId:'maw',abilityName:'Savage Maw',canvasSize:{width:size,height:size},coordinateSpace:'normalized full canvas; top-left origin',phaseOrder:['launch','travel','impact'],direction:'left-to-right',originAnchor:origin,contactAnchor:contact,phases:rows.map(r=>({phase:r.phase,image:r.registeredImage,keyedImage:r.registeredImage,imageSha256:r.registeredSha256,alphaBoundsPixels:r.registeredBounds,canvasSize:{width:size,height:size},originAnchor:origin,contactAnchor:contact,qualityAccepted:false})),qualityAccepted:false,registration:'Shared 1024-square intake canvas; visible launch/travel origins and impact contact aligned within half a pixel. Empty anchors are virtual. Visual motion acceptance pending; generated masters did not obey requested occupied bounds.'};
