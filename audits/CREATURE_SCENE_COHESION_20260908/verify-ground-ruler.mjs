...
const reportPath = path.join(base, 'static-ground-ruler.json');
...
for (const [name, command, args] of stages) {
 ...
 fs.writeFileSync(path.join(base,name+'-ground-ruler.log'),(result.stdout||'')+(result.stderr||''),{flag:'wx'});
 report.stages.push({name,command,args,exitCode:result.status,error:result.error?.message,log:name+'-ground-ruler.log'});
