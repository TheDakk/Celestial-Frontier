...
const dir=path.dirname(fileURLToPath(import.meta.url)), out=path.join(dir,'alpha-refined-v1');
if(fs.existsSync(out)) throw Error('Immutable alpha output already exists');
fs.mkdirSync(out);
const magick='/opt/homebrew/bin/magick';
const source=path.join(dir,'civet-reference-refinement-original.png');
...
const master=path.join(out,'civet-refined-v2.png');
run([source,matte,'-alpha','off','-compose','CopyOpacity','-composite',master]);
const derivative=path.join(out,'civet-refined-v2.webp');
...
fs.writeFileSync(path.join(out,'extraction.json'),JSON.stringify({schema:'cf-civet-alpha-extraction/v1',authority:'Nick: Use ImageMagick for alpha extraction',at:new Date().toISOString(),source:{path:'civet-reference-refinement-original.png',sha256:sha(source)},tool:run(['-version']).split('\n')[0],files,limits:'Heuristic matte extraction from opaque generated pixels; dark/light visual edge review required. No claim of original alpha recovery or identical antialias colors.'},null,2)+'\n',{flag:'wx'});
