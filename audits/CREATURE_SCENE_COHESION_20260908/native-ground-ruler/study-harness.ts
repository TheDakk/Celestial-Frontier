...
import { attachEarthSceneCohesion, type CohesionTestMode } from './cohesion-owner.js';
import { EARTH_RESIDENT_LAYER_PLAN_V1 } from '../../port/v2/packages/art/src/earth-resident-plan.js';

type Bounds = { x0: number; y0: number; x1: number; y1: number };
interface AssetManifest { path: string; sha256: string; width: number; height: number; alphaBounds: Bounds; contactY: number; civetPaws: Array<{x0:number;y0:number;x1:number;y1:number}> }
...
const grounding = document.getElementById('cohesion') as HTMLInputElement;
let cohesion: ReturnType<typeof attachEarthSceneCohesion> | undefined;
let cohesionRetirement: ReturnType<ReturnType<typeof attachEarthSceneCohesion>['snapshot']> | null = null;
...
function comparePixels(actor: Actor, baseline: Uint8Array | Uint8ClampedArray, changed: Uint8Array | Uint8ClampedArray) {
  // Source-x partitions come from the four separately visible paw clusters
  // in the immutable 440px rest capture, independently checked at 132px.
  // The front pair touches at 300px, so raster connected-component count
  // alone cannot establish four-foot coverage at every display scale.
  const contactFeet = [[270, 325], [325, 385], [428, 486], [486, 548]].map(([x0, x1]) => ({ sourceX0: x0!, sourceX1: x1!, solid: 0, changed: 0 }));
  const stats = { all: 0, head: 0, body: 0, contact: 0, headInk: 0, bodyInk: 0, contactInk: 0, contactFeet, maximumChannelDelta: 0 };
  // Lowest20 source rows preserve solid paw pixels at440/300/132. The former
  // two-row strip sampled zero solid ink at300/132; keep that first failure.
  // This band is tied to the measured asset contact, not a rig motion mask.
...
function captureScene() {
  const prior = { x: earth.x, y: earth.y, sx: earth.scale.x, sy: earth.scale.y };
  earth.position.set(0, 0); earth.scale.set(1);
  try { return app.renderer.extract.pixels({ target: earth, frame: new Rectangle(0, 0, 960, 430), resolution: 1, antialias: true }); }
  finally { earth.position.set(prior.x, prior.y); earth.scale.set(prior.sx, prior.sy); }
}
async function cohesionProbe(mode: CohesionTestMode | 'backgroundGrade' = 'none') {
  assert(cohesion && !cohesion.snapshot().disposed && visible(), 'Inactive cohesion study'); rest('scene comparison');
  const prior = grounding.checked, background = sceneSprites[0]!, originalTint = background.tint;
  try {
    cohesion!.setTestMode('none'); cohesion!.setEnabled(false); render(); const baseline = captureScene();
    background.visible = false; render(); const ink = captureScene(); background.visible = true;
    cohesion!.setEnabled(true); cohesion!.setTestMode('noShadows'); render(); const lightOnly = captureScene();
    cohesion!.setTestMode('missingCivetContact'); render(); const withoutPawContacts = captureScene();
    cohesion!.setTestMode(mode === 'backgroundGrade' ? 'none' : mode);
    if (mode === 'backgroundGrade') background.tint = 0x888888;
    render(); const sample = captureScene(), owner = cohesion!.snapshot(), rgba = sample.pixels;
    const allowed = new Uint8Array(960 * 430);
    ...
    for(const shadow of owner.declaredShadows)for(let y=Math.max(0,Math.floor(shadow.y-shadow.radiusY-2));y<=Math.min(429,Math.ceil(shadow.y+shadow.radiusY+2));y++){
      for(let x=Math.max(0,Math.floor(shadow.x-shadow.radiusX-2));x<=Math.min(959,Math.ceil(shadow.x+shadow.radiusX+2));x++)allowed[y*960+x]=1;
    }
    let outsideChanged=0, outsideObserved=0, allChanged=0;
    const regions={civet:{ink:0,before:0,after:0},legacy:{ink:0,before:0,after:0}};
    const luma=(a:Uint8Array|Uint8ClampedArray,i:number)=>.2126*a[i]!+.7152*a[i+1]!+.0722*a[i+2]!;
    const bodyBox={x0:earthActor.sourceX+manifest.alphaBounds.x0*earthActor.k,x1:earthActor.sourceX+(manifest.alphaBounds.x1+1)*earthActor.k,
      y0:earthActor.sourceY+manifest.alphaBounds.y0*earthActor.k,y1:EARTH_RESIDENT_LAYER_PLAN_V1.residents[0]!.groundY*430};
    for(let y=0;y<430;y++)for(let x=0;x<960;x++){
      const i=(y*960+x)*4, changed=[0,1,2,3].some(c=>rgba[i+c]!==baseline.pixels[i+c]);
      if(changed)allChanged++;if(!allowed[y*960+x]){outsideObserved++;if(changed)outsideChanged++;}
      if(ink.pixels[i+3]!>=250){const row=x>=bodyBox.x0&&x<=bodyBox.x1&&y>=bodyBox.y0&&y<=bodyBox.y1?regions.civet:regions.legacy;
        row.ink++;row.before+=luma(baseline.pixels,i);row.after+=luma(rgba,i);}
    }
    const contacts=owner.declaredShadows.filter(s=>s.role==='contact').map(shadow=>{
      ...
      for(let y=Math.max(0,Math.floor(shadow.y-shadow.radiusY-1));y<=Math.min(429,Math.ceil(shadow.y+shadow.radiusY+1));y++)for(let x=Math.max(0,Math.floor(shadow.x-shadow.radiusX-1));x<=Math.min(959,Math.ceil(shadow.x+shadow.radiusX+1));x++){
        const i=(y*960+x)*4; if(shadow.resident==='Civet'?ink.pixels[i+3]!==0:ink.pixels[i+3]!>=230)continue;
        const reference=shadow.resident==='Civet'?withoutPawContacts:lightOnly;
        const delta=luma(reference.pixels,i)-luma(rgba,i);pixels++;if(delta>1){darkened++;totalDarkening+=delta;}maxDarkening=Math.max(maxDarkening,delta);
      }
      return {id:shadow.id,resident:shadow.resident,x:shadow.x,y:shadow.y,radiusX:shadow.radiusX,radiusY:shadow.radiusY,pixels,darkened,maxDarkening,totalDarkening};
    });
    return {mode,owner,contacts,outsideChanged,outsideObserved,allChanged,
      lighting:Object.entries(regions).map(([name,row])=>({name,...row,ratio:row.after/row.before})),
      baselinePng:png(baseline.pixels,960,430),samplePng:png(rgba,960,430),lightingOnlyPng:png(lightOnly.pixels,960,430),withoutPawContactsPng:png(withoutPawContacts.pixels,960,430)};
  } finally { background.visible=true;background.tint=originalTint;cohesion!.setTestMode('none');cohesion!.setEnabled(prior);rest('scene comparison restored'); }
}
async function retireCohesion() {
  assert(cohesion && !cohesion.snapshot().disposed, 'Cohesion already retired');rest('retire scene owner');
  cohesion!.setEnabled(false);render();const before=captureScene();
  cohesionRetirement=cohesion!.dispose();grounding.disabled=true;render();const after=captureScene();
  let changed=0;for(let i=0;i<before.pixels.length;i++)if(before.pixels[i]!==after.pixels[i])changed++;
  const sibling=captureActor(actors[0]!);let siblingInk=0;for(let i=3;i<sibling.pixels.length;i+=4)if(sibling.pixels[i]!>230)siblingInk++;
  return {retired:cohesionRetirement,restoredSceneChannelChanges:changed,siblingInk,
    scenePng:png(after.pixels,960,430),siblingPng:png(sibling.pixels,sibling.width,sibling.height)};
}
...
function dispose() {
  ...
  if (cohesion) try { cohesionRetirement ??= cohesion.dispose(); } catch (error) { failures.push(String(error)); }
  for (const actor of actors) {
  ...
  if (earth) try { earth.destroy({ children: false }); } catch (error) { failures.push(String(error)); }
  ...
  effects.disabled = reduced.disabled = grounding.disabled = true; status.textContent = 'Study disposed · owned mesh and texture resources retired.';
  return snapshot();
}
...
const ready = (async () => {
  ...
  cohesion = attachEarthSceneCohesion({ earth, background: sceneSprites[0]!, residents: sceneSprites[1]!, civetActor: earthActor.root,
    recipe: { schema: 'cf-earth-scene-cohesion/v1', worldKey: EARTH_RESIDENT_LAYER_PLAN_V1.worldKey,
      plan: EARTH_RESIDENT_LAYER_PLAN_V1, civetAsset: { width: manifest.width, height: manifest.height,
        alphaBounds: manifest.alphaBounds, contactY: manifest.contactY, sha256: manifest.sha256 }, civetPaws: manifest.civetPaws },
    enabled: grounding.checked, allowTestControls: true });
  listen(grounding, 'change', event => { rest('scene treatment changed'); cohesion!.setEnabled(grounding.checked); render(); journal.push({event:'grounding-control',trusted:event.isTrusted,enabled:grounding.checked}); });
...
win.__CF_CIVET_STUDY__ = Object.freeze({ ready, snapshot, probe, assetControls, cohesionProbe, retireCohesion, dispose });
