 ...
 *   node audits/CREATURE_SCENE_COHESION_20260908/study-runner.mjs FRESH_OUTPUT_NAME
 */
...
const release = acquireWorkspaceLock('Earth scene cohesion immutable local study'); fs.mkdirSync(out);
const sha = bytes => crypto.createHash('sha256').update(bytes).digest('hex');
const write = (file, bytes) => fs.writeFileSync(path.join(out, file), bytes, { flag: 'wx' });
const report = { schema: 'cf-earth-scene-cohesion-native/v1', certification: false, status: 'RUNNING', startedAt: new Date().toISOString(), sourceCommit: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(), node: process.version, sources: {}, modes: [], runtimeErrors: [], cleanupErrors: [], pending: null, limitations: [
...
function acceptCohesion(value) {
  assert.equal(value.outsideChanged,0,'Cohesion changed unowned background pixels');assert(value.outsideObserved>200000,'Background ruler is empty');
  assert(value.allChanged>1000,'Visible scene treatment is absent');
  assert.deepEqual(value.owner.alphaRow,[0,0,0,1,0]);
  assert.deepEqual([...new Set(value.contacts.map(c=>c.resident))].sort(),['Civet','Cranberry',"Devil's Club",'Frog','Persimmon','Platypus'].sort());
  assert.deepEqual(value.contacts.filter(c=>c.resident==='Civet').map(c=>c.id).sort(),['Civet:paw:0','Civet:paw:1','Civet:paw:2','Civet:paw:3']);
  for(const contact of value.contacts){assert(contact.pixels>0,'No observable ground in '+contact.id);assert(contact.darkened>0 && contact.maxDarkening>1, 'Actual ground shadow missing at '+contact.id);}
  for(const region of value.lighting){assert(region.ink>100,'Empty opaque lighting region '+region.name);assert(region.ratio>.82 && region.ratio<.97,'Scene light absent or overstrong for '+region.name+': '+region.ratio);}
  assert(value.owner.borrowedSourcesIntact);assert.deepEqual(value.owner.failures,[]);
}
...
try {
  for (const name of ['study.html', 'study-harness.ts', 'study-runner.mjs', 'serve-study.mjs', 'study.tsconfig.json', 'asset.json', 'cohesion-owner.ts']) {
  ...
  for (const name of ['earth-background-original.webp', 'earth-residents-without-civet.png']) {
    const bytes = fs.readFileSync(path.join(baselineBase, name)), expected = name.endsWith('.webp') ? scene.background : scene.residents;
  ...
  browser = await openChromiumCdp({ label: 'painted Civet local study', userDataPrefix: 'cf-earth-scene-cohesion-20260908', commandTimeoutMs: 15000, onEvent: event => { if (event.method === 'Runtime.exceptionThrown') report.runtimeErrors.push(event.params.exceptionDetails); } });
  ...
  for (const mode of [{ name: 'desktop', width: 1440, height: 1200, dpr: 1, mobile: false }, { name: 'phone', width: 390, height: 844, dpr: 2, mobile: true }]) {
    ...
    result.cohesion = [];
    const keepCohesion = (label, value) => {
      value.baseline=png(`${mode.name}-${label}-before.png`,value.baselinePng);delete value.baselinePng;
      value.sample=png(`${mode.name}-${label}-after.png`,value.samplePng);delete value.samplePng;
      value.lightingOnly=png(`${mode.name}-${label}-lighting-only.png`,value.lightingOnlyPng);delete value.lightingOnlyPng;
      value.withoutPawContacts=png(`${mode.name}-${label}-without-paw-contacts.png`,value.withoutPawContactsPng);delete value.withoutPawContactsPng;
      result.cohesion.push(value);return value;
    };
    const cohesive=keepCohesion('cohesion',await evaluate(`${api}.cohesionProbe('none')`));acceptCohesion(cohesive);
    for(const mutation of ['missingCivetContact','shiftContacts','noShadows','strongLight','backgroundGrade']){
      const value=keepCohesion(mutation,await evaluate(`${api}.cohesionProbe(${JSON.stringify(mutation)})`));let rejection=null;
      try{acceptCohesion(value);}catch(error){rejection=String(error);}assert(rejection,'Cohesion mutant accepted: '+mutation);
      result.controls.push({kind:'cohesion-'+mutation,rejected:true,reason:rejection});
    }
    const restoredCohesion=keepCohesion('cohesion-restored',await evaluate(`${api}.cohesionProbe('none')`));acceptCohesion(restoredCohesion);
    assert.equal(restoredCohesion.baseline.sha256,cohesive.baseline.sha256);assert.equal(restoredCohesion.sample.sha256,cohesive.sample.sha256);
    const paintedCanvas = async label => {
      await evaluate('window.scrollTo(0,0)'); await frames();
      const rect=await evaluate(`(()=>{const r=document.querySelector('#study-view canvas').getBoundingClientRect();return{x:r.x+scrollX,y:r.y+scrollY,width:r.width,height:r.height,scale:1}})()`);
      const shot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:true,clip:rect});
      const bytes=Buffer.from(shot.data,'base64'),file=`${mode.name}-native-grounding-${label}.png`;write(file,bytes);
      return {path:file,bytes:bytes.length,sha256:sha(bytes),rect};
    };
    const beforeToggle=await paintedCanvas('before');
    await native('cohesion');const offState=await snapshot();assert.equal(offState.cohesion.enabled,false);
    assert(offState.journal.some(row=>row.event==='grounding-control'&&row.trusted===true&&row.enabled===false));
    const offToggle=await paintedCanvas('off');
    await native('cohesion');const onState=await snapshot();assert.equal(onState.cohesion.enabled,true);
    assert(onState.journal.some(row=>row.event==='grounding-control'&&row.trusted===true&&row.enabled===true));
    const onToggle=await paintedCanvas('restored');
    const acceptPaintedToggle=(before,off,on)=>{assert.deepEqual(before.rect,off.rect);assert.deepEqual(before.rect,on.rect);assert.notEqual(before.sha256,off.sha256,'Native grounding control did not repaint');assert.equal(before.sha256,on.sha256,'Native grounding paint did not restore');};
    acceptPaintedToggle(beforeToggle,offToggle,onToggle);
    let staleRejection=null;try{acceptPaintedToggle(beforeToggle,beforeToggle,onToggle);}catch(error){staleRejection=String(error);}assert(staleRejection);
    result.controls.push({kind:'native-painted-grounding-toggle',before:beforeToggle,off:offToggle,restored:onToggle,stalePixelsRejected:true,reason:staleRejection});
    const retiredCohesion=await evaluate(`${api}.retireCohesion()`);
    retiredCohesion.scene=png(`${mode.name}-cohesion-retired-scene.png`,retiredCohesion.scenePng);delete retiredCohesion.scenePng;
    retiredCohesion.sibling=png(`${mode.name}-cohesion-retired-sibling.png`,retiredCohesion.siblingPng);delete retiredCohesion.siblingPng;
    result.retiredCohesion=retiredCohesion;assert.equal(retiredCohesion.restoredSceneChannelChanges,0);assert(retiredCohesion.siblingInk>1000);
    const co=retiredCohesion.retired;assert(co.disposed&&co.shadowContainerDestroyed&&co.filterDestroyed&&co.borrowedSourcesIntact);
    assert(co.graphics.every(g=>g.destroyed&&g.contextDestroyed));assert(co.borrowed.every(b=>b.alive&&b.parentIntact&&b.indexRestored&&b.filtersRestored));assert.deepEqual(co.failures,[]);
