...
root=Path(__file__).resolve().parents[2];audit=Path(__file__).resolve().parent;out=audit/'samples-final'
...
for label,path in [('Previous construction study',out/'previous-440.webp'),('Fine coat study',out/'natural-001-440.webp'),('Broad coat study',out/'storybook-001-440.webp')]:
 ...
 run([magick,str(path),'-background','#142035','-alpha','remove','-alpha','off','-gravity','south','-splice','0x48','-font','/System/Library/Fonts/Supplemental/Arial.ttf','-pointsize','19','-fill','#dfeafa','-annotate','+0+14',label,str(tile)])
