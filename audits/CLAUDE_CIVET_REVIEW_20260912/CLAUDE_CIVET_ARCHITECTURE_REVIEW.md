...
## Addendum: dedicated combat arenas and attack effects (Nick, 2026-09-12)

Nick's aim: very smooth creature and attack animations that represent what each attack is; turn-based is the frame of reference, not a constraint. Battles need not take place on the landfall painting: a dedicated combat arena in the game's theme and hand, as in Pokémon, is preferred.

- **Arena scene profile (kit addition, Nick to approve):** a full-bleed scene under the planets class, painted from the same system card and frozen style as the biome plate, composed for a fight: quiet middle ground, two combatant stands at the left and right thirds, low horizon, atmosphere behind, no competing foreground. One arena per biome family (43 at most; fewer if grouped by planet type first). Easier than the landfall plate: no residents to hide, composition built for two combatants.
