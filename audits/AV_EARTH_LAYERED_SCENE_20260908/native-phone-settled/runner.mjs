...
// node audits/AV_EARTH_LAYERED_SCENE_20260908/native-earth-layers-settled-runner.mjs DIST FRESH_OUTPUT phone|desktop|blocked|default
...
const report = { schema: 'cf-native-earth-layered-scene/v1', certification: false, status: 'RUNNING',
  ...
  limitations: [
    ...
    'Pixel comparison pauses the app ticker and extracts the same explicit full-stage frame; separate DOM rectangle overlap and 35 hit samples establish visible panorama placement. Neither proves native driver release or individual-species art quality.',
    'Phone exit uses the existing native Survey / Leave world control; right-click exit coverage is not claimed.',
    'A passive journal samples at most120 animation frames for at most2000ms around native Land. Missing transient motion is reported as a limitation; the runner never replays Land or forces a shake, policy change or layout synchronization.',
  ] };
const sourceNames = [
  ...
  'audits/AV_EARTH_LAYERED_SCENE_20260908/native-earth-layers-visible-runner.mjs',
  'audits/AV_EARTH_LAYERED_SCENE_20260908/native-earth-layers-diagnostic-runner.mjs',
...
const acceptComposition = value => {
  ...
  if(painted){
    ...
    for(const property of ['bounds','cssBounds','position','scale','anchor','rotation','sourceSize','spriteFrame']){
...
const startSettlementJournal = `(()=>{
  ...
  const take=()=>{
    ...
    try{
      ...
      const layers=s.app.stage.children.filter(c=>c.label==='earth-painted-background'||c.label==='earth-canonical-residents')
...
const summarizeSettlementJournal = journal => {
  ...
  const restingPairs=ended.filter(r=>r.layers.length===2&&r.animations.every(a=>a.playState!=='running'&&!a.pending)
    ...
    try{
      const control=await evaluate(`(()=>{
        ...
        return{method:'Each actual layer independently visible/hidden/restored in one full-stage frame with paused ticker; real source alpha read separately',
          noOp:diff(baseline,repeat),layerControls,canvasAlpha,identities,pools:pools(),composition,globeControl,cloudControl,legacyPlacementControl,tickerStopped:!s.app.ticker.started};
      })()`);
