...
const sourcePaths = JSON.parse(fs.readFileSync(path.join(audit,'source-freeze-final.json'),'utf8'));
...
for(const [name,cmd,args,cwd]of steps){
 const log=path.join(audit,'audit-final-'+name+'.log'),fd=fs.openSync(log,'wx'),start=Date.now();
...
fs.writeFileSync(path.join(audit,'browser-free-audit-final.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});
