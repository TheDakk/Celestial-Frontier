...
try{const {capture}=await server.ssrLoadModule(path.join(root,'audits/AV_EARTH_LAYERED_SCENE_20260908/fact-entry.ts'));const facts=capture();fs.writeFileSync('audits/AV_EARTH_LAYERED_SCENE_20260908/canonical-earth.json',JSON.stringify(facts,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(facts.request));}
