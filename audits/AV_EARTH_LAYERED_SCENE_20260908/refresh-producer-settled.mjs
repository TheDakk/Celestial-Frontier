...
const report=JSON.parse(fs.readFileSync('audits/AV_EARTH_LAYERED_SCENE_20260908/producer-settled.json','utf8'));
...
assert.equal(previous.sha256,'d433e91759fc762b00cc8bb1380359a98a56d345ac682ef9f502fdebe21e4636');
...
budget.calibration.selectionRule+=' Opt-in exact Earth layered scene (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after the exact Earth scene restores its measured resting layout when the final landing camera impulse completes, retaining the same scene assets, original fallback and UI placement. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('audits/AV_EARTH_LAYERED_SCENE_20260908/producer-refresh-settled.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
