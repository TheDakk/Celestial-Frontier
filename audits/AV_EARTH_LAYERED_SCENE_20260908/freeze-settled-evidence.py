...
assert json.loads((audit/'browser-free-settled.json').read_text())['status']=='PASS'
previous=json.loads((audit/'browser-free-verified.json').read_text())
assert [x['exitCode'] for x in previous['steps'][:4]]==[0,0,0,0]
source=root/'port/v2/apps/game/dist';target=root/'port/v2/apps/game/smoke/earth-layered-settled-evidence-dist-20260908'
...
producer=json.loads((audit/'producer-settled.json').read_text())['build']
...
with (audit/'evidence-dist-settled.json').open('x') as out:json.dump(receipt,out,indent=2);out.write('\n')
