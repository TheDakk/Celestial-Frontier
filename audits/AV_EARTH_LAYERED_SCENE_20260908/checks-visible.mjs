...
const steps=[
 ['producer-final','node',[path.join(audit,'producer-visible.mjs')],root],
 ['producer-refresh-final','node',[path.join(audit,'refresh-producer-visible.mjs')],root],
...
for(const [name,cmd,args,cwd]of steps){
 const log=path.join(audit,'visible-'+name+'.log'),fd=fs.openSync(log,'wx'),start=Date.now();
...
fs.writeFileSync(path.join(audit,'browser-free-visible.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});
