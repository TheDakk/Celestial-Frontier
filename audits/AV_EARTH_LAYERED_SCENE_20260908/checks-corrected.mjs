...
for(const [name,cmd,args,cwd]of steps){
 const log=path.join(audit,'corrected-'+name+'.log'),fd=fs.openSync(log,'wx'),start=Date.now();
...
fs.writeFileSync(path.join(audit,'browser-free-corrected.json'),JSON.stringify(report,null,2)+'\n',{flag:'wx'});
