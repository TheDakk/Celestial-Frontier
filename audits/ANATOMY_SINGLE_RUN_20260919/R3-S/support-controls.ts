...
import {createFamilyContactSolver as previous} from '../R2c/previous-contact.ts';
...
const phase={actionId:'hit',elapsedMs:200,durationMs:1000},pose={root:{rotation:0,dx:.001,dy:.01}},s=createFamilyContactSolver(r),zero=Object.fromEntries(s.chains.map(c=>[c.end,r.landmarks[c.end]])),old=previous(r).resolve(pose,phase),current=createFamilyContactSolver(r,zero).resolve(pose,phase);
