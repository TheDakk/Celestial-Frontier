...
const fit=path.resolve(base,'../02-fish/fit-01'),sourcePrompt=path.resolve(base,'../02-fish/prompt.txt');
...
const keyedFile=path.join(fit,'parts/keyed.png'),atlasFile=path.join(fit,'parts/atlas/salmon.png');
...
const prepared=patterns.map(pattern=>{
  ...
  assert(prompt.subarray(0,compiled.length).equals(compiled),pattern+' must retain exact compiled prompt prefix');
