...
function assertVoiceRetirement(report) {
  const before=report.before.audio.runtime,after=report.final.audio.runtime;
  assert.equal(before.voices.active,1,'Expected the one native music voice before acceptance');
  assert.equal(after.voices.started,before.voices.started+1,'Exactly one additional voice');
  assert.equal(after.voices.completed+after.voices.stopped,
    before.voices.completed+before.voices.stopped+2,'Music and settlement must both retire');
  assert.equal(after.voices.stolen,before.voices.stolen,'No stolen voice substitutes for completion');
  assert.equal(after.voices.active,0,'No voice may remain active');
  assert.deepEqual(after.voices.ids,[],'No tracked voice may remain');
  assert.equal(after.voiceMix.activeOwners,0,'No mixing ownership may remain');
  assert(after.nodes.active<=before.nodes.active,'Settlement retained extra nodes');
  assert.equal(after.faults.total,0);
  assert(Object.values(after.cleanup).every(n=>n===0));
}
...
try{
  ...
  const receipts=report.durableAfter.receipts.filter(r=>!report.durableBefore.receipts.some(b=>b.ordinal===r.ordinal));assert.equal(receipts.length,1,'Exactly one durable acceptance receipt required');assert(/^cf-v2-starter-charter-accept-witness\/v1:[0-9a-f]{64}$/.test(receipts[0].witness));// Receipt consumes the pre-action ordinal; runtime diagnostics expose the next available cursor.
  const ordinals={before:report.before.persistence.runtime.sessionOrdinal,pending:report.pending.persistence.runtime.sessionOrdinal,receipt:receipts[0].ordinal,atSource:p.ordinal,after:report.after.persistence.runtime.sessionOrdinal};
  const ordinalTransitionValid=(before,receipt,next)=>Number.isInteger(before)&&before>=0&&before<0xffff_ffff&&receipt===before&&next===receipt+1;
  assert.equal(ordinals.pending,ordinals.before,'Held action consumed an ordinal before commit');
  assert(ordinalTransitionValid(ordinals.before,ordinals.receipt,ordinals.atSource)&&ordinalTransitionValid(ordinals.before,ordinals.receipt,ordinals.after),'Consumed receipt / next ordinal mismatch');
  report.ordinalProof={...ordinals,controls:[{name:'receipt incorrectly compared with next cursor',accepted:ordinalTransitionValid(ordinals.before,ordinals.atSource,ordinals.atSource)},{name:'next cursor did not advance',accepted:ordinalTransitionValid(ordinals.before,ordinals.receipt,ordinals.receipt)}]};
  assert(report.ordinalProof.controls.every(c=>c.accepted===false),'Ordinal negative control accepted');persist();
  ...
  await delay(900);report.final=await evaluate(snapshot);assert.equal(await evaluate(`${settlement}.length`),1,'Repeated settlement source');assert.equal(report.final.trace.faults.length,0);assertVoiceRetirement(report);
