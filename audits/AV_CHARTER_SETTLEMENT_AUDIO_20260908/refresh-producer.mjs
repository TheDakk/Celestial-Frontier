...
const report=JSON.parse(fs.readFileSync('audits/AV_CHARTER_SETTLEMENT_AUDIO_20260908/producer.json','utf8'));
...
assert.equal(previous.sha256,'ff1d6051b34b2f2b7027b7f99e45a9d5ee692f5b0568f7133b7321d7a01c3b62');
...
budget.calibration.selectionRule+=' Starter Charter settlement audio (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after binding the existing700ms settlement cue to exact Starter Charter durable publication and barrier release with a native captured same-route shared-context ticket and matching draft audio copy. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('audits/AV_CHARTER_SETTLEMENT_AUDIO_20260908/producer-refresh.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
