...
# Prior draft digest, app build and producer metadata passed. Only release-oracle/test
# sources change in this successor; app output and Compendium measurement stay unchanged.
steps=[('develop-profile-corrected',['node','port/v2/tools/check-profile.mjs','--profile=develop']),('root-validation',['node','tools/validate.js'])]
receipt=audit/'preparation-corrected.json';assert not receipt.exists();results=[]
for name,command in steps:
 ...
 if r.returncode:print(log.read_text()[-5000:]);sys.exit(r.returncode)
receipt.write_text(json.dumps({'status':'PASS','steps':results},indent=2)+'\n');print('CORRECTED PREPARATION PASS',flush=True)
