...
for (const [i,row] of rows.entries()) {
  ...
  const left=(i%3)*360,top=Math.floor(i/3)*345;
...
await sharp({create:{width:1080,height:1035,channels:3,background:'#eee9df'}}).composite(tiles).png().toFile(path.join(import.meta.dirname,'review-sheet.png'));
