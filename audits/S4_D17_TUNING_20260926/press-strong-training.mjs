...
const config={epoch:'training-10-press-strong',source:'same exact encounter source as integrated 2fae1e73',count:512,seedBase:50000000,poolSize:50000,careFed:[0,10,20,40,60],
 ...
 candidates:[1.6,1.7,1.8].map(dealt=>({press:{dealt,taken:1.7},guard:{dealt:.65,taken:.6,openerBlunt:.25},evade:{dealt:.64,dodge:.32}})),phase:{atFraction:.5,dealt:1.02,taken:.98}};
fs.writeFileSync(dir+'/press-strong-training-policy.json',JSON.stringify(config,null,2)+'\n');
...
fs.writeFileSync(dir+'/press-strong-training-corpora.json',JSON.stringify({corpora,solos},null,2)+'\n');
...
for(let index=0;index<config.candidates.length;index++){
 ...
 for(const [kind,plans] of Object.entries(corpora)){
  ...
  rows.push(r);console.log(index,JSON.stringify(r));fs.writeFileSync(dir+`/press-strong-training-${index}-${kind}-choices.json`,JSON.stringify(choices,null,2)+'\n');
 }
 results.push({index,tuning:globalThis.__s4,phase:globalThis.__s4phase,rows});fs.writeFileSync(dir+'/press-strong-training-results.json',JSON.stringify(results,null,2)+'\n');
}
