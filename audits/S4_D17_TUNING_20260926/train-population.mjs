...
const policy={schema:'cf-s4-d17-training/v1',trainingSeed:40000000,poolSize:50000,count:128,
 guardianSlots:[[.6,.8],[.7,.9],[.8,1]],wildPower:{easy:[180,220],normal:[250,350]},wildRatio:{easy:[1.8,2.2],normal:[.9,1.1]},soloRatio:[.8,1],
...
const solos={};for(const kind of ['guardian','titan']){const enemy=encounters[kind].defender.battleGenome,power=api.battleStats(enemy).total;solos[kind]=Array.from({length:policy.count},(_,i)=>({mode:'auto',defender:{name:kind,genome:enemy,phase:true},party:[{name:'solo',genome:choose(power*.8,power,i*17,[enemy.seed]).genome,stance:'balanced'}]}));}
