...
function stanceStats(stats, stance) {
	...
	if (stance === "press") {
		ab.dmg = num("dmg", 1) * globalThis.__s4.press.dealt;
		ab.taken = num("taken", 1) * globalThis.__s4.press.taken;
	} else if (stance === "guard") {
		ab.dmg = num("dmg", 1) * globalThis.__s4.guard.dealt;
		ab.taken = num("taken", 1) * globalThis.__s4.guard.taken;
	} else if (stance === "evade") {
		ab.dmg = num("dmg", 1) * globalThis.__s4.evade.dealt;
		ab.dodge = num("dodge", 0) + globalThis.__s4.evade.dodge;
...
function runEncounterV1(plan, decisions = []) {
	...
	const phasedB = () => {
		const ab = { ...B0.ab };
		ab.dmg = (typeof ab.dmg === "number" ? ab.dmg : 1) * globalThis.__s4phase.dealt;
		ab.taken = (typeof ab.taken === "number" ? ab.taken : 1) * globalThis.__s4phase.taken;
		return {
			...B0,
			ab
		};
	...
	for (let index = 0; index < plan.party.length && outcome === null; index++) {
		...
		const strike = () => {
			...
			if (att.first && (turnA ? firstA : firstB)) {
				const first = !turnA && guardOpener ? 1 + (att.first - 1) * globalThis.__s4.guard.openerBlunt : att.first;
				dmg *= first;
				fs = true;
			}
			if (att.enrage) {
		...
		for (let round = 0; round < 26 && hpA > 0 && hpB > 0; round++) {
			...
			turnA = !turnA;
			if (phaseEnabled && !phaseActive && hpA > 0 && hpB > 0 && hpB <= maxB * globalThis.__s4phase.atFraction) {
				phaseActive = true;
