...
const config={epoch:'training-03-care-and-forecast',source:'same exact encounter source as integrated 2fae1e73',count:128,seedBase:50000000,poolSize:50000,careFed:[0,10,20,40,60],
 ...
 candidates:[{press:{dealt:1.3,taken:1.4},guard:{dealt:.7,taken:.7,openerBlunt:.25},evade:{dealt:.9,dodge:.08}},
 {press:{dealt:1.4,taken:1.7},guard:{dealt:.65,taken:.575,openerBlunt:.25},evade:{dealt:.6,dodge:.32}},
 {press:{dealt:1.4,taken:1.7},guard:{dealt:.6,taken:.5,openerBlunt:.25},evade:{dealt:.6,dodge:.32}}],phase:{atFraction:.5,dealt:1.02,taken:.98}};
fs.writeFileSync(dir+'/forecast-training-policy.json',JSON.stringify(config,null,2)+'\n');
...
fs.writeFileSync(dir+'/forecast-training-corpora.json',JSON.stringify({corpora,solos},null,2)+'\n');
...
for(let index=0;index<config.candidates.length;index++){
 ...
 for(const [kind,plans] of Object.entries(corpora)){
  ...
  rows.push(r);console.log(index,JSON.stringify(r));fs.writeFileSync(dir+`/forecast-training-${index}-${kind}-choices.json`,JSON.stringify(choices,null,2)+'\n');
 }
 results.push({index,tuning:globalThis.__s4,phase:globalThis.__s4phase,rows});fs.writeFileSync(dir+'/forecast-training-results.json',JSON.stringify(results,null,2)+'\n');
}
