...
## Version 3 outcome and version 4 requirements (2026-09-12)

The two reference images painted from version 3 (`frontier-sheet-01`, `frontier-plate-01`) came out as fantasy token-library art and were rejected by Nick. Cause: section 2 names the TSR illustrators and rules that alien life is chitin and membrane "rather than fur and feather"; the sheet format itself is a token-library artifact. Version 3 is retired under its date. Version 4 keeps the structure and replaces the anchor with the approved Living Worlds language (see CODEX_HANDOFF.md, first actions, item 4). The reference lock is resolved to images that already exist: the Living Worlds triptych as the scene plate and the Discovery Atlas as the sheet. The star table becomes the game's thirteen star kinds. The full source-derived vocabulary the kit must cover is in GAME_VOCABULARY_COVERAGE.md.
...
## Updated program (version 3; superseded by program version 4 in CODEX_HANDOFF.md)
