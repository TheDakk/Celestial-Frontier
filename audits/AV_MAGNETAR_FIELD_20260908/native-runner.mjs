...
for (const file of [saveFile, starFile, addressFile, path.join(repo, 'port/v2/packages/scene/src/charter.ts'), path.join(repo, 'port/v2/apps/game/src/main.ts'), path.join(repo, 'port/v2/apps/game/src/system-star-field.ts'), fileURLToPath(import.meta.url)]) report.sources[path.relative(repo, file)] = hash(fs.readFileSync(file));
