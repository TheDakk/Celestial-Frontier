...
const report=JSON.parse(fs.readFileSync('/private/tmp/cf-av-producer-20260907.json','utf8'));
...
assert.equal(previous.sha256,'aef21961ea701ea7d01e3b4bc5b9fe01a8efff328ca3baa33637e98e3a3acafe');
...
budget.calibration.selectionRule+=' Playable landing/battle batch (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after finite Scout arrival/shared audio handoff, canonical Chronicle staging and exact Explorer Frontier Resolve metadata repair. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('/private/tmp/cf-av-refreshed-producer-20260907.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
