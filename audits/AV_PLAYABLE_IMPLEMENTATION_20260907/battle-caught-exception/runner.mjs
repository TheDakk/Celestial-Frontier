...
let browser, captureFailure; const pausedExceptions=[], resumeJobs=[], resumeErrors=[];
...
try {
  browser = await openChromiumCdp({ label:'native playable landing review', userDataPrefix:'cf-av-landing-20260907',
    onEvent:event=>{if(event.method==='Runtime.exceptionThrown')report.errors.push(event.params.exceptionDetails);if(event.method==='Debugger.paused'){pausedExceptions.push({reason:event.params.reason,data:event.params.data,frames:event.params.callFrames?.slice(0,10).map(f=>({functionName:f.functionName,url:f.url,location:f.location}))});resumeJobs.push(browser.send('Debugger.resume',{},event.sessionId).catch(error=>resumeErrors.push(String(error))));}}});
  ...
  captureFailure = async () => {
    const facts = await evaluate(`({bodyClass:document.body.className,battle:window.__cfBattleReview,landingInputs:window.__cfLandingReview?.inputs,panel:document.querySelector('#combatpanel')?.outerHTML,survey:document.querySelector('#survey')?.outerHTML,toast:document.querySelector('#toast')?.textContent})`);
    fs.writeFileSync(path.join(out,'failure-state.json'),JSON.stringify(facts,null,2)+'\n');
    const shot = await send('Page.captureScreenshot',{format:'png'});
    fs.writeFileSync(path.join(out,'failure.png'),Buffer.from(shot.data,'base64'));
  };
  const waitFor = async (name, condition) => {
    report.steps.push({name,condition,status:'waiting',at:new Date().toISOString()});persist();
    await evaluate(`new Promise((resolve,reject)=>{const end=performance.now()+12000;function tick(){if(${condition})resolve(true);else if(performance.now()>end)reject(Error(${JSON.stringify(name)}));else setTimeout(tick,25)}tick()})`);
  ...
  try { await captureFailure?.(); } catch (fault) { report.failureCaptureError=String(fault); }
  report.caughtExceptions=pausedExceptions; await Promise.all(resumeJobs); report.resumeErrors=resumeErrors;
} finally {
