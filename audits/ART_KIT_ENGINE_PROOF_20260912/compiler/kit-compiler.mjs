...
function compileEarthArtKitV4(input, kit) {
	...
	return freeze({
		...
		prompts,
		plate,
		familyReferences: exemplars.map((row) => {
...
function compileEarthKitEngineV4(input, kit, assets, settings) {
	...
	const subject = ["Finish the existing Earth temperate landfall composition in the frozen painted hand. Preserve all six placed organisms, their exact anatomy, counts, silhouettes, materials, locations and relative sizes; unify shared light, contact and atmosphere without adding or moving organisms.", ...model.recipe.residents.map((row) => `One ${row.name}, exactly one ${row.name}, at ${Math.round(row.placement.x * 100)} percent across and grounded at ${Math.round(row.placement.groundY * 100)} percent down: ${row.namedRule.diagnostics.map((d) => d.required).join("; ")}.`)].join("\n");
