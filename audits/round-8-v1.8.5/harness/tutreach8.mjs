...
const URL=process.env.CF_URL || 'http://127.0.0.1:8909/game.html';
const DEV=[{id:'iphone-14-pro',w:393,h:852,dpr:3},{id:'galaxy-s8',w:360,h:740,dpr:3},
           {id:'iphone-se',w:375,h:667,dpr:2},{id:'ipad-mini',w:744,h:1133,dpr:2},
           {id:'ipad-air',w:820,h:1180,dpr:2},{id:'desktop-1440',w:1440,h:900,dpr:1}];
const SURF=['#panel','#log','#codex','#vistabox','#reveal','#tutbox','#codexbtn','#logbtn','#cargobtn','#chbtn','#sheet','#yard','#tray','#searchres','#chpanel','#records','#setpanel'];
  ...
  try{
    ...
    while(guard++<200){
      ...
      if(!seen.has(ts.step)){ seen.add(ts.step); stall=0;
        ...
        if(ts.step>=5 && ts.step<=9) await p.screenshot({path:`/root/cf/out/tr_${d.id}_step${ts.step}.png`});
      } else stall++;
      // --- advance
