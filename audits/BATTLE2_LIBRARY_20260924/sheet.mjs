...
const O = import.meta.dirname, take = process.argv[2] ?? '01', set = process.argv[3] ?? 'land', films = (set === 'water' ? ['salmon-vs-octopus', 'eagle-vs-salmon', 'crab-vs-starfish'] : ['python-vs-tarantula', 'eagle-vs-beetle', 'chimpanzee-vs-centipede', 'treefrog-vs-fruitbat']).map((f) => f + '-' + take), stills = ['turn0-hit-approach-50.png', 'turn0-hit-impact.png', 'turn1-hit-impact.png'];
...
fs.writeFileSync(path.join(O, 'library-battles-sheet-' + (set === 'water' ? 'water-' : '') + take + '.png'), writePng(W, H, out)); console.log('sheet', W, H);
