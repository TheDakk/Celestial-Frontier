...
export function measureLayeredStanceReach(record:RecordInput,supports:Readonly<Record<string,ContactSupport>>,maximumPerStance:number,card:BodyCard=compileBodyCard(record,record.genome)){
 ...
 for(const row of poses){const failure=checkRow(row,maximumPerStance*candidate/grid);if(failure){firstRefusal=failure;restrict(row);}}
 for(;;){let changed=false;
  for(const row of poses){const failure=checkRow(row,maximumPerStance*candidate/grid);if(failure){firstRefusal=failure;restrict(row);changed=true;break;}}
  if(!changed)break;
 }
