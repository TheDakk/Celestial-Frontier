...
try {
  ...
  await wait('native trinary Survey offers Enter', `document.querySelector('#survey [data-act="travel"]')?.textContent.includes('Enter system')&&${ready}`);
  await click('#survey [data-act="travel"]');
  await wait('native trinary rendered entry', `${S}.api.state().mode==='system'&&${S}.api.state().star===${star.seed}&&${S}.api.state().renderedScene.starKey===${S}.api.state().navStarKey&&${ready}`);
  assert(await evaluate(`window.__cfTrinaryInputs.some(e=>e.trusted&&e.act==='travel')`));
