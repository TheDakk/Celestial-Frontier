...
const report=JSON.parse(fs.readFileSync('audits/AV_TRINARY_COMPANION_20260908/producer.json','utf8'));
...
assert.equal(previous.sha256,'66663069bae984202dfd0726e21103e1d8b9412516494e216ac4dbb56771768b');
...
budget.calibration.selectionRule+=' Trinary stellar companion (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after restoring the generated third companion and its canonical three-stop glow/orbit while correcting triple-system caption and matching draft copy. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('audits/AV_TRINARY_COMPANION_20260908/producer-refresh.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
