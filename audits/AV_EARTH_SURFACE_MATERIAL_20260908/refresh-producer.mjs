...
const report=JSON.parse(fs.readFileSync('audits/AV_EARTH_SURFACE_MATERIAL_20260908/producer.json','utf8'));
...
assert.equal(previous.sha256,'f5e16260e729a79e5dc865704525dd6f501b59a800d8b525171d19556d92ac1f');
...
budget.calibration.selectionRule+=' Opt-in canonical Earth material proof (2026-09-08): live producer '+previous.sha256+' becomes '+current.sha256+' after adding optional analytic grain/relief, water sheen and inner atmospheric rim to the unchanged canonical atlas, with one shared program/group and per-view material reset. '+Object.entries(current.inputs).map(([k,v])=>k+' '+v.relativePath+' '+v.sha256).join('; ')+'. Measurement/ruler/ceilings/history unchanged. Scoped native presentation diagnostics are not a fresh Compendium certificate; SceneMemory remains production-quarantined.';
...
fs.writeFileSync('audits/AV_EARTH_SURFACE_MATERIAL_20260908/producer-refresh.json',JSON.stringify({previous:previous.sha256,current,measurementUnchanged:true,rulerUnchanged:true,ceilingsUnchanged:true,sceneMemoryQuarantined:true},null,2)+'\n',{flag:'wx'});
