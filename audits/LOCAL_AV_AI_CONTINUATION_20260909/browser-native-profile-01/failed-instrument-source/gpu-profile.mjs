...
export function parseNativeProfile(rawJson,options={}) {
  ...
  for(const e of events){
    ...
    if(e.cat==='Api'){
      ...
      aggregate(gpu,JSON.stringify([e.name,shapes]),{programName:e.name,shapes:e.args.shapes},e);
...
export function parseProofOptions(args){
  ...
  const supported=new Set(['--without-reference','--identity-reference','--preflight','--profile']);
  ...
  if(outputs.length!==1||!outputs[0]||flags.some(x=>!supported.has(x))||new Set(flags).size!==flags.length)
    throw Error('Usage: run-browser-proof.mjs EVIDENCE_DIRECTORY [--without-reference | --identity-reference] [--preflight | --profile]');
  ...
  return {output:outputs[0],referenceEnabled:!has('--without-reference'),identityReference:has('--identity-reference'),
    preflight:has('--preflight'),profile:has('--profile')};
}
