...
import type {CreaturePoseV1,CreatureRigRecordV1,CreaturePartsBindingV1} from '../../port/v2/apps/game/src/creature-rig.js';
...
export function createDiagnosticFamilyContactSolver(record:CreatureRigRecordV1,paintedSupports:Readonly<Record<string,ContactSupport>>={},diagnostic:{passes:number;trace:(v:any)=>void}){
 ...
 return {chains,scaleLength,stride,resolve(input:CreaturePoseV1,phase:ContactPhase){
  ...
  for(let pass=0;pass<=(hasOffset?diagnostic.passes:0);pass++){
