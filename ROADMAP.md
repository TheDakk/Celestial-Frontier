...
Done this batch: bear merged, static RED reviewed on the stage, G6 built + tested + filmed (item -39). Next, in order: (1) the guardian eye findings as one constant each behind tests — rearing headroom (fill measured from the record's tallest pose, or 0.8) and a guardian stand offset in the arena composition — then re-film bear-vs-crab; (2) on Codex's report: re-merge, re-film mud/vent once their reach lifts, re-check the A2 outcome test; (3) the compiler: the two merged near-claw fingers (coconut/freshwater) and crab `leg3Far` remain the only crab defects — next lever is the fork-aware ridge; the roster batch through the compiler the moment IC-4 passes on the crabs (one sheet for Nick).
