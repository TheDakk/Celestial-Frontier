...
Done this batch: bear merged, static RED reviewed on the stage, G6 built + tested + filmed (items -39/-40; films -01 and -02 await Nick's eye — one constant each). Next, in order: (1) the bear as TARGET on film (crab attacks the bear: the guardian's hit/faint under the fill) and the guardian in `e1-outcomes.test.ts` as attacker and target (D2 G6's last line); (2) on Codex's report: re-merge, re-film mud/vent once their reach lifts, re-check the A2 outcome test; (3) the compiler: the two merged near-claw fingers (coconut/freshwater) and crab `leg3Far` remain the only crab defects — next lever is the fork-aware ridge; the roster batch through the compiler the moment IC-4 passes on the crabs (one sheet for Nick).
