...
### Codex (openai lane)
Holds with writers frozen. One ask when Nick releases it: apply `audits/INTAKE_COMPILER_20260921/FOLDED_DECLARATIONS.md` — a `folded` class in `cf.anatomy-presence` and the three declarations — in its writers (schema bump its call); the compiler already reads the field.
