...
### Codex (openai lane)
Holds with writers frozen (IC-4 not passing). Its hidden-rule note is answered and consumed (slice 20). Nothing further asked.
...
### Nick
**One decision from the compiler work (2026-09-21):** authorize an interior-edge stage for limbs painted over the body (colour/edge evidence inside the mask) — without it, folded rear legs stay empty slots and IC-4 cannot pass on the freshwater and crab fits. Otherwise nothing to decide until the stop. R9 addendum's two answers (finisher model = the accepted one; arena-scale crab on the sheet)
