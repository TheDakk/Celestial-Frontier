...
- **Life:** `makeGenome` / `crossGenome` / `evolveGenome`; per-kingdom SVG portraits via `speciesPortrait` (cache capped at 1,200). Fifteen rarity tiers, Common → Omnipotent (the "deep spectrum" and "summit" tiers past Unique arrived in v1.3, alongside named Apex Guardians who wear the summit grades and iridescent foil card styling for tier 12+).
