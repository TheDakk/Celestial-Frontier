...
> save schema, migration, transaction, receipt, recovery or player-data behavior. See `ROADMAP.md`
> for the live handoff and `port/v2/README.md` for the canonical battery commands.

> **2026-09-01 current D-TRAIN convergence green evidence overlay (matches exact signed code;
...
>
> D-TRAIN now waits under one bounded deadline for two identical primary reads bracketing the exact
> live/focus sample: Training is released, live and durable `rn` match the fixture, pending release
> state is clear, only the exact Guide bulletin owns focus, Training/chrome are released and Atlas
> is closed. Exact native write topology is judged only after its third row arrives. The same chain
> covers the deadline-clipped adaptive Guide-tail proof and pre-shift/exact-resume ceremony repair.
> Ceremony presentation ordering is the only product behavior change; the legacy game, Training
> transaction, distinct bulletin autosave, save schema/migrations, deterministic generation,
> creatures, plants,
> biomes, Guardians, loot, audio, CSS, numeric rulers, browser pin/version policy and no-retry policy
> are unchanged. Hosted run `33572309149` remains immutable red and its ninth authorization
> consumed; no new hosted attempt, push, merge, release, version bump, publication or deployment is
> authorized.
...
> strict. Exact clean SSH-signed source `a45220421195042a8702aa1265e96d40d839fc38` passed the
...
> Each browser stage ran once with no retry. No save schema, migration, retry, timeout, browser pin,
> persistence ruler or gameplay payload
...
> Slice `20260831155943782-24588-a98f13f2c7b7` stopped once/no-retry after **359,647 ms** with one
> `world-identity-collision-controls-failed` scope; named verification exited 2 and Glass did not
> run. The base collision product outcome was green and exposed no save or product defect.
>
> The runner mixed newest-first Atlas rows `[Beta, Alpha]` with receipt order `[Alpha, Beta]`, so
> the pointer mutant selected the same row and was inert. Signed repair/evidence commit
> `23eb6dabeaf40cf0bc7878272b4f4893ad422113` derives a distinct measured sibling, rejects inert/equal
> cases, causal-stops a red base and executes a reversed-order regression. It adds no save field,
> schema or migration and changes no gameplay, art, creature, biome, ruler, timeout, retry or Edge
> contract. Focused tests pass **1 file / 6 tests**; the
> complete develop profile is green at **253 files / 2,561 passed / 1 skipped**, all TypeScript
...
> `e9c978bfdb885da8cbc6002c0f9af416d96120ca26a617b3758b898652b85a01`; only the fresh clean
> signed candidate and its new browser chain remain pending.
...
> with **3 findings / 3 scopes**; Glass did not run. The two roots were whole-`style` restoration
...
> retains SHA-256 `11483b3d1e9c2760a00354e6511a27889e62a4f092ee6847589dc1b7a0bfb2c1`.
>
...
> their named runs as history):** exact signed source
...
> `20260830-pr35-execution-late-dc6004cf4426-compendium-certification` once/no-retry and failed again
> after **3,112 ms** at phone veteran-Earth Planetside: its worker reached `ready`, but its first
> painter import failed, zero outcomes ran and all 78 were blocked. Neither red was retried or
> followed by a downstream browser stage. The 38d carrier is
...
>
> Fetch-time adoption was falsified as a product repair. Worker construction stays lazy, while the
> species-art worker statically owns the complete painter and the biome-vista worker statically owns
> the complete renderer. Production builds reject `import()` and every external static JavaScript
> import in both worker entries. The generated service worker performs no fetch-time adoption, so
> every unpinned non-navigation request remains exact 503. This preserves cache-only mixed-build
> safety without a network fallback, cross-build guess, sleep or retry.
>
...
> `0af0f5884c0eec67cea7c6696c20a2c691c669fa93ee255fd1c54d17b56d5010`. The final derived,
> browser-uncertified changed-head producer / index / owner / generated-service-worker authorities are
...
> `927f3e1ac45346ec4b0fa5f69a0b4486008d98783b5bd966c7f9aced7f16530f`. Current Scene build /
> game-main authorities are `96bb633f5c92675709655a7c24c2eb92f442324f431087c02b752ad665fcfa9d` /
> `07bdf8aac9bd8224870f2749df18461576d733c55555698dd247ddeffb83f831`.
> Combined worker/painter asset `assets/species-art.worker-DnnSDKMy.js` remains
> `25519cabdf0963bdc722b591855e7c7fdaaecbead63fdfa2d499bf35382f7172`; current Compendium / Scene
> budget-file authorities are `eb286baba5843e46110055e29ffeb8098a7f4c9c301d85727ff07427c5ca813c` /
> `6ce69d1a6888a1a79c159f79a6cf9ba671f1aeec1e40430d54931da36e88cf5d`. The green printer binds a
> **964-module / 52-file** build. Exact clean signed source
> `941ba45a96e5baabadc255d53db86fa935cefe81` then passed Compendium **78/78** once/no-retry in
> **65,731 ms**, with both profiles, all six review PNGs and exact named verification. Its immutable
...
>
> The same exact source then ran Slice `20260830115041916-36220-7ed2dd2ef398` exactly once and
> stopped terminal red after **63,106 ms**, with **63 findings / 42 scopes**, six partial screenshots
> and no retry. The first 62 findings are Guide/Release instrument drift from two sources: probes
> synchronously inspected the previous DOM while archive publication deliberately waited a
> microtask, and the oracle still expected the superseded 26-partial/15-unavailable inventory plus
> older Scout, conquest, reward and audio copy instead of current 9/41/34/7 authority. The
> independent final product finding is named-CF1 Follow starvation after accepted custom-world
> naming. Glass and
> Recovery did not run. Exact Slice JSON is **40,180 gzip / 522,130 raw bytes** with SHA-256
...
> gzip / 253,140 raw bytes** with SHA-256
...
>
> The changed-head transaction repair removes the custom-world name transaction's early aggregate
> catch-up. Direct Travel owns its accepted route, arrival, galaxy visit and source-proved galaxy-
> event aggregate; Follow owns that arrival aggregate plus Follow/Jumps/`wayfarer`. A post-name route
> that refuses or otherwise does not join progression queues exactly one catch-up after the
> world-name transaction settles. The adapter reserves that composite handoff synchronously:
> default ordinary `persistView` attempts set one deferred latch instead of replacing `activePersist`,
> then one checkpoint is re-armed only after route settlement and catch-up enqueue. Exact private
> heartbeat and pagehide-lifecycle checkpoint owners remain nondeferrable; heartbeat already refuses
> to queue behind a product action, while lifecycle may validly supersede the route. Existing action
> receipts, canonical F4/F3
> CAS, checkpoint verification, refusal, no-retry and read-only convergence rules remain in force.
> The bounded timeout remains diagnostic. Guide/Release probes now await exact requested DOM
> identity and verify the canonical **9 categories / 41 unique topics / 34 partial / 7 unavailable**
> map with mapping and stale-publication controls.
>
> The sealed-worker portion changes disposable exact-app-byte routing only, while the named-CF1
> portion changes action ordering/aggregate ownership inside the existing transaction model. Neither
> adds an expedition field, save schema, migration, retry, rollback of IndexedDB, or product-state
> fallback. Painter pixels,
> seeds, genomes, anatomy, art structure, rulers, ceilings and the 78-outcome inventory are also
> unchanged. The current changed head has no browser certificate; Edge `152.0.4191.53` / CDP `1.3`
> remains provenance only and cannot trigger a rebaseline or threshold change.

> **2026-08-30 historical post-claim PWA/save boundary (matched code at that source; superseded by
> the sealed single-file boundary above):** exact signed source
...
> Edge `152.0.4191.53` / CDP `1.3`. All 78 outcomes completed: 74 passed; phone/desktop
> `cap-shrink` and `settled-jobs` stored red. Their evidence nevertheless proves exact 256 → 96 cap
> shrink, 6,690,816 decoded bytes, 160 disposals, four sealed warm cycles, restored device class and
> released/balanced workers. The shared false clause required a stale non-null paint `lastError`
> after replacement recovery. The exact report remains red and immutable; no successor browser
> stage ran.
>
> Current diagnostics v2 therefore requires `lastError === null` on every released/recovered
> current snapshot, including non-final and post-cap samples, while the deliberate fault remains
> bound by exact cumulative paint/phase/result arithmetic. A terminal current product error still
> requires its exact non-null trusted receipt; historical v1 records remain replayable without one.
> This is measurement lifecycle, not save authority: it adds no save field, migration, write or
> fallback. The preserved carrier is
...
> SHA-256 `e4eb2aba1079a1d42b1da5e7f97d236105917fd497035937b1f6855d63a4289e`, with 8/8 independent
> replay.
>
...
> Browser-free repair coverage is 591 Compendium selftest controls, independent 8/8 preserved-
> carrier replay, the full **239 files / 2,431 passed / 1 skipped** suite, all three TypeScript
> programs and the green authority printer. The fixed ruler, numeric ceilings and 78-outcome inventory are
> unchanged. Certification remains open; only a materially changed signed source may make one new
...
> request inherits the initiating client's selected build onto its `resultingClientId`; current
> species-art and biome-vista worker entries are statically complete, live-build retention inspects
> all client types, and an absent reserved
...
> Arc 5 compact authority, v4 mirror, F4 authority, immutable receipt and revision in one CAS.
> Every scenario contains exactly 18 Arc 4 replacement writes plus the five Arc 5 carrier
> replacements. That mirror is the exact full `codex`,
...
>
> **Implemented compact Arc 5 V2 carrier.** `player/arc5.ownership.migration` is the version-2
> manifest owner and `creatures/arc5.ownership.delta.0` through `.3` are exactly four fixed generic
> delta-shard owners. Every prepared replacement is therefore one exact five-write tuple. The
> manifest binds the exact Arc 4 source schema/mode/revision/digest, reconstructed V2 target
> schema/mode/revision/digest, canonical delta digest and row count, fixed shard count four and four
> ordered shard digests. Each deterministically packed shard binds its own index/range/count/total,
> digest and ordered rows; all four carriers are present even when their rows are empty.
>
> Every read independently parses the manifest and four shards, reconstructs
> `V2 = exact Arc 4 source + exact delta`, and recomputes the source, delta, target and per-shard
> fixed points. Missing, extra, misplaced, duplicated, future, corrupt, over-bound or drifted
> evidence protects rather than repairs. An absent carrier bootstraps only after the exact Arc 4
> fixed point. An aligned legacy-v1 single certificate is still readable and upgrades receipt-free
> to the exact five current carriers in the one shared boot CAS. An aligned current-v2 carrier is a
> strict zero-write fixed point. Any protected result cancels previously staged boot work and
> restores the durable saved view, Atlas routes, `items`, `equip` and `equipAff` before runtime
> creation.
>
> The shards contain only deterministic V2-exclusive or changed rows—bred acquisitions,
> changed/live creature or specimen rows, tombstones and the V2 scout override—not a second copy of
> unchanged Arc 4 ownership. Source-only Arc 4 growth changes fixed-size manifest source/target
> evidence while all four canonical empty-shard bytes remain identical; Arc 5 namespace and
> unchanged-state storage growth are explicitly O(1). Capture pre-certifies and atomically replaces
> exactly 18 Arc 4 plus five Arc 5 carriers before RNG. The internal V2-only successor authority also
...
> Arc 4 writes and all five Arc 5 carrier writes from the final detached candidate inside the same
...
> exact manifest/shards plus source/delta/target evidence before publication; any mismatch reloads without a second
...
> SHA-256 are `b19ba6f749cb12e5c8fe23bdc1e779fce8fb04ebbb47653e65313ef2f47784ad` /
> `5a5be42cea5a67401472fe214f663ce8ca1bed7b3c6dbccd29b83fd8d1ea9225`.
> Glass passed on the same Edge in 71,449 ms with 12/12 viewport and reload rows, 95/95 controls,
> 36/36 Arc 4 outcomes, no blocked/omitted control and zero findings/instrument failures/retries. Its
> five-carrier fingerprint, coherent current-v2 manifest/shard corruption and source-growth
> four-empty-shard byte-invariance controls are non-vacuous. Report SHA-256 is
> `c46b81fbac123c1df22b03949e64589bf1d8d52898613efe01c809b840df177e`.
> Both bind source commit `48ce0b1662a59b21070667be339a1e59503e1f19` on `openai/mac`, status
> `729e139b14a978c39457ed9ab24990b7e1fd3f3bb63fef3efeeca24b45e4fb9f` and working tree
> `a375f64327e00f9aeaa4e7f46b8f5b4af271aad5230ba301484114520ec8e361`; independent audits are
> CLEAR. The focused compact-v2 gate passed 109/109. Root/app/worker/noUnused TypeScript,
> `artunused` and the 876-module Vite build passed. The final full run recorded 102 passing files and
> one failing file, with 1,218 tests passed, one failed and one skipped (1,220 total); focused
> Compendium was 18 passed/one failed and its selftest passed 222 controls. The sole known red is the
> intentionally deferred Compendium measurement-authority seal (`6a961df8…` stored versus
> `2ab18865…` live), whose exact drift is `packageLock+appPackage`; the dependency graph is not frozen,
> so it has not been rebaselined.
