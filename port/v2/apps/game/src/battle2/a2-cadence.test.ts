...
const baseLayout = composeArena({ id: 'e1', groundLineNormalized: 0.78, plates: { far: TEX, mid: TEX, near: TEX } }, FRAME);
...
const contextFor = (name: string, mass: number, card: ReturnType<typeof compileBodyCard> | null, attackFor?: TurnOutcomeContext['attackFor'], over: Partial<TurnOutcomeContext> = {}): TurnOutcomeContext => ({
  ...
  arena: { groundLineY: baseLayout.groundLineY, stands: baseLayout.stands }, seed: 593405465, anchorsForTheme: (t) => (t === 'wild' ? wild() : null), readyMs: 800, commandMs: 300,
...
const contextTarget = (name: string, mass: number, card: ReturnType<typeof compileBodyCard>, over: Partial<TurnOutcomeContext> = {}): TurnOutcomeContext => ({
  ...
  arena: { groundLineY: baseLayout.groundLineY, stands: baseLayout.stands }, seed: 593405465, anchorsForTheme: (t) => (t === 'wild' ? wild() : null), readyMs: 800, commandMs: 300, ...over,
});
...
describe('A2 cadence — walk whole cycles planted, lunge the rest', () => {
  // the Brown Bear guardian (D2 G6) walks the same planted cadence on the guardian stands with the tallest-pose fill
  const CASES = [['crab', 'leg0NearFoot'], ['coconut-crab', 'leg0NearFoot'], ['freshwater-crab', 'leg0NearFoot'], ['mud-crab', 'leg0NearFoot'], ['vent-crab', 'leg0NearFoot'], ['civet', 'hindNearPaw'], ['brown-bear', 'hindNearPaw']] as const;
  for (const [fit, footJoint] of CASES) for (const side of ['left', 'right'] as const) it(`${fit} attacker from the ${side}: whole gait cycles ≤ the cap, stance feet fixed in the arena per stance window, zero refusals, run-up reached by impact`, async () => {
    const { rig, card } = fit === 'brown-bear' ? await loadFitDir('audits/VISION_D2_GUARDIAN_20260921/fit-01/') : await loadFit(fit);
    const layout = fit === 'brown-bear' ? composeArena({ id: 'e1g', groundLineNormalized: 0.78, plates: { far: TEX, mid: TEX, near: TEX } }, FRAME, { guardianSide: side }) : baseLayout;
    ...
    const arena = { groundLineY: layout.groundLineY, stands: layout.stands };
    const ctx = side === 'left' ? contextFor('X', card.massClass.multiplier, card, attackFor, { arena }) : contextTarget('X', card.massClass.multiplier, card, { attackFor: (s, o) => (s === 'B' ? attackFor('A', o) : null), arena });
