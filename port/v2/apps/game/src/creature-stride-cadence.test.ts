...
it('whole cycles cover composed distance, reset stance-local displacement and preserve body-length units in both directions',()=>{
 for(const targetDistancePx of [184.32,-184.32,1,0]){
  const c=createStrideCadence({targetDistancePx,bodyLengthPx:21,gaitDurationMs:800});
...
it('invalid scales and a pixel-valued replacement are not accepted as evidence of planting',()=>{
 expect(()=>createStrideCadence({targetDistancePx:1,bodyLengthPx:0,gaitDurationMs:1000})).toThrow();
 const c=createStrideCadence({targetDistancePx:184.32,bodyLengthPx:21,gaitDurationMs:800}),s=c.sample(390);
