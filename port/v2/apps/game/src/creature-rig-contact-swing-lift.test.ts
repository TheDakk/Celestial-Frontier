...
it('omitting the declaration preserves exact former compact refusals and the ordinary screen-up target',()=>{
 ...
 const real=read('fit-04/record.json'),binding=read('fit-04/binding.json'),actual=contracts.familyContractForRecord(real),{swingLift:removed,travelSubsteps:removedCadence,...oldStance}=actual.contactStance!;expect(removed).toBe('toward-socket');expect(removedCadence).toEqual({hit:2,dodge:2,tame:2});const old=vi.spyOn(contracts,'familyContractForRecord').mockReturnValue({...actual,contactStance:oldStance});
