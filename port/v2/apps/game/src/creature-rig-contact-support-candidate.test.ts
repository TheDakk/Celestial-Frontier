...
const supports=observedContactSupports(record,binding),current=contracts.familyContractForRecord(record),{travelSubsteps,...priorStance}=current.contactStance!,template={...current,contactStance:priorStance};
// These retained failures isolate fallback semantics before reaction cadence.
// The cadence owner has separate new opt-in outcomes; do not change this input.
beforeEach(()=>{vi.spyOn(contracts,'familyContractForRecord').mockReturnValue(template);});
