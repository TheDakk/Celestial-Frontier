  ...
  sections: Object.freeze([
    ...
    Object.freeze({
      ...
      bullets: Object.freeze([
        ...
        '🦋 COLD PLANETSIDE ART NO LONGER FREEZES THE DECK: Loading and painting the first specimen thumbnails now happens away from the renderer thread, so Search, navigation, and browser controls stay answerable while neutral tiles fill in; the globe starts at the resolution its fitted size needs and upgrades only when real zoom or display density asks for a sharper tier. On first install, activation now pins worker realms created during the claim gap before their lazy painter can load, so new specimen thumbnails cannot silently go blank; an actual worker failure keeps its exact bounded diagnosis.',
