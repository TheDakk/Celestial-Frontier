  ...
  sections: Object.freeze([
    Object.freeze({
      ...
      bullets: Object.freeze([
        ...
        '🗂 THE EXPLORER’S BOARDS RETURN: Current-slice versions of the Star Atlas, Compendium, Records, Charters, Settings, Field Training, and protected expedition import are open for development playtests.',
    ...
    Object.freeze({
      ...
      bullets: Object.freeze([
        ...
        '⚖ COMPARISON TELLS THE WHOLE STORY: Inventory opens from the phone dock or desktop rail, keeps a bounded 48-row page, and gives one selected item a focus-owned detail sheet whose comparison includes every base and affix delta, conditional context, provenance, and equipped state. On the narrowest phones, full item copy now reflows above its wrapping badges instead of being starved beside them.',
        ...
        '🏅 EVERY EXPEDITION HAS A RANKED RECORD: Records now renders the exact ten ranks from Cadet through Eternal Frontier, all six score factors, and all 96 achievements across 13 shelves. One nonoptimistic aggregate refresh can durably add the 68 aggregate milestones and permanent best-rank record; exact-event rows remain locked until their true owner fires. Twenty-three exact joins now belong to their true actions: Earth landing, world or companion Rename, Legendary-pair successful Breed, first verified conquest settlement, settled explorer injury below 20 HP, valid-world Share, accepted CF1 Follow, twelve source-derived Survey observations, wormhole/quasar/dwarf-galaxy travel, and first explicit Atlas Favorite. Accepted Follow folds the accepted route, Jumps, galaxy visit, wayfarer, and any proved quasar/dwarfg event into one receipt; ordinary navigation cannot counterfeit Follow. Records also houses the Binder’s six established type pages and seven current-proof Set claims. One completed unclaimed Set pays its established 25–150 Stardust, updates lifetime earned, records the claim, and refreshes achievements and rank in one receipt and one compare-and-swap with no retry or optimistic reward; the Fifty Paragons remain visible but unavailable, and imported Paragon evidence cannot become a claim button. The top bar shows the player name and rank in its earned permanent color or Eternal Frontier foil. Only a post-action durable promotion announces the new named rank—boot catch-up stays silent; a newly durable post-action achievement likewise announces only after commit. Achievement ceremonies use the authored name and description with a tier-3 sting; rank promotion uses a tier-5 sting and an at-most-40-particle gold burst that the effects, motion, and device budget may lower. Replay, already-durable recovery, Training, convergence, and refusals stay silent, and ceremonies never write or reward. Settings → Nameplate now offers Auto/current-rank or any permanently earned rank color through one receipt-bearing compare-and-swap with no retry or optimistic color change; locked choices refuse and durable ambiguity reloads instead of applying twice. Settings → Explorer name → Change name uses the shipped sanitizer and 24-character cap, changes only the explorer name, makes cleaned-empty or unchanged input receipt-free, and settles through one receipt-bearing compare-and-swap with no retry or optimistic name; durable ambiguity reloads instead of applying twice. Explorer self-rename deliberately does not grant the discovery-name namer achievement. Achievement rewards and exactly five event owners—daily, decade, survivor, fieldmedic, and gambler—remain open.',
        '✕ ONE SURFACE, ONE CLOSE: Every panel and Survey card owns exactly one 44-pixel top-right Close action with a reserved content gutter, so refilling or scrolling a surface cannot duplicate it, detach it to the upper-left, place another control beneath it, or strand keyboard focus behind a hidden surface. Spacing inside either desktop rail belongs to that command deck and leaves the active panel open; a genuine empty-sky press still dismisses it.',
    ...
    Object.freeze({
      ...
      bullets: Object.freeze([
        ...
        '🌿 ONE EXACT MEAL SETTLES ONCE: A real fauna Compendium detail can Feed one exact unassigned owned companion below the 200-Meal cap from one exact owned flora lot through Use 1. Same-species twins remain separate exact instances; assigned, recovering, and capped companions stay disabled and explain why. One receipt-bearing compare-and-swap raises Meals by 1 and removes exactly 1 flora, emptying that exact lot on its final unit, with no retry or optimistic change. Back and Close remain available. Refused, stale, and failed writes use and publish nothing; a durable result whose publication cannot be confirmed requires reload and cannot feed twice. With Sound and Creature voices enabled, only the trusted native Feed gesture, exact current ownership successor, and still-current accessible settled status may produce one deterministic synthesized acknowledgement after that status appears. That inline polite result is the only assistive-technology announcement; the simultaneous corner toast is visual-only. The same or any superseded successful Feed result also remains silent; refused, stale, converging, replayed, hidden, route-lost, and counterpart-lost paths remain silent. Tastes and flavours, stat or Power growth, injury care or healing, poison, bond, explorer eating, friendly duels, and missions remain unavailable. Companion Breed is a separate action with its own exact-parent eligibility, odds, lineage, and active-play Recovery; Rename is identity-only, while Field Scout changes only the exact role without intercepting injury or earning Scout XP yet.',
    ...
    Object.freeze({
      ...
      bullets: Object.freeze([
        ...
        '📋 THE CHARTER STOPS AT THE LIVE FRONTIER: The primary chip and Charter board show only landfall, mining, fixed-fabrication, successful-breeding, first-world alien bioscan, and conquest goals with real outcome writers. The board now exposes the two established starter chains one unfinished link at a time, requires Accept before tracking, and caps incomplete accepted work at three. Exact writers cover first planetfall beyond canonical Earth, one Mine, a non-null Field Scout assignment or switch, verified conquest, full-address Mercury, Mars, Uranus, or Neptune landfall, five manual Mine presses on Jupiter or Saturn, and any canonical T2 component. Full-address Sol hierarchy is required; a matching leaf seed elsewhere earns nothing. Each supported completion pays its established 10–25 Stardust once in the same receipt, and its supported gear uses the exact inventory carrier with empty-slot auto-equip only. A successful offspring banks its exact Chapter 3 milestone in the same durable Breed transaction; failure banks nothing. A verified conquest banks Chapter 2 conquest and can honor one accepted starter st-conq for +25 Stardust in the same combat save. Discover life stays visible but unavailable until its accepted-bioscan owner exists. Accepted wk-conq remains fail-closed because its weekly lifecycle owner is missing; wall-week, slate, acceptance, and rollover authority must all be complete before weekly work can settle. Survey Records and weekly rewards remain unavailable instead of being presented as settled work.',
        ...
        '🦋 COLD PLANETSIDE ART NO LONGER FREEZES THE DECK: Loading and painting the first specimen thumbnails now happens away from the renderer thread, so Search, navigation, and browser controls stay answerable while neutral tiles fill in; the globe starts at the resolution its fitted size needs and upgrades only when real zoom or display density asks for a sharper tier. On first install, activation pins every worker realm the browser can see, while a worker revealed only at its first controlled request may inherit the sole verified build only after its exact live identity is confirmed; update and mixed-build states still refuse, so new specimen thumbnails cannot silently go blank and an actual worker failure keeps its exact bounded diagnosis.',
