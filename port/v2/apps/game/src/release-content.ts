  ...
  sections: Object.freeze([
    Object.freeze({
      ...
      bullets: Object.freeze([
        ...
        '🛠 THE SHIPYARD READS CAPABILITY, NOT WISHES: A read-only Shipyard derives its Scout/Chemical, Jump/Interstellar, Survey Cruiser, or Frontier/IG chassis from the same canonical saved reach used by travel, and shows only actually owned systems and hardpoints. A completed veteran Charter without an owned Intergalactic Drive receives an honest generic legacy charter refit that never names or draws the missing drive; fabrication, Research Bench purchases, and upgrades remain unavailable.',
    ...
    Object.freeze({
      ...
      bullets: Object.freeze([
        ...
        '🔒 THE FRONTIER HONORS YOUR PROGRESS: The saved reach stage—derived from navigation systems or a completed veteran Charter—gates star travel; saved Prime Signatures gate galaxy reach. A blocked destination never moves the camera or quietly skips the requirement.',
        '🪐 FIRST PLANETFALL COUNTS: Only a world’s first landing banks the live landfall objective, current chapter landfall counter, and worlds-landed record; revisits bank and pay nothing. A revisit can only reconcile an imported Chapter already proved complete by its canonical progress and saved reach stage.',
        '🎓 FIELD TRAINING LIVES IN THE NEW SHELL: Six real lessons teach finding Earth, surveying, charting, opening the Atlas, and landing, followed by an honest Finish for now. Restart captures the exact pre-Training view. A normal Finish or Skip source-verifies and immediately restores the exact pre-Training view; if verification pauses, that exact view stays saved, and when Sol can still be verified, Training returns there so a reload can restart safely and retry. Older v1.8.9 Training checkpoints restore only the eleven pre-drill record groups they captured; every other expedition field is retained from the surrounding save. That older checkpoint contains no saved view: Skip from Welcome stays in Sol, while completing the drill after Land stays at Earth. An unrecognized checkpoint or unavailable recovery route locks exploration behind a recovery screen and leaves the stored expedition unchanged; reload after updating, or import a trusted complete expedition.',
      ]),
    }),
    Object.freeze({
      category: 'Bug Fixes',
