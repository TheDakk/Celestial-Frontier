  ...
  sections: Object.freeze([
    Object.freeze({
      ...
      bullets: Object.freeze([
        ...
        '🔊 THE FRONTIER SPEAKS: Survey pings and travel, planetfall, and ascent whooshes answer the live exploration loop. The persistent master Sound control immediately silences any sting already ringing, while Volume stays remembered for the next enabled sound. Creature voices now adds two exact deterministic synthesized expressions through one runtime: a greeting after a verified durable wild-fauna Tame and an acknowledgement after one exact durable nonconverging Feed commit. Each waits for its own current visible accessible status counterpart; misses, refusals, stale or converging results, repeats, reloads, hidden play, route or counterpart loss, and disabled Sound or Creature voices remain silent without retry or replay. Feed keeps only the latest successful ownership key, so the same or an older successful result never replays; its inline polite result is the sole screen-reader announcement while the simultaneous corner toast is visual-only. Distant ecology playback, other creature actions, ambience, music, and combat sound remain future work.',
      ]),
    }),
    Object.freeze({
    ...
    Object.freeze({
      ...
      bullets: Object.freeze([
        '🧱 A MODULAR CORE UNDER ONE UNIVERSE: Deterministic domain facades, strict root and app TypeScript programs, Pixi rendering, and IndexedDB persistence form the current playable Phase-4 foundation without changing seeded world identity. One versioned, digested 43-key domain biome-profile authority now binds the exact current-world roster environment fingerprint to art and to a pure already-surfaced distant-ecology hint plan; that audio join is presentation-only and non-playing, so it changes neither generation nor gameplay and does not claim ambience or Gate G.',
