...
export interface LandfallViewerV1 {
  open(original: AiLandfallOriginalV1, returnFocus?: () => void): Promise<boolean>;
  close(): void;
}

/** Capture before an asynchronous original read. Resolve the same live control
 * after a panel refill; if resize closed that panel, focus its visible opener. */
export function captureLandfallFocusReturnV1(document: Document | undefined = globalThis.document): () => void {
  if (!document?.defaultView) return () => {};
  const opener = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  const scope = opener?.closest<HTMLElement>('#notificationpanel,#survey')?.id ?? null;
  const action = opener?.dataset.aiAct, job = opener?.dataset.aiJob, id = opener?.id;
  const usable = (node: HTMLElement | null): node is HTMLElement => {
    if (!node?.isConnected || ('disabled' in node && node.disabled)) return false;
  ...
  return () => {
    const targets: Array<HTMLElement | null> = [opener];
    if (scope && action) for (const node of document.getElementById(scope)?.querySelectorAll<HTMLElement>('[data-ai-act]') ?? []) {
      if (node.dataset.aiAct === action && node.dataset.aiJob === job) targets.push(node);
    }
    if (id) targets.push(document.getElementById(id));
    for (const fallback of scope === 'notificationpanel' ? ['shelfnotifications', 'docknotifications'] : scope === 'survey' ? ['docksurvey'] : [])
      targets.push(document.getElementById(fallback));
    targets.find(usable)?.focus({ preventScroll: true });
  };
}
...
export function createLandfallViewerV1(document: Document = globalThis.document): LandfallViewerV1 {
  let current: { dialog: HTMLDialogElement; image: HTMLImageElement; url: string;
    restore: () => void; returnFocus: () => void } | null = null;
  let serial = 0;
  function close(): void {
    serial++;
  ...
  return Object.freeze({ close, async open(original: AiLandfallOriginalV1, returnFocus = captureLandfallFocusReturnV1(document)) {
    close();
    ...
    current = { dialog, image, url, restore, returnFocus };
    ...
    try {
      ...
      document.addEventListener('focusin', focus, true); document.defaultView?.addEventListener('pagehide', close);
      dialog.showModal(); exit.focus({ preventScroll: true }); image.src = url;
      await image.decode();
