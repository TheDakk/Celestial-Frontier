...
export const GUIDE_TOPIC_SUPPORT: Readonly<Record<GuideTopicId, GuideTopicSupport>> =
  Object.freeze({
    ...
    atlas: partial(
      ['atlas'],
      '<p>Use <b>Star Atlas</b> on a planet card to chart it. The Atlas lists saved galaxies, stars, and worlds. Each saved galaxy, star, or planet route is regenerated from the seeded universe and must produce a source-verified destination before its row can travel. Choosing a proven entry inside the expedition’s owned reach returns to that destination’s own navigation level. An accepted galaxy or system arrival records the source-proved galaxy visit and any quasar or dwarf-galaxy achievement in the same travel receipt; it does not counterfeit Follow or increment Jumps. An out-of-reach entry leaves you in place. A star beyond owned ship reach remains blocked until the required permanent system is built in Engineering. Saved Prime Signatures separately set galaxy distance; verified Titan victories can claim new Signatures, and the ninth distinct claim unlocks the Frontier. A world entry reopens its system survey, where Land remains separate. A stale, forged, or incomplete imported route remains visible but disabled with an honest route-unavailable label instead of pretending it can travel.</p><p>Each row keeps <b>Travel</b> and <b>Favorite</b> as separate controls. Favorite changes only that exact imported row in place so its proven route remains attached; the first explicit false-to-true choice owns <code>curator</code>, while unfavoriting never removes it and an unchanged choice writes nothing. Custom names and source-verified composite identities are preserved. The legacy chart view, home marker, undo, and hyperlane presentation are not yet ported.</p>',
      'List-based deterministic return travel and exact-row Favorite are live; the full legacy Atlas presentation remains open.',
    ...
    hp: partial(
      ['explorer-health'],
      '<p>The top bar displays the expedition’s imported/current <b>HP</b> and the maximum derived from its preserved explorer stats. The value is durable, not decorative: an unguarded stellar skim at a remnant star previews and then costs exactly <b>3 HP</b>. Engineering refuses that skim when HP is 4 or lower, and connected skim protection reduces the shown damage to zero.</p><p>The explorer can also lead a landed Surface conquest. A defeat applies the verified defender-scaled damage but keeps the expedition at a <b>1 HP mercy floor</b>; a settled drop below 20 HP unlocks <b>On the Brink</b>. Healing, death/reset, and restoration outcomes remain open, so this is not yet a complete health loop.</p>',
      'Imported/current HP, guarded remnant-skim damage, and mercy-floor conquest injury are live; healing, death, and restoration remain open.',
    ...
    achievements: partial(
      ['records'],
      '<p>The <b>Records</b> board preserves and displays imported exploration totals, Stardust earned, and Journal entries. First landfalls visibly update the worlds-landed total. Mine, Skim, and fixed Fabricator settlements also preserve their compatible expedition counters, but those Arc 3 counters are not yet listed as separate Records totals and live Journal writing is not connected.</p><p>Records renders all <b>96 achievements</b> in <b>13 shelves</b>. One nonoptimistic aggregate refresh can durably add the <b>68 aggregate</b> milestones and the best-rank record after a real product settlement; it owns one compare-and-swap, never retries, and publishes only after verification. The other 28 achievements require their exact live event owner rather than being guessed from totals.</p><p><b>Twenty-three</b> exact live event joins now cover Earth landing (<code>home</code>), first accepted world or companion Rename (<code>namer</code>), successful breeding from two Legendary-or-better parents (<code>bredlegend</code>), first verified conquest settlement (<code>settle1</code>), a settled explorer injury below 20 HP (<code>brink</code>), valid-world Share (<code>share</code>), accepted CF1 Follow (<code>wayfarer</code>), twelve source-derived Survey observations, wormhole traversal, arrival at a quasar or dwarf galaxy, and the first explicit Atlas Favorite (<code>curator</code>). Accepted Follow folds its accepted route, Jumps record, galaxy visit, <code>wayfarer</code>, and any proved <code>quasar</code>/<code>dwarfg</code> event into one receipt; ordinary navigation cannot counterfeit Follow. Explorer self-rename changes only identity and deliberately does not grant <code>namer</code>. Exactly five event owners remain blocked: <code>daily</code>, <code>decade</code>, <code>survivor</code>, <code>fieldmedic</code>, and <code>gambler</code>. Achievement reward claims also remain open; preserved unknown imported IDs retain rank credit without being reinterpreted.</p><p>After a real action’s exact durable successor is verified and published, each newly appended supported achievement may announce its authored name and description with a tier-3 sting. A newly raised best rank may announce its name with a tier-5 sting and a bounded gold burst of at most 40 particles; the current effects, motion, and device budget may lower that count. Boot catch-up, replay, already-durable recovery, Training, convergence, and refusals stay silent. These ceremonies write no save, evaluate no rule, and grant no reward.</p>',
      'Records now presents the ten-rank projection, all 96 achievements in 13 shelves, durable aggregate refresh, and 23 exact live event joins; daily, decade, survivor, fieldmedic, gambler, and achievement rewards stay open.',
    ...
    binder: partial(
      ['binder'],
      '<p>The <b>Binder</b> lives in <b>Records</b> and keeps its six established type pages: the Spectrum, Sixteen Realms, Body Plans, Ability Themes, Flora Flavors, and Size Classes. Slots are species types, not individual creatures; the current Compendium decides which are filled.</p><p>Seven established collection sets are live: Four Crowns, The Five Flavors, Master of Arts, The Bestiary, Warden of Realms, Against All Odds, and The Apex Court. A completed unclaimed set exposes one exact <b>Claim</b> action. Its one-time Stardust reward, lifetime-earned total, claimed-set record, achievements, and rank refresh settle in one receipt and one compare-and-swap with no retry or optimistic reward.</p><p>The Fifty Paragons remain visible but unavailable until their deterministic discovery and acquisition owner is ported. An imported Paragon-set claim is preserved as evidence and can never become a new claim button.</p>',
      'All six established Binder type pages and seven current-proof set rewards are live in Records; the Fifty Paragons remain protected.',
