...
export const GUIDE_TOPIC_SUPPORT: Readonly<Record<GuideTopicId, GuideTopicSupport>> =
  Object.freeze({
    ...
    landing: partial(
      ['planetfall'],
      '<p>Any galaxy, star, or planet route arriving from Search, the Star Atlas, or a saved location is regenerated from the seeded universe before it is accepted; navigation uses only the source-verified destination. A valid planet address inside the expedition’s owned reach returns to its live system survey; it never lands for you. A stale or forged route cannot act. An out-of-reach route leaves you in place. A star beyond owned ship reach remains blocked until the required permanent system is built in Engineering. A galaxy beyond the saved Prime Signature radius stays blocked because Prime Signature earning is not available in this development slice. Press <b>Land</b> on the current planet card to enter Planetside. Only the first landing earns new landfall progress, and Sol-versus-beyond-Sol Charter credit follows the complete verified hierarchy and planet ordinal rather than the planet seed alone.</p><p>On touch, press the minimum-size <b>Leave world</b> action to return to the system. Right-click or <b>Escape</b> also lifts off. The legacy descent-risk and wave-off model is not yet part of this slice.</p>',
    ...
    settings: partial(
      ['settings'],
      '<p><b>Settings</b> currently controls Sound, Volume, Creature voices, Star charts, Visual effects, Screen shake, panel tint, text size, text tone, font, motion, Field Training restart, and protected expedition import. <b>Visual effects</b> Off allocates no frontier fog particles and turns off the live bloom treatment. With Visual effects On, Motion Reduced keeps only bounded static atmosphere; touch/low-tier devices also stay static, while full-motion capable devices may animate capped fog, blazar bloom, and bright-star breathing. Motion Auto follows the device’s reduced-motion preference live. <b>Screen shake</b> adds only a short deterministic planetfall impulse, and only when Visual effects, Screen shake, and full motion are all enabled. Reduced motion disables shake; device policy may lower the enabled profile and concurrent-impulse cap, but it can never enable shake. Creature voices currently governs the two exact synthesized expressions after a verified successful wild-fauna Tame or exact durable nonconverging Feed commit; each waits for its own current accessible status counterpart. Turning Creature voices or master Sound off stops either expression immediately, and turning it back on never replays an earlier result. Preferences persist with the expedition. If the expedition is protected or this tab is read-only, every ordinary save-mutating preference and Training control—including Creature voices and both sliders—remains inspection-only; <b>Bring expedition</b> remains available for recovery. A lease-storage failure stops active-play eligibility and accrual immediately, then converges through a protected reload instead of silently reacquiring.</p><p>Restart begins the current six-lesson drill in Sol. A normal Finish or Skip source-verifies and immediately restores the exact pre-Training view. If verification pauses, that exact view stays saved; when Sol can still be verified, Training returns there so a reload can restart safely and retry. Older v1.8.9 Training checkpoints restore only the eleven pre-drill record groups they captured; every other expedition field is retained from the surrounding save. That older checkpoint contains no saved view: Skip from Welcome stays in Sol, while completing the drill after Land stays at Earth. An unrecognized checkpoint or unavailable recovery route locks exploration behind a recovery screen and leaves the stored expedition unchanged; reload after updating, or import a trusted complete expedition. If persistence fails before restart, restart is cancelled. See <span data-gt="saving">Your save & reset</span>.</p>',
