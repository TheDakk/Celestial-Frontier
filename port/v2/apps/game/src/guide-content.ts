...
export const GUIDE_TOPIC_SUPPORT: Readonly<Record<GuideTopicId, GuideTopicSupport>> =
  Object.freeze({
    ...
    duels: partial(
      ['duels'],
      '<p>Open an eligible owned companion’s <b>Friendly duel</b> panel in the Compendium. Paste a friend’s <b>CFB-</b> code and preview the exact challenger before confirming. Sharing a creature code writes no expedition and does not import a companion.</p><p>A deterministic duel records one saved result. Nobody is lost or wounded. A win can pay <b>8 XP</b>; participation can pay 2 XP, or 3 for a bout taken below 25% health. The panel previews the separate active-play reward cooldowns, and repeating or reloading cannot claim a reward twice. The settled Chronicle has the same named fighters and result as the durable receipt.</p>',
      'Friendly duels, CFB creature sharing, saved outcomes and bounded XP are live; expedition import is not.',
      'Friendly duels have not been connected in this build.',
    ),
    ...
    harvest: partial(
      ['harvesting'],
      '<p>A conquered world offers <b>Harvest</b> when its active-play cooldown is ready. The card previews its result. One receipt grants the harvest and starts the next cooldown; a duplicate claim, stale tab or failed write cannot grant it twice. Closing the game or changing the device clock cannot advance the timer.</p>',
      'Conquered-world Harvest runs on active play.',
...
export { V2_ADVANCED_BRIEFINGS } from './guide-briefings.js';
