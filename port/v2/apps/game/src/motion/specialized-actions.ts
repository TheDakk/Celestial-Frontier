...
export function specializedActions(id:string):Readonly<Record<string,MotionAction>>|undefined{
 ...
 const action=(id:string,family:string,effort:number,loop=false,travel=0):MotionAction=>{
  ...
  return {id,family,loop,...scaleLoading?{rootUnit:'motion-scale' as const}:{},poses:[P(times[0]!,loaded(0,effort*.6)),P(times[1]!,loaded(Math.PI/2,effort),spec.anchored?0:travel,loading),P(times[2]!,loaded(Math.PI,effort),spec.anchored?0:travel*.35),P(1,{})]};
 };
