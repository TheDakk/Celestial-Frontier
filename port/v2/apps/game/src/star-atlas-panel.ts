...
export function renderStarAtlasV1(
  ...
  const undo = projection.view !== 'list' || options.undo === null ? ''
  ...
  if (projection.rows.length === 0) {
    ...
    body = `<div class="atlas-list" role="list">${projection.rows.map((row) => {
      const unavailable = mutationDisabled(row.id);
      return `<article class="centry atlas-entry${row.home ? ' is-home' : ''}" role="listitem" data-atlas-id="${escapeHtml(row.id)}">`
    ...
    const positioned = projection.chartPoints.filter((point) => point.xPercent !== null);
    ...
    body = '<div class="atlas-chart" role="group" aria-label="Charted places">'
      ...
      + positioned.map((point) => {
        const state = `${point.home ? 'Home, ' : ''}${point.favorite ? 'Favorite, ' : ''}`;
        return `<button type="button" class="atlas-chart-point${point.home ? ' is-home' : ''}${point.favorite ? ' is-favorite' : ''}" data-atlas-travel="${escapeHtml(point.id)}" data-atlas-chart-point style="--atlas-x:${point.xPercent!.toFixed(3)}%;--atlas-y:${point.yPercent!.toFixed(3)}%"${disabled(!point.travelable)} aria-label="${escapeHtml(`${state}${point.title}`)}">`
          + `<span aria-hidden="true">${point.home ? '⌂' : point.favorite ? '★' : '·'}</span></button>`;
      }).join('')
      + '</div>'
      + (unmapped.length === 0 ? ''
  ...
  return '<section data-star-atlas-body aria-label="Star Atlas">'
    + `<h3>Star Atlas <span data-atlas-count>${projection.counts.all}</span></h3>`
