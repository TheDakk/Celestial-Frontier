...
import { buildLandfallConditioningV1, type LandfallConditioningRecipeV1 } from './landfall-conditioning.js';
...
export function assessLandfallFidelityV1(
  ...
  try {
    if (!record(contract) || !contracts.has(contract)) stop('invalid-contract');
    const bound = contract as unknown as LandfallFidelityContractV1;
    ...
    if (!record(review) || !keys(review, ['schema', 'contractSha256', 'imageSha256', 'recipeSha256', 'referenceSetSha256',
      'basis', 'observer', 'scene', 'residents']) || review.schema !== LANDFALL_VISUAL_REVIEW_SCHEMA_V1) stop('incomplete-review');
    ...
    if (!record(scene) || !keys(scene, ['faunaBodies', 'unmatchedFaunaBodies', 'distinctBodies', 'noCrossSpeciesFeatures', 'evidence'])
    ...
    for (const row of review.residents) {
      if (!record(row) || !keys(row, ['contractSha256', 'identityKey', 'expectedPlacement', 'instances', 'identity', 'placement', 'diagnostics'])) stop('incomplete-review');
