/** Implementation targets for the 58 missing identities. No automatic promotion
 * into candidateTemplates: each species still needs a source-bound observation,
 * painted fit, complete motion proof and review. */
...
export function missingBodyStructures(){return EARTH_FAUNA_PROFILES.filter(p=>!p.candidateTemplates.length).flatMap(p=>{
