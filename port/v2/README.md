...
> scheduler repair present; cold-start measurement authority fail-closed pending one refresh;
...
>
> The selftest's single real cold launch now owns caller-local startup/socket/command/shutdown caps of
> 45,000/15,000/1,500/2,000 ms. Portable controls pass at 38,657 ms and reject exact/late
> 38,658/38,659 ms with one child and complete cleanup. There is no warmup, relaunch, retry, fallback,
> or workflow change; the generic launcher and candidate startup remain 15 seconds, and product
...
> After one exact-head battery and one corresponding CI attempt, work returns to the gameplay roadmap.
> Exact-head re-certification, corresponding PR test-merge CI, and six-image HUMAN review remain open.
>
...
Highest-priority active v2 work is to finish PR #32's final bounded authority transition
without widening D-TRAIN-1's eleven-field compatibility boundary into ownership or receipt authority.
Preserve f9's complete local-green battery and run `32367902426`'s no-product cold-start instrument
red. Browser-free budget `bb4da2bf0b…` is active for measurement `6ba58522…` and producer
`e59685b1…`; run one complete unchanged-head battery and one corresponding PR test-merge attempt.
Do not expand timing policy or optimize the accepted 45-second CI allowance;
after this closure cycle, return to gameplay arcs.
...
`32350971816` remains the preserved timer-instrument red, `32367902426` the later cold-start
instrument red, and c095's 12.5px compact-phone Glass overlap is
