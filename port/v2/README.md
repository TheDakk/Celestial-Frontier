...
> **Current local candidate — signed `862a75b…` Layout is exact-green, SceneMemory is
> terminal-red, and the bounded hidden-panel/listener repair plus heap-only calibration remain
> open.** Six current-input Slice reds and both preserved SceneMemory reds remain diagnostic
...
> numeric budget in either ruler. The current worktree is not a final frozen producer: SceneMemory
> must complete the three-candidate and paired-red calibration below, then any supported heap-only
> activation, before exact certification; Compendium needs a fresh certificate bound to the eventual
> signed producer.
...
> also CLEAR. A signed clean repair/calibration source, three
> clean candidates, heap-only activation, and fresh serial browser IDs remain pending.
...
> numeric budget. The current worktree is not a final frozen producer: SceneMemory calibration and
...
>
> The current attempted authority is signed source `862a75b316142348636abea442dab15e87393642`:
...
> cannot certify or calibrate. Three independent clean no-retry candidates on one signed repair
> source must justify any heap-only ceiling activation before a fresh exact-budget certificate.
> Edge point-version changes alone never start that process.
>
...
| Tool | Role |
|---|---|
| **Current local candidate verification (2026-08-27)** | The current repair-worktree browserless checkpoint is green at 134 Vitest files / 1,466 passed / one intentional skip / zero failures, plus `root/app/worker/noUnused` TypeScript, `artunused` and the 887-module build. Signed `862a75b…` passed named Layout 787/787, then one-attempt/no-retry SceneMemory stopped at 40/42 on only phone/desktop heap/DOM budgets; its exact red is preserved and no later stage ran. The worktree retains the earlier protected-preview/F4 repairs, defers closed Inventory rows and subscriptions, delegates opener capture once, reports exact breached fields, and binds the rebuilt SceneMemory and Compendium producers without moving numeric rulers. Nodes/listeners are green only in a dirty diagnostic; three clean SceneMemory candidates, paired-red discrimination, heap-only activation, a signed exact certificate and fresh serial IDs remain pending. Edge point version is provenance only. No current Slice PASS, Glass result, recovery certificate, hosted/HUMAN whole-Gate or release authority exists. |
