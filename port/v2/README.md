...
> passed Layout 787/787, SceneMemory 42/42, Compendium 78/78, Slice zero-findings/ten-PNG and full
> 12-viewport Glass. Recovery `20260828-phase4-final11-1ca67156e27d-recovery` ran once for
> 1,291,034 ms and passed all 15 observation outcomes/309 samples, Node/browser/active elapsed
> 1,200,308/1,200,305/1,200,305 ms, exact next-cycle proof and recovered UI. It then failed only
> `activePlayProjection` and `closeCheckpoint`: recovered raw/rendered/runtime
> 1,285,118/1,285,098/1,285,404 ms has valid 20 ms render lag; exhausted raw/state/UI
> 79,709/84,738/84,740 ms and closed raw/state 85,062/85,062 ms show a causal 322 ms latest-live
> gap, while old logic selected stale raw and reported 5,353 ms. This is instrument-only, not product
> failure or a Recovery certificate. Final11 is immutable/no-retry. Repair is contract-only under
> the existing 10-second law, binds exact committed/lost hide evidence, mutation-tests both
> directions and requires a fresh signed clean full chain. No product/version/ruler/browser change;
> Edge point version remains provenance only. Final10 current wording below is historical where
> superseded.
...
> a Recovery certificate or product failure. Final10 is
> immutable and cannot resume. The next signed clean contract-repair source must restart
...
> synchronized signed clean docs-only descendant supplied immutable Final11; the next signed clean
> contract-repair source requires a full restart.
>
...
> because Final11's passed real 20-minute observation still needs a green repaired final assessor
> and HUMAN review; Arc 5 remains `[PARTIAL]`/infrastructure-only.
...
| **Current Final11 Arc 4 Recovery boundary** | The complete burn-down, offline no-credit proof, reactivation, full observation, exact boundary crossing and recovered UI passed. Recovered raw/rendered/runtime was 1,285,118/1,285,098/1,285,404 ms; exhausted UI runtime to closed raw was 322 ms. The old assessor rejected the permitted render lag and compared close against stale exhausted raw. The next signed clean assessor-only repair must bind latest-live close plus exact committed/lost hide evidence, pass isolated positive/negative controls, and restart the full chain. HUMAN journey/listening, Charter bioscan, targeted preview, hosted and release authority remain open. |
...
| **Historical Final10 Compendium evidence boundary (superseded by Final11)** | Signed `4405fb2…` passed Final10 Compendium run `20260828-phase4-final10-4405fb2b4ba7-compendium` 78/78 with six review PNG bindings after green Layout and SceneMemory predecessors, followed by green Slice and full Glass. Its raw/gzip SHA-256 are `6ee3aa6f2e4dd50b7e148302486a4381385df92a10828f1b8b63408c66353035` / `c93be41ed90be67236b10a2b57a79e252824b00091be9da55f84604de2ade8cd`. This certifies the narrow Inventory/Compendium/Settings repair for that exact signed source. Signed implementation/evidence repair `3fbfcd5…` changes source identity and the synchronized signed clean docs-only descendant supplied immutable Final11; the next signed clean contract-repair source changes no numeric Compendium ruler, sample or draft count and therefore requires a fresh full browser chain. Browser authority remains Edge family + CDP `1.3`; `.107` is provenance only, and compatible point updates never trigger rebaseline. HUMAN Compendium judgment, Recovery and whole-Gate closure remain open. |
| `npm run arc4recovery:selftest` / Recovery run and named verifier | The instrument retains the phase-specific Pertar receipts, exact deadline, runtime/RNG/revision binding and mutation controls described above. Final11 proved the uninterrupted real 20-minute observation, exact next cycle and recovered UI, then exposed the remaining temporal-oracle defect. The next contract-only repair must accept bounded UI lag under the existing 10-second runtime law and bind close to latest exhausted live runtime plus the exact committed/lost hide witness, with mutants in both directions. A fresh complete signed clean chain remains required. |
