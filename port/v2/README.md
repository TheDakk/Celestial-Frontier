...
> **Current evidence — signed Final7 stopped terminal-red at Glass; the repaired head requires a
> new full-chain restart.** Signed clean `53d030bb733beca1a68fd8e42358dfa4b10ed0e2` passed Layout
> `20260827-phase4-final7-53d030bb733b-layout` at 787/787, source-bound SceneMemory
> `…-scenemem` at 42/42, source-bound Compendium `…-compendium` at 78/78 and Slice `…-slice`
> with zero findings and the exact ten screenshots. Full Glass
> `20260827-phase4-final7-53d030bb733b-glass` then ran once for 75,032 ms across all 12 viewports
> and stopped with 25 findings, zero report-classified instrument failures and zero retries:
> 11 `CLIPPED_WITHOUT_SCROLL`, seven `CONTROL_OUTSIDE_VIEWPORT`, four
> `SETTINGS_CREATURE_VOICE_CONTROL`, two `COMPENDIUM_HOSTILE_ROW_GEOMETRY` and one
> `COMPENDIUM_SHORT_LANDSCAPE_WORKSPACE`. Recovery correctly did not start. These reports and their
> named predecessor bindings are immutable historical Final7 evidence; they are not rewritten or
...
>
> Browser-free negative controls pass for the repaired oracle, but the current worktree is not a
> browser certificate and no post-repair full Glass PASS exists. The changed product/release-content
> bytes rebind only live
> SceneMemory/Compendium producer inputs and their tracked budget/test authority records; no numeric
> ceiling, calibration sample, retry policy or browser authority changed. A newly signed clean
> successor must restart **Layout → SceneMemory → Compendium → Slice → Glass → recovery**. Edge
> `151.0.4129.107` / CDP `1.3` is Final7 run provenance only, never a rebaseline or threshold-change
> trigger. No post-repair chain, whole-Gate, hosted, HUMAN or release authority exists. The
...
> failures across all 12 viewports. Recovery did not run. The post-Final7 product/oracle repair is
> not browser-certified; a newly signed clean successor and full restart at Layout are required.
>
...
| **Current local candidate verification (2026-08-28)** | Signed clean `53d030b…` supplied Final7 Layout `20260827-phase4-final7-53d030bb733b-layout` at 787/787, SceneMemory `…-scenemem` at 42/42, Compendium `…-compendium` at 78/78 and Slice `…-slice` with zero findings and ten screenshots, each once and named-verified before its successor. Glass `…-glass` then ran once for 75,032 ms across 12 viewports and stopped terminal-red with 25 findings, zero report-classified instrument failures and zero retries; Recovery did not run. Exact carriers, sizes and hashes are in `audits/README.md`. The current repair narrowly stacks Inventory rows only at `<=360px`, removes only the redundant list-mode Compendium heading clear, requires mounted-and-contained hostile rows, and replaces the simultaneous Settings snapshot with independent settled native activations and exact raw restoration/classification controls. Browser-free controls pass, but the repaired worktree is not browser-certified. Final7 is immutable and cannot resume; a newly signed clean successor must restart Layout → SceneMemory → Compendium → Slice → Glass → recovery. Edge `.107` / CDP `1.3` is provenance only. No post-repair full Glass PASS, recovery, hosted/HUMAN whole-Gate or release authority exists. |
| **Current Compendium evidence boundary (Final7 is historical green; repaired producer is uncertified)** | Signed `53d030b…` passed Final7 Compendium run `20260827-phase4-final7-53d030bb733b-compendium` 78/78 with six review PNG bindings after green Layout and SceneMemory predecessors. Its raw/gzip SHA-256 are `617151d1b72529d260520171337e2977adb7e6e89a72fd7fafbac97347e0cb4d` / `adf44f5f06ba386065596d1e2578296bb3db6b633c79c2c1b50d91f88e3716f5`. That immutable report binds producer `bf9ad0b6…4dcb`. The narrow post-Final7 product and existing-bullet release-copy repair rebinds built index `7f49254d…8ff02`, owner `assets/main-DWdoD5Mw.js` / `839027a7…2f15`, live producer `e858722e6da4c20d0f618aeed23ea9e037feb09cac0887feaf27c6f5430b601f` and budget-file SHA-256 `726881b8ffec84621a950fa481ca9bc54a0adf4092625976683a40fd6c42ea74`; collector `c13a489d…3faab`, outcome contract `ac7eea39…6d17`, measurement `28b06f3c…276c`, numeric ceilings and historical samples are unchanged. The draft remains exactly 55 bullets. A fresh signed-successor chain is required for current-head authority. Browser authority remains Edge family + CDP `1.3`; exact `.107` is provenance only. HUMAN/full-Glass/recovery/whole-Gate closure remains open. |
...
| **Arc 2 extensions to Smoke + Glass (2026-08-24; Final7 evidence updated 2026-08-28)** | Smoke drives the real Inventory opener, exact carrier↔legacy↔raw↔runtime↔DOM parity, conditional comparison, one durable action/receipt, reload + Atlas continuity, rejected-bootstrap rollback, and genuine legacy-Training carrier replacement; source-deferred/current-view and corrupt/future controls prevent false repair. Native row reveal, raw exact Close owners and causal fail-fast sequencing passed in signed Final7 Slice with zero findings. Glass adds the exact ten-button/5×2 phone dock; real action receipts; bidirectional modal focus; complete pending-action runtime and raw-DOM equipped snapshots; and independent carrier, legacy mirror, raw authority, runtime and DOM mutations. Duplicate/missing rows, hidden pending items, conditional-copy loss, protection bypass, stale publication and convergence retry remain red even when sibling representations agree. Final7 Glass then found only the exact `<=360px` A++/Mono Inventory rows taller than their scrollport; the narrow product repair preserves every copy/badge/action field, stacks the row to one min-width-safe column and keeps the 44px target. That repaired CSS has no full-Glass certificate; current exact evidence requires a fresh signed-successor chain. |
...
| **Arc 4 focused + browser evidence (2026-08-25; historical inputs)** | Persistence, Training restore, acquisition planner/capacity/action, app bootstrap/wiring and coordinator tests cover strict 18-namespace bootstrap, exact legacy source evidence, full-roster/current-epoch two-draw settlement, every pre-draw miss/hit capacity scenario, one receipt/CAS, hit/miss spend, first-only/repeat ownership, full committed verification, targeted publication and synchronous overlap barriers. That exact-input Slice passed the nine-stage native-control ledger, including pending non-optimism, storage refusal, stale/publication convergence, hit, miss, disabled suppression and 14-step burn-down; Glass passed 12 viewports/36 Arc 4 outcomes with every planned control executed. The shared exact-input tree is `b83ccef5…`; `recoveryClaimed:false`. The separate uninterrupted 20-minute recovery observation and HUMAN review remain open, and no current-input, hosted, preview/publication, release or whole-Gate authority is claimed. |
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical Pixi inventory, heap/DOM/resource bounds, target answerability, independent heartbeat, zero pending work, bfcache survival and cleanup. Signed `53d030b…` passed Final7 run `20260827-phase4-final7-53d030bb733b-scenemem` 42/42 with zero findings and complete cleanup after its green Layout predecessor. The later product/release-copy repair changes live buildDist to `9db56745f9c32c446646daff084a9e285bd6183bbca85d522f7be6dfd30cb3a6`, so this report remains immutable historical evidence and a newly signed successor must restart from Layout. Current budget-file SHA-256 is `7fcb7757547d7b05243a224771b66776f58ed41b3ed39438327ea3c9942e168e`; numeric ceilings and calibration samples are unchanged. Exact Edge provenance is mandatory but not baseline identity. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-28)** | Signed `53d030b…` is the latest exact 42/42 report, Final7 run `20260827-phase4-final7-53d030bb733b-scenemem`; its raw/gzip SHA-256 are `dda4da30da3a9cee3550c4c52f88f1b7ecff61a23507af548fbc863f488558cd` / `73997c61e60ba655a7314eed8af8bbaa18a527c8d36da7bc96bdef7db8ed8d35`. Signed `862a75b…` remains the paired 40/42 heap/DOM red; signed `6c9ad855…` supplied three clean calibration candidates; signed activation `4a54c0d…` set 12 MiB V8 /18 MiB aggregate with every other ceiling and paired discrimination intact. The post-Final7 product/release bytes are rebound in current budget `7fcb7757…168e` without changing any ceiling or sample, but no browser certificate exists for that producer. The full chain must restart at Layout under a newly signed successor; Final7 cannot resume. A compatible Edge update remains provenance only and never triggers rebaseline, recalibration or threshold movement. |
