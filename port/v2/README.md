...
> scheduler repair present; shutdown lifecycle recalibration pending;
...
>
> Frozen browser CDP `6da9e2ef…` separates direct child exit from stdio-backed close and judges
> TERM→KILL on exit before releasing owned stderr and requiring close. Frozen collector
> `f4ad842c…` plus selftest `2713ed10…` keep the report lifecycle-pending through browser/server
> cleanup and workspace-lock release; only afterward can a sample and terminal success publish, and
> the verifier requires that lifecycle authority. New measurement is `a3b3bb9f…`; producer remains
> `d3223177…`. Fail-closed budget/test `ae4ab918…` / `60fa5e9f…` deliberately contain empty
> candidate samples, null ceilings, and a `measurement-required` paired baseline. Historical
> baseline8/candidate17/18/19 and `6ba58522…` / `74e88c2b…` / `485be9da…` remain truthful only for
> their old measurement. Next is baseline9 plus independent candidate20/21/22 from one clean
> committed source, once each without retry; then ruler activation, one exact-head battery, one push
> to existing PR #32, and one corresponding CI attempt. No timeout, browser/package, launch
> argument, product byte, producer, or retry policy changed.
>
...
Highest-priority active v2 work is to finish PR #32's shutdown-lifecycle authority without widening
D-TRAIN-1's eleven-field compatibility boundary. Exact local `89bfa05…` completed 78/78 and six
PNGs, then exited 2 during owned shutdown after report/verifier had prematurely published PASS.
Preserve terminal `b0bb8abc…` over false-green `66ba1366…` / `98664dca…` as post-measurement
instrument red. Browser CDP `6da9e2ef…` and collector/selftest `f4ad842c…` / `2713ed10…` establish
measurement `a3b3bb9f…`; producer remains `d3223177…`. Fail-closed budget/test
`ae4ab918…` / `60fa5e9f…` await baseline9 plus candidate20/21/22 from one clean committed source,
once each without retry, then activation, one exact-head battery, one PR #32 push, and one
corresponding CI. Do not expand timing policy or optimize accepted CI allowances; first red stops,
and after PR #32 closes, return to Arc 1B/gameplay.
...
premature PASS/report verification. Lifecycle repair `6da9e2ef…` / `f4ad842c…` / `2713ed10…`
establishes measurement `a3b3bb9f…`; producer remains `d3223177…`. Fail-closed ruler/test
`ae4ab918…` / `60fa5e9f…` await baseline9 plus candidate20/21/22, once each without retry, before
one exact-head battery and corresponding CI. No timing, retry, fallback, product, browser/package,
launch-argument, or producer drift occurred. First red stops.
...
| `npm run compendiummem:selftest` / `npm run compendiummem` / `node tools/compendiummem.mjs --verify-run=<run-id>` | Negative-controls the browser-free Compendium instrument, performs one standalone browser run without automatic retry, and independently verifies the named report. Exact-89 completed 78/78 and six PNGs but exited 2 during owned shutdown; its pre-cleanup PASS/report verification is false-green. Frozen collector `f4ad842c…` and selftest `2713ed10…` now hold RUNNING/lifecycle-pending through browser/server cleanup plus lock release, publish a sample and terminal success only afterward, and require complete lifecycle authority at verification. Measurement `a3b3bb9f…` leaves fail-closed budget/test `ae4ab918…` / `60fa5e9f…` calibration-required with empty samples/null ceilings and a `measurement-required` baseline; old baseline8/candidate17/18/19 and ruler `74e88c2b…` are history only. The gate still drives the exact 1,500-row fixture across phone/desktop, binds raw observations and all measurement/product/browser inputs, verifies the owner→worker→painter graph, retains six same-run review PNGs, and distinguishes pre-measurement instrument failure from product evidence. Next authority is baseline9 plus candidate20/21/22 from one clean committed source, once each without retry. Automated terminal success remains exact-run evidence and never supplies the open [HUMAN] judgment. |
