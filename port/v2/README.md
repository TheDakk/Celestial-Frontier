...
> renderer/RenderTexture; and a named, identity-safe `SurfacePlanetTextureAttachment`. Fabrication,
> the Research Bench, Cargo spending, build/upgrade actions and every inventory writer remain future.
>
...
> `20260822-arc1-local-certification` passed 42/42 under Edge `151.0.4129.101`, with complete
...
> `0d83e6ce339205beb0b5387008ca74ca9b1f95cb22bf61444c439da36405f2a6`. These are exact local automated
> authorities, not hosted CI, HUMAN visual judgment, integration, Gate closure, or release authority.
>
...
>
> The guarded `test-battery` owns that v2 budget as one serial, fail-fast step after Arc 1A's exact
> Edge `.86` certification and before the Chrome gates: install exact Edge `.101`, run the
> browser-free controls, perform one no-retry certification, always verify its named terminal
> report, and retain `v2-scene-memory-evidence`. It starts only inside one owner-label-authorized
...
dock is a measured 260px 5×2 nine-control layout with browser-backed non-overlap/hit-target
...
Arc 1's automated implementation is locally complete at the exact authorities recorded above;
...
40/40 scene-memory-v1 certificate remains historical, while Arc 1C's exact `59530da…` local 42/42
scene-memory-v2 certificate owns the real Shipyard extension. Neither local certificate may be
described as hosted, HUMAN, merge, preview, or release proof. PR #33 review, hosted CI and
integration remain open, as do phone/desktop ship-silhouette judgment, true GPU bytes and physical
...
| Tool | Role |
|---|---|
| **Compendium authority status (2026-08-23)** | PR #33 run `32611053651` passed the repaired D-ART stage and then correctly rejected stale pre-Arc-1 Compendium authority. The old Edge `.86` budget expected producer `d3223177…`; the current Arc-1 index/owner builds producer `5a316197…` under Edge `.101`. Historical c27/baseline11/c28/c29 and Linux evidence remain truthful for their old product, but are not current certification and were not hash-rebound. Fresh candidate1/paired-baseline1/candidate2/candidate3 each completed once with zero retries; their raw evidence is retained and the strict active budget is SHA-256 `9e36bdfb…`. Independent clean source `23e177a…` then passed 78/78 plus the named verifier. Both hosted workflow owners and the preflight now install/check that same exact `.101` package tuple. Later evidence/docs descendants are not retroactive certified heads. |
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. One Edge `.101` process owns four unmeasured warmups plus exactly four measured phone/desktop cycles through the seven populated routes Universe, Galaxy, Galaxy fine, Sol/System, Earth/Surface, the 1,500-row Compendium and real Shipyard before settled Universe. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical per-hash Pixi inventory, heap/DOM/resource bounds, target answerability, an independent browser heartbeat, zero pending work, same-document bfcache survival, and per-profile Shipyard lifecycle through its visible opener and owned Close. Product/ruler `a4de500…` is activated/certified by exact `59530da…`; run `20260822-arc1-local-certification` passed 42/42 locally under budget SHA-256 `3b71d14c…`, complete lifecycle/cleanup and named verification, with report raw/gzip SHA-256 `e24ceef86d17fb4a47bbb10e58f81d442cac6e3def28923672448f6c47eac3a5` / `0d83e6ce339205beb0b5387008ca74ca9b1f95cb22bf61444c439da36405f2a6`. Arc 1B's v1/40 evidence remains historical; later descendants do not become certified heads. Automation supplies neither hosted nor HUMAN authority. |
