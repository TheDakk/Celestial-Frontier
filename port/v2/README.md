...
> **Current local candidate — signed `862a75b…` Layout is exact-green, SceneMemory's paired red is
> preserved, and signed `6c9ad855…` completed the bounded repair calibration; heap-only activation
> remains open.** Six current-input Slice reds and both preserved SceneMemory reds remain diagnostic
> history; no later browser stage has been promoted. Gate B now recursively seals the exact
...
> numeric budget in either ruler. The current worktree is not a final frozen producer: SceneMemory's
> three-candidate calibration is complete, while heap-only activation and exact certification remain
> open; Compendium needs a fresh certificate bound to the eventual signed producer.
...
> 676/71 and 673/70. The reviewed worktree activation changes only V8 to 12 MiB (`12,582,912`) and
...
>
> The current hidden-panel/calibration repair browserless checkpoint is green at
> 134 Vitest files / 1,469 passed /
...
> also CLEAR. The activation worktree/tests are green; a signed activation source and fresh
> serial browser IDs remain pending.
...
> both eventual exact-producer certificates remain open. No current-input Slice PASS, Glass result or
> real recovery certificate exists; hosted, HUMAN, whole-Gate, release/version,
> preview/publication and deployment authority remain unclaimed.
...
> unchanged and the paired red still failing nodes/listeners. Activation and a fresh exact-budget
> certificate remain open. Edge point-version changes alone never start that process.
>
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. One compatible Microsoft Edge-family process satisfying CDP `1.3` and the sealed capability plus exact phone/desktop-profile contracts owns four unmeasured warmups and exactly four measured phone/desktop cycles through Universe, Galaxy, Galaxy fine, Sol/System, Earth/Surface, the 1,500-row Compendium and real Shipyard before settled Universe. Exact version/revision/JavaScript/path/UA remain mandatory per-run provenance, not baseline identity. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical per-hash Pixi inventory, heap/DOM/resource bounds, target answerability, an independent browser heartbeat, zero pending work, same-document bfcache survival, and per-profile Shipyard lifecycle through its visible opener and owned Close. `59530da…` / budget `3b71d14c…` / run `20260822-arc1-local-certification` remain historical authority for the former 250 ms local ruler. Repair `7d8dc380…`, budget `5c8a6e75…`, and Edge `.101` run `20260823-pr33-cross-host-sla-certification` remain historical exact-input evidence for the fixed strict `<1000 ms` SLA; that run passed 42/42 locally with named verification, and raw/gzip SHA-256 are `d16d40cd4d07f96683490eab920072fb9f3b42e0d0ee54434ffd4d312223f960` / `7c4100244abef8d50f93178aab7c8579ae93fa0b6bef76422cc5c0523edac55a`. Hosted run `32618995487` remains exact terminal-red 40/42: only the old 250 ms phone/desktop ruler failed against Linux 493–647 ms while every liveness and memory outcome passed. It was not retried and is not hosted-green. Signed `6c9ad855…` completed the exact three-candidate calibration; the reviewed worktree activation is green, while a signed activation source and exact certification remain pending. Version tolerance itself changed no numeric budget. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-27)** | **`[RED]` pending signed activation and certification.** Signed source `862a75b…` passed named Layout `20260827-phase4-successor-layout` 787/787, then no-retry run `20260827-phase4-successor-scenemem` passed 40/42 and stopped the chain on only phone/desktop `heap-dom-budget`. Its carrier binds clean source, Edge `.107` / CDP `1.3`, one attempt, zero retries and full cleanup. Product repair defers the closed Inventory row tree and six subscriptions, replaces per-opener focus-capture closures with one delegated owner, and makes every failed heap/DOM field name its value and ceiling. Signed source `6c9ad855…` (tree `a3896460…`) supplied exactly `20260827-phase4-repair-candidate1/2/3`, each one attempt, zero retries and full cleanup on the same compatible browser authority. Maxima were 11,566,152 / 11,630,936 V8 and 17,681,258 / 17,636,682 aggregate bytes for phone/desktop, with nodes/listeners at 676/71 and 673/70. The reviewed worktree activation sets 12 MiB V8 and 18 MiB aggregate, with exact headroom 1,016,760 / 951,976 and 1,193,110 / 1,237,686 bytes; all other ceilings stay unchanged. The paired `862a75b…` red still fails nodes/listeners under both the activated budget and unlimited heap ceilings. A compatible Edge update is provenance only and never triggers rebaseline or threshold movement. A signed activation source, fresh exact-budget certification and all later serial stages remain open. |
