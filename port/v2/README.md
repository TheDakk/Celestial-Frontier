...
> **2026-09-03 UTC current Capture-focus / atomic-Glass overlay (matches exact signed local
...
> `7a9f4c1370dd84292388d718c38ff34214f6203b`. The changed-input early small-phone Glass preflight
> passed on hosted Chrome; Layout **787/787**, Compendium **78/78** plus named verification and
> exact-bound Slice also passed. Full Glass stopped on its first small-phone row with one old-
...
>
> Sample retained a button rectangle centred at y `173.171875` and ancestor scroll top `2538`,
> then later hit-tested y `504.171875` on `NAV`. Between those commands, the five-second Capture
> authority heartbeat replaced the DOM and bare semantic-focus restoration moved scroll to `2207`.
> No one layout epoch can contain both facts; the hosted stop is instrument despite the old product
> label. The earlier same-source/browser preflight PASS demonstrates timing variability, not
> collector closure.
>
> Passive Capture authority rerenders now call `focus({preventScroll:true})` when restoring the
> matching semantic action; explicit action settlement retains default-focus reveal behavior.
> Glass uses `buildArc4AtomicGeometryEvidenceExpression()` for each action and Close, forces the
> small-phone Sample heartbeat through exact quiesce/resume authority, reacquires the replacement
> node and captures geometry, scroll, centre hit and focus in one browser-side chronology. Explicit
> carrier schemas and `assessArc4CaptureGeometryEvidenceCoherence()` classify partial/cross-epoch
> evidence instrument-red before product geometry assessment. A wholly missing target remains a
> product-assessor concern. Product geometry requires actual `focused:true` ownership for each
> action and Close witness and requires Close's captured scroll to be settled; isolated mutations
> prove an unfocused action, unfocused Close and unsettled Close red.
>
> No workflow job, timeout, retry, browser pin, viewport or control is added; **104 controls remain
> one Glass campaign, not 104 jobs**. Both hosted reports, the stop/repair audit and the exact local
> PASS audit are retained under `../../audits/`. Current Compendium measurement/producer authority is `b83cbb85…681da` /
> `c216cdc9…d8cae`; the built index, owner and service worker moved while worker/painter, ruler,
> ceilings, outcomes and historical samples remain unchanged. Focused repair coverage passes
> **53/53**, and the complete browser-free `develop` profile passes **265/265 files, 2,749 passed /
> 1 skipped** with all three TypeScript programs and static owners green. Exact clean SSH-signed
> source `7cfb42d993778211e9c27b671e6b4fc9b891ff95`, tree
> `090d2e4827b0eeeec334410ec9563dc778559dd0`, then passed its tracked-input rehearsal, the
...
> Five deterministic carriers are indexed in `../../audits/README.md`; the signed evidence/docs
> descendant containing this reference passes the final tracked-input `develop` profile. This is
> exact local evidence only; no hosted authority is claimed.
...
#35's recent Inventory/action Glass stops, and retains its own immutable `...-glass-preflight`
report. Test-only
...
original thirteen-stop rationale remain preserved in
`../../audits/PR35_FAILURE_SURFACE_AND_EARLY_GLASS_PREFLIGHT_20260902.md`.

The current small-phone Sample geometry owner also forces one deterministic ordinary F4 heartbeat
replacement. It focuses before scrolling, quiesces/resumes against the exact document, observes
the old node disconnect, reacquires its semantic replacement, proves prior focus and ancestor
scroll survived, then collects the replacement rectangle, translated layout, centre hit and focus
inside one browser-side chronology across stable frames. Each action/Close carrier is schema-bound;
`assessArc4CaptureGeometryEvidenceCoherence()` runs before the product geometry assessor, so a
partial or cross-epoch carrier is instrument-red and causal-stops rather than masquerading as bad
product geometry. A wholly missing target remains coherent evidence for the product assessor.
Actual focused ownership is mandatory for every action and Close witness, and Close's recorded
scroll must be settled; isolated mutations prove each omission red.

This closes the class exposed after the original canary itself passed on run `33694235427`: final
Glass later combined y `173.171875` / scroll `2538` with y `504.171875` / `NAV` after a heartbeat
replacement and passive-focus scroll rollback to `2207`. Both hosted reports and the full
classification are retained in
`../../audits/ARC4_GLASS_PR35_GEOMETRY_CHRONOLOGY_INSTRUMENT_RED_AND_REPAIR_20260903_6B59B45.md`.
The repair adds no stage, retry, timeout, browser pin or control. Exact clean signed source
`7cfb42d993778211e9c27b671e6b4fc9b891ff95` later passed its one-viewport diagnostic and one
unchanged-source, named-verified Compendium → Slice → Glass chain once/no-retry.
...
| Tool | Role |
|---|---|
| **Current Capture-focus / atomic-Glass boundary (2026-09-03 UTC; exact local chain green)** | Hosted run `33694235427` first passed its exact-source small-phone canary, then final Glass stopped when an old multi-command Sample carrier crossed a five-second heartbeat replacement: retained rect centre y `173.171875` / scroll `2538`, later hit y `504.171875` on `NAV` after bare focus restoration moved scroll to `2207`. The corrected classification is instrument. Passive `CaptureCardController` rerenders now restore semantic focus with `preventScroll`; explicit action settlement still reveals normally. `buildArc4AtomicGeometryEvidenceExpression()` focuses before scroll, forces and receipts the replacement on small-phone Sample, reacquires the node and captures same-chronology geometry/hit/focus. Coherence assessment runs instrument-first. Both hosted carriers are permanent; no job, control, timeout, retry, browser pin or gameplay mechanic changes. Measurement/producer authority is `b83cbb85…681da` / `c216cdc9…d8cae`; focused repair coverage is 53/53 and the complete browser-free `develop` profile is green at 265/265 files and 2,749 passed / 1 skipped. Exact signed `7cfb42d…` then passed its tracked-input rehearsal, one-viewport repaired-path diagnostic and one named-verified **78/78 Compendium → zero-finding Slice → 12/12 Glass / 104/104 controls** chain once/no-retry. Five deterministic carriers are indexed in `../../audits/README.md`; no hosted authority exists. |
