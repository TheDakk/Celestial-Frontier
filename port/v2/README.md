...
> **Arc 1A Compendium resource implementation (2026-08-20; product, serviced-turn
> scheduler repair, and active resource ruler present; live automation resolved from exact-source
...
> `budgets/compendium-memory-v1.json` is active. It retains measurement authority `bb03a3af…` and binds repaired
...
> paired baseline4 and independent one-attempt candidate5/6/7 under exact Edge .86. Every candidate
> completed 78/78 outcomes with zero retries. The active ruler replays their raw capsules, places
...
> `406edea11fec5f5a3cf11e6f9fc6dfea00cbdd2ce54fefed780bd1c9dafc9282`. The later D-TRAIN
> repair cannot borrow either historical carrier; every selected head must own its exact-source
> certification, whose ignored outcome is not cached here.
...
> truthful history but are stale for current certification. Baseline4 and candidate5/6/7 now activate
> the repaired producer's ruler. The first exact-788 Chrome Smoke run
...
> and waits semantically for `claim-rejected/busy`. Push eligibility for the selected repair head
> comes only from the complete same-head battery's durable carriers, verifiers, captured
> terminal-only checks, and repository-identity proof; Ready/merge requires
> terminal-green PR test-merge CI corresponding to that pushed head. This reference caches neither
> outcome. Fresh six-image HUMAN review remains separate.
>
...
carry the repaired Arc 1A producer's active ruler and ef6 certification through the D-TRAIN Smoke
...
then close remaining literal Gate-B boundaries and split-store/CAS persistence. Arc 1A's selected
repair head is push-eligible only when the complete same-head battery's durable carriers, verifiers,
captured terminal-only checks, and repository-identity proof are terminal green; Ready/merge
requires terminal-green PR test-merge CI corresponding to that pushed head. This reference caches
neither live outcome. Da0's local PASS, its CI red, 788/ef6
certifications, and both repaired-producer Smoke
instrument reds remain preserved.
