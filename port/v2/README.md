...
> scheduler repair present; displayed-demand producer recalibration in progress;
...
> `3412fcc0…` / `2800519e…`.
>
> The bounded repair leaves the hashed launcher, candidate collector, active measurement
...
> Product and development-copy bytes change producer to `d3223177…` (index `dee9af3a…`, owner
> `assets/main-Da536xWA.js` / `28382873…`; worker/painter unchanged), while measurement remains
...
> replayed 78/78, and baseline8 retained four faults with 14 phone / 13 desktop breaches. Active
> budget/test `74e88c2b…` / `485be9da…` (79,614 / 20,782 bytes) reuse all 40 strict ceilings above
...
> normalization hypothesis, not a proven fix until one new changed-head CI. Product, timing,
> launcher, ruler, browser authority, budgets, and one-attempt/zero-retry policy are unchanged.
> First red stops; work returns to Arc 1B/gameplay after PR #32 closes.
...
Highest-priority active v2 work is to finish PR #32's exact-Edge runner-image normalization without
widening D-TRAIN-1's eleven-field compatibility boundary into ownership or receipt authority.
Preserve exact-139's complete local-green battery and run `32383320206`'s no-retry Planetside
product red. Measurement remains `6ba58522…`; producer is `d3223177…`, and budget/test
`74e88c2b…` / `485be9da…` are active from clean source `75a996af…`'s baseline8 plus independent
candidate17/18/19. Exact activation head `96464d5…` passed its complete local battery; corresponding
run `32394244417`, attempt 1, stopped before Compendium when exact-Edge job `96507263338` on
`ubuntu24/20260816.277` performed an already-newest apt no-op and the unchanged live preflight found
no CDP endpoint. The two workflows now request same-package `--reinstall`, and the preflight
selftest has a green fail-closed static control for that ordered workflow contract. One changed-head
CI must test the normalization hypothesis. Do not expand timing policy or optimize accepted CI
allowances; first red stops, and after PR #32 closes, return to Arc 1B/gameplay.
...
generation/world owner. It changes producer to `d3223177…` while measurement stays `6ba58522…`;
clean source `75a996af…` supplied baseline8 plus independent candidate17/18/19, and active
budget/test `74e88c2b…` / `485be9da…` preserve strict discrimination. Activation is browser-free,
...
This remains a hypothesis pending one changed-head CI, with no timing, retry, fallback, product,
live/runtime-tool behavior, browser-authority, measurement, producer, or budget drift. First red stops.
