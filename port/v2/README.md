...
> **Current local candidate — signed Final6 stopped fail-closed at Slice; repaired signed successor
> next.** Signed clean `ea845d77d9783599c269f708462eb650e2c3e245` passed Layout
> `20260827-phase4-final6-ea845d77d978-layout` at 787/787, source-bound SceneMemory
> `…-scenemem` at 42/42 and source-bound Compendium `…-compendium` at 78/78. Slice
> `…-slice` then ran once for 420,570 ms and stopped with five findings in five scopes. Its root was
> instrumental: the exact thermal Inventory row was a valid 164px button whose raw centre at
> `y=751` was clipped below the panel scrollport. The old collector skipped the click and then
> judged four absent modal/Equip/Close/reload descendants. Those are cascades, not independent
> product verdicts. Final6 was not retried; Glass and recovery did not start.
>
> The browser-free repair performs native row reveal and render settlement, proves exact scrollport
> containment and centre-hit ownership, binds trusted row/action/Close receipts, proves detail Close
> leaves the parent Inventory visible/open/expanded, and binds exact receipt bytes/semantics plus F4
> RNG authority across reload. Every dependent judgment and mutation control requires its complete
> green causal prefix. Red actions
> prove quiescence and cleanup, then terminate before later mutable arcs. No product source or
> numeric ruler changed. Final6 cannot resume; a newly signed clean successor must restart at
> Layout. Edge `151.0.4129.107` / CDP `1.3` is run provenance only, never a rebaseline or
> threshold-change trigger. No post-repair Slice, Glass, recovery, whole-Gate, hosted, HUMAN or
> release authority exists. The user-authorized universe-wide visual polish across galaxies,
> systems, planets, biomes, creatures and plants remains queued after this stable checkpoint; it
> preserves deterministic authored structure and is not implemented in this candidate. Earlier
...
> one-attempt Compendium cardinality-oracle stop at 0/78. The current exact-membership repair changes
...
> Compendium before Slice stopped on the Inventory reachability/causal-prefix instrument defect. A
> newly signed clean successor and full restart at Layout are required; Slice, Glass, recovery and
> whole-Gate evidence remain open.
>
...
> preview/publication and deployment authority were unclaimed. The later Final4–Final6 history plus
...
| **Current local candidate verification (2026-08-27)** | Signed clean `ea845d7…` supplied Final6 Layout 787/787, source-bound SceneMemory 42/42 and source-bound Compendium 78/78, each once with named verification. Slice then ran once for 420,570 ms and stopped at one root Inventory row-reveal defect plus four unexercised descendants; it was not retried, and Glass/recovery did not start. Exact carriers, sizes and hashes are in `audits/README.md`. The browser-free repair now proves native reveal, settled containment/hit ownership, exact trusted row/action/detail-Close/panel-Close receipts, parent-panel survival, exact receipt keys/raw bytes/parsed semantics and stable F4 RNG authority across reload, plus full green causal prefixes through Atlas; every terminator and control is structurally negative-tested. No product or numeric ruler changed. The worktree must become a newly signed clean successor and restart at Layout; Final6 cannot resume. Edge `.107` / CDP `1.3` is provenance only. No post-repair Slice, Glass, recovery, hosted/HUMAN whole-Gate or release authority exists. |
| **Current Compendium evidence boundary (Final6 is green; later Slice repair changes current-head bytes)** | Signed `39e4f20…` reached Final5 Compendium after Layout/SceneMemory greens but stopped at 0/78 because `8 > 8` could not prove a cold key. Its exact-membership repair became signed `ea845d7…`, whose run `20260827-phase4-final6-ea845d77d978-compendium` passed 78/78 with six review PNG bindings after green predecessors. Its raw/gzip SHA-256 are `1416d7cad1512ad4bc251d8870d2ac4681f0aef059887a41f4bcd1ba96aa41aa` / `8567a36ca0c4d9d28a5dbe4373ae60c29364843b4657a85ee54c71c8593c85fc`. Current collector `c13a489d…3faab`, outcome contract `ac7eea39…6d17`, measurement `28b06f3c…276c` and budget-file `3e660742…2ae5` bind producer `bf9ad0b6…4dcb`; numeric ceilings and historical samples remain fixed. The later Slice-only repair requires a fresh signed-successor chain for current-head authority. Browser authority remains Edge family + CDP `1.3`; exact `.107` is provenance only. HUMAN/Glass/recovery/whole-Gate closure remains open. |
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical Pixi inventory, heap/DOM/resource bounds, target answerability, independent heartbeat, zero pending work, bfcache survival and cleanup. Signed `ea845d7…` passed Final6 run `20260827-phase4-final6-ea845d77d978-scenemem` 42/42 with zero findings and complete cleanup after its green Layout predecessor. The later Slice evidence repair makes that signed chain historical for current worktree bytes and requires a newly signed successor from Layout; it does not change SceneMemory budget `a23a3d80…8355`, its numeric ceilings or calibration samples. Exact Edge provenance is mandatory but not baseline identity. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-27)** | Signed `ea845d7…` is the latest exact 42/42 report, Final6 run `20260827-phase4-final6-ea845d77d978-scenemem`; its raw/gzip SHA-256 are `d23ec732491730bfcd718f0ca1ed6d758f83d5142d454ec29601c291965d4ade` / `541184b8affb1a637909644fc7fb88184f00fbfedf8aa60dff25af411696c720`. Signed `862a75b…` remains the paired 40/42 heap/DOM red; signed `6c9ad855…` supplied three clean calibration candidates; signed activation `4a54c0d…` set 12 MiB V8 /18 MiB aggregate with every other ceiling and paired discrimination intact. Budget `a23a3d80…8355` is unchanged. Because the current Slice instrument repair changes repository evidence bytes, the full chain must restart at Layout under a newly signed successor; Final6 cannot resume. A compatible Edge update remains provenance only and never triggers rebaseline, recalibration or threshold movement. |
