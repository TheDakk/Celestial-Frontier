...
> **Current evidence — signed Final10 passed all five responsive predecessors, then Recovery
...
>
> The current phase-specific successor keeps that suppression seal unchanged and repairs the later
> Pertar surface gate. `ready-visible`, `exhausted-visible` and `exhausted-offline` now have disjoint
...
> campaign source. No successor browser certificate, Recovery or product verdict exists. Final10 is
> immutable and cannot resume. That descendant must restart
> **Layout → SceneMemory → Compendium → Slice → Glass → recovery**. Edge
...
> geometry and is not implemented in this candidate. The live Phase-4 surface remains globe-only;
> Phase 6 must port the preserved authored procedural vistas from `packages/art/src/hdart.verbatim.js`
> for all 43 live biome profiles without changing their composition, placement, scale or structural
> identity before the shared treatment is carried through their flora, fungi, microbes and ambient
> actors. Earlier
> Slice and SceneMemory reds remain diagnostic history. Gate B now recursively seals the exact
> 62-file domain-source inventory against DOM,
...
> `child.fed = 0.5 * min(clamp(left.fed), clamp(right.fed))`, symmetrically and once. It remains a
> model invariant, not a public breeding/care writer or companion UI. Arc 7/8 is no longer confined
> to packages, but the player-live boundary is intentionally one event wide: the exact current registered
...
> verification, and any nonzero/red/instrument result stops the chain without automatic retry. The
...
> synchronized signed clean docs-only descendant is the fresh campaign source and requires a full restart.
>
...
The current **player** audio boundary is one creature event wider than the retained compatibility
...
plans, lab audit and empty rights validator remain package foundations. Compatibility stings remain
outside the runtime accounting. No other creature expression, authored/recorded asset, ambience,
...
open; the implemented greeting does not confer release/version authority.
...
| **Current local candidate verification (2026-08-28)** | Signed clean `4405fb2…` supplied Final10 Layout `20260828-phase4-final10-4405fb2b4ba7-layout` at 787/787, SceneMemory `…-scenemem` at 42/42, Compendium `…-compendium` at 78/78 with six PNGs, Slice `…-slice` with zero findings/ten screenshots and Glass `…-glass` across 12 viewports with zero findings or instrument failures. Each green stage ran once and passed named verification. Recovery `…-recovery` then ran once, passed fixture, the complete 16-attempt burn-down, disabled suppression, close/checkpoint and closed/offline proof, and stopped terminal `instrument-fail` at `offline-reopened` after 110,549 ms; cleanup passed and no retry occurred. The truthful read-only surface exposed all three controls as `unavailable`, but the phase-blind poll accepted only active `empty`/`depleted`. Active observation, boundary crossing and recovered stages did not run. Seven exact carriers, sizes and hashes are in `audits/README.md`. The signed implementation/evidence checkpoint `3fbfcd5…` separates offline/active predicates, follows a separate offline raw read with one same-document UI → state surface/runtime witness, refreshes/re-proves active exhaustion, and requires retained/replayed/cross-bound receipts for all three phase surfaces before a PASS can verify. Its six Recovery seals are full/production/dedicated/assessment/phase/suppression `c1b4798…` / `a96138c…` / `f568a7b…` / `c5a76e7…` / `b661d67…` / `22e8704…`; full values and byte lengths are recorded in the current-boundary block above. Its embedded SSH signature verified; this documentation changes source identity, so the signed clean docs-only descendant is the fresh campaign source. No successor browser run exists. Final10 is immutable; that descendant must restart Layout → SceneMemory → Compendium → Slice → Glass → recovery. Edge `.107` / CDP `1.3` is provenance only and never causes a rebaseline or threshold change. No Recovery, product-layout, hosted/HUMAN whole-Gate or release authority exists. |
| **Current Compendium evidence boundary (Final10 exact-source green; signed repair and signed docs descendant)** | Signed `4405fb2…` passed Final10 Compendium run `20260828-phase4-final10-4405fb2b4ba7-compendium` 78/78 with six review PNG bindings after green Layout and SceneMemory predecessors, followed by green Slice and full Glass. Its raw/gzip SHA-256 are `6ee3aa6f2e4dd50b7e148302486a4381385df92a10828f1b8b63408c66353035` / `c93be41ed90be67236b10a2b57a79e252824b00091be9da55f84604de2ade8cd`. This certifies the narrow Inventory/Compendium/Settings repair for that exact signed source. Signed implementation/evidence repair `3fbfcd5…` changes source identity and the synchronized signed clean docs-only descendant is the fresh campaign source without altering numeric Compendium rulers, samples or the 55-bullet draft, so it requires a fresh full browser chain. Browser authority remains Edge family + CDP `1.3`; `.107` is provenance only, and compatible point updates never trigger rebaseline. HUMAN Compendium judgment, Recovery and whole-Gate closure remain open. |
| `npm run arc4recovery:selftest` / `node tools/arc4recovery.mjs --slice-run=<Slice-run-id> --glass-run=<Glass-run-id>` / `node tools/arc4recovery.mjs --verify-run=<recovery-run-id> --slice-run=<Slice-run-id> --glass-run=<Glass-run-id>` | The mutation-sensitive selftest seals a dedicated real-time, one-attempt, no-forged-clock collector for the exact active-play next-cycle recovery outcome. Recovery and Slice share one exact disabled-suppression receipt. Recovery quiesces and joins the complete in-flight heartbeat, captures synchronized raw → state → UI exhausted evidence, reveals and settles the exact Tame owner, revalidates it immediately before the bound CDP dispatch, retains one trusted pointer and zero clicks, restores scroll/listeners/globals, resumes heartbeat and reads post-cleanup state before product attribution. The later Pertar gate separately binds `ready-visible`, active `exhausted-visible` (`empty`/`depleted`) and read-only `exhausted-offline` (`unavailable`) phases; the offline path reads raw, then captures one same-document UI → state surface/runtime witness, and reactivation runs a full heartbeat and re-proves active exhaustion before observation. Exact runtime revision/SessionRNG equality and nondecreasing active time distinguish stable product mismatch from malformed instrument evidence. A candidate PASS retains full schema-bound Pertar surface evidence in `exhausted`, `offline-reopened` and `active-observation`; terminal finalization and named verification independently replay those receipts and cross-bind phase/document/cycle/facts/SessionRNG/state/UI/first-active-sample evidence. Before observation, `active-observation:running` must be persisted and survive later failure. Each poll receipts the strict remaining share of one absolute 20-second deadline. The exhausted raw/live-state chain and reactivated→first-service binding require at most 20 seconds, the same cycle/RNG and revision delta at most one. Missing, swapped, coherently retokened, reversed-chronology, coherently recomputed route/card/runtime/pending, internally assessment-green retiming, +2-revision and next-cycle mutants are red. Recovery's exact full/production/dedicated/assessment/phase/suppression seals are `c1b4798eb21bad961d1dd984b515ca1cc884101ce28405c09613c1e361118f84` / `a96138cc33ace145c77e64de584f4062d0860d4e418c8d3c19d06a1293db56be` / `f568a7bb95a49d7dfb9839d2d11cc68743f87cf3af6eb52776f5551dba0e6045` / `c5a76e70c096a33df9bc12ba9a044c7d7bfddc1dc082d61e8365f5d7c99b35f5` / `b661d676f1679e9fc92590bf7849ee319ea0b8c78f444a91f46b06eccff29b6e` / `22e8704122103323d0dd0079ce0d2821d69f249a860f31e4062f51b9f8e68771`; byte lengths are 217,578 / 91,758 / 10,442 / 6,184 / 7,125 / 13,190. The production seal rejects a dead-wrapped/comment-shadowed sole operative span and the full seal rejects late helper rebinding. Slice's preparation/collector digest remains `baa284a736d9243df4a61de192e553111c9c5fbc9d6aa70ffd05af5b6e31e45f`. The verifier also replays the fixture witness/receipt/snapshots and rejects null, malformed, cleanup-red or forged evidence. The collector is non-standalone: it verifies both exact terminal-green predecessors, their nested Glass → Slice binding and the same clean committed source. Final10 stopped at the old offline phase oracle. Implementation/evidence commit `3fbfcd5…` is locally signed with a verified embedded SSH signature; the synchronized signed clean docs-only descendant is the fresh campaign source. No successor browser or uninterrupted real 20-minute candidate certificate exists yet. |
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical Pixi inventory, heap/DOM/resource bounds, target answerability, independent heartbeat, zero pending work, bfcache survival and cleanup. Signed `4405fb2…` passed Final10 run `20260828-phase4-final10-4405fb2b4ba7-scenemem` 42/42 with complete cleanup after its green Layout predecessor; later Compendium, Slice and Glass stages also passed. The local Recovery phase-oracle successor changes evidence source identity but not the SceneMemory product, ruler or samples, so a newly signed successor must restart from Layout. Exact Edge provenance is mandatory but not baseline identity; a compatible point update never triggers a rebaseline. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-28)** | Signed `4405fb2…` is the latest exact 42/42 report, Final10 run `20260828-phase4-final10-4405fb2b4ba7-scenemem`; its raw/gzip SHA-256 are `00161fd6c4d8b5457776dc1768ea2022a142a73c892c8a4cdff207073e88116a` / `99abfc6d0a084ce0ed874cf5516c833d1d73735e4ebb8c44c0d3ce870075a0e3`. Signed `862a75b…` remains the paired 40/42 heap/DOM red; signed `6c9ad855…` supplied three clean calibration candidates; signed activation `4a54c0d…` set 12 MiB V8 /18 MiB aggregate with every other ceiling and paired discrimination intact. Final10 changes no ceiling or sample and passed the full responsive chain through Glass. Signed implementation/evidence repair `3fbfcd5…` and the synchronized signed clean docs-only descendant is the fresh campaign source requiring a new full chain; compatible Edge updates remain provenance only and never trigger rebaseline, recalibration or threshold movement. |
...
- **`tests/nodom.test.ts`** — recursively seals the exact 62-file domain-source inventory against
