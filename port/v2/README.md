...
> **Arc 1A Compendium resource implementation (2026-08-17; automated product
> implementation present; the resource ruler is intentionally calibration-required,
...
> `budgets/compendium-memory-v1.json` is deliberately `calibration-required`: its
> candidate sample sets are empty, all candidate ceilings are null, and its paired
> baseline is `measurement-required`, so it cannot issue a certification PASS.
...
> owner-to-worker-to-painter authority. One fresh paired exact-3844701 baseline, three
> independent one-attempt current-candidate runs per profile, new ceiling rationale,
> and a later exact-head certification remain open.
>
