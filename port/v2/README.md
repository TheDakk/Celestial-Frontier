...
> **Current evidence — signed Final8 passed the complete responsive chain through Glass, then
> stopped at Recovery's fixture precondition.** Signed clean
> `c133c89ead736c9c7414af1e6242acd411339853` passed Layout
> `20260828-phase4-final8-c133c89ead73-layout` at 787/787, source-bound SceneMemory `…-scenemem`
...
> `…-recovery` then ran once and stopped terminal `fail` at its fixture precondition after 7,633 ms;
> the sole failed clause was `runtimeCaptureOrder`, cleanup passed and no retry occurred. Burn-down,
> exhaustion, closure/reopen, the 20-minute active observation, boundary crossing and recovered
> stages did not run. Final8 therefore proves the repaired Inventory, Compendium and Settings
> behavior through full Glass, but makes no Recovery product verdict or recovery claim.
>
...
>
> Post-run source review proved the Recovery red was an instrument chronology mismatch. The old
> collector sampled outer state first and then obtained the UI's nested state snapshot, while the
> shared precondition expected canonical UI → state order. Because active play advances from a
> monotonic clock, the later UI value could legitimately exceed the earlier state value; equality
> could also hide the reversed collector. The bounded repair now captures UI then state and binds a
> browser-derived document/order/timestamp/runtime witness. Malformed receipts are instrument-red;
> a trusted backward runtime remains product-red under the unchanged shared precondition. Positive
> and negative controls cover swapped distinct/equal values, ordinals, timestamps, document identity,
> exact projection, enforcement bypass and product-backward runtime. Trusted page sources and the
> production collector are additionally sealed against ordinary clock wrapper/binding shadows and
> direct, computed, destructuring, iteration, descriptor, alias, whole-object, prototype and legacy
> accessor writes. The full browser-free battery and two independent bounded reviews are clear.
>
> Browser-free tests pass for that instrument repair, but its edited worktree is not a browser
> certificate. Final8 is immutable and cannot resume. A newly signed clean successor must restart
> **Layout → SceneMemory → Compendium → Slice → Glass → recovery**. Edge
> `151.0.4129.107` / CDP `1.3` is Final8 run provenance only, never a rebaseline or threshold-change
...
> geometry and is not implemented in this candidate. Earlier
...
> later Recovery clock-seal repair is the uncertified WIP that now requires the fresh full restart.
>
...
| **Current local candidate verification (2026-08-28)** | Signed clean `c133c89…` supplied Final8 Layout `20260828-phase4-final8-c133c89ead73-layout` at 787/787, SceneMemory `…-scenemem` at 42/42, Compendium `…-compendium` at 78/78, Slice `…-slice` with zero findings/ten screenshots and Glass `…-glass` across 12 viewports with zero findings or instrument failures. Each ran once and passed named verification. Recovery `…-recovery` then stopped terminal `fail` at fixture after 7,633 ms with only `runtimeCaptureOrder` red, complete cleanup and zero retry. Static review proved its old state→nested-UI collector disagreed with the shared UI→state chronology; no burn-down, 20-minute observation or Recovery product verdict occurred. Seven exact carriers, sizes and hashes are in `audits/README.md`. The repair successor binds browser-derived order, timestamps, document identity, snapshots, projections, complete product-precondition replay and product/instrument classification, but it has no browser certificate yet. Final8 is immutable; the newly signed clean successor must restart Layout → SceneMemory → Compendium → Slice → Glass → recovery. Edge `.107` / CDP `1.3` is provenance only. No recovery, hosted/HUMAN whole-Gate or release authority exists. |
| **Current Compendium evidence boundary (Final8 is exact-source green; later tool-only successor is uncertified)** | Signed `c133c89…` passed Final8 Compendium run `20260828-phase4-final8-c133c89ead73-compendium` 78/78 with six review PNG bindings after green Layout and SceneMemory predecessors, followed by green Slice and full Glass. Its raw/gzip SHA-256 are `cfb11b3ba5c735c02c3e4ad0e3a6ba0ed6748a3edbac5157d47e3f9aaad723e8` / `f7dcc09f8a7ce419488a63e4c446ea384aea167caeb007ea321cc6f128de71ec`. This certifies the narrow Inventory/Compendium/Settings repair for that exact signed source. The later Recovery instrument/tests/docs change source identity without altering numeric Compendium rulers, samples or the 55-bullet draft, so the successor requires a fresh chain. Browser authority remains Edge family + CDP `1.3`; `.107` is provenance only. HUMAN Compendium judgment, Recovery and whole-Gate closure remain open. |
| `npm run arc4recovery:selftest` / `node tools/arc4recovery.mjs --slice-run=<Slice-run-id> --glass-run=<Glass-run-id>` / `node tools/arc4recovery.mjs --verify-run=<recovery-run-id> --slice-run=<Slice-run-id> --glass-run=<Glass-run-id>` | The mutation-sensitive selftest seals a dedicated real-time, one-attempt, no-forged-clock collector for the exact active-play next-cycle recovery outcome. Its ready-surface receipt requires browser-derived UI→state call order, exact capture count/value binding, page-monotonic timestamps, document-token continuity and exact snapshot/runtime projections. Malformed receipts are instrument-red; trusted backward product time stays product-red, with full witness retained on either path. The terminal verifier replays the fixture witness/receipt/snapshots and rejects null or forged evidence. The collector is non-standalone: it verifies both exact terminal-green predecessors, their nested Glass → Slice binding and the same clean committed source. No uninterrupted real 20-minute candidate certificate exists yet. |
...
| **Arc 4 focused + browser evidence (current boundary 2026-08-28)** | Persistence, Training restore, acquisition planner/capacity/action, app bootstrap/wiring and coordinator tests cover strict 18-namespace bootstrap, exact legacy source evidence, full-roster/current-epoch two-draw settlement, every pre-draw miss/hit capacity scenario, one receipt/CAS, hit/miss spend, first-only/repeat ownership, full committed verification, targeted publication and synchronous overlap barriers. Final8 Slice passed the nine-stage native-control ledger with zero findings and Glass passed every Arc 4 outcome across 12 viewports. Recovery reached only its fixture precondition and stopped before burn-down or the 20-minute observation on a post-run-diagnosed collector chronology mismatch. Thus recovery remains unproved and `recoveryClaimed:false`; the current tool-only repair needs a fresh signed full chain. HUMAN review, hosted, preview/publication, release and whole-Gate authority remain open. |
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical Pixi inventory, heap/DOM/resource bounds, target answerability, independent heartbeat, zero pending work, bfcache survival and cleanup. Signed `c133c89…` passed Final8 run `20260828-phase4-final8-c133c89ead73-scenemem` 42/42 with zero findings and complete cleanup after its green Layout predecessor; later Compendium, Slice and Glass stages also passed. Current Recovery-tool WIP changes evidence source identity but not the SceneMemory product, ruler or samples, so a newly signed successor must restart from Layout. Exact Edge provenance is mandatory but not baseline identity. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-28)** | Signed `c133c89…` is the latest exact 42/42 report, Final8 run `20260828-phase4-final8-c133c89ead73-scenemem`; its raw/gzip SHA-256 are `5bbac8b3c71a396e0ace5ce04e2124c15695f0af92580cca47d85b3057cb2e2a` / `a4ac0eeb323cd6558c690d5b1d57d1e5a0a36ec69997db22010e0d520bbef649`. Signed `862a75b…` remains the paired 40/42 heap/DOM red; signed `6c9ad855…` supplied three clean calibration candidates; signed activation `4a54c0d…` set 12 MiB V8 /18 MiB aggregate with every other ceiling and paired discrimination intact. Final8 changes no ceiling or sample and passed the full responsive chain through Glass. The current Recovery-tool repair still requires a new full signed chain; compatible Edge updates remain provenance only and never trigger rebaseline, recalibration or threshold movement. |
