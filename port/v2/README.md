...
| Tool | Role |
|---|---|
| **Current Slice Guide/Charter/D-TRAIN boundary (2026-09-01; repaired source awaits a fresh exact-source chain)** | Hosted run `33572309149` remains immutable terminal red at exactly three independent scopes. Guide tail proof now uses a pure bounded state machine: one native wheel, one fresh exact-document/tail/hit-owner/geometry/visibility observation, adaptive remaining delta, hard duration/attempt/stall limits, wrong-tail/owner/geometry controls and exact scroll/overflow restoration. Saturated Charter recovery holds one real Landing owner while a real ceremony callback must increment deferrals without shifting/delivering its preserved queue; removing/moving the product-action guard or clearing on defer is red, and release must yield the one aggregate outcome before the older ceremony resumes. D-TRAIN brackets live/focus state with equal raw-primary reads until release identity, pending state, Guide focus, removed Training, released chrome and closed Atlas converge; every predicate has an independent mutant, and the exact third native write must arrive before topology judgment. Only ceremony presentation ordering changes product behavior; Guide/Training/release timing, save schema, rewards, rulers and retry policy are unchanged. |
