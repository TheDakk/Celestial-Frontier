...
> every candidate replays 78/78. Active budget/test SHA-256 values are
...
> Focused replay 11/11, selftest 222/222, and semantic validation are green. The targeted compact-phone
> Glass diagnostic passes at `13efb5fa…`, but is non-certifying. Exact-head certification, the complete
> battery, corresponding PR test-merge CI, and six-image HUMAN review remain open.
>
...
Highest-priority active v2 work is to finish PR #32's exact-head transition
without widening D-TRAIN-1's eleven-field compatibility boundary into ownership or receipt authority.
Preserve c095's exact Compendium/Smoke PASS and its first full Glass product red. Baseline6 plus
independent candidate11/12/13 now activate producer `e59685b1…` from replayed raw capsules; run the
complete unchanged activation-head battery and corresponding PR test-merge CI next.
...
`32350971816` remains the preserved timer-instrument red; c095's 12.5px compact-phone Glass overlap is
the newer preserved product red. Baseline5/candidate8/9/10 remain historical authority only for
producer `1c8200d7…`; current paired baseline6/candidate11/12/13 activate the strict ruler for producer
`e59685b1…`.
...
clean baseline6/candidate11/12/13 activate budget `ebe5b5c3…` for that exact producer.
