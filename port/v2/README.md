...
> scheduler repair present; shared-timer recalibration required; live automation resolved from exact-source
...
> to `f9710bdfaac255d7df7e8c29f251c8387041abe99a0178667b7b3430110a0409`. Budget SHA-256
> `a41ff08b8a58e776c789f09e7294c1cb2c0f44da8406f81c55f0754076337c30` is therefore
> `calibration-required`: collect a fresh paired broken baseline plus three independent candidates
> before activation/certification. Fresh six-image HUMAN review remains separate.
>
...
`32350971816` is the current stopping red: its early shared-command timeout is instrument-only, so
the frozen `f9710bdf…` measurement authority remains `calibration-required` until a fresh paired
baseline and three independent candidates activate a successor ruler. Only a complete same-head
...
input, browser-CDP SHA-256 `36a832bc…` moves authority to `f9710bdf…`; the budget is intentionally
`calibration-required` pending fresh paired-baseline and three-candidate evidence.
