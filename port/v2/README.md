...
> **Current local candidate — source-complete bounded lanes with one preserved Slice red and a
> product-owner Compendium recalibration pending.** Gate B now recursively seals the exact 62-file domain-source inventory against DOM,
...
>
> The dedicated real-time Arc 4 recovery collector and mutation-sensitive selftest are ready; the
...
> product-owner/built-producer drift—not Edge `.107`—moves current authority to
...
> verification. This policy remains Compendium-only; SceneMemory and root Gate A are unchanged.
>
> Browser-free verification of these local bytes is green: 121 files / 1,356 passed / one
...
> reporter, Glass, recovery and Compendium selftests. Current-input Slice
> `20260826214541492-83064-b252b137f7a3` executed once from signed `8553bd7…` on Edge `.107` and
> failed after 92,772 ms with three findings and zero retries. Settings' 14-versus-15 control count
> and the Arc 3 stage-1-unreachable target were instrument false reds; retained Survey focus was a
> product regression. All three are repaired locally, but no rerun PASS exists. Fresh Compendium
> calibration/certification, current-input Glass, the real recovery certificate, hosted execution,
...
> 250 ms target ceiling; that 42/42 certificate is not rebound to the current budget. Active repair
> source `7d8dc380cd89ef53aac5a11c3850316e19e1aae9` keeps the product, collector, and verdict contract
...
> `7c4100244abef8d50f93178aab7c8579ae93fa0b6bef76422cc5c0523edac55a`. These are exact local
> automated authorities, not hosted-green, HUMAN, integration, Gate-closure, or release authority.
>
...
>
> The guarded `test-battery` now runs the shorter SceneMemory ruler before Compendium: it installs
...
> `v2-scene-memory-evidence`. Only then may the separately sealed Compendium chain run. Its browser
> authority accepts the selected canonical Edge by family, CDP `1.3` and sealed capability contract;
> `.101` is retained as that hosted run's exact provenance, not as Compendium threshold identity.
...
certificate is also historical; active `7d8dc380…` / `5c8a6e75…` and local run
`20260823-pr33-cross-host-sla-certification` own the current 42/42 real-Shipyard proof under the
...
| Tool | Role |
|---|---|
| **Current local candidate verification (2026-08-26)** | Browser-free source gates are green: 121 Vitest files / 1,356 passed / one intentional skip / zero failures; `root/app/worker/noUnused` TypeScript; 884-module Vite build; root validate/fingerprint and legacy jsdom smoke; contract/reporter/Glass/recovery/Compendium selftests. Current-input Slice `20260826214541492-83064-b252b137f7a3` ran once from signed `8553bd7…` on Edge `.107` and failed after 92,772 ms with three findings/zero retries: two instrument false reds and one retained-Survey focus regression. All three are locally repaired; no rerun PASS exists. Fresh Compendium, Glass, recovery, hosted, HUMAN, whole-Gate and release evidence remain open. |
| **Current Compendium authority (`calibration-required`)** | Measurement `cb5cd9f86ac99435028f98af800bc0d89de96bd7db88694214d832eed83fb15d`, former producer `587d3bdfab471370e625c71d1658e391067881fe824ce14ccfaf7200eb6e4d73`, selected c3/c5/c6 plus baseline1, signed activation `d33e540…` and certificate `20260826-phase4-certification` remain historical exact-source evidence. Real product-owner drift moves current producer authority to `f7c87f2263bdac4014e5f56be5efc5ceeca7fbd2e32e25549a6b9e0260354224`; it still needs three fresh candidates, one paired baseline, activation and one exact-budget certificate plus named verifier. Browser authority v2 remains Microsoft Edge family + CDP `1.3` + sealed capability-contract SHA-256 `6eed33ed9784f7c7774c4b1bf8d4e880986e31667324d9a1aa7b8dd62fe5a476`; exact Edge-build fields are same-run provenance. Auto-update alone never rebaselines or changes a ceiling. SceneMemory and root Gate A are unchanged. |
