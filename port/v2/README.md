...
> calibration, and signed `4a54c0d…` owns heap-only activation; exact certification is next.** Six
> current-input Slice reds and both preserved SceneMemory reds remain diagnostic
...
> numeric budget in either ruler. The current worktree is a documentation-only descendant of signed
> activation `4a54c0d…` with identical producer/budget bytes. Exact SceneMemory certification remains
> open; Compendium needs a fresh certificate bound to that producer.
...
> also CLEAR. Signed activation `4a54c0d…` owns the green worktree/tests; fresh serial browser IDs
> remain pending.
...
> numeric budget. The current worktree is a documentation-only descendant of signed activation
> `4a54c0d…`; both eventual exact-producer certificates remain open. No current-input Slice PASS, Glass result or
...
> complete; a fresh exact-budget certificate remains open. Edge point-version changes alone never
...
| Tool | Role |
|---|---|
| **Current local candidate verification (2026-08-27)** | Signed heap-only activation `4a54c0d…` (tree `ff11158a…`, parent `6c9ad855…`) is green at 134 Vitest files / 1,469 passed / one intentional skip / zero failures, plus `root/app/worker/noUnused` TypeScript, `artunused`, the 887-module build and root validation. Signed `862a75b…` remains the exact paired red. Signed `6c9ad855…` supplied exactly `20260827-phase4-repair-candidate1/2/3`, each once with zero retries and full cleanup on Edge `.107` / CDP `1.3`. Their maxima activate only 12 MiB V8 /18 MiB aggregate ceilings under budget SHA-256 `e6c4aeea…`; 676/71 and 673/70 nodes/listeners plus every other ceiling remain unchanged, both paired replays still fail 704/80, and current producer bindings match. This documentation-only descendant changes no producer/budget bytes. An exact SceneMemory certificate and fresh serial IDs are next. Edge point version is provenance only. No current Slice PASS, Glass result, recovery certificate, hosted/HUMAN whole-Gate or release authority exists. |
