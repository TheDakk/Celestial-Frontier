...
> scheduler repair present; explicit browser-authority recalibration pending;
...
> self-updated to `Edg/151.0.4129.93`, revision `@4a822b1b…`; null calibration browser authority
> let it collect profiles. Quarantine report/sample/log `175fac5e…` / `916dd12a…` / `7462144b…`
> as wrong-browser instrument evidence—not calibration, certification, product failure, or HUMAN
> review. Baseline9 did not run.
>
> The budget now requires explicit top-level Edge `.86` / revision `@083e7549…` / JavaScript
...
> complete report must replay a true match. Frozen budget/schema/contract/collector/selftest/test
...
> Samples stay empty, ceilings null, and baseline measurement-required. Materialize exact `.86`
> afresh per launch, then collect baseline9 plus candidate21/22/23 from one clean committed source,
> once each without retry; activate, run one exact-head battery, push PR #32 once, and require one
> corresponding CI. No timeout, launch argument, product byte, producer, or retry policy changed.
>
...
Highest-priority active v2 work is to finish PR #32's explicit browser authority without widening
...
Edge `.93`; quarantine `175fac5e…` / `916dd12a…` / `7462144b…`, and note baseline9 did not run.
Explicit `.86` authority and pre-profile candidate/baseline rejection establish measurement
`825fb386…`; producer `d3223177…` and browser CDP `6da9e2ef…` remain unchanged. Fail-closed budget
`71ffa46f…` awaits fresh-per-launch exact-.86 baseline9 plus candidate21/22/23 once each without
retry, then activation, one exact-head battery, one PR #32 push, and one corresponding CI. Do not
...
lifecycle, but its reused `.86`-named app reported Edge `.93`; its report/sample/log are quarantined,
and baseline9 did not run. Explicit top-level `.86` authority establishes measurement `825fb386…`;
producer `d3223177…` and browser CDP `6da9e2ef…` remain unchanged. Fail-closed ruler `71ffa46f…`
awaits fresh-per-launch exact-.86 baseline9 plus candidate21/22/23, once each without retry, before
one exact-head battery and corresponding CI. No timing, retry, fallback, product, launch-argument,
or producer drift occurred. First red stops; terminal-green PR #32 returns immediately to Arc 1B.
...
| `npm run compendiummem:selftest` / `npm run compendiummem` / `node tools/compendiummem.mjs --verify-run=<run-id>` | Negative-controls the browser-free Compendium instrument, performs one standalone browser run without automatic retry, and independently verifies the named report. Exact-89 remains an owned-shutdown red. Candidate20 completed 78/78, six PNGs, and clean lifecycle, but its reused `.86`-named app reported Edge `.93`; quarantine report/sample/log `175fac5e…` / `916dd12a…` / `7462144b…`, and note baseline9 did not run. Explicit top-level `.86` authority is required in every budget state and checked before candidate or paired-baseline profile collection; mismatch emits early instrument-fail evidence and no profile. Measurement `825fb386…` leaves fail-closed budget `71ffa46f…` with empty samples/null ceilings and a `measurement-required` baseline; producer `d3223177…` and browser CDP `6da9e2ef…` are unchanged. The gate still drives the exact 1,500-row fixture across phone/desktop, binds raw observations and all measurement/product/browser inputs, verifies the owner→worker→painter graph, retains six same-run review PNGs, and distinguishes pre-measurement instrument failure from product evidence. Materialize exact `.86` afresh for each baseline9/candidate21/22/23 launch and run each once without retry. Automated terminal success remains exact-run evidence and never supplies the open [HUMAN] judgment. |
