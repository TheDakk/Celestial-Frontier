...
> unchanged. Measurement remains `f9710bdf…`; budget/test `70c40013…` / `7e8f03d5…` are
> `calibration-required` with zero candidates, paired baseline `measurement-required`, and null
> ceilings. Fresh baseline6 plus independent candidate11/12/13, activation, exact-head battery,
> corresponding PR test-merge CI, and six-image HUMAN review remain open.
>
...
Highest-priority active v2 work is to finish PR #32's fresh producer ruler and exact-head transition
without widening D-TRAIN-1's eleven-field compatibility boundary into ownership or receipt authority.
Preserve c095's exact Compendium/Smoke PASS and its first full Glass product red; collect paired
baseline6 plus independent candidate11/12/13 for producer `e59685b1…`, activate only from replayed raw
capsules, then run the complete unchanged activation-head battery and corresponding PR test-merge CI.
...
producer `1c8200d7…`; the current `e59685b1…` producer is fail-closed pending fresh calibration.
...
budget `70c40013…` is fail-closed until baseline6/candidate11/12/13 populate a new ruler.
