...
> calibration, signed `4a54c0d…` owns heap-only activation, and signed `7362a0e…` owns exact
> SceneMemory plus Compendium certificates and the preserved seventh Slice harness red.** The
> current bounded repair changes evidence oracles/tests, audit/evidence carriers and docs only;
> product, producer, numeric ruler and deadline bytes are unchanged. It must be signed clean before the entire serial chain restarts
> at Layout. Earlier Slice and SceneMemory reds remain diagnostic history. Gate B now recursively seals the exact
...
>
> The current harness-repair browserless checkpoint is green at
> 134 Vitest files / 1,474 passed /
...
> final whole-diff review for that input is CLEAR. The current bounded repair's final
> documentation/release-copy consistency review and independent evidence/code whole-diff review are
> also CLEAR. Signed `7362a0e…` owns the source-bound SceneMemory/Compendium greens and is
> chronology-associated with the verifier-green Layout result; the current repair must receive a
> new signed clean checkpoint and fresh serial browser IDs.
> Current-input Slice remains RED; Glass, recovery and whole-Gate evidence remain open.
>
> Current-input Slice now retains **seven** one-attempt/zero-retry reds on Edge
...
| Tool | Role |
|---|---|
| **Current local candidate verification (2026-08-27)** | Signed heap-only activation `4a54c0d…` (tree `ff11158a…`, parent `6c9ad855…`) and signed evidence boundary `7362a0e…` (tree `7115732…`) preserve the selected 12 MiB V8 /18 MiB aggregate ceilings under budget SHA-256 `e6c4aeea…`; 676/71 and 673/70 nodes/listeners plus every other ceiling remain unchanged, both paired replays still fail 704/80, and producer bindings match. `7362a0e…` supplied source-bound standalone and serial SceneMemory 42/42 plus Compendium 78/78 with named verification. Its associated serial Layout carrier is 787/787 and verifier-green, but the schema embeds no Git/source identity and its non-commit-tagged run lacks a preserved execution record, so it is result provenance rather than standalone exact-source evidence. Slice `20260827-phase4-final-slice` stopped terminal red after 414,213 ms with 12 findings/11 scopes and no retry; Glass/recovery did not run. The bounded harness repair is green at 134 Vitest files / 1,474 passed / one intentional skip / zero failures, plus `root/app/worker/noUnused` TypeScript, `artunused`, the 887-module build and root validation. It changes no product/producer/ruler/deadline bytes and must receive a new signed clean checkpoint before the whole chain restarts at Layout. Edge point version is provenance only. No current Slice PASS, Glass result, recovery certificate, hosted/HUMAN whole-Gate or release authority exists. |
| **Compendium authority (current exact certificate on `7362a0e…`)** | Measurement `cb5cd9f86ac99435028f98af800bc0d89de96bd7db88694214d832eed83fb15d`, former producer `587d3bdfab471370e625c71d1658e391067881fe824ce14ccfaf7200eb6e4d73`, selected c3/c5/c6 plus baseline1, signed activation `d33e540…` and certificate `20260826-phase4-certification` remain historical exact-source evidence. Signed calibration source `8ffd2e2…` supplied `20260826-slice-repair-candidate1/2/3` plus paired `20260826-slice-repair-baseline1`, each one attempt/zero retries. Historical budget `6284a394…` bound former producer `f7c87f22…`, retained all four faults and exact 14-phone/13-desktop discrimination, and changed only the phone warm ceiling to `524288`. Historical clean signed activation `91f4e044…` passed `20260826-slice-repair-certification` 78/78 with complete lifecycle and named verification in one attempt/zero retries (44,847 ms; raw/gzip SHA-256 `81c27ed5…` / `6f3deb0…`). Current signed `7362a0e…` passed `20260827-phase4-final-compendium` 78/78 with zero findings, complete lifecycle and named verification in one attempt/zero retries (44,852 ms; raw/gzip SHA-256 `45849b7e…` / `742d7391…`) for producer `4bdd3e36…`. Browser authority v2 remains Microsoft Edge family + CDP `1.3` + sealed capability-contract SHA-256 `6eed33ed9784f7c7774c4b1bf8d4e880986e31667324d9a1aa7b8dd62fe5a476`; exact Edge `.107` is same-run provenance only. The next signed harness source must repeat Compendium inside the full predecessor chain; HUMAN review, Slice/Glass/recovery and whole-Gate closure remain open. |
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. One compatible Microsoft Edge-family process satisfying CDP `1.3` and the sealed capability plus exact phone/desktop-profile contracts owns four unmeasured warmups and exactly four measured phone/desktop cycles through Universe, Galaxy, Galaxy fine, Sol/System, Earth/Surface, the 1,500-row Compendium and real Shipyard before settled Universe. Exact version/revision/JavaScript/path/UA remain mandatory per-run provenance, not baseline identity. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical per-hash Pixi inventory, heap/DOM/resource bounds, target answerability, an independent browser heartbeat, zero pending work, same-document bfcache survival, and per-profile Shipyard lifecycle through its visible opener and owned Close. `59530da…` / budget `3b71d14c…` / run `20260822-arc1-local-certification` remain historical authority for the former 250 ms local ruler. Repair `7d8dc380…`, budget `5c8a6e75…`, and Edge `.101` run `20260823-pr33-cross-host-sla-certification` remain historical exact-input evidence for the fixed strict `<1000 ms` SLA; that run passed 42/42 locally with named verification. Hosted run `32618995487` remains exact terminal-red 40/42 and was not retried. Signed `6c9ad855…` completed calibration; signed `4a54c0d…` owns the green activation; signed `7362a0e…` passed standalone `20260827-phase4-activation-scenemem` and serial `20260827-phase4-final-scenemem`, both 42/42 with named verification. The later harness-source repair requires a fresh full-chain SceneMemory stage but does not reopen calibration. Version tolerance itself changed no numeric budget. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-27)** | **`[CURRENT-EXACT]` on signed `7362a0e…`; fresh chain repetition pending.** Signed source `862a75b…` remains the paired 40/42 heap/DOM red. Signed `6c9ad855…` supplied exactly `20260827-phase4-repair-candidate1/2/3`, each one attempt, zero retries and full cleanup on Edge `.107` / CDP `1.3`. Signed activation `4a54c0d…` sets 12 MiB V8 and 18 MiB aggregate under budget SHA-256 `e6c4aeea…`; all other ceilings stay unchanged and both paired-red replays still fail nodes/listeners. Signed `7362a0e…` passed two independent 42/42 exact-budget runs with complete cleanup and named verification; archived serial report raw/gzip SHA-256 are `97fb1859…` / `5bef9ef3…`. A compatible Edge update is provenance only and never triggers rebaseline or threshold movement. The harness repair changes source and therefore requires a fresh SceneMemory stage inside the restarted chain, not recalibration. |
