...
> **Current evidence — signed Final9 passed the complete responsive chain through Glass, then
> stopped at Recovery's exhausted disabled-control oracle.** Signed clean
> `a85e0edf9b7ceca0f13ecf32bb8ac3c88db6ceb1` passed Layout
> `20260828-phase4-final9-a85e0edf9b7c-layout` at 787/787, source-bound SceneMemory `…-scenemem`
> at 42/42, source-bound Compendium `…-compendium` at 78/78 with six PNGs, Slice `…-slice` with
> zero findings and the exact ten screenshots, and Glass `…-glass` across all 12 viewports with
> zero findings or instrument failures. Each stage ran once and passed its named verifier. Recovery
> `…-recovery` then ran once and stopped terminal `fail` at exhausted after 87,589 ms. Fixture and
> the complete 16-attempt burn-down passed; cleanup passed and no retry occurred. Close/checkpoint,
> closure/reopen, the 20-minute active observation, boundary crossing and recovered stages did not
> run. Final9 therefore proves the responsive predecessor chain and burn-down for that exact source,
> but makes no Recovery product-layout verdict or recovery claim.
>
...
>
> Post-run geometry and source review proved the Final9 Recovery red was an instrument oracle defect.
> The old collector sampled the Tame button before revealing it inside the Survey scrollport,
> collapsed target geometry and hit ownership into one `ok` bit and retained no actual hit owner.
> The semantically disabled 292×44 control was clipped below the scrollport; unchanged-source Glass
> independently revealed the same control class and proved containment/ownership at the same phone
> viewport, while Slice passed its native disabled-suppression path.
>
...
>
> Browser-free tests pass for that instrument repair, but its edited worktree is not a browser
> certificate. Final9 is immutable and cannot resume. A newly signed clean successor must restart
> **Layout → SceneMemory → Compendium → Slice → Glass → recovery**. Edge
> `151.0.4129.107` / CDP `1.3` is Final9 run provenance only, never a rebaseline or threshold-change
...
> 16-attempt burn-down before the exhausted-control oracle stopped the run. The later shared
> disabled-suppression repair is the uncertified WIP that now requires the fresh full restart.
>
...
| **Current local candidate verification (2026-08-28)** | Signed clean `a85e0ed…` supplied Final9 Layout `20260828-phase4-final9-a85e0edf9b7c-layout` at 787/787, SceneMemory `…-scenemem` at 42/42, Compendium `…-compendium` at 78/78, Slice `…-slice` with zero findings/ten screenshots and Glass `…-glass` across 12 viewports with zero findings or instrument failures. Each green stage ran once and passed named verification. Recovery `…-recovery` then ran once, passed fixture plus the complete 16-attempt burn-down and stopped terminal `fail` at exhausted after 87,589 ms; cleanup passed and no retry occurred. Close/checkpoint, offline/reopen, the real 20-minute observation, boundary crossing and recovered stages did not run. Seven exact carriers, sizes and hashes are in `audits/README.md`. The local successor shares one exact Slice/Recovery suppression receipt, synchronizes a quiesced raw → state → UI exhausted baseline, joins/resumes the complete heartbeat cycle with overlap-counter proof, seals Recovery at `22e87041…` and Slice at `baa284a73…`, and requires cleanup/instrument truth before product attribution. Its exact identity is this batch's clean signed `HEAD`, but it has no browser certificate yet. Final9 is immutable; that successor must restart Layout → SceneMemory → Compendium → Slice → Glass → recovery. Edge `.107` / CDP `1.3` is provenance only and never causes a rebaseline or threshold change. No Recovery, hosted/HUMAN whole-Gate or release authority exists. |
| **Current Compendium evidence boundary (Final9 is exact-source green; local successor is uncertified)** | Signed `a85e0ed…` passed Final9 Compendium run `20260828-phase4-final9-a85e0edf9b7c-compendium` 78/78 with six review PNG bindings after green Layout and SceneMemory predecessors, followed by green Slice and full Glass. Its raw/gzip SHA-256 are `bd009387b9a3f1430135c2ab264bf9ef32429967c02e0dfdc6da79f414b60441` / `7612cd6e850057c6a7b5eb33154cbe71ede25e54cdb1a3c6a8ebc5714b125f07`. This certifies the narrow Inventory/Compendium/Settings repair for that exact signed source. The local suppression-instrument/tests/docs successor changes source identity without altering numeric Compendium rulers, samples or the 55-bullet draft, so it requires a fresh chain. Browser authority remains Edge family + CDP `1.3`; `.107` is provenance only, and compatible point updates never trigger rebaseline. HUMAN Compendium judgment, Recovery and whole-Gate closure remain open. |
| `npm run arc4recovery:selftest` / `node tools/arc4recovery.mjs --slice-run=<Slice-run-id> --glass-run=<Glass-run-id>` / `node tools/arc4recovery.mjs --verify-run=<recovery-run-id> --slice-run=<Slice-run-id> --glass-run=<Glass-run-id>` | The mutation-sensitive selftest seals a dedicated real-time, one-attempt, no-forged-clock collector for the exact active-play next-cycle recovery outcome. Recovery and Slice share one exact disabled-suppression receipt. Recovery quiesces and joins the complete in-flight heartbeat, captures synchronized raw → state → UI exhausted evidence, reveals and settles the exact Tame owner, revalidates it immediately before the bound CDP dispatch, retains one trusted pointer and zero clicks, restores scroll/listeners/globals, resumes heartbeat and reads post-cleanup state before product attribution. Exact heartbeat receipts plus stable lease-read, revision-read and lease-heartbeat counters reject interval/checkpoint overlap. Recovery's preparation/collector/verdict seal is `22e8704122103323d0dd0079ce0d2821d69f249a860f31e4062f51b9f8e68771`; Slice's preparation/collector digest is `baa284a736d9243df4a61de192e553111c9c5fbc9d6aa70ffd05af5b6e31e45f`. The terminal verifier replays the fixture witness/receipt/snapshots and rejects null, malformed, cleanup-red or forged evidence. The collector is non-standalone: it verifies both exact terminal-green predecessors, their nested Glass → Slice binding and the same clean committed source. No uninterrupted real 20-minute candidate certificate exists yet. |
...
| **Arc 4 focused + browser evidence (current boundary 2026-08-28)** | Persistence, Training restore, acquisition planner/capacity/action, app bootstrap/wiring and coordinator tests cover strict 18-namespace bootstrap, exact legacy source evidence, full-roster/current-epoch two-draw settlement, every pre-draw miss/hit capacity scenario, one receipt/CAS, hit/miss spend, first-only/repeat ownership, full committed verification, targeted publication and synchronous overlap barriers. Final9 Slice passed its native-control ledger with zero findings and Glass passed every Arc 4 outcome across 12 viewports. Recovery passed fixture and the complete 16-attempt burn-down, then its old exhausted-control oracle stopped at 87,589 ms before close/checkpoint, offline/reopen or the 20-minute observation. Thus recovery remains unproved and `recoveryClaimed:false`; the shared suppression-tool successor is browser-uncertified and needs a fresh signed full chain. HUMAN review, hosted, preview/publication, release and whole-Gate authority remain open. |
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical Pixi inventory, heap/DOM/resource bounds, target answerability, independent heartbeat, zero pending work, bfcache survival and cleanup. Signed `a85e0ed…` passed Final9 run `20260828-phase4-final9-a85e0edf9b7c-scenemem` 42/42 with zero findings and complete cleanup after its green Layout predecessor; later Compendium, Slice and Glass stages also passed. The local suppression-tool successor changes evidence source identity but not the SceneMemory product, ruler or samples, so a newly signed successor must restart from Layout. Exact Edge provenance is mandatory but not baseline identity; a compatible point update never triggers a rebaseline. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-28)** | Signed `a85e0ed…` is the latest exact 42/42 report, Final9 run `20260828-phase4-final9-a85e0edf9b7c-scenemem`; its raw/gzip SHA-256 are `b9734acd5c6614e1c7b6f908aaf1e6a4547efef5f0d9d0451359bd96dbf8a3a4` / `a195d31731899136d699343b736fe6940d2729a50d101caa268d08b3cf063e23`. Signed `862a75b…` remains the paired 40/42 heap/DOM red; signed `6c9ad855…` supplied three clean calibration candidates; signed activation `4a54c0d…` set 12 MiB V8 /18 MiB aggregate with every other ceiling and paired discrimination intact. Final9 changes no ceiling or sample and passed the full responsive chain through Glass. The local suppression-tool successor still requires a new full signed chain; compatible Edge updates remain provenance only and never trigger rebaseline, recalibration or threshold movement. |
