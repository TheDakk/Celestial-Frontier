...
> **Current local candidate — signed clean `041d1cfdff28c4217d699bdb26eacd5f792f7a80`
> reached Final4 Glass before the chain stopped.** Layout passed 787/787, SceneMemory 42/42,
> Compendium 78/78 and Slice passed with ten screenshots and zero findings; all four exact reports
> passed named verification. Glass then ran once across all 12 viewports, retained 46 findings and
> five instrument failures, and stopped without an automatic retry. Recovery correctly did not
> start. The independent diagnosis found stale harness assumptions around a valid four-character
> **Mars** title, generic-audit scroll restoration, an off-card orbital control that neither centred
> the already-rendered row nor proved exact restoration, and the live contact effects for
> `earpiece`, `diplobeacon` and `rl-mind`. It also confirmed two product defects:
> laptop-720p Sound content overlapped the sticky Close target, and canonical Exotic text reached
> only 2.86:1 over bright glass. Bounded repairs for all diagnosed roots are now applied locally but are
> uncertified; they require a new signed clean source and a complete fresh Final5 restart at Layout.
> Edge `151.0.4129.107` / CDP `1.3` is run provenance only, never a rebaseline or threshold-change
> trigger. No recovery, whole-Gate, hosted, HUMAN or release authority exists. Earlier Slice and
> SceneMemory reds remain diagnostic history. Gate B now recursively seals the exact
...
> whole-diff, and documentation/evidence reviews are CLEAR. The bounded Glass/product repairs are
> applied locally but remain uncertified. A new signed clean source and full fresh Final5 restart at
> Layout are required; current Glass, recovery and whole-Gate evidence remain open.
>
...
> preview/publication and deployment authority were unclaimed. The Final4 successor and pending
> current-source Final5 state are recorded in the current status above.
>
...
> The current repair atomically resets/stages all direct primary, backup and absent-primary fixtures,
...
| **Current local candidate verification (2026-08-27)** | Signed clean `041d1cf…` supplied Final4 Layout 787/787, SceneMemory 42/42, Compendium 78/78 and Slice PASS with ten screenshots and zero findings; every named verifier passed. Glass then ran once across all 12 viewports, stopped `instrument-fail` with 46 findings/five instrument failures, was not retried, and prevented recovery. The exact carriers and hashes are preserved in `audits/README.md`. The bounded product/harness repair is browser-free green at 135 Vitest files / 1,479 passed / one intentional skip, all TypeScript/no-unused programs, the 887-module build, authority printer, Glass/Slice/recovery selftests and root validation/fingerprint. Current SceneMemory budget SHA-256 is `a23a3d80…8355`; current Compendium budget is `858c2503…8f6` with producer `bf9ad0b6…4dcb`; no numeric ruler or historical sample changed. Independent diagnosis, patch and doc reviews found no remaining actionable blocker. The worktree must become one signed clean checkpoint and restart Final5 at Layout with fresh IDs. Edge `.107` / CDP `1.3` is provenance only. No current-source browser PASS, recovery, hosted/HUMAN whole-Gate or release authority exists. |
| **Compendium authority (latest signed exact certificate is historical for the current worktree)** | Signed `041d1cf…` passed Final4 run `20260827-phase4-final4-041d1cfdff28-compendium` 78/78 with complete lifecycle and named verification in one attempt/zero retries (46,414 ms). Its report raw/gzip SHA-256 are `e7c26d37cc653da74900ca03700b175c3020e8d07a293542a7e9d9dfa9849585` / `b8ce7e6bcbecaff7ab6f150afc32856d68422b1d90ae250c65782e917552d1ca`. The later Glass/product repair changes app producer bytes, so that certificate remains truthful history rather than current authority. Browser-free derivation now binds live budget `858c2503…8f6` to producer `bf9ad0b6…4dcb` while preserving measurement authority, fixed ruler, calibration samples and all numeric ceilings. A fresh Final5 predecessor chain must certify it. Browser authority remains Microsoft Edge family + CDP `1.3` + capability contract; exact `.107` is provenance only. HUMAN/Glass/recovery/whole-Gate closure remains open. |
...
| `npm run compendiummem:selftest` / `npm run compendiummem` / `node tools/compendiummem.mjs --verify-run=<run-id>` | Negative-controls the browser-free Compendium instrument, performs one standalone browser run without automatic retry, and independently verifies the named report. The current browser authority is version-tolerant Edge family + CDP `1.3` + a hashed, source-inventoried CDP capability contract. Exact version/revision/JavaScript/path/UA stay in every run; same-run phone/desktop provenance must match exactly, while independent compatible Edge updates never rebaseline or change thresholds. Chrome, malformed/incomplete Edge, protocol/capability mismatch and any real metric breach fail closed. Exact `aecf386…` adds an immutable monotonic 2,000 ms static-server close boundary: just-before succeeds; exact/late/missing/error force one connection cleanup and reject; cleanup red suppresses PASS/sample. Clean `6d8f184…` selected candidate3/5/6 plus baseline1, each one attempt and zero retries. Signed activation `d33e540…` installed strict ceilings above every selected candidate maximum while preserving the four baseline faults and exact 14-phone/13-desktop breach inventory; run `20260826-phase4-certification` passed 78/78 and named verification with raw/gzip SHA-256 `3afe4103…` / `5677d9ed…`. That is historical evidence for producer `587d3bdf…`. Signed calibration source `8ffd2e2…` selected `20260826-slice-repair-candidate1/2/3` plus paired `20260826-slice-repair-baseline1`, each one attempt/zero retries; historical budget `6284a394…` binds former producer `f7c87f22…`, retains the four faults and exact 14/13 inventory, and changes only phone warm to `524288`. Historical clean signed activation `91f4e044…` passed `20260826-slice-repair-certification` 78/78 with complete lifecycle and named verification in one attempt/zero retries (`2026-08-26T23:42:19.150Z`–`23:43:03.997Z`, 44,847 ms); raw/gzip SHA-256 are `81c27ed5…` / `6f3deb0f…`. Product-owner bytes, not Edge `.107`, required that activation. The eventual signed Compendium producer requires a fresh bound certificate; the version-tolerant authority change alters no numeric ceiling. Older c27/baseline11/c28/c29 and cross-host evidence remain historical for their own authorities. The gate still owns the 1,500-row fixture, authority graph, six review PNGs, and instrument/product distinction; automation never supplies HUMAN judgment. |
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. One compatible Microsoft Edge-family process satisfying CDP `1.3` and the sealed capability plus phone/desktop-profile contracts owns the complete Compendium/Shipyard travel cycle. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical Pixi inventory, heap/DOM/resource bounds, target answerability, independent heartbeat, zero pending work, bfcache survival and cleanup. Historical certificates remain bound to their own producers. Signed `041d1cf…` passed Final4 run `20260827-phase4-final4-041d1cfdff28-scenemem` 42/42 plus named verification before later green Compendium/Slice stages and the Glass instrument stop. The current app repair moves standard-build authority to budget `a23a3d80…8355` without changing any numeric ceiling or calibration sample, so Final4 is historical and fresh Final5 certification is required. Exact Edge version/revision/JavaScript/path/UA remains mandatory provenance, not baseline identity. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-27)** | Signed `041d1cf…` is the latest exact 42/42 certificate, from Final4 run `20260827-phase4-final4-041d1cfdff28-scenemem`; its raw/gzip SHA-256 are `1350dc76914dbd6b9cb26411896a7e92d2b04e482439b7af2aa3d35650d0f987` / `544ebab6c01b59c59c45b44c5e3ead7fbc473262a0573d7f8a485486ce758a85`. The current presentation/Glass repair changes source/dist producer bytes, so that certificate is historical for this worktree. Signed `862a75b…` remains the paired 40/42 heap/DOM red; signed `6c9ad855…` supplied three clean calibration candidates; signed activation `4a54c0d…` set 12 MiB V8 / 18 MiB aggregate with all other ceilings and paired discrimination intact. Current source-derived budget `a23a3d80…8355` matches the standard build and changes no numeric ruler. A compatible Edge update remains provenance only and never triggers rebaseline, recalibration or threshold movement. Fresh Final5 is required for current authority. |
