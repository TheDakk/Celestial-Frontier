...
> **Current local candidate — signed `862a75b…` remains the paired red, signed `6c9ad855…` owns
> calibration, signed `4a54c0d…` owns heap-only activation, signed `b206cf0…` owns the final2
> greens/eighth Slice red, and signed `5ddddbf…` owns the second harness repair plus the preserved
> final3 stop.** Final3 Layout passed 787/787 with named verification. SceneMemory then stopped
> `instrument-fail` before measurement because the live budget retained stale `buildDist` and
> `gameMain` hashes; Compendium's measurement authority still matched, but a browser-free audit
> found its built index/owner producer stale too. Compendium, Slice, Glass and recovery did not run.
> The current bounded repair rebinds only those live producer records, keeps all rulers/samples and
> numeric ceilings unchanged, and adds a source-derived standard-build test with source, dist,
> duplicate-constant and recomputed-index/owner mutants. The complete repair battery is green at
> 135 Vitest files /1,478 passed /one intentional skip, all TypeScript/no-unused programs, the
> 887-module build, fail-closed authority printer and root validation/fingerprint. Three independent
> final binding/test, whole-diff, and documentation/evidence reviews are CLEAR. The signed clean
> commit containing this record is the final4 repair checkpoint; verify its signature and clean tree
> before restarting at Layout with new IDs. Earlier Slice and
...
> The current repair sets SceneMemory budget SHA-256 `47d24080…775b` and Compendium budget
...
> record owns the fresh final4 source. Current-input Slice remains RED; Glass,
> recovery and whole-Gate evidence remain open.
>
> Current-input Slice now retains **eight** one-attempt/zero-retry reds on Edge
...
> changes no product or ruler bytes. No current-input Slice PASS, Glass result or real recovery
> certificate exists; hosted, HUMAN, whole-Gate, release/version, preview/publication and deployment
...
| **Current local candidate verification (2026-08-27)** | Signed heap-only activation `4a54c0d…` preserves the selected 12 MiB V8 /18 MiB aggregate ceilings and every historical calibration sample. Signed `5ddddbf…` owns the fully verified second harness repair. Final3 Layout `20260827-phase4-final3-layout` passed 787/787 and named verification; SceneMemory `20260827-phase4-final3-scenemem` then stopped `instrument-fail` before measurement because only live `buildDist` and `gameMain` bindings were stale. The no-retry stop preserved complete cleanup and prevented Compendium/Slice/Glass/recovery. The current repair rebinds SceneMemory budget `47d24080…775b` and Compendium budget `f0bedb67…3c64`, with all numeric rulers unchanged, and adds the source-derived pre-browser regression above. The complete 135-file /1,478-pass /one-skip battery, all TypeScript/no-unused programs, 887-module build, authority printer and root validator pass; three independent final binding/test, whole-diff, and documentation/evidence reviews are CLEAR. The signed clean commit containing this record is the fresh final4 source from Layout. Edge point version is provenance only. No current SceneMemory product verdict, Compendium/Slice PASS, Glass result, recovery certificate, hosted/HUMAN whole-Gate or release authority exists. |
| **Compendium authority (latest signed exact certificate is historical for the current worktree)** | Signed `b206cf0…` passed `20260827-phase4-final2-compendium` 78/78 with zero findings, complete lifecycle and named verification in one attempt/zero retries (45,728 ms). Raw/gzip SHA-256 are `fe70ee276ec97e9954d5299b4c00682014caa52296b9c443a255f2bc6bf67dbc` / `0ab5536b76a08c85c882f1bc33f819da14dd9a2b356fbda5ec19be6287c48616`; its budget `91b91b53…a012` and producer `4bdd3e36…2cb4` remain historical. Browser-free current derivation now binds live budget `f0bedb67…3c64` to producer `4b5aa3a3…` while preserving measurement authority, fixed ruler, calibration samples and all numeric ceilings. That rebind is not a certificate: Compendium did not start after final3's SceneMemory instrument stop and must run inside a fresh predecessor chain after the repair is signed. Browser authority remains Microsoft Edge family + CDP `1.3` + capability-contract SHA-256 `6eed33ed9784f7c7774c4b1bf8d4e880986e31667324d9a1aa7b8dd62fe5a476`; exact Edge `.107` is run provenance only. HUMAN/Slice/Glass/recovery/whole-Gate closure remains open. |
...
| `npx vitest run tests/scenemem-contract.test.ts tests/scenemem-budget.test.ts tests/scenemem-tool.test.ts tests/scenemem-workflow.test.ts` / `node tools/scenemem.mjs --budget=budgets/scene-memory-v2.json` / `node tools/scenemem.mjs --verify-run=<run-id> --budget=budgets/scene-memory-v2.json` | Negative-controls the Arc 1 scene-memory contract, tracked v2 budget, workflow binding and terminal verifier; then performs one no-retry raw-CDP certification and independently verifies the exact named report. One compatible Microsoft Edge-family process satisfying CDP `1.3` and the sealed capability plus exact phone/desktop-profile contracts owns four unmeasured warmups and exactly four measured phone/desktop cycles through Universe, Galaxy, Galaxy fine, Sol/System, Earth/Surface, the 1,500-row Compendium and real Shipyard before settled Universe. Exact version/revision/JavaScript/path/UA remain mandatory per-run provenance, not baseline identity. Forty-two outcomes bind exact scene ownership/work, transient peaks, canonical per-hash Pixi inventory, heap/DOM/resource bounds, target answerability, an independent browser heartbeat, zero pending work, same-document bfcache survival, and per-profile Shipyard lifecycle through its visible opener and owned Close. `59530da…` / budget `3b71d14c…` and `7d8dc380…` / budget `5c8a6e75…` remain historical authorities. Hosted run `32618995487` remains exact terminal-red 40/42 and was not retried. Signed `6c9ad855…` completed calibration; signed `4a54c0d…` owns green activation; signed `7362a0e…` and `b206cf0…` supplied historical 42/42 certificates. Signed `5ddddbf…` passed final3 Layout, then SceneMemory stopped pre-measurement because only live `buildDist`/`gameMain` binding was stale. Current budget `47d24080…775b` is now standard-build-derived and fully browser-free green; three independent final binding/test, whole-diff, and documentation/evidence reviews are CLEAR. The signed clean commit containing this record owns the fresh final4 SceneMemory stage; calibration remains closed. Version tolerance itself changed no numeric budget. Automation supplies neither HUMAN nor merge authority. |
| **Current SceneMemory evidence and calibration boundary (2026-08-27)** | **Latest signed exact certificate `b206cf0…` remains `[HISTORICAL]`; final3 produced instrument evidence, not a product verdict.** Signed `862a75b…` remains the paired 40/42 heap/DOM red. Signed `6c9ad855…` supplied exactly three clean calibration candidates; signed activation `4a54c0d…` sets 12 MiB V8 and 18 MiB aggregate under historical budget `e6c4aeea…`, with all other ceilings unchanged and paired discrimination intact. Signed `5ddddbf…` passed final3 Layout, then `20260827-phase4-final3-scenemem` stopped once after 1,662 ms before measurement because tracked `buildDist` `46e47365…`/`gameMain` `7ff00481…` lagged observed `6575498b…`/`02b85f74…`; cleanup and source binding were complete. Repaired current budget is `47d24080…775b`, derived by ordinary `npm test` rather than duplicate constants. A compatible Edge update is provenance only and never triggers rebaseline, recalibration or threshold movement. The complete browser-free battery, authority printer and root validator are green; three independent final binding/test, whole-diff, and documentation/evidence reviews are CLEAR. The signed clean commit containing this record owns fresh final4 certification. |
