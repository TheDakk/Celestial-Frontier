...
> calibration, signed `4a54c0d…` owns heap-only activation, and signed `b206cf0…` owns the final2
> Layout/SceneMemory/Compendium greens plus the preserved eighth Slice harness red.** The current
> second repair changes the browser-only fixture hook (and therefore built producer bytes), Slice/
> Arc 4 evidence controls, Guide/release truth copy, audit carriers and docs. Numeric rulers and
> deadlines are unchanged. Its complete current-byte checkpoint passes 134 Vitest files / 1,475
...
> controls; three independent current-byte reviews are CLEAR. Signing still precedes a complete restart at
> Layout. Earlier Slice and SceneMemory reds remain diagnostic history. Gate B now recursively seals the exact
...
> carriers are archived in `audits/`; the current second repair changes producer/source bytes, so
...
> `artunused`, syntax/import selftests, the 887-module Vite build and root validation. The current
> second repair passes the complete current-byte checkpoint at 134 files / 1,475 tests plus one
...
> independent current-byte reviews are CLEAR. Because its diagnostic hook and player copy change
> built source/producer bytes, it must receive a new signed clean checkpoint and fresh serial IDs.
...
| Tool | Role |
|---|---|
| **Current local candidate verification (2026-08-27)** | Signed heap-only activation `4a54c0d…` preserves the selected 12 MiB V8 /18 MiB aggregate ceilings under budget SHA-256 `e6c4aeea…`; 676/71 and 673/70 nodes/listeners plus every other ceiling remain unchanged, and both paired replays still fail 704/80. Latest signed browser boundary `b206cf0…` retained final2 Layout 787/787, source-bound SceneMemory 42/42 and Compendium 78/78 with named verification, then Slice stopped terminal-red after 424,225 ms with six findings/five scopes and no retry; Glass/recovery did not run. The second repair changes the browser-only staging hook and built producer, evidence controls, Guide/release copy and docs without changing numeric rulers/deadlines. Its complete current-byte checkpoint passes 134 Vitest files / 1,475 tests plus one intentional skip, all TypeScript/no-unused programs, the 887-module build, root validation/fingerprint, evidence and preview selftests, and isolated real-browser Slice outcome controls; three independent current-byte reviews are CLEAR. A signed clean checkpoint remains before the whole chain restarts at Layout. Edge point version is provenance only. No current Slice PASS, Glass result, recovery certificate, hosted/HUMAN whole-Gate or release authority exists. |
| **Compendium authority (latest signed exact certificate is historical for the second worktree)** | Signed `b206cf0…` passed `20260827-phase4-final2-compendium` 78/78 with zero findings, complete lifecycle and named verification in one attempt/zero retries (45,728 ms). Raw/gzip SHA-256 are `fe70ee276ec97e9954d5299b4c00682014caa52296b9c443a255f2bc6bf67dbc` / `0ab5536b76a08c85c882f1bc33f819da14dd9a2b356fbda5ec19be6287c48616`; budget SHA-256 is `91b91b53…a012` and producer SHA-256 is `4bdd3e36…2cb4`. Historical calibration/activation evidence remains truthful for its own producers. Browser authority v2 remains Microsoft Edge family + CDP `1.3` + sealed capability-contract SHA-256 `6eed33ed9784f7c7774c4b1bf8d4e880986e31667324d9a1aa7b8dd62fe5a476`; exact Edge `.107` is same-run provenance only. The second repair changes producer bytes and must repeat Compendium inside the full predecessor chain; numeric ceilings are unchanged and HUMAN/Slice/Glass/recovery/whole-Gate closure remains open. |
