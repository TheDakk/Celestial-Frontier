...
> then requires the full prepared save before targeted publication. Arc 5 has only a receipt-
...
> it owns and, when the Arc 4 target is absent, derives all 18 ownership writes from the same final
> candidate. Their 19 product writes share the one replacement transaction; postcommit Arc 4
> verification remigrates the committed canonical state and binds exact source evidence. Current-
> view/source-deferred restore preserves the carriers, while existing/future/corrupt authority
> protects against rewind.
>
...
> **Arc 3 current-source browser boundary (`c4a02be`, 2026-08-25):** no-retry Slice Smoke run
