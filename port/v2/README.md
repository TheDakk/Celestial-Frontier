...
> scheduler repair present; shared-browser-identity recalibration pending;
...
> ceilings are null, and the measurement-required paired baseline has a null collector commit and
> empty profiles. Materialize exact `.86`
> afresh for baseline10 and candidate24/25/26 from one clean committed source, once each with zero
> retries; activate, run one exact-head battery, push PR #32 once, and require one corresponding
> CI. No timeout, launch argument, workflow, product byte, producer, or retry policy changed.
>
...
candidate24/25/26 once each with zero retries, then activation, one exact-head battery, one PR #32
push, and one corresponding CI. Do not expand timing policy or optimize accepted CI allowances;
first red stops, and terminal-green PR #32 returns immediately to Arc 1B/gameplay.
...
diagnostics cannot cross corrected contract `e7dfea1d…`. Measurement `2318f57b…`, producer
`d3223177…`, and browser CDP `6da9e2ef…` bind fail-closed ruler `ac2c084a…`, which awaits fresh-
per-launch exact-.86 baseline10 plus candidate24/25/26, once each with zero retries, before one
exact-head battery and corresponding CI. No timing, retry, fallback, workflow, product, launch-
argument, or producer drift occurred. First red stops; terminal-green PR #32 returns immediately
to Arc 1B.
...
| `npm run compendiummem:selftest` / `npm run compendiummem` / `node tools/compendiummem.mjs --verify-run=<run-id>` | Negative-controls the browser-free Compendium instrument, performs one standalone browser run without automatic retry, and independently verifies the named report. Exact-89 remains an owned-shutdown red; candidate20 remains wrong-browser instrument evidence. Candidate21/22/23 plus paired baseline9 are individually clean exact-.86 diagnostics but cannot activate because the old shared identity compared fresh executable paths and host UAs. Raw path/UA remain mandatory per-run provenance; product/revision/JavaScript/protocol are exact shared authority. Measurement `2318f57b…` leaves fail-closed budget `ac2c084a…` with empty candidate samples and null ceilings; its measurement-required baseline has a null collector commit and empty profiles. Producer `d3223177…` and browser CDP `6da9e2ef…` are unchanged. The gate still drives the exact 1,500-row fixture across phone/desktop, binds raw observations and all measurement/product/browser inputs, verifies the owner→worker→painter graph, retains six same-run review PNGs, and distinguishes pre-measurement instrument failure from product evidence. Materialize exact `.86` afresh for each baseline10/candidate24/25/26 launch and run each once with zero retries. Automated terminal success remains exact-run evidence and never supplies the open [HUMAN] judgment. |
