...
export function projectCreatureLevelProgressV1(xpValue: unknown): CreatureLevelProgressV1 {
  if (typeof xpValue !== 'number' || !Number.isFinite(xpValue) || xpValue < 0
    || xpValue > CREATURE_XP_MAX_V1) {
    throw new RangeError(`creature XP must be a finite number from 0 through ${CREATURE_XP_MAX_V1}`);
  }
  const xp = xpValue as number;
  const level = levelOf({ xp });
  const maximumLevel = levelOf({ xp: CREATURE_XP_MAX_V1 });
  /* Search integer thresholds independently of the exact instance XP. */
  let levelFloorXp = 0;
  while (levelFloorXp < CREATURE_XP_MAX_V1 && levelOf({ xp: levelFloorXp }) < level) {
    levelFloorXp++;
  }
  let nextLevelXp: number | null = null;
  for (let candidate = levelFloorXp + 1; candidate <= CREATURE_XP_MAX_V1; candidate++) {
    if (levelOf({ xp: candidate }) > level) {
      nextLevelXp = candidate;
      break;
    }
  }
  /* The legacy floating-point curve can round up immediately below a
     threshold. Keep its level and exact XP; displayed span progress starts at 0. */
  const levelProgressXp = nextLevelXp === null ? 0 : Math.max(0, xp - levelFloorXp);
