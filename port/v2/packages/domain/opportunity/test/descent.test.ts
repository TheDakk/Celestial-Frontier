...
describe('@cf/domain-opportunity — deterministic descent authority', () => {
  ...
  it('keeps failure in orbit, applies Hull then struts, and never reduces HP below one', () => {
    const venus = world(132);
    const braced = capabilities([['stabil', 1]], { module: 'stabil' });
    const approach = policy(venus, { capabilities: braced, reinforcedHull: true });
    ...
    expect(outcome).toMatchObject({
      ...
      hullAdjustedDamage: outcome.rawDamage - 1,
      gearAdjustedDamage: Math.max(0, outcome.rawDamage - 1 - 3),
  ...
  it('rejects address/capability/state/policy clones and malformed draws before an outcome', () => {
    ...
    const input = {
      ...
      stormActive: false, trainingActive: false, alreadyLanded: false, reinforcedHull: false,
    };
