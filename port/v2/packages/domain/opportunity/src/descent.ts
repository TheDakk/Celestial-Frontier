...
export interface DescentPolicyInputV1 {
  ...
  /** Exact `hull1` research membership; the v1 descent also removes one
   * point from a nonzero scrape before worn struts. */
...
function capturedInput(value: DescentPolicyInputV1): DescentPolicyInputV1 | null {
  try {
    ...
    const fields = [
      ...
      'trainingActive', 'alreadyLanded', 'reinforcedHull',
    ] as const;
    ...
    if (!isCanonicalCF1Address(captured.address) || !('planet' in captured.address)
      ...
      || typeof captured.alreadyLanded !== 'boolean'
      || typeof captured.reinforcedHull !== 'boolean') return null;
...
export function projectDescentApproachV1(
  ...
  const policy: DescentApproachPolicyV1 = Object.freeze({
    ...
    reinforcedHull: input.reinforcedHull,
...
export function resolveDescentAttemptV1(
  ...
  const hullAdjustedDamage = policy.reinforcedHull && rawDamage > 1
    ? rawDamage - 1 : rawDamage;
  const gearAdjustedDamage = Math.max(0,
    hullAdjustedDamage - policy.waveOffDamageReduction,
  );
  ...
  return Object.freeze({
    ...
    rawDamage, hullAdjustedDamage, gearAdjustedDamage, damage,
    waveOffCountBefore: policy.waveOffCount,
