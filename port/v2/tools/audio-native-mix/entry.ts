...
import { createAudioRuntime, AUDIO_NEUTRAL_VOICE_MIX_INTENT_V1, type AudioContextLike,
  type AudioCounterpartReceipt, type AudioVoiceRequest, type AudioVoiceGraph } from '../../packages/audio/src/runtime.js';
    ...
    for (const category of ['music', 'ambience'] as const) {
      const request: AudioVoiceRequest = { key: `calibration-${category}`, category, priority: 1, cooldownGroup: `calibration-${category}`, cooldownMs: 0,
        ...
        create: (_context, reservation) => {
          ...
          // Native onended receives an Event; the runtime installs a zero-argument
          // callback that ignores that extra argument. Bridge only this audit-native
          // graph boundary; actual node validation and runtime types stay unchanged.
          return { source, sources: [source], output: source, nodes: [source], reservation } as unknown as AudioVoiceGraph;
        } };
