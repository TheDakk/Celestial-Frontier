...
export function exportAudioBytes(source,profileName,{ffmpeg='ffmpeg',ffprobe='ffprobe'}={}){
 ...
 need(duration<profile.maximumSeconds||(profileName==='bed'&&duration===40),'source duration exceeds profile');
 ...
 const options=['-nostdin','-hide_banner','-v','error','-fflags','+bitexact','-f','wav','-i','pipe:0',
  ...
  '-application','audio','-b:a',String(profile.bitrate),'-vbr','on','-compression_level','10','-threads','1',
  '-flags:a','+bitexact','-fflags','+bitexact','-serial_offset','0','-f','opus','pipe:1'];
 const opus=command(ffmpeg,options,source).stdout;need(opus.length>0&&opus.length<profile.maximumBytes,'Opus byte budget exceeded');
 const probe=JSON.parse(command(ffprobe,['-v','error','-f','ogg','-i','pipe:0','-show_streams','-of','json'],opus).stdout.toString());
 ...
 return {opus,receipt:{schema:'cf.audio-export/v1',recipe,recipeHash:sha256(Buffer.from(JSON.stringify(recipe))),source:facts,sourceMeasurement,encodedMeasurement,
  encoded:{sha256:sha256(opus),bytes:opus.length,frames:facts.frames,channels:profile.channels,codec:'opus'},
