...
export interface FamilyContract extends SkeletonDefinition {
 readonly id:string;readonly version:number;readonly clipSetId:string;readonly joints:readonly string[];readonly anatomyModel?:'myriapod-rigid-trunk-v1';
 readonly legs:readonly string[];readonly hiddenChains?:readonly string[];readonly hiddenJoints?:readonly string[];
 readonly contactStance?:{readonly default:'all'|'hind'|'none';readonly swingLift?:'toward-socket';readonly travelSubsteps?:Readonly<Partial<Record<'hit'|'dodge'|'tame',2>>>;readonly hind?:readonly string[];readonly gaits?:Readonly<Record<string,'alternating'|'bounding'>>;readonly travel?:Readonly<Record<string,'source-steps'>>;readonly actions:Readonly<Record<string,'all'|'hind'|'none'>>};
...
export function familyContractForRecord(record:{readonly template:{readonly id:string};readonly anatomy?:import('./anatomy-inventory.mjs').AnatomyPresence;readonly geometry?:{readonly fixedAttachments?:Readonly<Record<string,readonly [number,number]>>}}):FamilyContract;
