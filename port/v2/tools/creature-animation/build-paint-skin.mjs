...
export async function buildPaintSkin(baseDirectory,seamBinding,record,options={}){
 ...
 // can meet an upper leg although neither joint parents the other. Retain
 // every nonzero-alpha ownership edge for the continuous skin compiler.
 const ownershipEdges=[];
 for(let y=0;y<h;y++)for(let x=0;x<w;x++){const i=y*w+x;if(!owner[i])continue;for(const q of[x+1<w?i+1:-1,y+1<h?i+w:-1]){if(q<0||!owner[q]||owner[i]===owner[q])continue;ownershipEdges.push({ownerParts:[parts[owner[i]-1].id,parts[owner[q]-1].id],edge:q===i+1?[[x+1,y],[x+1,y+1]]:[[x,y+1],[x+1,y+1]]});}}
 ...
 const result=new Map(parts.map(p=>[p.id,{id:p.id,vertices:[],indices:[],fieldTriangles:[]} ]));let triangles=0;
 ...
 for(const b of leaves){const edge=[...horizontal.get(b.y).filter(x=>x>=b.x&&x<b.x+b.w).map(x=>[x,b.y]),...vertical.get(b.x+b.w).filter(y=>y>=b.y&&y<b.y+b.h).map(y=>[b.x+b.w,y]),...horizontal.get(b.y+b.h).filter(x=>x>b.x&&x<=b.x+b.w).reverse().map(x=>[x,b.y+b.h]),...vertical.get(b.x).filter(y=>y>b.y&&y<=b.y+b.h).reverse().map(y=>[b.x,y])],center=vertex(b.x+b.w/2,b.y+b.h/2),ids=new Set();
  ...
  for(let i=0;i<edge.length;i++){const tri=[center,vertex(...edge[i]),vertex(...edge[(i+1)%edge.length])],v=tri.map(i=>vertices[i]);triangles++;fieldTriangles.push(...tri);
   for(const id of ids){const p=parts[id-1],box=p.cutout,out=result.get(p.id);let poly=v.map(p=>[p.x,p.y]);for(const [axis,bound,above]of [[0,box.x,true],[0,box.x+box.width,false],[1,box.y,true],[1,box.y+box.height,false]])poly=clip(poly,axis,bound,above);if(poly.length<3)continue;
    ...
    for(let k=1;k+1<poly.length;k++){
     const a=poly[0],b=poly[k],c=poly[k+1],area=(b[0]-a[0])*(c[1]-a[1])-(b[1]-a[1])*(c[0]-a[0]);
     // Rectangle clipping can repeat a corner or add a collinear vertex. Such
     // zero-area source triangles contain no surface and have no pose Jacobian.
     if(Math.abs(area)>1e-8){out.indices.push(local[0],local[k],local[k+1]);out.fieldTriangles.push(...tri);}
    }
   }
  }
 }
