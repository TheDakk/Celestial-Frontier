...
export function createWasmOrientationForward(c,runtime=globalThis.WebAssembly){
 ...
 try{
  ...
  fixedD.set(d);fixedSigns.set(signs);fixedFloors.set(floors);fixedMovable.set(movable);
  ...
  const run=(position,iterations)=>{
   ...
   p.set(position);const result=leaf(pOffset,dOffset,signOffset,floorOffset,movableOffset,count,iterations),passes=result&127;
