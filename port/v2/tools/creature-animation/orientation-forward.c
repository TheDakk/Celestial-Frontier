...
static __attribute__((always_inline)) inline void mark_incident(unsigned dof,
    const unsigned *starts, const unsigned *incident, unsigned char *dirty) {
  const unsigned v = dof / 2;
  for (unsigned i = starts[v]; i < starts[v + 1]; ++i) dirty[incident[i]] = 1;
}
...
unsigned cf_orientation_forward(double *p, const unsigned *dofs,
    ...
    const unsigned *starts, const unsigned *incident, unsigned char *dirty) {
  unsigned passes = 0;
  for (unsigned t = 0; t < count; ++t) dirty[t] = 1;
  for (unsigned pass = 0; pass < iterations; ++pass) {
    unsigned changed = 0;
    int progressed = 0;
    for (unsigned triangle = 0; triangle < count; ++triangle) {
      /* Skip only a previously satisfied constraint whose three vertices
       * are bit-identical since its last ordered evaluation. Violated,
       * degenerate and unordered constraints retain every original visit. */
      if (!dirty[triangle]) continue;
      ...
      if (constraint >= 0) { dirty[triangle] = 0; continue; }
      ++changed;
      ...
      if (mask & 1) {
        ...
        if (!same_number(x, p[a]) || !same_number(y, p[a + 1])) { progressed = 1; mark_incident(a, starts, incident, dirty); }
      }
      if (mask & 2) {
        const double x = p[b], y = p[b + 1];
        p[b] += scale * bx; p[b + 1] += scale * by;
        if (!same_number(x, p[b]) || !same_number(y, p[b + 1])) { progressed = 1; mark_incident(b, starts, incident, dirty); }
      }
      if (mask & 4) {
        const double x = p[c], y = p[c + 1];
        p[c] += scale * cx; p[c + 1] += scale * cy;
        if (!same_number(x, p[c]) || !same_number(y, p[c + 1])) { progressed = 1; mark_incident(c, starts, incident, dirty); }
      }
    }
    passes = pass + 1;
