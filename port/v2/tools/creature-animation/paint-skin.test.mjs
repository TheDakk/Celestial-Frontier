...
import {createArapScratch}from'./arap-skin.mjs';
...
test('thin nonzero painted triangles cannot bypass orientation checks',()=>{
 const s=data();s.vertices[0].x=0;s.vertices[0].y=0;s.vertices[1].x=40;s.vertices[1].y=0;s.vertices[2].x=0;s.vertices[2].y=1e-8;
 const p=Float64Array.from([0,0,1,0,0,1e-8/40]);
 assert.doesNotThrow(()=>assertPaintPartShape(s.parts[0],s,p,40,40));
 p[5]*=-1;assert.throws(()=>assertPaintPartShape(s.parts[0],s,p,40,40),/folded triangle/);
 s.vertices[2].y=0;assert.throws(()=>assertPaintPartShape(s.parts[0],s,p,40,40),/degenerate source/);
});

test('asset solver profile cannot override orientation defaults or add inherited tuning fields',()=>{
 ...
 for(const [key,value]of [['orientationIterations',1],['orientationIterations',24],['minimumAreaRatio',.01],['minimumAreaRatio',.12]]){
  ...
  assert.equal(createArapScratch(changed.vertices,changed.triangles,20,20,changed.solver)[key],value);
  assert.throws(()=>validatePaintSkin(changed,parts,20,20,['head','jaw']),/shared solver profile keys/);
 }
 const inherited=structuredClone(s);inherited.solver=Object.assign(Object.create({minimumAreaRatio:.01}),s.solver);
 assert.throws(()=>validatePaintSkin(inherited,parts,20,20,['head','jaw']),/shared solver profile keys/);
 for(const value of [null,false,0,'',[]]){const changed=structuredClone(s);changed.solver=value;assert.throws(()=>validatePaintSkin(changed,parts,20,20,['head','jaw']),/shared solver profile keys/);}
 for(const mutate of [p=>p.iterations=3,p=>p.globalIterations=8,p=>p.targetWeight=1,p=>p.pins=[0,0]]){
  const changed=structuredClone(s);mutate(changed.solver);assert.throws(()=>validatePaintSkin(changed,parts,20,20,['head','jaw']),/shared solver profile/);
 }
});
