...
export function partAncestry(parts,remainderPart){
 const declared=new Map(parts.map(p=>[p.joint,p.id])),remainder=parts.find(p=>p.id===remainderPart);
 if(!remainder)throw Error('Missing explicit remainder part');
 const owners=new Map();
 const ownerOf=j=>{if(owners.has(j))return owners.get(j);const id=declared.get(j)??(['root','pelvis'].includes(j)?remainder.id:parent.has(j)?ownerOf(parent.get(j)):null);owners.set(j,id);return id;};
 for(const j of ['root',...GRAPH.map(([j])=>j)])ownerOf(j);
 return {owners:Object.fromEntries(owners),chain(ancestor,descendant){const chain=[];
  for(let j=descendant.joint;j;j=parent.get(j)){
   if(j!==descendant.joint&&(declared.has(j)||j==='root'||j==='pelvis')&&ownerOf(j)===ancestor.id)return chain;
   chain.push(j);
   if(!declared.has(j)&&j!=='root'&&j!=='pelvis'&&ownerOf(j)===ancestor.id)return chain;
  }return null;
 }};
}
export function bandDepth(width,distance,chain,limits,descendantMinDimension=Infinity){let degrees=0;for(const j of chain){const l=limits[j];if(!l)throw Error('Missing joint limit: '+j);degrees+=Math.max(Math.abs(l.min),Math.abs(l.max));}return Math.min(descendantMinDimension*.5,width/8,Math.max(width*.02,distance*Math.sin(Math.min(Math.PI/2,degrees*Math.PI/180))*1.1));}
...
export function buildBandPixels({width:w,height:h,owner,rgba,parts,record,limits,remainderPart}){
 const ancestry=partAncestry(parts,remainderPart),cuts=new Map(),groups=new Map(),other=new Set();
 ...
 for(let y=0;y<h;y++)for(let x=0;x<w;x++){
  ...
  for(const q of [x+1<w?i+1:-1,x>0?i-1:-1,y+1<h?i+w:-1,y>0?i-w:-1]){
   ...
   const chain=ancestry.chain(parts[a],parts[d]);if(!chain){if(!ancestry.chain(parts[d],parts[a]))other.add([parts[a].id,parts[d].id].sort().join('|'));continue;}
   ...
   const pv=cut.pivot,depth=bandDepth(w,Math.hypot(x+.5-pv[0]*w,y+.5-pv[1]*h),chain,limits,Math.min(parts[d].cutout.width,parts[d].cutout.height));cut.seeds++;cut.minDepthPx=Math.min(cut.minDepthPx,depth);cut.maxDepthPx=Math.max(cut.maxDepthPx,depth);
 ...
 return {bands,rawCuts:[...cuts.values()],jointOwners:ancestry.owners,cuts:[...cuts.values()].map(({pixels,...c})=>({...c,pixels:pixels.size})),nonAncestorAdjacencies:[...other].sort()};
}
export async function buildBandAtlas({id,baseDirectory,recordFile,cardFile,output,remainderPart}){
 ...
 fs.mkdirSync(output,{recursive:true});fs.mkdirSync(path.join(output,'pair-bands'));
 for(let k=0;k<built.rawCuts.length;k++){const c=built.rawCuts[k],ids=[...c.pixels];let x0=w,y0=h,x1=-1,y1=-1;for(const i of ids){x0=Math.min(x0,i%w);x1=Math.max(x1,i%w);y0=Math.min(y0,Math.floor(i/w));y1=Math.max(y1,Math.floor(i/w));}if(!ids.length)throw Error('Empty pair band');const box={x:x0,y:y0,width:x1-x0+1,height:y1-y0+1},data=new Uint8Array(box.width*box.height*4);for(const i of ids)data.set(rgba.subarray(i*4,i*4+4),((Math.floor(i/w)-y0)*box.width+i%w-x0)*4);const bytes=await sharp(Buffer.from(data),{raw:{width:box.width,height:box.height,channels:4}}).png().toBuffer(),file='pair-bands/'+c.ancestor+'--'+c.descendant+'.png';fs.writeFileSync(path.join(output,file),bytes);Object.assign(built.cuts[k],{pairBand:{file,cutout:box,sha256:await hashBytes(bytes)}});}
 fs.writeFileSync(path.join(output,'declaration.json'),JSON.stringify({kind:'band',recordRecipeHash:record.recipeHash,cardHash:await hashJSON(card),floorFraction:.02,capFraction:.125,descendantMinDimensionCapFraction:.5,remainderPart,jointOwners:built.jointOwners,margin:1.1,cumulativeAngleCapDegrees:90,ink:'opaque descendant pixels only',cuts:built.cuts,nonAncestorAdjacencies:built.nonAncestorAdjacencies},null,2)+'\n');
