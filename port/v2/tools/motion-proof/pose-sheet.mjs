 ...
 * Usage: node tools/motion-proof/pose-sheet.mjs <record.json> <outDir> [--genome=<genome.json>] [--name=<id>]
 * Deterministic: the same inputs write byte-identical SVG/JSON. */
...
const ACTIONS = ['idle', 'melee:bite', 'hit'];
...
const S = 200, CELL_W = 230, CELL_H = 280, PAD = 16, HEAD = 110, COLOR = { body: '#1d3557', head: '#457b9d', legs: '#2a9d8f', tail: '#e76f51', ears: '#f4a261' };
const esc = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;');
const fmt = (n) => (Math.round(n * 1000) / 1000).toString();
function figure(card, pose, x0, y0, label, sub) {
  ...
  return [`<g><rect x="${x0 - 6}" y="${y0 - 6}" width="${S + 12}" height="${S + 12}" fill="#fafafa" stroke="#ddd"/>`,
    ...
    `<text x="${x0}" y="${y0 + S + 36}" font-family="monospace" font-size="10" fill="#666">${esc(sub)}</text></g>`].join('');
}
...
const info = [
  ...
  `stick figure: joints as circles, bones as lines, dashed ground line; root offsets in body lengths (${fmt(card.bodyLength)}) · legend: body ${COLOR.body} head ${COLOR.head} legs ${COLOR.legs} tail ${COLOR.tail} ears ${COLOR.ears}`,
  '+ rotation = clockwise (front dips) for a right-facing body · A1 mechanical evidence, not visual acceptance',
];
...
rows.forEach(({ tl, times }, r) => {
  ...
  times.forEach((ms, c) => {
    ...
    out.push(figure(card, pose, x0, y0, `t=${fmt(ms / tl.bodyMs)} · ${fmt(ms)} ms`, `dx ${fmt(pose.root.dx)} dy ${fmt(pose.root.dy)} jaw ${fmt(pose.joints.jaw * 180 / Math.PI)}° head ${fmt(pose.joints.head * 180 / Math.PI)}° tail0 ${fmt(pose.joints.tail0 * 180 / Math.PI)}°`));
  });
});
out.push('</svg>');
