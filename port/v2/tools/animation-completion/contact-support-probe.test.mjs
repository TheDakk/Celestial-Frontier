...
test('separates exact triangle LBS, weighted-point covariance and published ARAP residual',()=>{
 const skin={vertices:[{x:100,y:100,weights:[['a',1]]},{x:200,y:100,weights:[['b',1]]},{x:100,y:200,weights:[['a',.5],['b',.5]]}]},mark={joint:'a',xy:[.125,.15],vertex:{triangle:[0,1,2],barycentric:[.25,.25,.5]}},matrices={a:[1,0,0,1,0,0],b:[0,1,-1,0,.5,0]},base={skin,mark,matrices,target:[.225,.15],width:1000,height:1000};
 const near=(actual,expected)=>actual.forEach((v,i)=>assert(Math.abs(v-expected[i])<1e-10));
 const exact=sampleSupportResidual({...base,current:[.225,.15]});near(exact.lbsPrediction,[.225,.15]);near(exact.kinematicResidualPx,[0,0]);near(exact.arapResidualPx,[0,0]);near(exact.weightPositionCovariancePx,[-12.5,12.5]);
