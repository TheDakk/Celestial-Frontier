...
export function controlledLibraryLayout(family){
 ...
 const rows={
  quadruped:{legs:4,accuracy:'four separately visible legs and feet; one tail of its real species length',pose:'Quiet neutral standing pose. Exactly FOUR separately visible legs, two fore and two hind; far legs offset horizontally so all four feet are readable, naturally connected and resting at one level. No crossed, raised or merged legs. Preserve real ears, horns if present, nose and eyes.'},
  'biped-bird':{legs:2,accuracy:'two separately readable legs and feet, two natural wings and one complete feather tail; never add limbs to expose hidden ones',pose:'Neutral standing bird in strict side profile. Two legs offset front-to-back with both ankles and all natural toes readable on the same level. Both wings rest naturally folded; show the far wing edge separately only where anatomically visible, never invent an extra wing. Feather tail extends naturally left of the body with its complete tip separated from feet; species with a short tail keep a short but clearly visible tail. No spread display, flying or raised foot.'},
  ...
  serpent:{legs:0,accuracy:'zero legs and zero wings; one head, one uninterrupted body and one natural terminal tail tip',pose:'One continuous serpent in a shallow open S curve lying sideways, head right and tapered tail left. NO overlapping coils, knots, crossings, upright rearing or tucked tail. Clear negative space between bends. The entire real-length body and taper must fit within the frame; preserve natural head and scale anatomy, with no added fins or appendages.'},
  insect:{legs:6,accuracy:'exactly six legs attached to the thorax, two antennae, natural head/thorax/abdomen and only species-correct wings',pose:'Grounded adult insect, side profile. Exactly SIX separate legs, three near and three far, offset front-to-back with every natural tarsal endpoint visible and clear gaps. Two antennae visibly distinct, never mistaken for legs. Preserve actual wing-cover or folded-wing structure, thorax and complete abdomen tip/cerci where the species has them. Insects have an abdomen, NOT an added vertebrate tail. No flight, spread display, larval anatomy or extra legs.'}
 };
