...
const counts=COUNTS[record.template.id];if(!counts)throw Error('No count line for template '+record.template.id);
...
const receipt={schema:'cf.painted-master-packet/v1',status:'PACKET',creatureId:path.basename(fit),earthName,template:record.template,recordRecipeHash:record.recipeHash,painterMaster:{file:record.source,sha256:record.geometry.cutoutAssetHash,width:master.width,height:master.height},pigment,counts,subject,promptSha256:sha(prompt),kitSha256:sha(kit),systemCard:card,head:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),
