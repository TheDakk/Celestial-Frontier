...
if(!arg||process.argv.length!==3)throw Error('Usage: roster-runner.mjs NEW_OUTPUT_DIRECTORY');
...
try{
 if(report.dirtyDiagnostic)throw Error('Roster requires committed producer');
 release=acquireWorkspaceLock('ordered source family roster intake');
 const bundle=await rolldown({input:path.join(import.meta.dirname,'roster-entry.mjs'),platform:'browser',plugins:[{name:'source-hashes',transform(_,id){if(path.isAbsolute(id)&&fs.existsSync(id)&&fs.statSync(id).isFile())sources.set(id,sha(fs.readFileSync(id)));}}]});
 ...
 browser=await openChromiumCdp({label:'Ordered source family roster intake',userDataPrefix:'cf-fauna-census',commandTimeoutMs:60000});report.browser=browser.browser;
