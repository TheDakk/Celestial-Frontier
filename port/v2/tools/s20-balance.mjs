...
// Cohorts are selected by predeclared raw power ratios, never by battle outcomes.
const pools=new Map();
for(const base of Object.values(POLICY.seedBases))pools.set(base,Array.from({length:POLICY.poolSize},(_,i)=>{
 const genome=api.makeGenome(base+i,'fauna',.5);return {genome,power:api.battleStats(genome).total};
}));
const cachedEligible=new Map();
const choose=(base,min,max,index,excluded=[])=>{
 const key=base+':'+min+':'+max;
 if(!cachedEligible.has(key))cachedEligible.set(key,pools.get(base).filter(x=>x.power>=min&&x.power<=max));
 const eligible=cachedEligible.get(key);if(eligible.length<3)throw Error('insufficient power-matched corpus');
 for(let j=0;j<eligible.length;j++){const g=eligible[(index+j)%eligible.length].genome;if(!excluded.includes(g.seed))return g;}
 throw Error('duplicate party corpus');
};
const kinds=['easy','normal','guardian','titan'];
const fixture=(kind,index,base)=>{
 const isWild=kind==='easy'||kind==='normal';
 const defender=isWild?{name:kind,genome:choose(base,...POLICY.wildDefenderPower[kind],index*13)}:
 {name:defenders[kind].defender.name,genome:defenders[kind].defender.battleGenome,phase:true};
 const power=api.battleStats(defender.genome).total;
 const bands=isWild?[POLICY.powerRatios[kind]]:POLICY.powerRatios.partySlots;
 const used=[defender.genome.seed];
 const party=bands.map(([lo,hi],j)=>{const genome=choose(base,power*lo,power*hi,index*7+j*127,used);used.push(genome.seed);return {name:`slot${j}`,genome,stance:'balanced'};});
 return {mode:'auto',party,defender};
};
...
for(const kind of kinds){
 ...
 const candidates=policies(train[0].party.length),selected=choosePolicy(train,candidates,run);
 write(kind+'-corpus.json',{train,evaluate:evals});
