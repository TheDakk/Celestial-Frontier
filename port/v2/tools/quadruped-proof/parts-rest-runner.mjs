...
const mode=process.argv[3]??'civet';if(!['civet','fox','painter','procedural'].includes(mode))throw Error('Mode must be civet, fox, painter or procedural');
...
try{
 ...
 if(mode==='fox')Object.assign(assets,{'record.json':'audits/CIVET_2D_PROOF_20260912/fox.landmarks.json','master.png':'audits/ART_KIT_ENGINE_FIRST_20260912/masters/family-mammal-quadruped.png','keyed.png':'audits/C2_PARTS_ATLAS_20260913/fox/keyed.png','binding.json':'audits/C2_PARTS_ATLAS_20260913/fox/binding.json','atlas.png':'audits/C2_PARTS_ATLAS_20260913/fox/atlas/fox.png'});
 if(mode==='procedural')Object.assign(assets,{'record.json':'audits/C2_PARTS_ATLAS_20260913/native-painter-parts-03/record.json','master.png':'audits/C2_PARTS_ATLAS_20260913/native-painter-parts-03/master.png','keyed.png':'audits/C2_PARTS_ATLAS_20260913/procedural/keyed.png','binding.json':'audits/C2_PARTS_ATLAS_20260913/procedural/binding.json','atlas.png':'audits/C2_PARTS_ATLAS_20260913/procedural/atlas/procedural.png'});
 if(mode==='painter'){for(const name of Object.keys(assets))delete assets[name];assets['genome.json']='audits/CIVET_2D_PROOF_20260912/procedural-genome.json';}
 ...
 if(mode==='painter'&&report.observation.status==='PASS'){
  const payload=await send('Runtime.evaluate',{expression:'window.cfPartsArtifacts',returnByValue:true});
  const artifacts=payload.result?.value;if(!artifacts)throw Error('Missing painter artifacts');
  for(const name of ['master.png','labels.png','record.json','declaration.json']){const value=artifacts[name];if(!value)throw Error('Missing '+name);fs.writeFileSync(path.join(output,name),name.endsWith('.png')?Buffer.from(value,'base64'):JSON.stringify(value,null,2)+'\n',{flag:'wx'});}
 }
