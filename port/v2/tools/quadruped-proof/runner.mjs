...
try{
 ...
 for(const id of ['civet','fox','procedural']){
  const result=await evaluate(`window.cfQuad.capture(${JSON.stringify(id)})`);fs.writeFileSync(path.join(out,id+'-10s.webm'),Buffer.from(result.video,'base64'));delete result.video;report.captures.push(result);persist();
