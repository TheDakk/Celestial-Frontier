...
import {inspectEncodedFrames} from './motion-proof-contract.mjs';
import {inspectCaptureAudio} from './capture-audio.mjs';
import {openChromiumCdp} from '../browsercdp.mjs';import {acquireWorkspaceLock} from '../workspacelock.mjs';import {requireTenSecondMedia,requireSkinCaptureGate,requireSkinCaptureSnapshot} from './capture-contract.mjs';
const root=path.resolve(import.meta.dirname,'../../../..'),output=path.resolve(process.argv[2]),producer=path.resolve(process.argv[3]);
const extras=process.argv.slice(4),mode=extras.find(a=>!a.startsWith('--candidate=')&&a!=='--diagnostic'&&a!=='--cpu-profile')??'parts',diagnostic=extras.includes('--diagnostic'),cpuProfile=extras.includes('--cpu-profile'),candidateArg=extras.find(a=>a.startsWith('--candidate='))?.slice(12);
if(!['parts','--portrait-fallback','--repair-gates','--repaired-parts','--band-gates','--pair-gates','--seam-gates','--seam-parts','--skin-gates','--hinge-gates','--hinge-parts','--skin-parts','--facing-review','--facing-stills'].includes(mode))throw Error('Unknown proof mode');
if((mode==='--facing-review'||mode==='--facing-stills')&&!diagnostic)throw Error('Facing study is diagnostic-only pending shape review');
if(cpuProfile&&(!diagnostic||mode!=='--skin-parts'))throw Error('CPU profiling is diagnostic skin capture only');
if(fs.existsSync(output))throw Error('New output directory required');const sha=b=>createHash('sha256').update(b).digest('hex');
let skinGate=null;
if(mode==='--skin-parts'){
 const gateFile=extras.find(a=>a!==mode&&!a.startsWith('--candidate=')&&a!=='--diagnostic'&&a!=='--cpu-profile');if(!gateFile)throw Error('Skin captures require a prior native gate report');
 const gate=JSON.parse(fs.readFileSync(path.resolve(gateFile)));
 requireSkinCaptureGate(gate,{diagnostic});
 skinGate=gate;
 if(!Array.isArray(gate.sources)||!gate.sources.length)throw Error('Missing gate source binding');for(const r of gate.sources)if(sha(fs.readFileSync(r.path))!==r.sha256)throw Error('Source changed after skin gates: '+r.path);
}
if(mode==='--seam-parts'||mode==='--hinge-parts'){
 const gateFile=extras.find(a=>a!==mode&&!a.startsWith('--candidate=')&&a!=='--diagnostic'&&a!=='--cpu-profile');if(!gateFile)throw Error('Seam captures require a prior native gate report');
 const gate=JSON.parse(fs.readFileSync(path.resolve(gateFile)));
 if(gate.mode!==(mode==='--hinge-parts'?'--hinge-gates':'--seam-gates')||!(gate.status==='PASS'||diagnostic&&gate.status==='DIAGNOSTIC_PASS')||gate.seamGates?.status!=='PASS'
...
const dirty=execFileSync('git',['status','--porcelain','--untracked-files=all'],{cwd:root,encoding:'utf8'}).trim().split('\n').filter(s=>s&&s!=='?? .DS_Store');if(dirty.length&&!diagnostic)throw Error('Commit before native motion proof (or explicitly use --diagnostic; diagnostic evidence cannot qualify a committed capture)');
fs.mkdirSync(output,{recursive:true});const scratch=fs.mkdtempSync(path.join(os.tmpdir(),'cf-parts-motion-')),sources=new Map(),report={status:'RUNNING',mode,source:execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim(),diagnostic,cpuProfile,dirtyAtStart:dirty,candidate:candidateArg??null,errors:[],captures:[]};let browser,server,release,candidateManifestPath=null;
...
try{
 ...
 if(mode==='--skin-gates'||mode==='--skin-parts'){
  const candidate='audits/C2_DEFORMING_SEAMS_20260914/candidate-04/';
  candidateManifestPath=path.join(root,candidate+'manifest.json');remember(candidateManifestPath);
  const manifest=JSON.parse(fs.readFileSync(candidateManifestPath));
  for(const row of manifest.results){assets[row.id+'.record.json']=row.record;assets[row.id+'.binding.json']=candidate+row.id+'.binding.json';assets[row.id+'.atlas.png']=row.base+'/atlas/'+row.id+'.png';}
 }
 if(candidateArg){
  const candidateFile=path.resolve(candidateArg),manifest=JSON.parse(fs.readFileSync(candidateFile)),directory=path.dirname(candidateFile);
  candidateManifestPath=candidateFile;
  if(JSON.stringify(manifest.results?.map(r=>r.id))!==JSON.stringify(['civet','fox','procedural']))throw Error('Candidate must contain the three ordered proof creatures');remember(candidateFile);
  for(const row of manifest.results){const id=row.id;assets[id+'.record.json']=row.record;assets[id+'.binding.json']=row.binding??path.join(directory,id+'.binding.json');assets[id+'.atlas.png']=row.atlas??row.base+'/atlas/'+id+'.png';if(row.master)assets[id+'.master.png']=row.master;if(row.keyed)assets[id+'.keyed.png']=row.keyed;}
 }
 if((mode==='--facing-review'||mode==='--facing-stills'))Object.assign(assets,{
  'platypus.record.json':'audits/BATTLE_FACING_20260916/platypus-02/record.json','platypus.binding.json':'audits/BATTLE_FACING_20260916/platypus-02/binding.json','platypus.atlas.png':'audits/BATTLE_FACING_20260916/platypus-02/parts/atlas/platypus.png','platypus.keyed.png':'audits/BATTLE_FACING_20260916/platypus-02/parts/keyed.png',
  'old-head-gap.png':'audits/BATTLE_NECK_REPAIR_20260916/motion-01/body-restored-idle.png','head-views.json':'audits/BATTLE_THROAT_JOIN_20260916/head-04/views.json','head-source.png':'audits/CIVET_ANIMATION_PROOF_20260912/civet-turnaround-01.png','head-profile.png':'audits/BATTLE_THROAT_JOIN_20260916/head-04/profile.png','head-front.png':'audits/BATTLE_THROAT_JOIN_20260916/head-04/front.png'});
 for(const[n,p]of Object.entries(assets)){const absolute=path.resolve(root,p);remember(absolute);fs.copyFileSync(absolute,path.join(scratch,n));}remember(import.meta.filename);remember(path.join(import.meta.dirname,'capture-contract.mjs'));remember(path.join(import.meta.dirname,'capture-audio.mjs'));
 ...
 if(mode==='--skin-gates'||mode==='--skin-parts'){
  report.captureSnapshot={schema:'cf.motion-capture-source/v1',candidateManifest:sources.get(candidateManifestPath),servedFiles:[...files].map(([name,data])=>({name,sha256:sha(data),sourcePath:Object.hasOwn(assets,name)?path.resolve(root,assets[name]):null})).sort((a,b)=>a.name<b.name?-1:a.name>b.name?1:0)};
  if(skinGate)requireSkinCaptureSnapshot(skinGate,{sources:[...sources.values()],captureSnapshot:report.captureSnapshot});
  persist();
 }
 ...
 await send('Page.enable');await send('Runtime.enable');await send('Emulation.setDeviceMetricsOverride',{width:1536,height:740,deviceScaleFactor:1,mobile:false});await send('Page.navigate',{url:'http://127.0.0.1:'+server.address().port+(mode==='--portrait-fallback'?'/?mode=portrait-fallback':mode==='--repair-gates'?'/?mode=repair-gates':mode==='--band-gates'?'/?mode=band-gates':mode==='--pair-gates'?'/?mode=pair-gates':(mode==='--seam-gates'||mode==='--hinge-gates')?'/?mode=seam-gates':mode==='--skin-gates'?'/?mode=skin-gates':(mode==='--facing-review'||mode==='--facing-stills')?'/?mode=facing-review':'/')});
 ...
 if((mode==='--facing-review'||mode==='--facing-stills')){report.facingGate=await evaluate('window.cfPartsMotion.facingGate()');const coverage=await evaluate('window.cfPartsMotion.facingCoverage()');for(const[name,data]of Object.entries(coverage.artifacts))fs.writeFileSync(path.join(output,name),Buffer.from(data,'base64'));delete coverage.artifacts;report.facingCoverage=coverage;persist();if(coverage.status!=='PASS')throw Error('Body coverage regression');const joins=await evaluate('window.cfPartsMotion.attachmentCoverage()');for(const[name,data]of Object.entries(joins.artifacts))fs.writeFileSync(path.join(output,name),Buffer.from(data,'base64'));delete joins.artifacts;report.attachmentCoverage=joins;persist();if(joins.status!=='PASS')throw Error('Rendered head/throat attachment gap');const impact=await evaluate('window.cfPartsMotion.impactWitness()');for(const[name,data]of Object.entries(impact.artifacts))fs.writeFileSync(path.join(output,name),Buffer.from(data,'base64'));delete impact.artifacts;report.impactWitness=impact;persist();if(impact.status!=='PASS')throw Error('Impact focus witness failed');}
 if(mode==='--skin-gates'){
  const g=await evaluate('window.cfPartsMotion.skinGates()');for(const[name,data]of Object.entries(g.artifacts))fs.writeFileSync(path.join(output,name),Buffer.from(data,'base64'));delete g.artifacts;report.skinGates=g;persist();if(g.status!=='PASS')throw Error('Native continuous paint gate failed');
 }
 if(mode==='--seam-gates'||mode==='--hinge-gates'){
 ...
 if(mode!=='--repair-gates'&&mode!=='--band-gates'&&mode!=='--pair-gates'&&mode!=='--seam-gates'&&mode!=='--skin-gates'&&mode!=='--hinge-gates')for(const id of ['civet','fox','procedural']){
  ...
  if(mode!=='--facing-stills'){
  if(cpuProfile){await send('Profiler.enable');await send('Profiler.setSamplingInterval',{interval:250});await send('Profiler.start');}
  let result;try{result=await evaluate('window.cfPartsMotion.capture('+JSON.stringify(id)+')');}finally{if(cpuProfile){const {profile}=await send('Profiler.stop');fs.writeFileSync(path.join(output,id+'-cpu.cpuprofile'),JSON.stringify(profile));await send('Profiler.disable');}}
  fs.writeFileSync(path.join(output,id+'-10s.webm'),Buffer.from(result.video,'base64'));delete result.video;
  const media=JSON.parse(execFileSync('ffprobe',['-v','error','-count_frames','-show_entries','format=duration,size:stream=codec_name,codec_type,nb_read_frames,width,height','-of','json',path.join(output,id+'-10s.webm')],{encoding:'utf8'}));result.encodedMedia=media;report.captures.push(result);persist();requireTenSecondMedia(Number(media.format.duration));result.encodedFrames=inspectEncodedFrames(media.streams);result.encodedAudio=inspectCaptureAudio(path.join(output,id+'-10s.webm'));persist();if(result.admissionError)throw Error('Capture admission: '+result.admissionError);
  }
  // Recording cleanup resets the performance players. Re-select before explicit-time stills.
  await evaluate('window.cfPartsMotion.select('+JSON.stringify(id)+')');
 ...
 for(const[p,r]of sources)if(sha(fs.readFileSync(p))!==r.sha256)throw Error('Source changed: '+p);report.exactSourceSnapshot=true;
 report.status=(mode==='--repair-gates'||mode==='--band-gates'||mode==='--pair-gates'||mode==='--seam-gates'||mode==='--skin-gates'||mode==='--hinge-gates')?'PASS':'REVIEW';if(report.errors.length)throw Error('Browser errors');if(diagnostic)report.status='DIAGNOSTIC_'+report.status;
