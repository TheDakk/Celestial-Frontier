...
const mode=process.argv[4]??'parts';if(!['parts','--portrait-fallback','--repair-gates','--repaired-parts','--band-gates','--pair-gates','--seam-gates','--seam-parts','--skin-gates','--hinge-gates','--hinge-parts'].includes(mode))throw Error('Unknown proof mode');
...
if(mode==='--seam-parts'||mode==='--hinge-parts'){
 ...
 if(gate.mode!==(mode==='--hinge-parts'?'--hinge-gates':'--seam-gates')||gate.status!=='PASS'||gate.seamGates?.status!=='PASS'
...
try{
 ...
 if(['--seam-gates','--seam-parts','--hinge-gates','--hinge-parts'].includes(mode)){
  const candidate='audits/C2_DEFORMING_SEAMS_20260914/'+(mode.startsWith('--hinge-')?'candidate-05/':'candidate-02/');
 ...
 if(mode==='--skin-gates'){
  ...
  assets['skin-poses.json']='audits/C2_DEFORMING_SEAMS_20260914/native-gates-03/report.json';
  const manifest=JSON.parse(fs.readFileSync(path.join(root,candidate+'manifest.json')));
 ...
 report.status=(mode==='--repair-gates'||mode==='--band-gates'||mode==='--pair-gates'||mode==='--seam-gates'||mode==='--skin-gates'||mode==='--hinge-gates')?'PASS':'REVIEW';if(report.errors.length)throw Error('Browser errors');
