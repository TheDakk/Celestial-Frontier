...
window.cfPartsRest={status:'RUNNING'};const diagnostics={};
...
try{
 ...
 let changed=0,max=0;for(let i=0;i<a.length;i++)if(a[i]!==b[i]){changed++;max=Math.max(max,Math.abs(a[i]-b[i]));}Object.assign(diagnostics,{normalPainterChangedChannels:changed,maxChannelDifference:max});requireValue(changed===0,'observation changed normal painter pixels');
 const corrupted=a.slice();corrupted[0]^=255;requireValue(corrupted.some((v,i)=>v!==b[i]),'render comparison failed to reject changed pixel');diagnostics.renderMutationRefused=true;
...
}catch(e){window.cfPartsRest={status:'FAIL',diagnostics,error:String(e.stack??e)};}
