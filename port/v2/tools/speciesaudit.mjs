...
for (let i = 0; i < 900; i++) {   /* up to ~7.5 min — the full catalog is >1000 paints */
  await sleep(500);
  const st = await evalIn(`(()=>{ const A=window.__CF_AUDIT__; if(A&&A.done) return { total:A.total, ok:A.ok, fails:A.fails.slice(0,20), nf:A.fails.length, keys:Object.keys(A.sheetUrls) };
...
console.log(`SPECIES AUDIT: ${done.ok}/${done.total} painted · ${done.nf} failures`);
