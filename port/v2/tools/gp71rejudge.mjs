...
async function openCdp(serverUrl) {
  const userData = path.join(os.tmpdir(), `cf-gp71-${process.pid}-${Date.now()}`);
  ...
  for (let attempt = 0; attempt < 50 && !debuggerUrl; attempt++) {
    await sleep(400);
    try { debuggerUrl = (await (await fetch(`http://127.0.0.1:${port}/json/version`)).json()).webSocketDebuggerUrl; }
    catch { /* browser is still booting */ }
  }
  if (!debuggerUrl) { edge.kill(); fail('GP7.1 rejudge: Edge CDP did not start'); }
  ...
  const navigate = async (url) => { await send('Page.navigate', { url }, sessionId); };
  return {
    url: serverUrl,
    navigate,
    evaluate,
    close() { try { ws.close(); } finally { edge.kill(); } },
  };
}

function buildCurrentAudit() {
  assert(fs.existsSync(EDGE), `GP7.1 rejudge: Edge is required at ${EDGE}`);
...
async function drainCurrentAudit(browser, stage) {
  ...
  for (let spins = 0; spins < 36000; spins++) {
    const item = await browser.evaluate(`(()=>{ const full=window.__CF_FULL__; if(!full) return null; if(full.q.length) return full.q.shift(); return full.done ? 'DONE' : 'WAIT'; })()`).catch(() => 'WAIT');
    if (item === 'DONE') { complete = true; break; }
    if (item === 'WAIT' || item === null) { await sleep(100); continue; }
