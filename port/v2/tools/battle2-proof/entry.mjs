...
const state = { status: 'PREPARING', errors: [], labels: {}, plans: [], captures: [] };
...
try {
  ...
  const dot = canvas(8, 8), dg = dot.getContext('2d'); dg.fillStyle = '#fff'; dg.beginPath(); dg.arc(4, 4, 4, 0, Math.PI * 2); dg.fill();
  ...
  const stage = new BattleStage({ factory: { container: () => new Container(), sprite: (t) => new Sprite(t), text: (t) => new Text({ text: t, style, anchor: 0.5 }), graphics: () => new Graphics() }, clock, layout,
    ...
    worldLife: life, effects: { host: createPixiEffectHost({ Sprite, Particle, ParticleContainer }), particleTexture: texture(dot), seed: recipe.seed, phaseTextures: (a) => a.phases.map((p) => phaseTex.get(p.keyedImage)) } });
  app.stage.addChild(stage.root);
  const caption = new Text({ text: '', style: { fontFamily: 'system-ui', fontSize: 15, fill: '#eee2c9' } }); caption.position.set(12, FRAME.height - 44); app.stage.addChild(caption);
  const ctx = { A: { side: 'A', name: 'Civet', mass: civetCard.massClass.multiplier, card: civetCard, theme: 'wild', seed: civetRecord.identity.seed >>> 0 }, B: { side: 'B', name: 'Platypus', mass: 0.85, card: null, theme: 'wild', seed: 424242 },
    arena: { groundLineY: layout.groundLineY, stands: layout.stands }, seed: recipe.seed, anchorsForTheme: () => anchors, readyMs: 900, commandMs: 400 };
  ...
  const startTurn = (i, at) => { turnIndex = i; turnStart = at; tNow = at; plans[i] = stage.play(turns[i].input); state.plans[i] = { startMs: at, beats: plans[i].beats, hitstopMs: plans[i].hitstopMs, attacker: plans[i].attacker.label, delivery: plans[i].delivery }; };
  ...
  const reset = () => { turnIndex = -1; plans = []; state.plans = []; frame(0); };
  const capture = async () => {
    ...
    const result = { durationMs: 10000, frames: updates.length, updateP95Ms: sorted[Math.floor(sorted.length * 0.95)], updateMaxMs: Math.max(...updates), phases, plans: state.plans, mimeType: blob.type, bytes: b.byteLength, sha256: await sha256(b), video: b64(b) };
