...
try {
  ...
  for (const [label, ms] of [['ready', 400], ['command', p0.readyEnd + 120], ['approach', p0.commandEnd + 150], ['strike', strikeMs], ['hitstop', strikeMs + 20], ['flash-fade', strikeMs + 80], ['impact+40', strikeMs + 40], ['impact+170', strikeMs + 170], ['number', strikeMs + 300], ['return', p0.actionEnd + 100], ['turn2-strike', result.plans[1].startMs + result.plans[1].beats.impactAt + 0.5 /* inside the first hitstop frame; the exact sum lands one ulp short after the JSON round-trip */], ['turn2-launch', result.plans[1].startMs + result.plans[1].beats.actionStart + 60], ['turn2-impact+40', result.plans[1].startMs + result.plans[1].beats.impactAt + 40], ['turn2-impact+170', result.plans[1].startMs + result.plans[1].beats.impactAt + 170]]) {
