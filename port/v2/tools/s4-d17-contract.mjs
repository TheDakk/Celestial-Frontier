...
export function planFromForecast(api, plan, samples = 16) {
...
export function assessD17(report, candidate) {
  const failures = [], fail = x => failures.push(x), n = 512;
  const count = x => Number.isSafeInteger(x) && x >= 0 && x <= n;
  if (report.schema !== 'cf-s4-d17-evaluation/v1' || report.count !== n) fail('schema/count');
  if (report.candidateHash !== hash(candidate)) fail('candidate authority');
  if (report.seedBase !== 70000000 || report.hookSeedBase !== 80000000 || report.forecastSeedBase !== 60000000) fail('disjoint seed authority');
  ...
  if (report.replayCases !== 2048 || report.parityCases !== 1024 || report.replayMismatches !== 0 || report.parityMismatches !== 0) fail('replay/phase-off parity');
