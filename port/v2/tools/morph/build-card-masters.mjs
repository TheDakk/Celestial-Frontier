...
import { ART_LIBRARY_PUBLIC, artTierOf } from './art-library-tiers.mjs';
...
/** G3 (Generated Creature Pipeline): a LIBRARY archetype's card files are served on demand from `public/library/cards/<key>/` (outside
 * the precached pack, admitted by the art-library manifest's pins); a CORE archetype's stay bundled `?url` assets under SHIPPED_ROOT. */
export const LIBRARY_CARD_ROOT = 'port/v2/apps/game/public/library/cards/';
export const cardRootOf = (a) => (artTierOf(a.earthName) === 'core' ? SHIPPED_ROOT : LIBRARY_CARD_ROOT);
...
export function generatedSources(archetypes, markingsFilesOf) {
  ...
  const registry = head + "import type { PaintedCardArchetype } from './painted-card-source.js';\nexport const CARD_ARCHETYPES: readonly PaintedCardArchetype[] = Object.freeze([\n"
    + archetypes.map((a) => `  { ${field('earthName', a.earthName)}, ${field('dir', cardRootOf(a) + a.key + '/')} },\n`).join('') + ']);\n';
  const imports = [], rows = [], libraryRows = [];
  archetypes.forEach((a, n) => { const files = [...SHIPPED_FILES, ...markingsFilesOf(a)], entries = [];
    if (artTierOf(a.earthName) === 'library') { // served on demand: the site-relative served path of each file (no bundle import)
      files.forEach((f) => entries.push(`'${f}': '${ART_LIBRARY_PUBLIC}cards/${a.key}/${f}'`)); libraryRows.push(`  ['${cardRootOf(a)}${a.key}/', { ${entries.join(', ')} }],`); return; }
    files.forEach((f, m) => { const id = `a${n}_${m}`; imports.push(`import ${id} from '../assets/painted-cards/${a.key}/${f}?url';`); entries.push(`'${f}': ${id}`); });
    rows.push(`  ['${cardRootOf(a)}${a.key}/', { ${entries.join(', ')} }],`); });
  const assets = head + imports.join('\n') + '\n/** CORE: repo-relative shipped archetype dir → its bundled asset URLs by file (card, record, and the painted masks when it has them). */\n'
    + 'export const CARD_ASSET_URLS: ReadonlyMap<string, Readonly<Record<string, string>>> = new Map([\n' + rows.join('\n') + '\n]);\n'
    + '/** LIBRARY (G3): repo-relative library archetype dir → each file\'s SERVED path under the site base (fetched on demand, admitted by the art-library manifest). */\n'
    + 'export const CARD_LIBRARY_FILES: ReadonlyMap<string, Readonly<Record<string, string>>> = new Map([\n' + libraryRows.join('\n') + '\n]);\n';
  ...
  const arena = head + "/** One entry per painted archetype that can FIGHT: its source paint-skin fit (and its painted masks' folder when they\n * live outside the fit), relative to the arena proof directory the battle2 wiring resolves against. */\n"
    + 'export interface Battle2PartsFit { readonly earthName: string; readonly dir: string; readonly markingsDir?: string; readonly weaponDeclaration?: string; readonly contactSupports?: \'observed\';\n  /** G3: served on demand from the art library (outside the pack); absent = a CORE pack painting. */ readonly library?: true; }\n'
    + 'export const BATTLE2_PARTS_FITS: readonly Battle2PartsFit[] = Object.freeze([\n'
    + archetypes.map((a) => `  Object.freeze({ ${field('earthName', a.earthName)}, ${field('dir', rel(a.dir))}${a.markings ? ', ' + field('markingsDir', rel(a.markings)) : ''}${a.weapons ? ', ' + field('weaponDeclaration', rel(a.weapons)) : ''}${a.contactSupports ? ', ' + field('contactSupports', a.contactSupports) : ''}${artTierOf(a.earthName) === 'library' ? ', library: true' : ''} }),\n`).join('') + ']);\n';
...
if (import.meta.url === new URL(process.argv[1], 'file:').href) {
  ...
  for (const a of CARD_ARCHETYPES) {
    ...
    const ship = path.join(R, cardRootOf(a), a.key); fs.mkdirSync(path.join(ship, 'card'), { recursive: true });
    // a tier move leaves no stale copy in the other root (the bundle must not keep a library card, nor the library a core one)
    for (const other of [SHIPPED_ROOT, LIBRARY_CARD_ROOT]) if (other !== cardRootOf(a)) fs.rmSync(path.join(R, other, a.key), { recursive: true, force: true });
    ...
    console.log(JSON.stringify({ earthName: a.earthName, from: [master.width, master.height], to: [dw, dh], masterSha256: receipt.card.masterSha256.slice(0, 12), masks: Math.max(0, files.length - 1), shipped: cardRootOf(a) + a.key + '/' }));
  }
