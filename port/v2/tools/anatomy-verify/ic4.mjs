...
if(process.argv[1]&&fileURLToPath(import.meta.url)===path.resolve(process.argv[1])){
  const rows=[];const positives={};const run=(id,template,png,declHidden,tag)=>{const opts=process.env.ASSIGN_OPTS?JSON.parse(process.env.ASSIGN_OPTS):{};const res=assignLegs(png.data,png.width,png.height,null,{template,declaredHidden:declHidden,declaredFolded:FOLDED[id]??[],...opts});res.unusedAllowance=Number(process.env.IC4_UNUSED??0);const v=verdict(res,template,declHidden,tag==='positive'?null:positives[id]);if(tag==='positive')positives[id]=res;rows.push({id,tag,template,...v,hidden:res.hidden});const {alpha}=alphaOf(png.data,png.width,png.height);const det=detectTips(alpha,png.width,png.height,{solidAlpha:128});const cls=classifyTips(det,template);const declaredVisible=[...templateRest(template).slots.Far,...templateRest(template).slots.Near].filter(s=>!declHidden.includes(s.id)).length;rows[rows.length-1].tipFeet=cls.counts.feet;rows[rows.length-1].declaredVisible=declaredVisible;const tipCol='tips feet '+cls.counts.feet+'/'+declaredVisible+' pincer '+cls.counts.pincer+' ';const cnt=tipCol+(res.legsBySide?['Far','Near'].map(sd=>{const k=res.legsBySide[sd];const T=templateRest(template);const declared=T.slots[sd].filter(s=>!declHidden.includes(s.id)).length;return sd+' e'+k.filter(x=>x==='end').length+'/l'+k.filter(x=>x==='loop').length+'/t'+k.filter(x=>x==='touch').length+' vs '+declared;}).join(' '):'');const kinds=cnt+' || '+Object.entries(res.assigned).filter(([k])=>/Foot$|Paw$/.test(k)).map(([k,a])=>k.replace(/Foot$|Paw$/,'')+':'+a.kind[0]).join(' ');console.log(id.padEnd(16),tag.padEnd(14),template.padEnd(11),v.verdict.padEnd(7),'unused',v.unusedStrong,'hidden',JSON.stringify(res.hidden),'|',kinds,'|',v.reasons.join('; '));return res;};
