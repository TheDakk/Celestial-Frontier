...
const {assignLegs}=await import('./assign.mjs');const {FOLDED}=await import('./score.mjs');const {alphaOf,detectTips,classifyTips}=await import('./tips.mjs');const {templateRest}=await import('./template-rest.mjs');const {SUBJECTS,truthOf,scoreSubject}=await import('./score.mjs');
export function verdict(res,template,declaredHidden){const T=templateRest(template);const reasons=[];
  ...
  for(const side of ['Far','Near'])for(const sl of T.slots[side]){const a=res.assigned[sl.terminal];const filled=!!a&&(!strict||a.kind!=='loop'||folded.includes(sl.id)),hid=declaredHidden.includes(sl.id);if(hid&&filled)reasons.push(`declared hidden ${sl.id} was found`);if(!hid&&!filled)reasons.push(`visible ${sl.id} not found`);}
...
if(process.argv[1]&&fileURLToPath(import.meta.url)===path.resolve(process.argv[1])){
  const rows=[];const run=(id,template,png,declHidden,tag)=>{const opts=process.env.ASSIGN_OPTS?JSON.parse(process.env.ASSIGN_OPTS):{};const res=assignLegs(png.data,png.width,png.height,null,{template,declaredHidden:declHidden,declaredFolded:FOLDED[id]??[],...opts});res.unusedAllowance=Number(process.env.IC4_UNUSED??0);const v=verdict(res,template,declHidden);rows.push({id,tag,template,...v,hidden:res.hidden});const {alpha}=alphaOf(png.data,png.width,png.height);const det=detectTips(alpha,png.width,png.height,{solidAlpha:128});const cls=classifyTips(det,template);const declaredVisible=[...templateRest(template).slots.Far,...templateRest(template).slots.Near].filter(s=>!declHidden.includes(s.id)).length;rows[rows.length-1].tipFeet=cls.counts.feet;rows[rows.length-1].declaredVisible=declaredVisible;const tipCol='tips feet '+cls.counts.feet+'/'+declaredVisible+' pincer '+cls.counts.pincer+' ';const cnt=tipCol+(res.legsBySide?['Far','Near'].map(sd=>{const k=res.legsBySide[sd];const T=templateRest(template);const declared=T.slots[sd].filter(s=>!declHidden.includes(s.id)).length;return sd+' e'+k.filter(x=>x==='end').length+'/l'+k.filter(x=>x==='loop').length+'/t'+k.filter(x=>x==='touch').length+' vs '+declared;}).join(' '):'');const kinds=cnt+' || '+Object.entries(res.assigned).filter(([k])=>/Foot$|Paw$/.test(k)).map(([k,a])=>k.replace(/Foot$|Paw$/,'')+':'+a.kind[0]).join(' ');console.log(id.padEnd(16),tag.padEnd(14),template.padEnd(11),v.verdict.padEnd(7),'unused',v.unusedStrong,'hidden',JSON.stringify(res.hidden),'|',kinds,'|',v.reasons.join('; '));return res;};
  for(const [id,template,master,rec,presence] of SUBJECTS){const png=readPng(fs.readFileSync(master));const declHidden=rec?(JSON.parse(fs.readFileSync(rec,'utf8')).anatomy?.hidden??[]):(JSON.parse(fs.readFileSync(presence,'utf8')).hidden??[]);
