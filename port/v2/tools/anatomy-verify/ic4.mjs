...
if(process.argv[1]&&fileURLToPath(import.meta.url)===path.resolve(process.argv[1])){
  const rows=[];const positives={};const declFoldedOf={};const run=(id,template,png,declHidden,tag,rec=null)=>{const opts=process.env.ASSIGN_OPTS?JSON.parse(process.env.ASSIGN_OPTS):{};const res=assignLegs(png.data,png.width,png.height,null,{template,declaredHidden:declHidden,declaredFolded:(declFoldedOf[id]??[]),...opts});res.unusedAllowance=Number(process.env.IC4_UNUSED??0);const v=verdict(res,template,declHidden,tag==='positive'?null:positives[id]);if(tag==='positive')positives[id]=res;// §6 pass = verdict ADMIT AND (when a hand record exists) every named foot within IC4_BOUND master px of the record
