...
export function assignLegs(rgba,w,h,guide,{template='brachyuran',bodyFraction=null,termFactor=3,thinSpread=1.8,centreMode='spine',units='body',refine='tip',touchInterior=false,touchProfile=false,touchAgainstBody=true,angleWeight=0,slotLenWeight=0,lenMode='exit',loopMode='near',costMode='rest',thickFinger=true,touchBodyDistMax=1e9,spacingWeight=0,gapWeight=0,wristCut=true,wristCuts=2,interiorEdges=0,edgeMinLen=0.3,edgeBlur=0,interiorTipMax=0.5,thickMaxLen=0.7,thinWeight=0.4,emptyScale=1.0,unusedEnd=1.0,contactRefine=true,thickNeedsFork=false,touchNotSep=true,loopThinFrac=0,declaredHidden=[],declaredFolded=[],declaredAbsent=[],orderBy='sep',rootMode='none',orderFrac=0.5,loopConfirm=false,thickNeedsPalm=false,clawAttached=0,tuftReach=true,kneeFrom='exit',kneeMode='fraction',longest=512,bodyUnitsP2=false,clawEdgeCut=0,gapeMin=0.3,rearGap=0,sideLenWeight=0.25}={}){
  ...
  const strongContours=(thr,blur,minLen)=>{const lum=new Float32Array(W*H);const r=Math.max(1,Math.round(0.5/scale));for(let y=0;y<H;y++)for(let x=0;x<W;x++){const mx=box.x+(x-2)/scale,my=box.y+(y-2)/scale;let sum=0,n=0;for(let yy=Math.round(my-r);yy<=Math.round(my+r);yy++)for(let xx=Math.round(mx-r);xx<=Math.round(mx+r);xx++){if(xx<0||yy<0||xx>=w||yy>=h)continue;const i=(yy*w+xx)*4;if(rgba[i+3]<128)continue;sum+=0.299*rgba[i]+0.587*rgba[i+1]+0.114*rgba[i+2];n++;}lum[y*W+x]=n?sum/n:0;}
    ...
    vals.sort((a,b)=>a-b);const med=vals[Math.floor(vals.length/2)]||1;const strong=new Uint8Array(W*H);for(let i=0;i<W*H;i++)if(mag[i]>=thr*med)strong[i]=1;
    const seen=new Uint8Array(W*H);const cut=new Uint8Array(W*H);for(let i=0;i<W*H;i++){if(!strong[i]||seen[i])continue;const comp=[i];seen[i]=1;const st=[i];let minx=1e9,maxx=-1,miny=1e9,maxy=-1;while(st.length){const c=st.pop();const x=c%W,y=(c-x)/W;if(x<minx)minx=x;if(x>maxx)maxx=x;if(y<miny)miny=y;if(y>maxy)maxy=y;for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){const nx=x+dx,ny=y+dy;if(nx<0||ny<0||nx>=W||ny>=H)continue;const j=ny*W+nx;if(strong[j]&&!seen[j]){seen[j]=1;comp.push(j);st.push(j);}}}if(Math.hypot(maxx-minx,maxy-miny)>=minLen)for(const j of comp)cut[j]=1;}return cut;};
  ...
  const chainsIn=lc.chains.slice();{const seen=new Set();for(const c of lc.chains){const last=c.edges[c.edges.length-1];if(last.length>=minLimbTerm||c.edges.length<2)continue;let node=c.rootNode.id;for(let k=0;k<c.edges.length-1;k++){const e=c.edges[k];node=(e.a===node)?e.b:e.a;}const j=node;if(seen.has(j)||bodyIds.has(j))continue;
      ...
      const edges=c.edges.slice(0,-1);const len=edges.reduce((a,e)=>a+e.length,0);const jn=g.nodes[j];const tuftLen=Math.max(...outs.map(o=>o.length),c.edges[c.edges.length-1].length);const leaves=[...outs,c.edges[c.edges.length-1]].map(o=>g.nodes[o.a===j?o.b:o.a]).filter(n=>n&&n.kind==='end');let far=null,fd=-1;for(const n of leaves){const d=Math.hypot(n.x-jn.x,n.y-jn.y);if(d>fd){fd=d;far=n;}}chainsIn.push({endNode:jn,length:+len.toFixed(1),meanDt:c.meanDt,maxDt:c.maxDt,endDt:jn.dt,edges,rootNode:c.rootNode,tuft:true,tuftLen,tuftTip:far?[far.x,far.y]:null});}}
  ...
  // a lone thick terminal is a finger only if a PALM lies behind it: the chain's thickness outside the body region
  // reaches ≥ 2 × the terminal's own (a short thick leg behind the claw has no palm — the crab's leg3Far)
  const palmMax=c=>{if(!c.chain)return 0;let node=c.chain.rootNode.id,mx=0;for(const e of c.chain.edges){for(const i of e.path)if(bodyDist[i]>0&&dt[i]>mx)mx=dt[i];node=(e.a===node)?e.b:e.a;}return mx;};
  const hasPalm=c=>palmMax(c)>=2*c.termDt;
  // twin stub: at the candidate's last junction, the thickest other SHORT edge (a merged finger leaves a thick stub;
  // a leg's knee spurs are thin) — reported as stubDt / termDt
  const twinStub=c=>{if(!c.chain||c.chain.edges.length<1)return 0;const last=c.chain.edges[c.chain.edges.length-1];const j=(g.nodes[last.a].id===c.chain.endNode.id)?last.b:last.a;let mx=0;for(const e of g.edges){if(e===last||(e.a!==j&&e.b!==j))continue;if(e.length>=minLimbTerm)continue;if(e.meanDt>mx)mx=e.meanDt;}return mx;};
  const isClaw=c=>forkOf.has(c)||(thickFinger&&c.kind==='end'&&c.termDt>=1.6*thinRefT&&c.len<=thickMaxLen*legLen&&(!thickNeedsFork||besideFork(c))&&(!thickNeedsPalm||hasPalm(c)));
  // CLAW-ATTACHED endpoints (`clawAttached` = minimum body distance, in R, of a node shared with a fork member's
  // chain): an endpoint whose chain joins a claw's chain OUTSIDE the body belongs to the claw (an outer finger
  // edge, an arm spur), not to a leg; sharing near the body (a leg beside the claw base) does not count
  const forkNodes=new Map();if(clawAttached>0){for(const f of forkOf.keys()){if(!f.chain)continue;let node=f.chain.rootNode.id;for(const e of f.chain.edges){node=(e.a===node)?e.b:e.a;const n=g.nodes[node];const i=Math.round(n.y)*W+Math.round(n.x);if(i>=0&&i<W*H&&bodyDist[i]>=clawAttached*R)forkNodes.set(node,true);}}}
  const clawAttachedTo=c=>{if(!clawAttached||!c.chain||forkOf.has(c))return false;let node=c.chain.rootNode.id;for(const e of c.chain.edges){node=(e.a===node)?e.b:e.a;if(forkNodes.has(node))return true;}return false;};
  // FORK-AWARE RIDGE (`clawEdgeCut` = contour threshold × median, 0 = off): a thick UNFORKED endpoint may be two
  // fingers whose closed gape lies inside the alpha (one blob, one ridge). Cut the strong interior contours inside a
  // disc of 3 × its terminal thickness around the tip, re-thin THAT disc only, and if the cut ridge shows two thin
  // endpoints there, the candidate is a fork (claw) — never a leg. The body and every other limb are untouched.
  const forkedByCut=new Set();if(clawEdgeCut>0){const thickLone=pool.filter(c=>c.kind==='end'&&!forkOf.has(c)&&c.termDt>=1.6*thinRefT);if(thickLone.length){const cut=strongContours(clawEdgeCut,1,0.1*legLen);
      for(const c of thickLone){const rad=3*c.termDt;const m2=new Uint8Array(W*H);let any=false;for(let y=Math.max(1,Math.round(c.y-rad));y<Math.min(H-1,Math.round(c.y+rad));y++)for(let x=Math.max(1,Math.round(c.x-rad));x<Math.min(W-1,Math.round(c.x+rad));x++){const i=y*W+x;if(!mask[i]||Math.hypot(x-c.x,y-c.y)>rad)continue;if(cut[i])continue;m2[i]=1;any=true;}if(!any)continue;
        const dt2=distanceTransform(m2,W,H);const g2=ridgeGraph(m2,dt2,W,H,{spurFactor:1.5,spurFloor:Math.max(3,Math.round(8*kP2)),junctionRadius:Math.max(2,Math.round(3*kP2))});
        const ends=g2.nodes.filter(n=>n.kind==='end'&&Math.hypot(n.x-c.x,n.y-c.y)<=rad*0.9&&dt2[Math.round(n.y)*W+Math.round(n.x)]<=1.2*c.termDt);
        // two endpoints within a finger gap of each other (≤ forkTipMax) that are not the same point → a fork
        // GAPE TEST: the two tips must have the gape between them — ≥ 30 % of the straight segment's samples on cut
        // contour or outside the mask; spur endpoints on one solid limb have solid paint between them
        let fork=false,gapeOf=0;for(let i=0;i<ends.length&&!fork;i++)for(let j=i+1;j<ends.length;j++){const d=Math.hypot(ends[i].x-ends[j].x,ends[i].y-ends[j].y);if(d<0.5*c.termDt||d>forkTipMax)continue;const n=Math.max(4,Math.ceil(d));let gap=0;for(let k=1;k<n;k++){const x=Math.round(ends[i].x+(ends[j].x-ends[i].x)*k/n),y=Math.round(ends[i].y+(ends[j].y-ends[i].y)*k/n);const q=y*W+x;if(mask[q]&&cut[q])gap++;}gapeOf=Math.max(gapeOf,gap/(n-1));if(gap/(n-1)>=gapeMin){fork=true;break;}} // INTERIOR gape only: background between two spur ends is what every thin leg tip shows
        if(process.env.ASSIGN_DEBUG)console.log('  gape',gapeOf.toFixed(2));
        if(process.env.ASSIGN_DEBUG)console.log('clawEdgeCut',toM([c.x,c.y]).map(Math.round),'ends',ends.length,'fork',fork);if(fork)forkedByCut.add(c);}}}
  const legs=pool.filter(c=>!isClaw(c)&&!clawAttachedTo(c)&&!forkedByCut.has(c)),clawList=pool.filter(c=>isClaw(c)||clawAttachedTo(c)||forkedByCut.has(c));
  // pathOf: the limb's ridge path from its exit from the thick region (default) — or from its separation node / body
  // root (`kneeFrom`) — to its tip; used for joints, labels and ordering
  const pathOf=c=>{if(c.kind==='touch'){const e=c.edge;const path=(e.a===c.fromBody)?e.path:[...e.path].reverse();const pts=[];let out=false;for(const i of path){if(!out&&dt[i]<lc.bodyDt)out=true;if(out)pts.push([i%W,Math.floor(i/W)]);}return pts.length>1?pts:null;}
    ...
    if(c.kind!=='end'&&c.kind!=='interior')return null;let node=c.chain.rootNode.id;const pts=[];let out=kneeFrom==='root';const sepId=kneeFrom==='sep'&&c.attach?null:null;for(const e of c.chain.edges){const path=(e.a===node)?e.path:[...e.path].reverse();if(kneeFrom==='sep'&&!out){const n=g.nodes[node];if(c.attach&&Math.hypot(n.x-c.attach[0],n.y-c.attach[1])<1)out=true;}for(const i of path){if(!out&&kneeFrom==='exit'&&dt[i]<lc.bodyDt)out=true;if(out)pts.push([i%W,Math.floor(i/W)]);}node=(e.a===node)?e.b:e.a;if(kneeFrom==='sep'&&!out){const n=g.nodes[node];if(c.attach&&Math.hypot(n.x-c.attach[0],n.y-c.attach[1])<1)out=true;}}return pts.length>1?pts:null;};
  ...
  const refineTip=c=>{if((c.kind!=='end'&&c.kind!=='interior')||refine==='none')return [c.x,c.y];
    if(tuftReach&&c.chain&&c.chain.endNode.kind==='junction'){ // a chain ending AT a junction (a collapsed tuft): the landmark is the
      // farthest LEAF within 0.6 R of the junction measured from the chain's approach point (fur tips hang off several
      // micro-junctions, so graph hops from this one junction miss them)
      const j0=c.chain.endNode;const prevE=c.chain.edges[c.chain.edges.length-1];const approach=prevE?(prevE.a===j0.id?g.nodes[prevE.b]:g.nodes[prevE.a]):j0;let best=null,bd=-1;
      for(const n of g.nodes){if(n.kind!=='end')continue;if(Math.hypot(n.x-j0.x,n.y-j0.y)>0.6*R)continue;const d=Math.hypot(n.x-approach.x,n.y-approach.y);if(d>bd){bd=d;best=[n.x,n.y];}}
      if(best)return best;}
    ...
    let dx=tip%W-back%W,dy=Math.floor(tip/W)-Math.floor(back/W);const L=Math.hypot(dx,dy)||1;dx/=L;dy/=L;let x=c.x,y=c.y;const reach=(c.chain.tuft&&tuftReach?c.chain.tuftLen:0)+r+1;for(let s=0;s<=reach;s++){const nx=Math.round(c.x+dx*s),ny=Math.round(c.y+dy*s);if(nx<0||ny<0||nx>=W||ny>=H||!mask[ny*W+nx])break;x=nx;y=ny;}return [x,y];};
  ...
  if(T.view==='front'){
    ...
    const rec=(si,i,acc,cost)=>{if(si===stations.length){const used=acc.flat().filter(Boolean).length;const total=cost+(cs.length-used)*0.7;allS.push({cost:total,acc:acc.map(s=>s.slice())});if(!best||total<best.cost)best={cost:total,acc:acc.map(s=>s.slice())};return;}
      ...
      // a REAR appendage (tail) separates from the body a clear gap behind every leg (rearGap × R along the axis); a
      // leg that merely sorts first (a stub-tailed bear's rear-most foot) does not qualify
  ...
  for(const [name,a] of Object.entries(assigned)){const e=slotByTerminal.get(name);if(!e||!a._c)continue;const {sl}=e;const c=a._c;const pts=pathOf(c);const tipM=a.master;
    ...
    joints[chainNames[0]]=pts?toM(pts[0]).map(Math.round):a.attach;
    const nJ=(sl.jointFractions??[]).length;
    if(kneeMode==='bend'&&pts&&pts.length>12&&nJ>0){ // turning angle at each path sample over a window w; the nJ strongest bends, in path order, are the joints
      const w=Math.max(4,Math.round(0.08*total));const turn=pts.map((_,i)=>{if(i<w||i+w>=pts.length)return 0;const a=pts[i-w],b=pts[i],cc=pts[i+w];const v1=[b[0]-a[0],b[1]-a[1]],v2=[cc[0]-b[0],cc[1]-b[1]];const l1=Math.hypot(...v1)||1,l2=Math.hypot(...v2)||1;return Math.acos(Math.max(-1,Math.min(1,(v1[0]*v2[0]+v1[1]*v2[1])/(l1*l2))));});
      const picks=[];const taken=new Set();for(let n=0;n<nJ;n++){let bi=-1,bv=-1;for(let i=w;i<pts.length-w;i++){if(turn[i]>bv&&![...taken].some(t=>Math.abs(t-i)<2*w))bv=turn[i],bi=i;}if(bi<0)break;picks.push(bi);taken.add(bi);}
      picks.sort((a,b)=>a-b);picks.forEach((pi,k)=>{joints[chainNames[k+1]]=toM(pts[pi]).map(Math.round);});for(let k=picks.length;k<nJ;k++)joints[chainNames[k+1]]=at(sl.jointFractions[k]);}
    else (sl.jointFractions??[]).forEach((f,k)=>{joints[chainNames[k+1]]=at(f);});}
  ...
  return {margins,declaredFolded,working:{W,H,scale,box,mask,dt,bodyDt:lc.bodyDt,bodyDist},clawPaths,bodyRidge,legsBySide:T.view==='front'?{Far:legs.filter(c=>sideFront(c)==='Far').map(c=>c.kind),Near:legs.filter(c=>sideFront(c)==='Near').map(c=>c.kind)}:null,joints,inferred,pool:pool.map(c=>({kind:c.kind,tip:toM([c.x,c.y]).map(Math.round),sep:c.attach?toM(c.attach).map(Math.round):null,term:c.term,len:+c.len.toFixed(1),termDt:c.termDt,fork:forkOf.has(c),claw:isClaw(c),used:usedSet.has(c),diag:c.diag,palmMax:+palmMax(c).toFixed(1),stubDt:+twinStub(c).toFixed(1),tuft:!!(c.chain&&c.chain.tuft),tuftTip:c.chain&&c.chain.tuftTip?toM(c.chain.tuftTip).map(Math.round):null,endKind:c.chain?c.chain.endNode.kind:null})),assigned,hidden,claws:{Far:claws.Far.map(c=>toM([c.x,c.y]).map(Math.round)),Near:claws.Near.map(c=>toM([c.x,c.y]).map(Math.round))},centre:toM(centre).map(Math.round),axis:axis.map(v=>+v.toFixed(3)),spine:spine?{a:toM([g.nodes[spine.a].x,g.nodes[spine.a].y]).map(Math.round),b:toM([g.nodes[spine.b].x,g.nodes[spine.b].y]).map(Math.round),len:spine.length,minDt:spine.minDt,meanDt:spine.meanDt}:null,bodyDt:lc.bodyDt,R,feetFound:feet.Far.length+feet.Near.length};
}
