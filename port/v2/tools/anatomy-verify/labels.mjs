...
export function labelParts(res,template,{bodyBand=0,seedMode='ridge'}={}){const T=templateRest(template);const {W,H,mask,bodyDist}=res.working;
  ...
  const legPaths={};for(const side of ['Far','Near'])for(const sl of T.slots[side]){const a=res.assigned[sl.terminal];if(!a||!a.path)continue;legPaths[sl.id]=a.path;seed(sl.id,a.path);}
