...
export function renderSheet(id,template,master,rec,outFile){
  const png=readPng(fs.readFileSync(master));const truth=rec?truthOf(JSON.parse(fs.readFileSync(rec,'utf8'))):null;
  const opts=process.env.ASSIGN_OPTS?JSON.parse(process.env.ASSIGN_OPTS):{};const res=assignLegs(png.data,png.width,png.height,null,{template,...opts});
...
if(process.argv[1]&&fileURLToPath(import.meta.url)===path.resolve(process.argv[1])){const out=process.argv[2]??(root+'audits/INTAKE_COMPILER_20260921/sheet-01');const want=process.argv.slice(3);const rows=[];
  for(const [id,template,master,rec] of SUBJECTS){if(want.length&&!want.includes(id))continue;const r=renderSheet(id,template,master,rec,path.join(out,id+'.png'));rows.push(r);console.log(id.padEnd(16),JSON.stringify(r.score),'hidden',JSON.stringify(r.hidden));}
