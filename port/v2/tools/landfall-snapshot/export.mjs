...
export async function exportCanonicalEarthSnapshot(directory){
  ...
  const receipt={schema:'cf.landfall-snapshot-export/v1',status:'FAIL',sources:[],virtualSources:[],files:[],warnings:[]};
  ...
  try{
      ...
      plugins:[{name:'landfall-source-binding',
        async transform(code,id){
          if(!path.isAbsolute(id)){
            if(id!=='\0rolldown/runtime.js')throw Error('Unknown virtual snapshot source refused: '+id);
            receipt.virtualSources.push({id,bytes:Buffer.byteLength(code),sha256:sha(code)});return null;
          }
          await remember(id);return null;
        },
    ...
    await bundle.write({dir:output,format:'es',entryFileNames:'canonical-producer.mjs',codeSplitting:false});
