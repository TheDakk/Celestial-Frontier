...
function fixture() {
  ...
  return { request: structuredClone(pilotEarthVistaRequest()) as Mutable<ReturnType<typeof pilotEarthVistaRequest>>,
    // A deep clone is deliberately mutable for adversarial fixtures; no canonical data is mutated.
    roster: structuredClone(outcome.roster) as unknown as Mutable<typeof outcome.roster> };
}
