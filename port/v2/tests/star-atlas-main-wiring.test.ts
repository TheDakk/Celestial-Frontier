...
function atlasProjectionErrors(main: string): string[] {
  ...
  if (!ordered(fill, [
    ...
    'restoreFocus();',
  ])) errors.push('atlas-refill-semantic-focus');
  if (!ordered(fill, [
...
function atlasActionErrors(main: string): string[] {
  ...
  if (!homeBeforeCommit.includes('atlasRouteStates.get(targetEntry) !== priorRoute')) {
    errors.push('home-stale-route-before-commit');
  }
  ...
  if (!removeBeforeCommit.includes('priorRows.some(([, entry], index) => atlasRouteStates.get(entry) !== priorRoutes[index])')) {
    errors.push('remove-stale-route-before-commit');
  }
...
describe('mature Atlas refill focus owner', () => {
  it('keeps semantic focus restoration and rejects its removal', () => {
    expect(atlasProjectionErrors(mainSource)).toEqual([]);
    const missing = replaceInSectionExact(mainSource, FILL_START, FILL_END,
      'restoreFocus();', '/* restoration omitted */');
    expect(atlasProjectionErrors(missing)).toContain('atlas-refill-semantic-focus');
  });
});

describe('mature Atlas route authority after heartbeat', () => {
  it('refuses lost route or absence authority before Home and Remove commit', () => {
    expect(wiringErrors(mainSource)).toEqual([]);
    const staleHome = replaceInSectionExact(mainSource, HOME_START,
      'const outcome = await commitArc9AtlasHomeV1({',
      '      || atlasRouteStates.get(targetEntry) !== priorRoute', '');
    const staleHomePublication = replaceInSectionExact(mainSource,
      'publishArc9AtlasHomeFieldsV1(save, outcome);', HOME_END,
      '        || atlasRouteStates.get(targetEntry) !== priorRoute', '');
    const staleRemove = replaceInSectionExact(mainSource, REMOVE_START, REMOVE_END,
      '      || priorRows.some(([, entry], index) => atlasRouteStates.get(entry) !== priorRoutes[index])', '');
    expect(wiringErrors(staleHome)).toContain('home-stale-route-before-commit');
    expect(wiringErrors(staleHome)).not.toContain('home-fixed-point-order');
    expect(wiringErrors(staleHomePublication)).toContain('home-fixed-point-order');
    expect(wiringErrors(staleHomePublication)).not.toContain('home-stale-route-before-commit');
    expect(wiringErrors(staleRemove)).toContain('remove-stale-route-before-commit');
  });
});

function clusterSelectionErrors(main: string): string[] {
  const errors: string[] = [];
  const handler = section(main, HANDLER_START, HANDLER_END);
  const chooser = section(handler,
  ...
  if (!ordered(chooser, [
    ...
    "button.dataset.atlasCluster === priorCluster",
    "'[data-atlas-view=\"chart\"]'",
    "?? panel?.querySelector<HTMLButtonElement>('[data-pnx]'))?.focus();",
  ])) errors.push('cluster-selection-return-focus');
  if (chooser.includes('commit') || chooser.includes('jumpTo') || chooser.includes('atlasRouteStates')
    || chooser.includes('save.') || chooser.includes('persistView')) errors.push('cluster-created-action-owner');
  const fill = section(main, FILL_START, FILL_END);
  if (!fill.includes("'data-atlas-cluster', 'data-atlas-cluster-back',")
    || !fill.includes('clusterId: arc9AtlasClusterId,')) errors.push('cluster-refill-state-focus');
  return errors;
}

describe('Chart candidate selection reuses the Atlas list owner', () => {
  it('opens and returns with semantic focus without a new route or mutation owner', () => {
    expect(clusterSelectionErrors(mainSource)).toEqual([]);
    const lostOrigin = mainSource.replace('button.dataset.atlasCluster === priorCluster',
      'button.dataset.atlasCluster === null');
    expect(clusterSelectionErrors(lostOrigin)).toContain('cluster-selection-return-focus');
    const changedOwner = mainSource.replace('arc9AtlasClusterId = clusterButton.dataset.atlasCluster ?? null;',
      'arc9AtlasClusterId = clusterButton.dataset.atlasCluster ?? null; save.homeId = arc9AtlasClusterId;');
    expect(clusterSelectionErrors(changedOwner)).toContain('cluster-created-action-owner');
  });
});
