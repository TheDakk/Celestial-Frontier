...
function executableDeclaration<T>(name: string, nextDeclaration: string): T {
  ...
  return Function(
    ...
    `return (${expression});`,
  )(hasUnnegatedSentenceClaim, 120) as T;
}
...
describe('sixth Slice red contract repairs', () => {
  ...
  it('binds Guide navigation to publication and the exact current 41-topic capability identity', () => {
    ...
    expect(expected.filter((topic) => topic.availability === 'partial')).toHaveLength(37);
    expect(expected.filter((topic) => topic.availability === 'unavailable')).toHaveLength(4);
  ...
  it('keeps every rendered Guide and release oracle green on current exact copy before browser spend', () => {
    ...
    for (const [label, expression, verdict] of [
      ...
      expect(sample(), `${label} audio baseline: ${JSON.stringify(sample())}`).toMatchObject({
        [verdict]: true, audioContract: true, audioContradiction: false, bulletCount: 120,
      });
      for (const omitted of audioOmissions) {
        ...
        try {
          ...
          expect(sample(), `${label}: missing ${omitted}`).toMatchObject({
            [verdict]: false, audioContract: false, audioContradiction: false, bulletCount: 120,
          });
      ...
      for (const claim of audioOverclaims) {
        try {
          ...
          expect(sample(), `${label}: ${claim}`).toMatchObject({
            [verdict]: false, audioContract: false, audioContradiction: true, bulletCount: 120,
          });
        } finally { audioRow.innerHTML = audioHtml; }
    ...
    for (const [current, stale] of [
      ...
      try {
        ...
        expect(releaseDom.window.eval(releaseCheck)).toMatchObject({ complete: false, populated: true,
          canonical: true, bulletCount: 120, liveProgressionContract: true, liveProgressionContradiction: false });
  ...
  it('keeps a fixed 120-row Guide oracle with five independent population controls', () => {
    expect(sliceSource).toContain('const V2_DRAFT_BULLET_COUNT = 120;');
    const owner = section(
      sliceSource,
    ...
    expect(glassExpectedBulletCount).toBe(120);
    expect(glassMissingBulletCount).toBe(119);
    expect(glassMissingBulletCount).toBe(glassExpectedBulletCount - 1);
    expect(glassSource).toContain('120-outcome development inventory');
