...
function executableDeclaration<T>(name: string, nextDeclaration: string): T {
  ...
  return Function(
    ...
    `return (${expression});`,
  )(hasUnnegatedSentenceClaim, 86) as T;
}
...
describe('sixth Slice red contract repairs', () => {
  ...
  it('keeps every rendered Guide and release oracle green on current exact copy before browser spend', () => {
    ...
    for (const [label, expression, verdict] of [
      ...
      expect(sample(), `${label} audio baseline`).toMatchObject({
        [verdict]: true, audioContract: true, audioContradiction: false, bulletCount: 86,
      });
      for (const omitted of audioOmissions) {
        ...
        try {
          ...
          expect(sample(), `${label}: missing ${omitted}`).toMatchObject({
            [verdict]: false, audioContract: false, audioContradiction: false, bulletCount: 86,
          });
      ...
      for (const claim of audioOverclaims) {
        try {
          ...
          expect(sample(), `${label}: ${claim}`).toMatchObject({
            [verdict]: false, audioContract: false, audioContradiction: true, bulletCount: 86,
          });
        } finally { audioRow.innerHTML = audioHtml; }
    ...
    for (const [current, stale] of [
      ...
      try {
        ...
        expect(releaseDom.window.eval(releaseCheck)).toMatchObject({ complete: false, populated: true,
          canonical: true, bulletCount: 86, liveProgressionContract: true, liveProgressionContradiction: false });
  ...
  it('keeps a fixed 86-row Guide oracle with five independent population controls', () => {
    expect(sliceSource).toContain('const V2_DRAFT_BULLET_COUNT = 86;');
    const owner = section(
      sliceSource,
    ...
    expect(glassExpectedBulletCount).toBe(86);
    expect(glassMissingBulletCount).toBe(85);
    expect(glassMissingBulletCount).toBe(glassExpectedBulletCount - 1);
    expect(glassSource).toContain('86-outcome development inventory');
