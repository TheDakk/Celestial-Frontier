...
function solWorldOfType(types: ReadonlySet<string>): CanonicalCF1WorldAddress {
  for (let seed = 100; seed < 1_000; seed++) {
    const resolved = resolveCF1WorldAddress({
      galaxy: HOME_GALAXY, star: SOL, planet: { seed },
    });
    if (!resolved.ok) continue;
    const address = resolved.address;
    if (types.has(projectWorldOpportunity(address).source.planetType)) return address;
  }
  throw new Error('landing fixture found no requested Sol world type');
}

function weeklyWeekContaining(...ids: string[]): number {
  for (let week = 0; week < 10_000; week++) {
    const slate = new Set(weeklyCharterSlateForWeekV1(week).map(({ id }) => id));
    if (ids.every((id) => slate.has(id as never))) return week;
  }
  throw new Error(`weekly landing fixture found no slate containing ${ids.join(',')}`);
}
...
describe('Arc 0 durable landing action', () => {
  ...
  it('lets one permanent first hostile Landing settle wk-land and wk-hostile in the same receipt', async () => {
    const week = weeklyWeekContaining('wk-land', 'wk-hostile');
    const codecNow = week * WEEKLY_CHARTER_MS_V1 + 1;
    const address = solWorldOfType(new Set(['venus', 'lava', 'gas']));
    const state = baseState();
    state.tutDone = true;
    state.chDone = ['st-land', 'st-mine', 'st-scan', 'st-scout', 'st-conq'];
    state.chWeek = week;
    state.chacc = ['wk-land', 'wk-hostile'];
    state.chProg = { 'wk-land': 2, 'wk-hostile': 0 };
    const fixture = await runtimeFixture({ state, sessionSeed: 0xA0A0_0001 });
    const outcome = await commitArc0LandingAction({
      ...actionInput(fixture, address),
      codecNow,
    });
    ...
    expect(outcome.kind).toBe('committed');
    if (outcome.kind !== 'committed') return;
    expect(outcome.witness.facts.weeklyCharters).toMatchObject({
      changed: true,
      events: [{ kind: 'landfall', worldKey: address.key, first: true }],
      progressIds: ['wk-land', 'wk-hostile'],
      completions: [
        { id: 'wk-land', stardust: 20 },
        { id: 'wk-hostile', stardust: 35 },
      ],
    });
    expect(outcome.transaction.state.chacc).toEqual([]);
    expect(outcome.transaction.state.chProg).toMatchObject({
      'wk-land': 3,
      'wk-hostile': 1,
    });
    expect(outcome.transaction.state.stats.charters).toBe(2);
    expect(outcome.transaction.receipt.kind).toBe(ARC0_LANDING_RECEIPT_KIND);
    expect(fixture.receiptCas()).toBe(1);
    expect(verifyArc0LandingPostcommit({
      transaction: outcome.transaction,
