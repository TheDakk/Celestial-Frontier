...
function fixture(xps: readonly number[] = [24, 54, 216, 486]): Readonly<{
...
describe('exact-instance Compendium creature progression', () => {
  ...
  it('keeps valid fractional-XP twins visible without changing ownership bytes', () => {
    const value = fixture([24.5, 54, 485.5, 486]);
    ...
    expect(value.ids.map((id) => byId.get(id)!).map((row) => ({
      xp: row.xp,
      level: row.level,
      floor: row.levelFloorXp,
      next: row.nextLevelXp,
      remaining: row.nextLevelXp === null ? null : row.nextLevelXp - row.xp,
      slots: row.awakenedInnateSlots,
    }))).toEqual([
      { xp: 24.5, level: 2, floor: 24, next: 54, remaining: 29.5, slots: 1 },
      { xp: 54, level: 3, floor: 54, next: 96, remaining: 42, slots: 2 },
      { xp: 485.5, level: 8, floor: 384, next: 486, remaining: 0.5, slots: 3 },
      { xp: 486, level: 9, floor: 486, next: null, remaining: null, slots: 3 },
    ]);
    expect(result.rows.every((row) => row.className === 'Sorcerer')).toBe(true);
    expect(result.rows.every((row) => row.innateArts.length === row.awakenedInnateSlots)).toBe(true);
    const html = renderCompendiumCreatureProgressionV1(result);
    expect(html.match(/data-creature-progression-id=/gu)).toHaveLength(4);
    expect(html).toContain('0.5 / 30 XP to next · 24.5 total');
    expect(html).toContain('101.5 / 102 XP to next · 485.5 total');
    expect(html).toContain('486 XP · maximum level');
