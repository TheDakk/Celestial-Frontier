...
describe('explicit browser model delivery: synthetic OPFS-shaped controls, no weights downloaded', () => {
  ...
  it('refuses excess bytes and an ignored resumed Range with no partial ready publication', async () => {
    ...
    // An oversized stream may be refused before any prefix is committed. Build
    // the Range control from an independently observed canceled prefix instead.
    const ranged = fixture([bytes(C + 17)]), controller = new AbortController();
    expect((await ranged.make(status => {
      if (status.storedBytes >= C && status.phase === 'downloading') controller.abort();
    }).install({ signal: controller.signal })).phase).toBe('canceled');
    ranged.mode('ignore-range'); expect((await ranged.make().install()).error).toBe('invalid-range-response');
    expect(ranged.calls.at(-1)!.range).toBe(`bytes=${C}-`); expect(readyMarkers(ranged.disk)).toEqual([]);
    ranged.mode('good'); expect((await ranged.make().install()).ready).toBe(true);
  });
