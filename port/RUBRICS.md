...
**Current evidence boundary (2026-08-28):** signed clean source
`53d030bb733beca1a68fd8e42358dfa4b10ed0e2` supplied Final7 Layout 787/787, source-bound
SceneMemory 42/42, source-bound Compendium 78/78 and Slice with zero findings and ten screenshots.
Glass then ran exactly once for 75,032 ms across 12 viewports and stopped terminal-red with
25 findings, zero report-classified instrument failures and zero retries: 11 `CLIPPED_WITHOUT_SCROLL`, seven
`CONTROL_OUTSIDE_VIEWPORT`, four `SETTINGS_CREATURE_VOICE_CONTROL`, two
`COMPENDIUM_HOSTILE_ROW_GEOMETRY` and one `COMPENDIUM_SHORT_LANDSCAPE_WORKSPACE`. Recovery did not
run. Final7 is immutable historical evidence and cannot resume.

The bounded product repair stacks Inventory copy/badges into one min-width-safe column only at
`<=360px`, preserving every field and 44px target. Compendium retains its bounded virtual scroller
and 58px sticky-Close gutter while its list heading stops clearing the already-reserved Close float.
Hostile-row acceptance now requires the exact row to be mounted and fully contained, rejecting an
overscan-only mount before combined geometry acceptance.

The Settings oracle independently reveals and activates Sound and Creature voices. Two fresh
node/geometry samples bind control/panel/bounds/Close rectangles, panel and document scroll,
containment, overlap and centre ownership; the coordinate chain binds preparation, dispatch-time
target, trusted touch/mouse receipt and exact post-state/focus. Phase and outer restoration run in
`finally`. Malformed/unsettled/transport/restoration evidence is instrument failure; stable wrong
geometry/ownership/state/focus is product failure. Stable missing Close is product-red, whereas
incoherent absence or node replacement is instrument-red. Viewport completion requires all four
activations plus restored-off settlement, disjoint from product-blocked coverage.

Browser-free controls pass, but the repaired worktree is not browser-certified and has no
post-repair full Glass PASS. Live producer/budget test records are rebound to the changed
product/release-content bytes without
changing any numeric ruler, calibration sample, retry policy or browser authority. A newly signed
clean successor must restart Layout → SceneMemory → Compendium → Slice → Glass → recovery. No
post-repair chain, publication, hosted attempt, release, deployment, version bump or external
authority exists. Compatible Edge/Chrome point updates, including `.107` at CDP `1.3`, are
...
latest evidence boundary and stopped at final2's preserved Slice harness red; no Glass result, recovery certificate,
hosted, HUMAN, whole-Arc/Gate, preview, version, release or deployment claim exists.
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup for the current producer | **`[HISTORICAL]` for the repaired worktree.** Product virtualization, art ownership and focus/filter/detail/Close outcomes are implemented. Signed `53d030b…` passed Final7 Compendium 78/78 with six review PNG bindings after green Layout/SceneMemory predecessors; raw/gzip SHA-256 are `617151d1…e0cb4d` / `adf44f5f…3716f5`. Final7 Glass later exposed a separate list-layout defect: generic heading clear spent a second 44px Close row and clipped the bounded inner scroller. The narrow repair keeps the 58px gutter/virtualization and removes only that redundant list-mode clear. Hostile-row acceptance additionally requires the exact row mounted **and contained**, so overscan-only mount cannot satisfy geometry. Current collector `c13a489d…3faab`, outcome contract `ac7eea39…6d17`, measurement `28b06f3c…276c`, numeric ceilings and historical samples are unchanged; the product and existing 55-bullet draft-copy repair rebinds live producer from Final7's `bf9ad0b6…4dcb` to `e858722e…b601f` under budget `726881b8…ea74`. No post-repair browser certificate exists. Browser authority remains Edge family + CDP `1.3` + sealed capability contract; `.107` is provenance only. Six-image HUMAN judgment, full Glass, recovery and whole-Gate closure remain outstanding. |
...
| `[EXEC]` | Ship art, installed-system captions and travel reach derive from one normalized state | **`[HISTORICAL]` for the repaired worktree.** Product/ruler `a4de5007ffc9131b8bc952a0a4cb469d9139039e` makes travel, captions, four SVG chassis, exact `array`/`autoext`/`cscoop` hardpoints and diagnostics consume one unsaved projection. Unit/integration/browser controls cover 64 normalized cases, all eight hardpoint permutations, reload-shaped reconstruction, imported-veteran fallback, friendly-alias rejection, deliberately mismatched selectors, one-preview ownership and real opener/Close state parity. Repair `7d8dc380…` passed historical scene-memory-v2 42/42 under budget `5c8a6e75…`; run `32646110946` merged it in PR #33. Signed `041d1cf…` supplied Final4 SceneMemory 42/42 before later green Compendium/Slice stages and the Glass stop. Signed `39e4f20…` contains the Shipyard oracle repair and supplied Final5 SceneMemory 42/42 before Compendium stopped instrument-only. Signed `ea845d7…` supplied Final6 SceneMemory 42/42 before green Compendium and the later Slice instrument stop. Signed `53d030b…` then supplied Final7 SceneMemory 42/42 plus green Compendium/Slice predecessors before Glass stopped terminal-red with 25 findings and zero report-classified instrument failures; post-run review separated product defects from oracle artifacts. The post-Final7 changed app bytes require a fresh signed-successor chain from Layout; no repaired-source full-Glass/recovery claim follows. |
...
| `[EXEC]` | Gear instances migrate, equip, unequip, salvage, pending-claim and persist by exact identity | **`[HISTORICAL]` for the repaired worktree.** Strict codecs/fixed-point tests cover duplicate-base identity, equipped legacy affix ownership, corrupt/future rows, full-inventory pending state and lossless capacity/byte protection. Signed Final7 Slice exercised native reveal, trusted row/action/detail-Close/panel-Close ownership, surviving parent-panel state, exact receipt key/raw/parsed durability, stable F4 RNG authority and Atlas continuity with zero findings. Final7 Glass independently found the exact small-phone A++/Mono rows taller than their scrollport. The narrow `<=360px` repair retains every field/badge/action, stacks copy and badges to one min-width-safe column, allows copy to wrap and preserves the 44px button. No post-repair full Glass exists; a fresh signed-successor chain must re-exercise the product CSS. |
...
| `[EXEC]` | Tame/Scavenge/Sample are the finite acquisition writers for catalogue pages, specimens and owned fauna | **`[RED]` until the bound final campaign passes.** Native Survey controls bind exact nav/address, the production full roster/current epoch and all-scenario capacity before two F4 draws; they spend on hit or miss and commit first-only reward, repeat individual/lot, one receipt/CAS, exact committed verification and targeted publication. Source-bound random-pool odds/budget, presentation fencing, native Close/reopen and historical exact-input outcomes remain. One exact durable fauna Tame result may join an accessible asset-free greeting. The first nine Slice reds remain historical diagnosis. Signed `53d030b…` supplied Final7 Slice PASS after green Layout/SceneMemory/Compendium predecessors; Glass then stopped terminal-red with 25 findings and zero report-classified instrument failures. Post-run review separated the Inventory/Compendium product defects from the Settings and hostile-row oracle artifacts; Recovery did not run. The post-Final7 repair changes source/evidence bytes, so a fresh signed-successor chain from Layout remains required. Post-start audio and real recovery remain unproved. Separate HUMAN journey/listening review, Charter bioscan, targeted preview, hosted and release authority remain open. |
...
| `[EXEC-TODO]` | Accessibility audit | Final4 Glass reached the real 12-viewport audit and correctly retained Exotic `#9A5CFF` at 2.86:1 over effective bright glass as a product red, separately from stale instrument assumptions. Signed `39e4f20…` contains the repair preserving that hue on opaque `#05070d`, restoring exclusive sticky-Close geometry and adding live contrast/hit/scroll controls. Signed Final7 reached all 12 viewports but stopped terminal-red with 25 findings / zero report-classified instrument failures; Recovery did not run. Its four Settings findings came from an oracle that demanded simultaneously hit-testable off-card controls. The repaired contract reveals Sound and Creature voices independently, requires two equal current-node geometry samples, exact control/panel/bounds/Close and panel/document-scroll settlement, 44px contained centre ownership, no Close overlap, dispatch-time target self-coordinate integrity, prepared→target product stability, target→trusted touch/mouse receipt coherence, exact post-state/focus and unconditional restoration. Missing/unsettled/malformed/transport/restoration evidence is instrument failure; stable wrong product outcome is product failure. Stable missing Close is product-red; incoherent absence/node replacement is instrument-red. Completion requires all four toggles plus restored-off settlement. Browser-free controls pass, but a fresh signed-successor full Glass and physical assistive-technology review remain required. The queued universe-wide lighting/bloom pass must preserve these contrast and reduced-motion contracts. |
| `[EXEC-TODO]` | Performance and heat budgets | Historical Arc 1A/1B/1C certificates and the signed heap-only activation remain bound to their exact producers. Signed `041d1cf…` supplied historical Final4 Layout/SceneMemory/Compendium/Slice greens before Glass stopped. Signed `ea845d7…` supplied Final6 Layout/SceneMemory/Compendium before Slice stopped instrument-only. Signed `53d030b…` supplied Final7 Layout 787/787, SceneMemory 42/42, Compendium 78/78 and Slice before Glass stopped terminal-red with 25 findings and zero report-classified instrument failures; post-run review separated product defects from oracle artifacts. Current SceneMemory/Compendium numeric ceilings, rulers and samples are unchanged; only live producer/budget-test bindings follow the post-Final7 changed product/release-content bytes. A fresh signed-successor chain from Layout is required. Both authorities are compatible Edge family + CDP `1.3`; `.107` and later compatible point updates are provenance only. True GPU bytes, the queued bloom/atmosphere/particle costs, and physical-device heat/battery remain open, so this row and Gate I stay open. |
