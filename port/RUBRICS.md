...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact-89 through candidate26/baseline10 remain truthful history. Exact `aecf386…` adds one immutable 2,000 ms static-server close deadline; exact/late/missing/error cleanup force-closes once, rejects, and suppresses PASS/sample. Collector/measurement/budget are `0c7ec3ba…` / `23aacc2c…` / `c711c8a5…`; producer stays `d3223177…`. Current state is calibration-required with empty samples/null ceilings and a measurement-required/null paired baseline; c27/baseline11/c28/c29 remain serial one-attempt exact-.86 work. Run `32420327368` remains no-verdict historical evidence. The repository is public, but Actions remain FROZEN by intent. This row also awaits Arc 1B's scene Canvas/Pixi and combined travel → Compendium → Shipyard plateau. |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, art ownership, focus/filter/detail/Close outcomes, and the owner→worker→painter graph are implemented. Candidate24/25/26+baseline10 and budget `70145575…` are truthful historical authority only. Exact `aecf386…` bounds static-server shutdown and changes collector/measurement; budget `c711c8a5…` is calibration-required with empty candidates/null ceilings and a measurement-required paired baseline. Recollect c27/baseline11/c28/c29 once each under fresh exact Edge `.86` before activation, exact-head battery, or PR CI. Arc-local Edge 151 still does not repin Gate A. |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's old rulers through candidate26/baseline10 remain history. Exact `aecf386…` invalidates the prior numeric authority by changing collector shutdown; current budget `c711c8a5…` is calibration-required pending c27/baseline11/c28/c29. Exact `731b2e2…` and hosted no-verdict run `32420327368` remain historical only. Arc 1B scene/Pixi/GPU plateaus and physical-device heat/battery evidence remain open. |
