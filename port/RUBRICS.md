...
**Current local candidate boundary (2026-08-27):** signed clean source
`ea845d77d9783599c269f708462eb650e2c3e245` began Final6. Layout passed 787/787,
source-bound SceneMemory passed 42/42 and source-bound Compendium passed 78/78. Slice then ran once
for 420,570 ms and stopped with five findings in five scopes. The exact thermal Inventory row was
a valid 164px button whose raw centre lay below the panel scrollport. The old collector skipped the
click and manufactured four absent modal/Equip/Close/reload descendants. This is one instrument
reachability/causal-prefix defect, not five product verdicts. Final6 was not retried; Glass and
recovery did not start.

The browser-free repair performs native reveal and settled scrollport/hit proof, binds exact trusted
row/action/Close receipts, proves detail Close leaves the parent Inventory visible/open/expanded,
and binds exact receipt bytes/semantics plus F4 RNG authority across reload. It gates every dependent
outcome and mutation control on its full green causal prefix. Red action state proves quiescence and cleanup, then terminates before later mutable
arcs. No product or numeric ruler changed. A newly signed clean successor must restart the complete
chain at Layout; Final6 cannot resume. No post-repair Slice, Glass, recovery, publication, hosted
attempt, release, deployment, version bump or external authority exists. Compatible Edge/Chrome
point updates, including `.107` at CDP `1.3`, are provenance only and never trigger a rebaseline or
threshold change.
...
| `[EXEC-TODO]` | Universe-wide visual polish preserves authored creature/plant structure and deterministic identity | Before/after fixed-seed proof sheets across every biome must retain exact seed, species/biome key, anatomy, silhouette, proportions and scale while varying only deterministic palette, light, material, atmosphere and bounded particles. Reduced-motion and mobile-budget variants are required; HUMAN review judges appeal |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup for the current producer | **`[HISTORICAL]` for the repaired worktree.** Product virtualization, art ownership and focus/filter/detail/Close outcomes are implemented. Earlier calibration/certificate sources remain truthful historical proof. Signed `39e4f20…` reached Final5 Compendium after Layout/SceneMemory greens but stopped instrument-only at 0/78 because equal-size key sets were mistaken for identical membership. The exact-membership repair became signed `ea845d7…`, whose Final6 Compendium run passed 78/78 with six review PNG bindings after green Layout and SceneMemory predecessors. Its raw/gzip report SHA-256 are `1416d7ca…41aa` / `8567a36c…85fc`. Current collector `c13a489d…3faab`, outcome contract `ac7eea39…6d17`, measurement `28b06f3c…276c`, budget-file `3e660742…2ae5` and producer `bf9ad0b6…4dcb` changed no numeric ceiling. The later Slice-only repair makes a fresh signed-successor chain necessary for current-head evidence. Browser authority remains Edge family + CDP `1.3` + sealed capability contract; `.107` is provenance only. Six-image HUMAN judgment, Slice, Glass, recovery and whole-Gate closure remain outstanding. |
...
| `[EXEC]` | Gear instances migrate, equip, unequip, salvage, pending-claim and persist by exact identity | **`[DIRTY-DIAGNOSTIC]`.** Strict codecs/fixed-point tests cover duplicate-base identity, equipped legacy affix ownership, corrupt/future rows, full-inventory pending state and lossless capacity/byte protection. Older local real Smoke/Glass drives action publication, raw/runtime/DOM parity and reload. Final6 never clicked the clipped thermal row, so it supplied no current Inventory behavior verdict; its four descendant findings were instrument cascades. The repaired harness now proves native reveal, trusted row/action/Close ownership, surviving parent-panel state, full receipt key/byte/semantic durability and stable F4 RNG authority before Atlas. A fresh signed-successor Slice must exercise that causal chain before this row gains current exact evidence. |
...
| `[EXEC]` | Tame/Scavenge/Sample are the finite acquisition writers for catalogue pages, specimens and owned fauna | **`[RED]` until the bound final campaign passes.** Native Survey controls bind exact nav/address, the production full roster/current epoch and all-scenario capacity before two F4 draws; they spend on hit or miss and commit first-only reward, repeat individual/lot, one receipt/CAS, exact committed verification and targeted publication. Source-bound random-pool odds/budget, presentation fencing, native Close/reopen and historical exact-input outcomes remain. One exact durable fauna Tame result may join an accessible asset-free greeting. The first eight Slice reds remain historical diagnosis. Signed `041d1cf…` supplied the historical Final4 Slice PASS after green predecessors; Glass then stopped. Signed `ea845d7…` supplied Final6 Layout/SceneMemory/Compendium greens before Slice's ninth red stopped in the earlier Arc 2 Inventory chain, so Arc 4 was not reached and no new capture or recovery verdict exists. The reachability repair must complete a fresh signed-successor chain from Layout. Post-start audio and real recovery remain unproved. Separate HUMAN journey/listening review, Charter bioscan, targeted preview, hosted and release authority remain open. |
...
| `[EXEC-TODO]` | Accessibility audit | Final4 Glass reached the real 12-viewport audit and correctly retained Exotic `#9A5CFF` at 2.86:1 over effective bright glass as a product red, separately from stale instrument assumptions. Signed `39e4f20…` contains the repair preserving that hue on opaque `#05070d`, restoring exclusive sticky-Close geometry and adding live contrast/hit/scroll controls. Final6 passed Layout/SceneMemory/Compendium but stopped at Slice before Glass. A fresh signed-successor Glass result and physical assistive-technology review remain required. The queued universe-wide lighting/bloom pass must preserve these contrast and reduced-motion contracts. |
| `[EXEC-TODO]` | Performance and heat budgets | Historical Arc 1A/1B/1C certificates and the signed heap-only activation remain bound to their exact producers. Signed `041d1cf…` supplied historical Final4 Layout/SceneMemory/Compendium/Slice greens before Glass stopped. Signed `ea845d7…` supplied Final6 Layout 787/787, SceneMemory 42/42 and Compendium 78/78 before Slice stopped instrument-only. Current SceneMemory/Compendium numeric ceilings, rulers and samples are unchanged; a fresh signed-successor chain from Layout is required. Both authorities are compatible Edge family + CDP `1.3`; `.107` and later compatible point updates are provenance only. True GPU bytes, the queued bloom/atmosphere/particle costs, and physical-device heat/battery remain open, so this row and Gate I stay open. |
