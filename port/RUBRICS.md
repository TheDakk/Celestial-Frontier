...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Historical baseline5/candidate8/9/10 and baseline6/candidate11/12/13 remain truthful only for their exact measurement/producer inputs. Exact f9 passed the complete local battery, but corresponding run `32367902426` stopped before `Browser.getVersion` or Compendium on its one real Edge cold launch, so it made no product verdict and was not retried. The bounded selftest-only 45-second startup envelope retains 15-second socket/generic/candidate startup and 2-second product observation; launcher `6892dea6…` changes measurement to `6ba58522…` while producer remains `e59685b1…`. Clean one-attempt/no-retry baseline7 plus candidate14/15/16 activate browser-free budget/test `bb4da2bf0b…` / `d242705ad9…`; all candidates replayed 78/78, all 40 ceilings are strict, and the retained baseline breaches 14 phone / 13 desktop fields. This is non-certifying. This row remains `[EXEC-TODO]` pending one exact-head battery, corresponding terminal-green PR test-merge CI, and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, Planetside lifecycle, worker disposal, and the exact owner→worker→painter graph are implemented. Exact f9 passed the complete local battery; corresponding run `32367902426` stopped before browser provenance/product on the first real Edge cold launch and produced no Compendium outcome. Selftest-only startup is now 45 seconds with all game candidate/product bounds unchanged; browser-CDP `6892dea6…` changes measurement to `6ba58522…`. Clean source `374049536e…` supplied one-attempt/no-retry baseline7 plus candidate14/15/16, activating non-certifying browser-free budget/test `bb4da2bf0b…` / `d242705ad9…` with 78/78 per candidate and all 40 ceilings strict. This row remains `[EXEC-TODO]` pending one exact-head battery and terminal-green corresponding PR test-merge CI. Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's prior rulers, exact-f9 local battery, and no-product CI cold-start red remain preserved history. Launcher `6892dea6…` changes measurement to `6ba58522…`; browser-free budget `bb4da2bf0b…` is active for producer `e59685b1…`, with all 40 ceilings strict and the four-fault baseline still breaching 14 phone / 13 desktop fields. One exact-head battery and corresponding PR test-merge CI remain. The 45-second selftest-only CI allowance is not a game optimization target; generic/candidate startup and 2-second product observation are unchanged. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
