...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A's maximum-Compendium virtualization and owner→worker→painter ruler passed exact run `32462323775` and merged in PR #32 at `d4ab7e6…`. Arc 1B product/ruler `79c605f…` adds explicit Canvas/Pixi scene scopes, release-boundary managed-resource cleanup and BFCache survival. Three clean Edge-.93 calibrations activated budget `78a9e81…`; exact `e244c9e…` then passed one local no-retry 40/40 certification plus independent verification. Evidence descendant `b30b6d4…` does not move that exact certification. Draft PR #33 does not yet contain the unpushed Arc 1B head; there is no hosted attempt or integration. The current gate covers Universe → Galaxy → Galaxy fine → Sol → Earth Surface → 1,500-row Compendium; Shipyard is explicitly `future-arc-1c`, so the combined future loop keeps this row open. |
...
| `[EXEC-TODO]` | GPU/resource budget | Exact viewport backing-store caps remain. Arc 1B now locally budgets scene Canvas leases/bytes, live TextureSources, Pixi managed-texture pixels and per-hash inventories, render-target and cache proxies, DOM counters, heap/backing storage, slope/range, BFCache survival and answerability under exact Edge .93. Chromium exposes no portable true GPU-byte counter; future filters/particles, Shipyard/HD attachment, hosted integration, and physical-device GPU/heat evidence remain open. |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability; Arc 1A's Compendium ruler passed exact run `32462323775` and merged in PR #32. Arc 1B locally adds exact-Edge-.93 phone/desktop budgets for scene leases/canvas bytes, managed-resource and GPU proxies, heap/backing storage, DOM, slopes/ranges, BFCache and answerability. Exact `e244c9e…` passed one 40/40 local certification; `b30b6d4…` only retains its evidence. Draft PR #33 has no hosted attempt or integration, and true GPU bytes plus physical-device heat/battery remain open. |
