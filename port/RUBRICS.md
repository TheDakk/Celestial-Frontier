...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Da0's historical phone/desktop ruler embedded baseline3 plus candidate2/3/4, aggregate page/embedder/backing heap, a fixed-window last-three-cycle plateau, four sealed baseline faults, and 14 phone / 13 desktop breaches; its local certification passed. PR run `32334254714` then retained a no-retry phone Planetside `product-unanswerable` red. Every repaired default broker pump now crosses a rendering opportunity plus a later task, with bfcache pump-generation invalidation; that changes producer authority to `1c8200d7…`. Baseline4 plus three independent one-attempt candidate5/6/7 runs historically activated the new producer's strict replayed-raw ruler under measurement `bb03a3af…`; all candidates completed 78/78 outcomes and the baseline retained four faults with 14 phone / 13 desktop breaches. The frozen shared-timer repair moves measurement authority to `f9710bdf…`; the current budget is fail-closed `calibration-required` pending a fresh paired broken baseline plus three independent candidates before successor activation/certification. This row remains `[EXEC-TODO]` unless the selected head's ignored exact-source artifacts and its corresponding PR test-merge CI are terminal green; the live outcome is not cached here. Ordinary scene Canvas/Pixi ownership and the combined travel → Compendium → Shipyard warm GPU/resource plateau remain Arc 1B work |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, Planetside lifecycle, worker disposal, and the exact owner→worker→painter graph are implemented. Historical da0 authority used baseline3/candidate2/3/4 under measurement `bb03a3af…`, producer `291b794e…`, and exact Edge 151.0.4129.86; it replayed raw capsules, enforced strict aggregate-heap ceilings, sealed the fixed-window last-three-cycle plateau, retained four baseline faults, and breached 14 phone / 13 desktop ceilings. Da0's local cert passed, but exact PR run `32334254714` produced a valid no-retry phone Planetside answerability red. The frozen rAF→later-task pump and bfcache-generation repair changes producer authority to `1c8200d7…`; clean `f47cd381…` baseline4 and candidate5/6/7 historically activated its strict ruler under measurement `bb03a3af…` and exact Edge .86. That last pre-timer ruler's focused replay is 11/11 and the selftest is 222/222. The frozen shared-timer repair moves measurement authority to `f9710bdf…`; the current budget is fail-closed `calibration-required` pending a fresh paired broken baseline plus three independent candidates before successor activation/certification. This row remains `[EXEC-TODO]` unless the selected head's ignored exact-source artifacts and its corresponding PR test-merge CI are terminal green; the live outcome is not cached here. The Arc-local Edge 151 authority still does not repin Gate A |
