...
authority exists. Edge `151.0.4129.107` / CDP `1.3` is Final10 provenance only; compatible point
...
| `[EXEC]` | Tame/Scavenge/Sample are the finite acquisition writers for catalogue pages, specimens and owned fauna | **`[RED]` until the bound full campaign passes.** Native Survey controls bind exact nav/address, the production full roster/current epoch and all-scenario capacity before two F4 draws; they spend on hit or miss and commit first-only reward, repeat individual/lot, one receipt/CAS, exact committed verification and targeted publication. Source-bound random-pool odds/budget, presentation fencing and native Close/reopen remain. Final8 and Final9 are immutable historical evidence. Current immutable Final10 `4405fb2…` passed Layout, SceneMemory, Compendium, Slice and full Glass, then passed Recovery fixture, the complete 16-attempt burn-down, exhausted suppression, close/checkpoint and closed/offline proof. It stopped terminal `instrument-fail` at `offline-reopened` because the old phase-blind oracle rejected truthful read-only `unavailable` rows. Active observation, boundary crossing and recovered judgment did not run; no Recovery product or recovery verdict follows. Signed implementation/evidence repair `3fbfcd5…` has the six-region source seals and retained/replayed receipt controls above browser-free green; the synchronized signed clean docs-only descendant is the fresh campaign source, remains browser-uncertified and requires a full-chain restart. Post-start audio and real Recovery remain unproved; HUMAN journey/listening, Charter bioscan, targeted preview, hosted and release authority remain open. |
...
| `[EXEC-TODO]` | Accessibility audit | Final4 established the Exotic contrast repair. Final7's four Settings findings came from an oracle that demanded simultaneously hit-testable off-card controls. The repaired contract independently reveals Sound and Creature voices, binds two equal current-node samples, control/panel/bounds/Close and scroll settlement, 44px centre ownership, no overlap, dispatch-time coordinates, trusted receipt, exact post-state/focus and unconditional restoration. Final8 and Final9 remain historical exact-source Glass greens. Current immutable Final10 `4405fb2…` passed full 12-viewport Glass with zero findings/instrument failures. Signed implementation/evidence repair `3fbfcd5…` has the six-region source seals and retained/replayed receipt controls above browser-free green; the synchronized signed clean docs-only descendant is the fresh campaign source and requires a full-chain restart; physical assistive-technology review remains required. The queued universe-wide lighting/bloom pass must preserve contrast and reduced-motion contracts. |
| `[EXEC-TODO]` | Performance and heat budgets | Historical Arc 1A/1B/1C certificates and the signed heap-only activation remain bound to their exact producers. Final8 and Final9 remain historical exact-source evidence. Current immutable Final10 `4405fb2…` supplied Layout 787/787, SceneMemory 42/42, Compendium 78/78, Slice and full Glass with zero findings/instrument failures on unchanged current numeric ceilings, rulers and samples. Recovery passed through closed/offline proof before the `offline-reopened` instrument-only stop; active observation, boundary crossing and recovered judgment did not run. Signed implementation/evidence repair `3fbfcd5…` has the six-region source seals and retained/replayed receipt controls above browser-free green; the synchronized signed clean docs-only descendant is the fresh campaign source and requires a full-chain restart. Both browser authorities remain compatible Edge family + CDP `1.3`; `.107` and later compatible point updates are provenance only. True GPU bytes, queued bloom/atmosphere/particle costs and physical-device heat/battery remain open, so this row and Gate I stay open. |
