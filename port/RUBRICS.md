...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact-89, candidate20, and individually clean but unusable candidate21/22/23+baseline9 remain history. Clean exact `fb321f2…` candidate24/25/26 plus paired baseline10 ran once each with zero retries and distinct fresh `.86` paths; candidates completed 78/78 with zero findings and complete lifecycle, while baseline10 retained four faults. Active budget/test `70145575…` / `0fa2e89d…` embed 3/3 samples per profile, measured 1/1 baseline, and strict ceilings with 14 phone / 13 desktop breaches under measurement `2318f57b…`, producer `d3223177…`, and browser CDP `6da9e2ef…`. Focused activation is 13/13 after its phone-only synthetic control received matching desktop identities; browser evidence did not change. This row remains `[EXEC-TODO]` pending one exact-head local battery, one corresponding PR test-merge CI, and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau. No timing, workflow, product, producer, or retry-policy drift occurred; Arc 1B begins immediately after terminal-green PR #32 |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, asynchronous decoded 440px detail settlement, Planetside lifecycle, worker disposal, and the owner→worker→painter graph are implemented. Historical rulers, exact-89, candidate20, and candidate21/22/23+baseline9 remain truthful only for their own authorities. Clean exact `fb321f2…` candidate24/25/26 plus paired baseline10 ran once each with zero retries under distinct fresh `.86` paths. Active budget `70145575…` embeds 3/3 samples per profile, measured 1/1 baseline, strict ceilings, and 14 phone / 13 desktop breaches under measurement `2318f57b…` and unchanged producer `d3223177…`. The row remains `[EXEC-TODO]` pending exact-head battery and PR CI; Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's older rulers and exact-f9/c49/139/964/89, candidate20, and candidate21/22/23+baseline9 carriers remain preserved history. Clean exact `fb321f2…` candidate24/25/26 plus paired baseline10 ran once each with zero retries and activated budget `70145575…`: 3/3 samples per profile, measured 1/1 baseline, strict ceilings, 14 phone / 13 desktop breaches, measurement `2318f57b…`, unchanged producer `d3223177…`. One exact-head battery and one corresponding CI remain. No timeout, launch argument, workflow, product, producer, or retry-policy drift occurred. First red stops. Arc 1B scene/Pixi/GPU plateaus begin immediately after terminal-green PR #32; physical-device heat and battery evidence remain open |
