...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact-89's shutdown false-green and candidate20's wrong-browser carriers remain instrument history. Candidate21/22/23 plus paired baseline9 each completed once without retry under exact `.86` and complete lifecycle; candidates replayed 78/78 with zero findings and baseline9 retained four faults. They are individually clean diagnostic history only because the old shared identity incorrectly compared fresh path/UA provenance. Corrected contract `e7dfea1d…` accepts distinct path/UA while requiring exact product/revision/JavaScript/protocol; measurement is `2318f57b…`, producer `d3223177…`, and browser CDP `6da9e2ef…`. Fail-closed budget `ac2c084a…` has empty candidate samples and null ceilings; its measurement-required baseline has a null collector commit and empty profiles. It awaits fresh-per-launch exact-.86 baseline10 plus candidate24/25/26, one attempt each with zero retries. This row remains `[EXEC-TODO]` pending activation, one exact-head local battery, one corresponding PR test-merge CI, and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau. No timing, workflow, product, producer, or retry-policy drift occurred; Arc 1B begins immediately after terminal-green PR #32 |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, asynchronous decoded 440px detail settlement, Planetside lifecycle, worker disposal, and the owner→worker→painter graph are implemented. Historical rulers, exact-89, and candidate20 remain truthful only for their own authorities. Candidate21/22/23 plus paired baseline9 are individually clean exact-.86 diagnostic history but cannot activate because the old shared identity compared fresh path/UA provenance. Corrected contract `e7dfea1d…` preserves mandatory raw path/UA while sharing exact product/revision/JavaScript/protocol. Measurement `2318f57b…` and fail-closed budget `ac2c084a…` require fresh-per-launch exact-.86 baseline10 plus candidate24/25/26 from one clean committed source, one attempt each with zero retries, before exact-head certification. The row remains `[EXEC-TODO]`; Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's older rulers and exact-f9/c49/139/964/89 carriers remain preserved history. Candidate20's clean `.93` run stays quarantined. Candidate21/22/23 plus paired baseline9 are individually clean exact-.86 diagnostics, not product reds, but cannot cross the corrected shared-identity authority because their fresh path/UA provenance legitimately differs. Contract `e7dfea1d…` changes measurement to `2318f57b…`; producer remains `d3223177…`. Fail-closed budget `ac2c084a…` awaits fresh-per-launch exact-.86 baseline10 plus candidate24/25/26 once each with zero retries, then one exact-head battery and one corresponding CI. No timeout, launch argument, workflow, product, producer, or retry-policy drift occurred. First red stops. Arc 1B scene/Pixi/GPU plateaus begin immediately after terminal-green PR #32; physical-device heat and battery evidence remain open |
