...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC]` | Zero unapproved app authority in domain packages | `tests/nodom.test.ts` recursively seals the exact 62-file inventory against DOM, storage, `navigator`, network, wall/monotonic clock and uncontrolled randomness. Its only two waivers are the exact CombatCore `document.createElement('')` avatar-painter expressions; inventory, duplicate, reason/matcher-drift and injected-forbidden-access controls fail closed. D-HAZE moved byte-verbatim to app-owned `GalaxyArt` and is no longer waived |
| `[EXEC]` | No uncontrolled clock or randomness in every port domain package | The same exact-inventory scanner rejects `Math.random`, `Date.now`, `new Date`, `requestAnimationFrame` and `performance.now` across all 62 sources; deliberate injected clock/random violations prove discrimination |
...
| `[EXEC-TODO]` | Universe-wide visual polish preserves all authored structure, deterministic identity and interaction geometry | Before/after fixed-seed proof sheets across galaxy/system space, planets, every biome, creatures, plants, ships and effects must retain exact seed and owner keys, anatomy, silhouette, proportions, scale, placement and interaction geometry while varying only deterministic palette, light, material, atmosphere and bounded particles. Reduced-motion and mobile-budget variants are required; HUMAN review judges appeal |
...
| `[EXEC-TODO]` | Accessibility audit | Final4 established the Exotic contrast repair. Final7's four Settings findings came from an oracle that demanded simultaneously hit-testable off-card controls. The repaired contract independently reveals Sound and Creature voices, binds two equal current-node samples, control/panel/bounds/Close and scroll settlement, 44px centre ownership, no overlap, dispatch-time coordinates, trusted receipt, exact post-state/focus and unconditional restoration. Final8 and Final9 remain historical exact-source Glass greens. Historical immutable Final10 `4405fb2…` passed full 12-viewport Glass with zero findings/instrument failures. Signed implementation/evidence repair `3fbfcd5…` has the six-region source seals and retained/replayed receipt controls above browser-free green; the synchronized signed clean docs-only descendant supplied immutable Final11; the historical assessor/tests/docs-only repair supplied Final12, whose distinct immutable Slice control stop is recorded above; physical assistive-technology review remains required. The queued universe-wide lighting/bloom pass must preserve contrast and reduced-motion contracts. |
| `[EXEC-TODO]` | Performance and heat budgets | Historical Arc 1A/1B/1C certificates and the signed heap-only activation remain bound to their exact producers. Final8 and Final9 remain historical exact-source evidence. Historical immutable Final10 `4405fb2…` supplied Layout 787/787, SceneMemory 42/42, Compendium 78/78, Slice and full Glass with zero findings/instrument failures on unchanged current numeric ceilings, rulers and samples. Recovery passed through closed/offline proof before the `offline-reopened` instrument-only stop; active observation, boundary crossing and recovered judgment did not run. Signed implementation/evidence repair `3fbfcd5…` has the six-region source seals and retained/replayed receipt controls above browser-free green; the synchronized signed clean docs-only descendant supplied immutable Final11; the historical assessor/tests/docs-only repair supplied Final12, whose distinct immutable Slice control stop is recorded above. Both browser authorities remain compatible Edge family + CDP `1.3`; `.107` and later compatible point updates are provenance only. True GPU bytes, queued bloom/atmosphere/particle costs and physical-device heat/battery remain open, so this row and Gate I stay open. |
