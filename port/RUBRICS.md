...
disconnected-effect rows remain unavailable. Arc 4 now has a diagnostics-only headless durable
writer: strict 18-namespace boot/Training migration, full-roster/current-epoch capture, two F4
draws after all-scenario capacity proof, one receipt/CAS and committed-state verification/publication.
It has no player control, copy/toast, Guide capability, browser outcome or HUMAN proof. Arc 5's V2
model/digest certificate remains a package foundation, not a companion action. Remaining rows below
therefore stay `[EXEC-TODO]` where complete outcome/browser/HUMAN proof is absent.
...
| `[EXEC-TODO]` | Complete economy source/formula/faucet parity | Arc 3 supplies canonical finite mining/skimming sources and receipt-backed settlement. Arc 4 now headlessly settles one genuinely-new capture Stardust award with first-only catalogue facts while repeats create only the eligible individual/lot; that focused writer does not certify complete capture rates/recovery/pacing. The Arc 2 source-neutral ledger correctly retains its historical `arc3-deferred` field; combat/mission/new-random-loot faucets, broader pacing, five non-Deep-Scanner research purchases, Survey orbital mineral rows and unavailable fixed outputs remain open |
...
| `[EXEC-TODO]` | Every legacy outcome roll uses that authority through its real action | The exact 24-site audit classifies 14 outcome and 10 presentation calls. Arc 4's headless real action now consumes isolated persisted `captureCandidate` then `captureSuccess` domains after pre-draw certification and proves counter/value binding; care, combat, training and the other audited outcome writers remain open |
...
| `[EXEC-TODO]` | Every later reward-bearing/destructive mutation is revision-checked and stale-tab safe | Arc 3 fixed fabrication and Arc 4 capture now use the shared coordinator plus one lease-fenced receipt/CAS and focused stale/overlap controls. Same-parent breed, same-world settlement, mission claim, combat settlement and other later writers must still reject the stale second writer; implemented slices do not certify absent products |
...
| `[EXEC-TODO]` | Catalogue species and owned creatures are separate; every mutation targets a stable creature ID | Ownership-v1/v2 codecs model stable catalogue/specimen/fauna identities, receipt provenance, ordered lineage and tombstones. The headless Arc 4 writer now creates first-only catalogue facts and stable-ID fauna individuals or specimen lots, with repeat controls. Duplicate-species migration plus breed/feed/combat/delete/assignment outcomes and player/browser reach remain required |
| `[EXEC-TODO]` | Tame/Scavenge/Sample are the finite acquisition writers for catalogue pages, specimens and owned fauna | A diagnostics-only action now binds exact nav/address, full roster/current epoch, all-scenario capacity before two F4 draws, hit/miss spend, first-only reward, repeat individual/lot, one receipt/CAS, exact committed verification and targeted publication. Visible controls, presentation policy, real-browser/reload outcomes, Guide capability and HUMAN proof remain open |
