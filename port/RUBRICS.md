...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, art ownership, focus/filter/detail/Close outcomes, and the owner→worker→painter graph are implemented. Exact run `32462323775` passed the complete changed-head battery and PR #32 merged at `d4ab7e6…`. PR #34 run `32665404776` later exposed a native virtual-row activation ruler defect, not a leak: the click never opened detail and the downstream art wait misdiagnosed its final clipped poll. Collector `50c28928…` now requires full containment, independent exact hit ownership and an immediate detail receipt. Three retained candidates plus paired baseline1 activate measurement `cfc40f89…` and budget `208af955…` with unchanged numeric ceilings and all four baseline faults preserved. Clean source `7de42c6…` passed local run `20260823-pr34-row-activation-certification` 78/78 with complete lifecycle and named verification. Six-image HUMAN judgment remains the separate next row, and Arc-local Edge 151 does not repin Gate A. |
