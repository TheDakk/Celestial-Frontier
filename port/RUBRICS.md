...
`587d3bdfab471370e625c71d1658e391067881fe824ce14ccfaf7200eb6e4d73` is historical. Clean committed source
...
`524288`, with every other numeric ceiling unchanged. Clean signed activation source
...
SceneMemory/root Gate A are unchanged. Browser-free verification is green at 122 files /1,362 passed /one intentional skip /zero failures,
`root/app/worker/noUnused` TypeScript, 884-module Vite, root validation/fingerprint, legacy smoke
and applicable selftests. Current-input Slice retains three one-attempt/zero-retry reds on Edge `.107`.
...
the Arc 3 harness finding. Settings and Survey focus are outcome-cleared. The reachable replacement
target exposed a second instrument defect whose oracle incorrectly joined system navigation to
nav-owned planet identity. Clean signed source `8a23e22…` then supplied
`20260827025804458-2742-c0c871ee52b6`, which failed after 93,582 ms with three cascading findings
because retained Survey correctly hid the right rail before Slice attempted zero-rect
`railshipyard`. That run outcome-clears the card-context repair; Deep Scanners and every later stage
did not run, and the lifecycle repair is unrerun. No
