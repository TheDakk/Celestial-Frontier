...
20-minute recovery observation and HUMAN review remain open. Arc 5A now activates its exact compact
V2 manifest plus four fixed generic delta shards in the same boot, genuine legacy Training and every
Arc 4 hit/miss transaction, with paired postcommit publication; it remains infrastructure, not a
companion action. Aligned legacy-v1 upgrades in one receipt-free CAS, aligned current-v2 writes
nothing, and source-only growth preserves the four canonical empty-shard bytes. Retained Arc 4
browser evidence predates compact Arc 5 V2. Final current-input Slice
...
root/app/worker/noUnused TypeScript, `artunused` and the 876-module build passed. The full run has
