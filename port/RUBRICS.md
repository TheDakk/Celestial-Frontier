...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact activation head `96464d5…` passed its complete local battery. Corresponding run `32394244417`, attempt 1, stopped before product in exact-Edge job `96507263338`: image `ubuntu24/20260816.277` already carried verified .86, apt performed no unpack/setup, and the unchanged one-launch 45-second preflight published no CDP endpoint. There is no candidate, report, outcome, or review PNG. Both workflows now request same-package `--reinstall`; a green preflight selftest statically controls its ordered package/preflight placement, but one changed-head CI is required to test this runner-image normalization hypothesis. Measurement `6ba58522…`, producer `d3223177…`, budget/test `74e88c2b…` / `485be9da…`, all timing, authority, and one-attempt/zero-retry policy remain unchanged. This row remains `[EXEC-TODO]` pending terminal-green corresponding PR test-merge CI and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, asynchronous decoded 440px detail settlement, Planetside lifecycle, worker disposal, and the exact owner→worker→painter graph are implemented. Run `32383320206` reached product and preserved its no-retry Planetside answerability red; the bounded displayed-demand repair changed producer to `d3223177…` while measurement stayed `6ba58522…`. Clean source `75a996af…` supplied baseline8 plus three independent candidates; active budget/test `74e88c2b…` / `485be9da…` preserve all four baseline faults, 14 phone / 13 desktop breaches, and all 40 strict ceilings. Exact activation head `96464d5…` passed its complete local battery, but run `32394244417` stopped before product when image `ubuntu24/20260816.277`'s already-resident .86 produced an apt no-op and no CDP endpoint within the unchanged live-preflight bound. Same-package `--reinstall` plus its green static workflow control is a normalization hypothesis pending one changed-head CI, not a browser resource verdict. The row remains `[EXEC-TODO]`; Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's prior rulers and exact-f9/c49/139 local batteries remain preserved history. Run `32383320206` is product evidence, not a request to optimize accepted CI envelopes. The displayed-demand/zoom-owner repair preserves supported 1024 sharpness only where real zoom/DPR demands it. Launcher/measurement `6892dea6…` / `6ba58522…` remain unchanged; producer is `d3223177…`. Clean source `75a996af…` supplied baseline8 plus candidate17/18/19, and active budget/test `74e88c2b…` / `485be9da…` retain strict discrimination. Exact `96464d5…` passed locally; run `32394244417` is a pre-product hosted Edge/CDP environment red, with one attempt and zero retries. Same-package `--reinstall` and its green static ordered-workflow control change no timing, product, live/runtime-tool behavior, browser, measurement, producer, budget, or authority; the runner-image normalization hypothesis still needs one changed-head CI. First red stops. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
