...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Historical calibration remains source-bound. Measurement `6ba58522…`, producer `e59685b1…`, and active browser-free budget/test `bb4da2bf0b…` / `d242705ad9…` remain unchanged. Exact c49 passed the complete local battery, but corresponding run `32375329693` preserved root pre-endpoint, Edge-selftest command, and unadjudicated asynchronous-detail Smoke instrument reds without retry. The bounded repair gives root layout a 45/15/30/5-second caller, exact-Edge workflows a fresh-target 45/15/sealed-5/2-second preflight outside hashed measurement inputs, and Smoke one semantic 30-second 440px detail-settlement phase. This row remains `[EXEC-TODO]` pending one repair-head battery, corresponding terminal-green PR test-merge CI, and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, asynchronous decoded 440px detail settlement, Planetside lifecycle, worker disposal, and the exact owner→worker→painter graph are implemented. Exact c49 passed the complete local battery. Corresponding run `32375329693` opened exact Edge but stopped in the generic selftest's 1.5-second `Runtime.enable` before Compendium; it produced no resource outcome. The bounded exact-Edge preflight uses one fresh target under 45/15/sealed-5/2-second bounds outside the unchanged `6892dea6…` / `6ba58522…` authority graph. This row remains `[EXEC-TODO]` pending one repair-head battery and terminal-green corresponding PR test-merge CI. Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's prior rulers and exact-f9/c49 local batteries remain preserved history. Launcher `6892dea6…`, measurement `6ba58522…`, browser-free budget `bb4da2bf0b…`, and producer `e59685b1…` remain unchanged; all 40 ceilings are strict and the four-fault baseline still breaches 14 phone / 13 desktop fields. Run `32375329693` produced no Compendium outcome and its other reds were root cold-start plus an unadjudicated one-shot asynchronous detail sample. One repair-head battery and corresponding PR test-merge CI remain. The caller/preflight 45-second CI allowances and Smoke's semantic 30-second owner wait are instrument bounds, not game optimization targets. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
