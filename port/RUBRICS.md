...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Da0's historical phone/desktop ruler embedded baseline3 plus candidate2/3/4, aggregate page/embedder/backing heap, a fixed-window last-three-cycle plateau, four sealed baseline faults, and 14 phone / 13 desktop breaches; its local certification passed. PR run `32334254714` then retained a no-retry phone Planetside `product-unanswerable` red. Every repaired default broker pump now crosses a rendering opportunity plus a later task, with bfcache pump-generation invalidation; that changes producer authority to `1c8200d7…`. Baseline4 plus candidate5/6/7 historically activated its pre-timer `bb03a3af…` ruler. The frozen shared-timer repair moves measurement to `f9710bdf…`; exact-.86 baseline5 plus independent candidate8/9/10 from clean `cbe7868…` now activate strict budget/test `8ffd0d8e…` / `121ab8cd…`. All candidates replay 78/78, every one of 40 ceilings exceeds its maximum, warm maxima/ceilings are 6,196/65,536 phone and 306,704/524,288 desktop bytes, and the four-fault baseline breaches 14/13 fields. This browser-free activation is not certification. This row remains `[EXEC-TODO]` unless the selected head's ignored exact-source artifacts and its corresponding PR test-merge CI are terminal green; the live outcome is not cached here. Ordinary scene Canvas/Pixi ownership and the combined travel → Compendium → Shipyard warm GPU/resource plateau remain Arc 1B work |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, Planetside lifecycle, worker disposal, and the exact owner→worker→painter graph are implemented. The historical da0 ruler used baseline3 plus candidate2/3/4 under measurement `bb03a3af…` and exact Edge 151.0.4129.86; only the candidate runs carry producer `291b794e…`. Its local cert passed before PR `32334254714` produced a valid no-retry phone Planetside answerability red. The rAF→later-task/bfcache repair changes producer to `1c8200d7…`; baseline4/candidate5/6/7 historically activated its pre-timer ruler. The timer repair moves measurement to `f9710bdf…`; exact baseline5 plus candidate8/9/10 from clean `cbe7868…` now activate budget/test `8ffd0d8e…` / `121ab8cd…`. Focused replay is 11/11, selftest 222/222, each candidate run replays 78/78, all 40 ceilings exceed maxima, and the four-fault baseline breaches 14 phone / 13 desktop fields. This browser-free activation is not certification. This row remains `[EXEC-TODO]` unless the selected head's ignored exact-source artifacts and its corresponding PR test-merge CI are terminal green; the live outcome is not cached here. The Arc-local Edge 151 authority still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's da0 ruler and local certification are preserved history, but PR CI exposed a phone product-answerability failure; the serviced-turn repair changes producer authority. Baseline4/candidate5/6/7 historically activated that producer's pre-timer ruler. The timer repair moves measurement to `f9710bdf…`; exact-.86 baseline5 and candidate8/9/10 now activate budget `8ffd0d8e…` from clean `cbe7868…`, with all 40 ceilings above maxima and four baseline faults breaching 14/13 fields. This browser-free activation is not certification. This row remains `[EXEC-TODO]` unless the selected head's ignored exact-source artifacts and its corresponding PR test-merge CI are terminal green; the live outcome is not cached here. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
