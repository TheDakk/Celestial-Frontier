...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact `aecf386…` bounds static-server close and suppresses PASS/sample on cleanup red. Clean `6736ef4…` c27/baseline11/c28/c29 evidence activated the macOS ruler. Linux run `32441023665` isolated native PNG/embedder variance to three ceilings; budget/test `e3a71c8a…` / `0793772d…` admit the committed raw report while preserving just-below reds and 14/13 baseline breaches. Exact changed-head run `32462323775` passed the complete battery once and PR #32 merged at `d4ab7e6…`. Arc 1B's scene Canvas/Pixi ownership and the combined travel → Compendium → Shipyard plateau remain open. |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Clean `6736ef4…` c27/baseline11/c28/c29 evidence owns Arc 1A's candidates and paired baseline. Budget/test `e3a71c8a…` / `0793772d…` replay retained Linux report `a486fe8e…` 78/78 with just-below negatives and unchanged 14/13 baseline breaches. Exact changed-head run `32462323775` passed and PR #32 merged at `d4ab7e6…`; Arc 1B scene/Pixi/GPU plateaus and physical-device heat/battery evidence remain open. |
