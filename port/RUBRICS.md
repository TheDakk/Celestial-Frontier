...
**Current local candidate boundary (2026-08-27):** signed source
`5ddddbfb79ea984d44c86e2107e5e4013f84f1b3` owns the fully verified second harness repair. Final3
Layout `20260827-phase4-final3-layout` passed 787/787 across ten viewports in 76,133 ms plus named
verification. SceneMemory `20260827-phase4-final3-scenemem` then ran once, stopped
`instrument-fail` after 1,662 ms before measurement with zero outcomes, retained exact clean
begin/end source and complete cleanup, and was not retried. Compendium, Slice, Glass and recovery
correctly did not run. Layout remains chronology-associated result provenance because schema v2
does not embed source identity; the SceneMemory report is source-bound instrument evidence, not a
product verdict.

Only SceneMemory's live `buildDist` and `gameMain` bindings were stale. Browser-free current-byte
derivation also found Compendium's measurement authority current but its built index/owner producer
stale. This is not product, numeric-ruler, deadline, cleanup or Edge-version drift. The current
repair rebinds only those live records (SceneMemory budget `47d24080…775b`, Compendium budget
`f0bedb67…3c64`), preserves every historical sample/ruler/ceiling, and adds a standard-build
source-derived test plus source/build/duplicate-constant/recomputed-index/owner mutants. The full
repair battery is green at 135 Vitest files /1,478 passed /one intentional skip, all
TypeScript/no-unused programs, the 887-module build, authority printer and root validator. Three
independent final binding/test, whole-diff, and documentation/evidence reviews are CLEAR. The signed
clean commit containing this record is the final4 source; verify its signature and clean tree before
restarting with fresh IDs. No later certifying browser result, publication, hosted attempt, release, deployment, version
bump or external authority exists. Compatible Edge/Chrome point updates are provenance only and
never trigger a rebaseline or threshold change.
...
| `[EXEC]` | No leaks across repeated travel, Compendium and real Shipyard cycles | Signed `b206cf0…` is `[HISTORICAL]` for the current repaired worktree; earlier certificates remain `[HISTORICAL]`. Arc 1A's maximum-Compendium ruler passed exact hosted run `32462323775` and merged in PR #32 at `d4ab7e6…`. Arc 1B's `79c605f…` / `e244c9e…` 40/40 scene-memory-v1 certificate remains historical pre-Shipyard authority. Arc 1C product/ruler `a4de5007ffc9131b8bc952a0a4cb469d9139039e` adds normalized ship state, one SVG/DOM Shipyard preview owner and the named HD-surface attachment. `59530da…` / budget `3b71d14c…` / run `20260822-arc1-local-certification` remain historical 42/42 authority for the former Mac-derived 250 ms ruler. Repair `7d8dc380…`, budget `5c8a6e75…`, and no-retry run `20260823-pr33-cross-host-sla-certification` passed historical 42/42 plus named verification under Edge `.101`; run `32646110946` later passed and merged PR #33 as `8998ffb…`. Signed source `862a75b…` passed named Layout 787/787, then no-retry SceneMemory `20260827-phase4-successor-scenemem` stopped the chain at 40/42 on only phone/desktop `heap-dom-budget`; the carrier remains the paired red. The repair defers closed Inventory DOM/subscriptions, delegates opener capture once and emits exact field/value/ceiling diagnoses. Signed calibration source `6c9ad855…` supplied exactly `20260827-phase4-repair-candidate1/2/3`, each once with zero retries and complete cleanup on Edge `.107` / CDP `1.3`. Their maxima select only 12 MiB V8 and 18 MiB aggregate ceilings with exact phone/desktop headroom 1,016,760 / 951,976 and 1,193,110 / 1,237,686 bytes; nodes/listeners remain 676/71 and 673/70 and every other ceiling is unchanged. The paired red still fails 704/80. A compatible Edge update alone never calibrates. Signed activation `4a54c0d…` is complete; signed `7362a0e…` and `b206cf0…` passed their historical certificates. Signed `5ddddbf…` reached final3 Layout green, then SceneMemory stopped before measurement on stale live source/dist binding. The current repair rebinds budget `47d24080…775b` and source-derives both memory authorities during `npm test`; three independent final binding/test, whole-diff, and documentation/evidence reviews are CLEAR. It remains `[RED]`/not `[CURRENT-EXACT]` until the signed clean commit containing this record completes a fresh final4 chain. |
...
| `[EXEC-TODO]` | GPU/resource budget | Exact viewport backing-store caps remain. Arc 1B budgets scene Canvas leases/bytes, live TextureSources, Pixi managed-texture pixels and per-hash inventories, render-target/cache proxies, DOM, heap/backing storage, slope/range, BFCache and answerability. Arc 1C's historical `7d8dc380…` / `5c8a6e75…` local Edge `.101` 42/42 certificate extended those proxies through the real Shipyard, its one SVG/DOM preview owner and the named HD-surface attachment; the former `59530da…` / `3b71d14c…` certificate and PR #33 integration remain historical. Signed `862a75b…` is the preserved paired `[RED]` at 40/42: resource ownership, plateau, slope, answerability and cleanup passed, while the combined heap/DOM outcomes exposed both avoidable closed-panel DOM/listener ownership and fixed product-growth heap. Signed `6c9ad855…` supplied the exact three clean repair candidates; their fixed 676/71 phone and 673/70 desktop DOM/listener values preserve 704/80. Signed `4a54c0d…` activates 12 MiB V8 /18 MiB aggregate ceilings with every other ruler unchanged; the paired red still fails nodes/listeners. Signed `7362a0e…` and `b206cf0…` passed historical serial SceneMemory 42/42. Signed `5ddddbf…` reached final3 Layout green, then SceneMemory stopped before measurement on stale `buildDist`/`gameMain` binding; current budget `47d24080…775b` is source-derived and fully browser-free green but not certified until the signed final4 chain. Edge/Chrome point updates are provenance, never calibration or threshold-change triggers. Chromium still exposes no portable true GPU-byte counter; later filters/particles and physical-device GPU/heat evidence remain open. |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup for the current producer | **`[HISTORICAL]` for the repaired worktree.** Product virtualization, art ownership and focus/filter/detail/Close outcomes are implemented. Measurement `cb5cd9f86ac99435028f98af800bc0d89de96bd7db88694214d832eed83fb15d`, former producer `587d3bdfab471370e625c71d1658e391067881fe824ce14ccfaf7200eb6e4d73`, selected candidate3/5/6, baseline1, activation `d33e540…`, and certificate `20260826-phase4-certification` remain truthful historical proof. Signed calibration source `8ffd2e2…` supplied `20260826-slice-repair-candidate1/2/3` plus paired `20260826-slice-repair-baseline1`, each one attempt/zero retries. Historical budget `6284a394…` bound former producer `f7c87f22…`, retained all four faults and exact 14-phone/13-desktop discrimination, and changed only the phone warm ceiling to `524288`. Historical clean signed activation `91f4e044…` passed `20260826-slice-repair-certification` 78/78 with complete lifecycle and named verification in one attempt/zero retries (44,847 ms; raw/gzip SHA-256 `81c27ed5…` / `6f3deb0f…`). Signed `7362a0e…` and `b206cf0…` each passed historical serial Compendium 78/78. Signed `5ddddbf…` reached final3 Layout green, but SceneMemory's pre-measurement instrument stop prevented Compendium from running. Browser-free derivation now binds live producer `4b5aa3a3…` under budget `f0bedb67…3c64` while preserving measurement/ruler/samples/ceilings; that rebind is not a certificate and still requires signed final4 from Layout. This is real product-owner drift, not an Edge auto-update trigger; `.107` is provenance only. Browser authority remains Edge family + CDP `1.3` + sealed capability-contract SHA-256 `6eed33ed9784f7c7774c4b1bf8d4e880986e31667324d9a1aa7b8dd62fe5a476`. Six-image HUMAN judgment remains separate and outstanding; Slice/Glass/recovery and whole-Gate claims remain open. Root Gate A separately uses compatible Chromium family + CDP `1.3` + its source-derived root capability/provenance contract; neither authority uses point version as a rebaseline trigger. |
...
| `[EXEC]` | Ship art, installed-system captions and travel reach derive from one normalized state | **`[HISTORICAL]` for the repaired worktree.** Product/ruler `a4de5007ffc9131b8bc952a0a4cb469d9139039e` makes travel, captions, four SVG chassis, exact `array`/`autoext`/`cscoop` hardpoints and diagnostics consume one unsaved projection. Unit/integration/browser controls cover 64 normalized cases, all eight hardpoint permutations, reload-shaped reconstruction, imported-veteran fallback, friendly-alias rejection, deliberately mismatched selectors, one-preview ownership and real opener/Close state parity. Repair source `7d8dc380cd89ef53aac5a11c3850316e19e1aae9` passed scene-memory-v2 42/42 and its named verifier under budget `5c8a6e75…`; `59530da…` / `3b71d14c…` is older historical authority. Terminal-green run `32646110946` merged that evidence in PR #33. Signed `7362a0e…` and `b206cf0…` supplied historical SceneMemory 42/42 certificates. Signed `5ddddbf…` reached final3 Layout green before SceneMemory stopped pre-measurement on stale live binding. The repaired source's full browser-free battery is green and three independent final binding/test, whole-diff, and documentation/evidence reviews are CLEAR; the signed clean commit containing this record owns fresh final4. |
...
| `[EXEC]` | Gear instances migrate, equip, unequip, salvage, pending-claim and persist by exact identity | **`[DIRTY-DIAGNOSTIC]`.** Strict codecs/fixed-point tests cover duplicate-base identity, equipped legacy affix ownership, corrupt/future rows, full-inventory pending state and lossless capacity/byte protection. Local real Smoke/Glass drive action publication, raw/runtime/DOM parity and reload; the evidence is dirty-input/local, not current frozen-producer, hosted or Gate closure |
...
| `[EXEC]` | Tame/Scavenge/Sample are the finite acquisition writers for catalogue pages, specimens and owned fauna | **`[RED]` until the bound final campaign passes.** Native Survey controls bind exact nav/address, the production full roster/current epoch and all-scenario capacity before two F4 draws; they spend on hit or miss and commit first-only reward, repeat individual/lot, one receipt/CAS, exact committed verification and targeted publication. Source-bound random-pool odds/budget, presentation fencing, native Close/reopen and historical exact-input outcomes remain. One exact durable fauna Tame result may join an accessible asset-free greeting. The first six Slice reds remain historical diagnosis. Signed `7362a0e…` retained three green predecessors before its seventh terminal Slice red; signed `b206cf0…` repeated three green predecessors before final2's eighth terminal red retained six findings across five scopes. Glass/recovery did not run. Signed `5ddddbf…` contains that browser staging/evidence repair. Final3 Layout passed, then SceneMemory stopped pre-measurement on stale live producer binding. The current source-derived binding repair's full browser-free battery is green and three independent final binding/test, whole-diff, and documentation/evidence reviews are CLEAR; the signed clean commit containing this record owns final4 from Layout. Post-start audio, the exact nine-stage/14-burn ledger and real recovery remain unproved. Separate HUMAN journey/listening review, Charter bioscan, targeted preview, hosted and release authority remain open. |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability; Arc 1A's Compendium ruler passed exact run `32462323775` and merged in PR #32. Arc 1B's Edge `.93` 40/40 and Arc 1C's `59530da…` / `3b71d14c…` plus `7d8dc380…` / `5c8a6e75…` Edge `.101` certificates remain historical; repaired hosted run `32646110946` passed and merged PR #33. Signed `862a75b…` remains the paired 40/42 SceneMemory red. Signed repair source `6c9ad855…` completed the exact three-candidate calibration and paired-red discrimination; signed `4a54c0d…` activates only 12 MiB V8 /18 MiB aggregate. Signed `7362a0e…` and `b206cf0…` supplied historical serial SceneMemory/Compendium greens. Signed `5ddddbf…` reached final3 Layout green, then SceneMemory stopped before measurement on stale live binding. Current SceneMemory `47d24080…775b` and Compendium `f0bedb67…3c64` budgets are source-derived, and three independent final binding/test, whole-diff, and documentation/evidence reviews are CLEAR; the signed clean commit containing this record owns the fresh final4 chain. Compendium and SceneMemory retain separate version-tolerant Edge-family + CDP `1.3` authorities; compatible Edge/Chrome point updates neither rebaseline nor move thresholds. True GPU bytes and physical-device heat/battery remain open, so this row and Gate I stay open. |
