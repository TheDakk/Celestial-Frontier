...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Historical baseline5/candidate8/9/10 activated measurement `f9710bdf…`, producer `1c8200d7…`, and budget/test `8ffd0d8e…` / `121ab8cd…`; those exact capsules cannot cross a producer boundary. Exact c095 Compendium and Smoke passed, then the first full Glass run preserved one 12.5px compact-phone Survey/Planetside product overlap with 12/12 rows, 58/58 controls, zero instrument failures/retries. The bounded 44px Survey / 8px gap / 72px scrollable Planetside repair and revised draft copy change producer to `e59685b1…`. Clean `2a105d51…` baseline6/candidate11/12/13 now activate measurement `f9710bdf…` and budget/test `ebe5b5c3…` / `ec956b8a…`: all runs were one-attempt/no-retry, candidates replay 78/78, all 40 ceilings exceed their maxima, and the four-fault baseline breaches 14 phone / 13 desktop fields. This row remains `[EXEC-TODO]` pending one complete exact-head battery and corresponding terminal-green PR test-merge CI. Ordinary scene Canvas/Pixi ownership and the combined travel → Compendium → Shipyard warm GPU/resource plateau remain Arc 1B work |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, Planetside lifecycle, worker disposal, and the exact owner→worker→painter graph are implemented. Historical baseline5/candidate8/9/10 activated producer `1c8200d7…` under `f9710bdf…`; c095's exact Compendium and Smoke passed before its first full Glass run preserved the compact-phone stack product red. The bounded stack repair changes producer to `e59685b1…`, while measurement stays `f9710bdf…`; paired baseline6 plus candidate11/12/13 from clean `2a105d51…` now activate budget/test `ebe5b5c3…` / `ec956b8a…`. Focused replay 11/11, selftest 222/222, and semantic validation prove the strict ruler, not selected-head browser certification. This row remains `[EXEC-TODO]` pending the exact-source battery and corresponding terminal-green PR test-merge CI. Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's prior rulers and certifications remain preserved history. The bounded Planetside stack/copy change creates producer `e59685b1…`; measurement remains `f9710bdf…`, and clean baseline6/candidate11/12/13 activate strict replayed budget `ebe5b5c3…`. Exact-head local gates and corresponding PR test-merge CI remain required. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
