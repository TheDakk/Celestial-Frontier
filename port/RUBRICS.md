...
**Current local candidate boundary (2026-08-27):** signed evidence source
`7362a0ea32e90b24e4988c81d566b82e20549e66` first passed standalone SceneMemory 42/42, then
retained a serial Layout 787/787 result carrier plus source-bound SceneMemory 42/42 and Compendium
78/78 carriers. Layout schema v2 embeds no Git/source identity and that non-commit-tagged run lacks
a preserved execution record, so it is chronology-associated provenance rather than standalone
exact-source evidence. Its one Slice attempt then stopped terminal-red on preserved stale harness
assumptions; Glass and recovery correctly did not run. This is the latest committed
browser-evidence boundary, not a complete chain.

The repaired worktree changes only evidence oracles, their focused tests, audit/evidence carriers
and live documentation; product, producer, numeric-budget and deadline bytes are unchanged. Its browser-free
battery is green at 134 Vitest files / 1,474 passed / one intentional skip, plus all
`root/app/worker/noUnused` TypeScript, `artunused`, syntax/import selftests and the 887-module build.
It must become a new signed clean commit and restart the full chain from Layout with fresh IDs. No
later browser result, publication, hosted attempt, release, deployment, version bump or external
authority exists. Compatible Edge/Chrome point updates are provenance only and never trigger a
...
preserved and unclaimed. The later signed `7362a0e…` campaign superseded this as the latest evidence
boundary and stopped at its own preserved Slice harness red; no Glass result, recovery certificate,
...
| `[EXEC]` | Tame/Scavenge/Sample are the finite acquisition writers for catalogue pages, specimens and owned fauna | **`[RED]` until the bound final campaign passes.** Native Survey controls bind exact nav/address, the production full roster/current epoch and all-scenario capacity before two F4 draws; they spend on hit or miss and commit first-only reward, repeat individual/lot, one receipt/CAS, exact committed verification and targeted publication. Source-bound random-pool odds/budget, presentation fencing, native Close/reopen and historical exact-input outcomes remain. One exact durable fauna Tame result may join an accessible asset-free greeting. The first six Slice reds remain historical diagnosis. Signed evidence source `7362a0e…` then supplied source-bound standalone/serial SceneMemory and Compendium greens plus an associated verifier-green Layout result carrier without standalone source binding before its one Slice attempt stopped terminal-red with 12 findings across 11 scopes on preserved stale harness assumptions. Glass/recovery did not run. The repaired worktree changes only evidence oracles/tests, audit/evidence carriers and docs and must be signed before the full chain restarts from Layout. Post-start audio, the exact nine-stage/14-burn ledger and real recovery remain unproved. Separate HUMAN journey/listening review, Charter bioscan, targeted preview, hosted and release authority remain open. |
