...
disconnected-effect rows remain unavailable. Arc 4's roster/ownership/snapshot/planner
and Arc 5's V2 model/digest certificate are package foundations, not real acquisition/companion
actions. Their remaining rows below therefore stay `[EXEC-TODO]` where outcome/browser/HUMAN proof
is absent.
...
| `[EXEC-TODO]` | Complete economy source/formula/faucet parity | Arc 3 now supplies canonical finite mining/skimming sources and receipt-backed settlement. The Arc 2 source-neutral ledger correctly retains its historical `arc3-deferred` field; capture/combat/mission/new-random-loot faucets, complete rates/recovery/pacing, five non-Deep-Scanner research purchases, Survey orbital mineral rows and unavailable fixed outputs remain open |
