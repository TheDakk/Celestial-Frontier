...
20-minute recovery observation and HUMAN review remain open. Arc 5A now activates its digest-only
V2 projection certificate in the same boot, genuine legacy Training and every Arc 4 hit/miss
transaction, with paired postcommit publication; it remains infrastructure, not a companion action.
Retained Arc 4 browser evidence predates Arc 5A. Final current-input Slice
`20260825171148040-77701-fb5c63191544` and Glass report
`7fc8fbae9b8d01af2d3e9b61fbdee8d2f8a5dad24c9fb28c2407b97a7526546d` are terminal PASS on
Edge `151.0.4129.101`; their full report/log hashes and exact base/status/tree provenance are recorded
in `v2/README.md`, and independent audits are CLEAR. Non-Compendium Vitest passed
`101 files / 1,187 passed / 1 skipped`; full Vitest is
`102 files / 1,205 passed / 1 failed / 1 skipped`, solely the deferred Compendium seal. Remaining
...
| `[EXEC]` | Arc 4 and reconstructed V2 ownership authority cannot split across boot, Training, or capture | Arc 5A focused controls cover exact-source boot creation/zero-write fixed point, full rollback of staged route/Atlas/Arc 2 mirrors on protection, genuine legacy one+18+one write composition, source-deferred absence, exact-parent hit/miss 18+1 successors, pre-draw refusal, and paired postcommit V1/V2 verification/publication or read-only convergence. Non-Compendium Vitest passed `101 files / 1,187 passed / 1 skipped`; Slice `20260825171148040-77701-fb5c63191544` and the 95/95-control Glass run are terminal PASS with CLEAR audits and exact evidence in `v2/README.md` |
...
| `[EXEC-TODO]` | Catalogue species and owned creatures are separate; every mutation targets a stable creature ID | Ownership-v1/v2 codecs model stable catalogue/specimen/fauna identities, receipt provenance, ordered lineage and tombstones. Arc 5A reconstructs the exact V2 projection after boot/Training/capture and refuses a split source/certificate. The player-live Arc 4 writer creates first-only catalogue facts and stable-ID fauna individuals or specimen lots. The design-only fixed-four-shard, O(1)-namespace V2 delta carrier plus duplicate-species migration and breed/feed/combat/delete/assignment outcomes remain required |
