...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Its active phone/desktop ruler embeds exact baseline3 plus independent candidate2/3/4 raw capsules from clean `21af3fa2…`, measures aggregate page/embedder/backing heap, and proves a fixed retained-window last-three-cycle plateau under strict ceilings; baseline discrimination is four sealed faults and 14 phone / 13 desktop breaches. Exact-head certification on the eventual activation commit remains open; ordinary scene Canvas/Pixi ownership and the combined travel → Compendium → Shipyard warm GPU/resource plateau remain Arc 1B work |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, Planetside lifecycle, worker disposal, and the exact built owner→worker→painter graph are implemented. The active ruler embeds paired `20260820-arc1a-baseline3-21af3fa` plus candidate2/3/4 from clean `21af3fa2…`, with measurement authority `bb03a3af…`, producer `291b794e…`, and exact Edge 151.0.4129.86. It replays raw capsules, uses strict ceilings above the three-run maxima, includes aggregate page/embedder/backing heap, and seals stable identity plus unchanged job/disposal/worker counters over the last three cycles of one retained window. The baseline retains four expected faults and breaches 14 phone / 13 desktop ceilings. Exact-head certification on the eventual activation commit remains required; the Arc-local Edge 151 authority still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's maximum-Compendium resource ruler is active from exact baseline3/candidate2/3/4 evidence but still awaits certification on the eventual activation commit; Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
