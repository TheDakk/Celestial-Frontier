...
`c133c89ead736c9c7414af1e6242acd411339853` supplied Final8 Layout 787/787, source-bound
...
green stage ran once and passed named verification. This exact-source chain certifies the bounded
Inventory, Compendium and Settings repairs that Final7 exposed.

Recovery then ran once and stopped terminal `fail` at its fixture precondition after 7,633 ms with
only `runtimeCaptureOrder` red, complete cleanup and zero retry. Burn-down, exhaustion,
closure/reopen, the real 20-minute active observation, boundary crossing and recovered stages did
not run. Post-run review proved the old collector sampled state before its nested UI snapshot while
the shared contract expects canonical UI → state chronology. This is an instrument/oracle mismatch,
not a Recovery product verdict. Final8 is immutable and cannot resume.

The bounded tool repair captures UI then state and binds exact capture count/value ownership,
page-monotonic timestamps, document-token continuity and snapshot/runtime projections. Malformed,
swapped, copied or unbound receipts are instrument-red; a trusted backward runtime remains
product-red under the unchanged shared 10-second chronology/lag contract. Red reports retain the
full witness and the terminal verifier replays fixture evidence. Mutation controls cover equal-value
reversal, forged timestamps/tokens, unreceipted state, projection drift, enforcement/classification
bypass and missing/forged terminal evidence.

Browser-free controls pass, but this Recovery instrument WIP is not browser-certified. A newly
signed clean successor must restart Layout → SceneMemory → Compendium → Slice → Glass → recovery.
No recovery, whole-Gate, publication, hosted attempt, release, deployment, version bump or external
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup for the current producer | Signed `53d030b…` passed historical Final7 Compendium 78/78 before Glass exposed the list-workspace defect. The narrow repair keeps the 58px gutter/virtualization, removes only the redundant list-mode heading clear and requires the exact hostile row mounted **and contained**. Signed `c133c89…` then passed Final8 Compendium 78/78 with six review PNG bindings and later full 12-viewport Glass with zero findings/instrument failures for that exact source. Current collector `c13a489d…3faab`, outcome contract `ac7eea39…6d17`, measurement `28b06f3c…276c`, numeric ceilings and historical samples remain unchanged; the 55-bullet draft count also remains unchanged. The later Recovery-instrument WIP is uncertified and requires a fresh signed full-chain restart. Browser authority remains Edge family + CDP `1.3`; `.107` is provenance only. Six-image HUMAN judgment, real Recovery and whole-Gate closure remain outstanding. |
...
| `[EXEC]` | Gear instances migrate, equip, unequip, salvage, pending-claim and persist by exact identity | Strict codecs/fixed-point tests cover duplicate-base identity, equipped legacy affix ownership, corrupt/future rows, full-inventory pending state and lossless capacity/byte protection. Final7 exposed the exact small-phone A++/Mono overflow. The narrow `<=360px` repair retains every field/badge/action, stacks copy and badges to one min-width-safe column, allows copy to wrap and preserves the 44px button. Signed `c133c89…` then passed Final8 Slice and full 12-viewport Glass with zero findings for that exact source, including native reveal, receipt, modal/parent Close, reload and Atlas continuity. The later Recovery-instrument WIP is not covered and requires a fresh signed full-chain restart. |
...
| `[EXEC]` | Tame/Scavenge/Sample are the finite acquisition writers for catalogue pages, specimens and owned fauna | **`[RED]` until the bound full campaign passes.** Native Survey controls bind exact nav/address, the production full roster/current epoch and all-scenario capacity before two F4 draws; they spend on hit or miss and commit first-only reward, repeat individual/lot, one receipt/CAS, exact committed verification and targeted publication. Source-bound random-pool odds/budget, presentation fencing and native Close/reopen remain. Signed `c133c89…` passed Final8 Slice and full Glass after green Layout/SceneMemory/Compendium predecessors. Recovery then stopped terminal `fail` at its sole `runtimeCaptureOrder` precondition; burn-down and the 20-minute observation never ran, and post-run diagnosis identified an instrument capture-order mismatch rather than a product verdict. The later repair is uncertified and requires a fresh signed full-chain restart. Post-start audio and real Recovery remain unproved; HUMAN journey/listening, Charter bioscan, targeted preview, hosted and release authority remain open. |
...
| `[EXEC-TODO]` | Accessibility audit | Final4 established the Exotic contrast repair. Final7's four Settings findings came from an oracle that demanded simultaneously hit-testable off-card controls. The repaired contract independently reveals Sound and Creature voices, binds two equal current-node samples, control/panel/bounds/Close and scroll settlement, 44px centre ownership, no overlap, dispatch-time coordinates, trusted receipt, exact post-state/focus and unconditional restoration. Signed `c133c89…` passed Final8 full 12-viewport Glass with zero findings/instrument failures for that exact source. The later Recovery-instrument WIP requires a fresh signed full-chain restart; physical assistive-technology review remains required. The queued universe-wide lighting/bloom pass must preserve contrast and reduced-motion contracts. |
| `[EXEC-TODO]` | Performance and heat budgets | Historical Arc 1A/1B/1C certificates and the signed heap-only activation remain bound to their exact producers. Signed `c133c89…` supplied Final8 Layout 787/787, SceneMemory 42/42, Compendium 78/78, Slice and full Glass with zero findings/instrument failures on unchanged current numeric ceilings, rulers and samples. Recovery stopped at its precondition-only instrument mismatch before active observation; the later tool repair requires a fresh signed full-chain restart for current-WIP authority. Both browser authorities remain compatible Edge family + CDP `1.3`; `.107` and later compatible point updates are provenance only. True GPU bytes, queued bloom/atmosphere/particle costs and physical-device heat/battery remain open, so this row and Gate I stay open. |
