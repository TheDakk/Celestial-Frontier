...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact-139 passed the complete local battery, but corresponding run `32383320206` preserved a valid no-retry Planetside `product-unanswerable` red after 29 phone stages: target `Runtime.evaluate` took `2001.132592` ms against 2,000 ms while the root heartbeat answered in `10.401960` ms; zero outcomes, 78 blocked, no review PNG. The displayed-demand/zoom-owner repair starts standard phone/desktop at 609/420px → 512 and preserves identity-safe 768/1024 upgrades through real zoom/DPR demand. Measurement remains `6ba58522…`; producer is `d3223177…`, and budget/test `9f53ebe69e…` / `c2b68b3ec…` fail closed at `calibration-required`. This row remains `[EXEC-TODO]` pending baseline8 plus candidate17/18/19, one exact-activation-head battery, corresponding terminal-green PR test-merge CI, and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau |
