...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact-139 passed the complete local battery, but corresponding run `32383320206` preserved a valid no-retry Planetside `product-unanswerable` red after 29 phone stages: target `Runtime.evaluate` took `2001.132592` ms against 2,000 ms while the root heartbeat answered in `10.401960` ms; zero outcomes, 78 blocked, no review PNG. The displayed-demand/zoom-owner repair starts standard phone/desktop at 609/420px → 512 and preserves identity-safe 768/1024 upgrades through real zoom/DPR demand. Measurement remains `6ba58522…`; producer is `d3223177…`. Clean source `75a996af…` supplied baseline8 plus independent candidate17/18/19; active budget/test `74e88c2b…` / `485be9da…` retain four-fault baseline discrimination and all strict ceilings. This row remains `[EXEC-TODO]` pending one exact-activation-head battery, corresponding terminal-green PR test-merge CI, and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, asynchronous decoded 440px detail settlement, Planetside lifecycle, worker disposal, and the exact owner→worker→painter graph are implemented. Run `32383320206` reached the product and preserved its no-retry Planetside answerability red. The bounded product repair derives cold globe demand from display size/scale/DPR and uses an exact generation/world owner for post-bake publication and higher-tier zoom upgrades. It changes producer to `d3223177…` while measurement remains `6ba58522…`. Clean source `75a996af…` supplied one no-retry baseline8 plus three independent no-retry candidates; active budget/test `74e88c2b…` / `485be9da…` preserve all four baseline faults, 14 phone / 13 desktop breaches, and all 40 strict ceilings above candidate maxima. This browser-free activation is not a selected-head browser resource verdict. The row remains `[EXEC-TODO]` pending one exact-activation-head battery and terminal-green corresponding PR test-merge CI. Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's prior rulers and exact-f9/c49/139 local batteries remain preserved history. Run `32383320206` is product evidence, not a request to optimize the accepted CI envelopes: its target missed the unchanged 2,000 ms bound while the root heartbeat stayed timely. The displayed-demand/zoom-owner repair preserves supported 1024 sharpness only where real zoom/DPR demands it. Launcher/measurement `6892dea6…` / `6ba58522…` remain unchanged; producer becomes `d3223177…`. Clean source `75a996af…` supplied baseline8 plus candidate17/18/19, and active budget/test `74e88c2b…` / `485be9da…` retain strict discrimination. Activation is browser-free and non-certifying; one exact-activation-head battery and corresponding PR test-merge CI remain. First red stops without further timing expansion. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
