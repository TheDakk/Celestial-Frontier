...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact-89 remains a post-measurement shutdown red. Clean `c49e525…` candidate20 reached 78/78 and complete lifecycle, but its reused `.86`-named app reported Edge `.93`; quarantine report/sample/log `175fac5e…` / `916dd12a…` / `7462144b…` as wrong-browser instrument evidence. Baseline9 did not run. Explicit top-level `.86` authority and pre-profile candidate/baseline rejection establish measurement `825fb386…`; producer `d3223177…` and browser CDP `6da9e2ef…` remain unchanged. Fail-closed budget `71ffa46f…` awaits fresh-per-launch exact-.86 baseline9 plus candidate21/22/23, once each without retry. This row remains `[EXEC-TODO]` pending activation, one exact-head local battery, one corresponding PR test-merge CI, and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau. No timing, product, producer, or retry-policy drift occurred; Arc 1B begins immediately after terminal-green PR #32 |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, asynchronous decoded 440px detail settlement, Planetside lifecycle, worker disposal, and the owner→worker→painter graph are implemented. Historical rulers and exact-89 remain truthful only for their own authorities. Clean `c49e525…` candidate20 reached 78/78 and complete lifecycle on self-updated Edge `.93`; its `175fac5e…` / `916dd12a…` / `7462144b…` carriers are quarantined, and baseline9 did not run. Explicit top-level `.86` authority changes measurement to `825fb386…`; fail-closed budget `71ffa46f…` requires fresh-per-launch exact-.86 baseline9 plus candidate21/22/23 from one clean committed source, one attempt each, before exact-head certification. The row remains `[EXEC-TODO]`; Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's older rulers and exact-f9/c49/139/964/89 carriers remain preserved history. Candidate20's 78/78 and clean lifecycle do not override its self-updated Edge `.93`; quarantine it as wrong-browser instrument evidence, and note baseline9 did not run. Explicit `.86` authority changes measurement to `825fb386…`; producer remains `d3223177…`. Fail-closed budget `71ffa46f…` awaits fresh-per-launch exact-.86 baseline9 plus candidate21/22/23 once each with no retry, then one exact-head battery and one corresponding CI. No timeout, launch argument, product, producer, or retry-policy drift occurred. First red stops. Arc 1B scene/Pixi/GPU plateaus begin immediately after terminal-green PR #32; physical-device heat and battery evidence remain open |
