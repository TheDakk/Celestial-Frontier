...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's prior rulers and certifications remain preserved history. The bounded Planetside stack/copy change creates producer `e59685b1…`; measurement remains `f9710bdf…`, but budget `70c40013…` is `calibration-required` until baseline6/candidate11/12/13 supply a strict replayed ruler. Exact-head local gates and corresponding PR test-merge CI remain required after activation. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
