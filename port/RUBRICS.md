...
| `[EXEC]` | No leaks across repeated travel, Compendium and real Shipyard cycles | Arc 1A's maximum-Compendium ruler passed exact hosted run `32462323775` and merged in PR #32 at `d4ab7e6…`. Arc 1B's `79c605f…` / `e244c9e…` 40/40 scene-memory-v1 certificate remains the historical pre-Shipyard authority. Arc 1C product/ruler `a4de5007ffc9131b8bc952a0a4cb469d9139039e` adds normalized ship state, one SVG/DOM Shipyard preview owner and the named HD-surface attachment. Exact local activation/certification source `59530da3bf40965adf9c54f169b310e11ccdd0f8` binds scene-memory-v2 budget `3b71d14ca297ec4d536669d2edf960ac4d01671dd7a0c9eb11a2fb76e4fc43f7`; run `20260822-arc1-local-certification` passed 42/42 once without retry under Edge `151.0.4129.101`, complete lifecycle/cleanup and named verification, across Universe → Galaxy/fine → Sol/System → Earth/Surface → 1,500-row Compendium → real Shipyard → settled Universe. Report raw/gzip SHA-256 are `e24ceef86d17fb4a47bbb10e58f81d442cac6e3def28923672448f6c47eac3a5` / `0d83e6ce339205beb0b5387008ca74ca9b1f95cb22bf61444c439da36405f2a6`. This makes the criterion executable locally; PR #33 review, hosted CI and integration remain open, and the other Gate D rows still prevent whole-Gate closure. |
...
| `[EXEC-TODO]` | GPU/resource budget | Exact viewport backing-store caps remain. Arc 1B budgets scene Canvas leases/bytes, live TextureSources, Pixi managed-texture pixels and per-hash inventories, render-target/cache proxies, DOM, heap/backing storage, slope/range, BFCache and answerability. Arc 1C's local Edge `.101` scene-memory-v2 certificate extends those proxies through the real Shipyard, its one SVG/DOM preview owner, and the named HD-surface attachment. Chromium still exposes no portable true GPU-byte counter; later filters/particles, hosted integration, and physical-device GPU/heat evidence remain open. |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability; Arc 1A's Compendium ruler passed exact run `32462323775` and merged in PR #32. Arc 1B's exact Edge `.93` 40/40 certificate remains historical. Arc 1C source `59530da…` passed one local Edge `.101` 42/42 scene-memory-v2 certification and named verifier under budget `3b71d14c…`, including the real Shipyard and named HD attachment. PR #33 still has no hosted evidence or integration; true GPU bytes and physical-device heat/battery remain open, so this row and Gate I stay open. |
