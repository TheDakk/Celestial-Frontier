...
| `[EXEC-TODO]` | No leaks across repeated travel, Compendium and future Shipyard cycles | Arc 1A implements maximum-Compendium virtualization and lease/cache/worker ownership. Exact-89 completed 78/78 and six PNGs, then owned shutdown exited 2 after a premature PASS/report verification; preserve it as post-measurement instrument red. Lifecycle repair `6da9e2ef…` / `f4ad842c…` / `2713ed10…` moves measurement to `a3b3bb9f…` while producer stays `d3223177…`. Fail-closed budget/test `ae4ab918…` / `60fa5e9f…` await baseline9 plus candidate20/21/22 from one clean committed source, once each without retry. This row remains `[EXEC-TODO]` pending activation, one exact-head local battery, one corresponding PR test-merge CI, and Arc 1B's scene Canvas/Pixi plus combined travel → Compendium → Shipyard plateau; no timing, browser/package, product, producer, or retry-policy drift occurred |
...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC-TODO]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup | Product virtualization, focus/filter/detail/Close outcomes, 132px lease cancellation/dedupe/error recovery, asynchronous decoded 440px detail settlement, Planetside lifecycle, worker disposal, and the exact owner→worker→painter graph are implemented. Historical baseline8/candidate17/18/19 and ruler `74e88c2b…` remain truthful only for measurement `6ba58522…`. Exact-89 reached 78/78 and six images, then exposed a false-green shutdown lifecycle: terminal log `b0bb8abc…` records exit 2 while report/verifier `66ba1366…` / `98664dca…` had already claimed PASS. Current measurement `a3b3bb9f…` and fail-closed ruler/test `ae4ab918…` / `60fa5e9f…` require baseline9 plus candidate20/21/22, one clean committed source and one attempt each, before exact-head certification. The row remains `[EXEC-TODO]`; Arc-local Edge 151 still does not repin Gate A |
...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's older rulers and exact-f9/c49/139/964 carriers remain preserved history. Exact-89's 78/78 product observations do not override its owned-shutdown exit 2 or premature PASS/verifier; it is post-measurement instrument red, not a request to optimize accepted timing envelopes. Frozen browser/collector lifecycle repair changes measurement to `a3b3bb9f…`; producer remains `d3223177…`. Fail-closed `ae4ab918…` / `60fa5e9f…` awaits baseline9/candidate20/21/22 under exact Edge .86 once each with no retry, then one exact-head battery and one corresponding CI. No timeout, browser/package, launch argument, product, producer, or retry-policy drift occurred. First red stops. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
