...
| `[EXEC-TODO]` | Performance and heat budgets | `budgets.json` sets bundle + answerability. Arc 1A's prior rulers, exact-f9 local battery, and no-product CI cold-start red remain preserved history. Launcher `6892dea6…` changes measurement to `6ba58522…`; current budget `41482f1c…` fails closed pending baseline7/candidate14/15/16, one exact-head battery, and corresponding PR test-merge CI. The 45-second selftest-only CI allowance is not a game optimization target; generic/candidate startup and 2-second product observation are unchanged. Arc 1B scene/Pixi/GPU plateaus plus physical-device heat and battery evidence remain open |
