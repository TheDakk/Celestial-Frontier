...
| | Criterion | Evidence |
|---|---|---|
| `[EXEC]` | Maximum-size Compendium mounts/paints only a bounded visible window and plateaus after warmup for the current producer | Signed `53d030b…` passed historical Final7 Compendium 78/78 before Glass exposed the list-workspace defect. The narrow repair keeps the 58px gutter/virtualization, removes only the redundant list-mode heading clear and requires the exact hostile row mounted **and contained**. Signed `c133c89…` supplied historical Final8 Compendium 78/78 plus full Glass; historical Final9 repeated both. Historical immutable Final10 `4405fb2…` passed Compendium 78/78 with six review PNG bindings, then Slice and full 12-viewport Glass with zero findings/instrument failures. Current collector `c13a489d…3faab`, outcome contract `ac7eea39…6d17`, measurement `28b06f3c…276c`, numeric ceilings and historical samples remain unchanged; the 55-bullet draft count also remains unchanged. Signed implementation/evidence repair `3fbfcd5…` has the six-region source seals and retained/replayed receipt controls above browser-free green and later led through immutable Final11 to Final12; Final12's three green predecessors and distinct immutable Slice control stop are recorded above. Browser authority remains Edge family + CDP `1.3`; `.107` is provenance only. Six-image HUMAN judgment, real Recovery and whole-Gate closure remain outstanding. |
