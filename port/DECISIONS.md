...
| Gate C — real veteran save | **Resolved 2026-09-05 (Nick).** No legacy save import: “nobody is running old saves… treat it as a brand-new game; we want saves going forward for v2.” The Settings “Bring expedition” door, its paste/pick/import controls and the `cf_v2_import_original` keepsake were removed the same day; the Field Training recovery sheet offers reload/update only. Gate C is re-scoped to v2's own persistence on a real device (persist, reload, recover, and the Safari 7-day ITP eviction question). The v1.8.9 codec remains v2's load path; the evidence-build `importBlob` seam remains the Slice/Glass replacement driver. Open product decision: how v2 protects saves from ITP eviction (Home Screen install guidance, export/backup, or account). |
